...
>
> ADR-057 adds one more, deliberately: **`childTS.routingSessionID = parentTS.routingSessionID` (`pkg/agent/subturn.go:1130`) is a REQUIRED, load-bearing inheritance, not a bug.** It visually pattern-matches the prohibited shape and it is not covered by the prohibition. ADR-057's D2/FR-011 identity split gives a delegated child its **own** `transcriptSessionID` (`subturn.go:1113`, `TranscriptSessionID: childID`) for persistence, and keeps a **separately inherited** `routingSessionID` as the cancel/interrupt reachability key. **Delete `subturn.go:1130` and every chat-wide Stop silently stops reaching delegated sub-turns** — no error, no test failure in the obvious places, just a cancel that no longer cascades. Full 37-line contract: `routingSessionID`'s doc comment in `pkg/agent/turn.go` (search the symbol; `loop.go`/`turn.go` line numbers churn).
