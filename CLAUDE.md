...
6. **Deny-by-default for security, opt-in for features.** Documented exception: `bash` (ADR-036 unified the retired `exec`/`workspace_shell`/`workspace_shell_bg` tools into it) is registered for every agent regardless of sandbox mode — the kernel sandbox is the protective layer — and is governed exclusively by each agent's tool policy (allow/ask/deny), not a feature flag; Jim's seed grants him `bash: allow` so he has shell access on a fresh install. Sandbox `off` (god-mode) applies no implicit defaults — operator opt-in required.
