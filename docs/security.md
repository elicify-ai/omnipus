...
Before an entry is written, Omnipus replaces recognised credentials with `[REDACTED]` in the command, tool parameters and event details. Recognised formats: `sk-…` and `key-…` keys, `Bearer …` tokens, GitHub `ghp_…`/`gho_…` tokens, Slack `xoxb-…`/`xoxp-…` tokens, AWS access-key IDs (`AKIA…`/`ASIA…`), Google `ya29.…` access tokens, JSON Web Tokens, and the password in a web address such as `https://user:PASSWORD@host` (only the password is replaced). A value is also replaced when its field is named exactly like a secret (`password`, `token`, `api_key`, `authorization` and a few similar names). This is always on and cannot be switched off. Email addresses are kept, so mail recipients and message IDs appear in full. Anything else is not caught: a plain password typed as a command argument, an AWS secret access key, a GitHub fine-grained token (`github_pat_…`) or a Google API key (`AIza…`).
