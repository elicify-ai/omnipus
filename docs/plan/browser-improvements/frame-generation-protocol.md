...
Authenticated socket replacement retains admission ownership independently of
displayed generation. The relay reserves a binding token and increasing offer ID
under the installation lock, checks both again at installation, and cancels
superseded gathering. Beginning a new binding and publishing the gateway epoch
are coordinated under the capture-session binding lock. Healthy installed media
remains available while a replacement negotiates. The earlier older-offer
installation defect is corrected; focused regressions exercise negotiation
ordering and cancellation. This does not establish final-browser acceptance.
...
## Required evidence and historical validation checkpoints
...
The focused capture-health review reproduced seven failing cases: server-owned binding identity, five superseded-observation cases, and packets received before a loss incorrectly completing recovery. The current observation is now checked while claiming a loss, and recovery requires packet progress after the loss baseline. Three deliberate mutations (removing server identity stamping, accepting a superseded sample, and accepting pre-loss progress) each caused the corresponding regression tests to fail. Restored focused capture-health/recovery tests passed with exit 0 in 3.163 seconds. This validates the capture-session policy. The stateful gateway tracker is integrated as `e980edac7`; combined focused gateway health/tracker tests passed in 7.171 seconds. Received-packet and viewer-write-failure accounting is now integrated. These historical focused results do not establish the subsequent complete implementation or final runtime/CI acceptance; current correction status is recorded in `/Users/danielpiatkowski/AI-Agent-Workspace/omnipus/repo/docs/internal/browser-review-closeout-2026-09-08.md`.
