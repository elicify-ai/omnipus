...
On macOS, Windows, and Linux kernels older than 5.13, `sandbox.SelectBackend()` returns a `FallbackBackend` and `applySandbox` follows the graceful-degradation path (`pkg/gateway/sandbox_apply.go:271-289`). The gateway logs `sandbox.degraded` at boot and continues serving with application-level path checks only. Specifically:
...
- The Landlock ruleset is built and rules are added, so any malformed rule still surfaces at boot.
- `PR_SET_NO_NEW_PRIVS` is set (required for the seccomp filter).
- `landlock_restrict_self` is **deliberately skipped** — calling it would enforce the policy, the opposite of what the operator asked for.
- The seccomp BPF program is installed with `SECCOMP_RET_LOG` instead of `SECCOMP_RET_ERRNO(EPERM)` (`pkg/sandbox/seccomp_linux.go:64,139,148-153`). Blocked syscalls proceed but emit a kernel audit entry.
...
- An exact IPv4 or IPv6 address — `127.0.0.1`, `::1`, `192.168.1.5`.
- A CIDR range — `10.0.0.0/8`, `fc00::/7`.
- An exact hostname (case-insensitive) — `localhost`, `internal.corp`.
...
- **`productionNagBanner`** — fires when `mode=off` and `OMNIPUS_ENV=production` (`pkg/gateway/sandbox_apply.go:257-263`). Reads: `SANDBOX DISABLED IN PRODUCTION ENVIRONMENT`.
- **`permissiveNagBanner`** — fires whenever `mode=permissive`, regardless of `OMNIPUS_ENV` (`pkg/gateway/sandbox_apply.go:417-428`). Reads: `SANDBOX IN PERMISSIVE MODE — NOT ENFORCED. DO NOT USE IN PRODUCTION.`
