...
The CIDR allow-list interacts with the kernel layer asymmetrically. Landlock `NET_CONNECT_TCP` (ABI v4+) filters by destination port only — the kernel cannot match by IP. So a compiled binary spawned through `bash` (ADR-036 merged the former `exec`/`workspace_shell`/`workspace_shell_bg` into `bash`) that dials `https://192.168.1.1/` on port 443 is permitted by the kernel (443 is on the connect allow-list for legitimate HTTPS) and only the gateway-controlled HTTP clients (`web_search`, `web_fetch`, the skills installer, MCP fetches) see the CIDR filter via `SSRFChecker`. This gap is documented in `pkg/config/sandbox.go:244-251`; operators concerned about it should set `bash`'s tool-policy to `deny` or `ask` (CLAUDE.md hard constraint 6 — `bash` has no feature-flag gate, only an explicit per-agent tool-policy entry) on agents that handle untrusted content.
