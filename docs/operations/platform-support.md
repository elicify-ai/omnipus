...
- **Windows amd64** — Tracked as [#113](https://github.com/elicify-ai/omnipus/issues/113). Roughly 15 unit tests assume POSIX semantics (file mode bits, advisory `flock`, fork-time signals). The kernel-sandbox story (Job Objects + Restricted Tokens + DACL) described in `docs/BRD/Omnipus Windows BRD appendic.md` is specified but unimplemented; on Windows the sandbox falls back to application-level checks like on macOS.
