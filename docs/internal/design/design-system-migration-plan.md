...
| C3 | **Fix:** a dialog opened on top of a Sheet leaves the Sheet exposed to screen readers. Radix hides the layers below a modal with the `aria-hidden` package, which never hides an element that contains a live region. Since Stage A, every action button carries its own `role="status"` announcer, so any Sheet with a button stays visible to assistive technology. Found through `src/test/integration/provider-save.test.tsx`; the provider Sheet has a local fix (`ProvidersSection.tsx::ProviderConfigSheet` `obscuredByDialog`). C3 replaces that local fix with a system-level one: a single shared announcer instead of one per button, or the Sheet making itself inert while a child dialog is open. Then remove the local prop and cover every stacked Sheet-and-dialog pair. |
