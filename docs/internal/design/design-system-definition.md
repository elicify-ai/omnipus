...
**Second amendment, 2026-09-20 — one step at the top: 80px (`--space-9`).** The scale stopped at `--space-8` (64px), and `py-20` (80px at a 16px root) appears on the Usage screen and the landing page. There is no "next multiple" to snap to above the top rung, so the repair had only two options: leave the value raw, or register it. It is registered, because the alternative -- an exception entry -- is the wrong mechanism here. Exceptions exist for values that CANNOT be a token, such as a device-reported safe-area inset or a per-row tree depth; a plain hero-section padding is simply a value nobody had named yet, and excepting it would hide that. 80px continues the scale's own arithmetic: the top of the ladder already steps 40 → 48 → 64, and 64 → 80 is the same +16. The description follows the file's existing convention of naming the 8px grid multiple ("ten-grid spacing step").

**Two values are deliberately NOT registered and stay as accepted debt.** `pl-[188px]` in `AgentProfile.tsx` is a computed alignment offset -- a `w-44` label (176px) plus a `--space-2-5` gap (12px) -- not a spacing gap, and it has no sound rung. Removing it honestly means restructuring the row onto a shared grid, which is a layout change rather than a token swap. `lineHeight: 1` in `brand-icon.tsx` centres a single glyph by pixel maths inside a fixed circular chip; the nearest registered line height is 1.1, and snapping it would shift a brand mark off-centre. Both stay raw, recorded here so the next reader knows they were examined rather than missed.

**Amendment, founder-approved 2026-09-20 — two intermediate steps: 2px (`--space-0-5`) and 12px (`--space-2-5`).** The scale as first written (0, 4, 8, 16, 24, 32, 40, 48, 64) carries this rule's own mapping only for the even Tailwind steps: 7→8, 14→16, 21→24, 28→32. The odd steps had no rung. Measured against the installed ledger, that left 523 repairs with no sound target: the `p-3` / `px-3` / `gap-3` family at 10.5px (349 items) could only move to 8px (−2.5px, a quarter tighter in 349 places) or 16px (+5.5px), and the `gap-0.5` family at 1.75px (174 items) could only move to 0px, deleting the gap, or 4px. Carrying the same "next grid multiple" rule to the odd steps gives 1.75→2 and 10.5→12, so those two rungs are added. Existing token numbers are unchanged; the new names sit between them. With this amendment the 21px and 28px steps map as this rule already states, and the repair moves those 523 values by +0.25px and +1.5px instead of by a quarter of their size.
