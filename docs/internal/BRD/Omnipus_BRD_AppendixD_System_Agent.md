...
>
> The 37 `system.*` tools described here are ordinary builtins registered on the central `BuiltinRegistry` at boot. There is no distinct `omnipus-system` agent at runtime. Per-agent `ToolPolicyCfg` (allow/ask/deny) governs which agents can call which tools.
>
...
The Omnipus agent has access to 37 system tools that are exclusively available to it. These tools are NOT available to user agents, not exposed via MCP, and not configurable in tool allow/deny lists. All system tool invocations are logged to the audit trail (SEC-15).
...
**Schema redaction for cloud providers:** When the system agent uses a cloud LLM provider (Anthropic, OpenAI, DeepSeek, etc.), the full 37-tool schema is sent to the provider's API. This exposes the complete system tool API surface to the cloud provider. To reduce this exposure:
