...
The consolidated checks for commit `7ada86e18` reported unchecked type assertions in the new media-owner test, a test-fixture data race from changing the browser executor after asynchronous attachment discovery had started, and tab-switch recapture failures. The first two have local test-only corrections. Five focused repetitions after the fixture correction reported no race but still reproduced tab-switch functional failures. Competing initial-document discovery and tab-switch refresh are an investigation lead, not yet an established root cause or completed fix. No latency threshold or failing assertion has been weakened.
