...
- **UAT-3 (FIXED — `121b0303`): coordinator tests were silently SKIPPING (latent, unmasked by UAT-1).** With the X-Goog-Hash download broken, `EnsureChromium` failed, so the entire coordinator suite (crash-recovery, multi-agent-one-Chrome, shutdown-is-sole-kill, 5-agent stress) skipped for who-knows-how-long. Once downloads worked they ran — and hit a `t.TempDir()`-inside-`sync.Once` bug (shared Chrome installed under the first caller's per-test temp dir, deleted when that test returned → later tests got a stale path). Fixed to a process-lifetime `os.MkdirTemp`. **Net: real coordinator-launch coverage restored.**
- **UAT-4 (FIXED — `734bf938`): integration harness fired a wasteful Chrome download per gateway boot (latent, unmasked by UAT-1).** Working downloads meant every `StartTestGateway` spawned a manifest-fetch + ~130 MB download goroutine whose in-flight network call delayed `RunContext`'s shutdown return past `Close`'s 10s goroutine-stop deadline under CI load — a load-induced flake (different unrelated tests each run). Gated boot preprovision on `OMNIPUS_SKIP_BROWSER_PREPROVISION`, set in the harness. Production preprovisions normally.

**Cascade note:** UAT-3 and UAT-4 were both *latent* bugs that the broken X-Goog-Hash check (UAT-1) had been masking — fixing it made Chrome downloads succeed, which exposed them. All three are genuine codebase improvements, not regressions introduced by this work.
