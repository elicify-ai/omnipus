...
> **Update 2026-09-20 (issue #761):** D10's god-mode closure is superseded. God mode does not erase an agent's own tool ceiling, so the Judge no longer refuses to adjudicate while it is on. **Read §12 before D10** — the bullet there, §2.1's C2 row, and the spec's JUDGE-FR-057 / FR-057a all describe behaviour the product no longer has.
...
- **God mode.** ~~`resolveEffectivePolicyWith`'s `cfg.GodMode` short-circuit floors every tool at `allow` regardless of seed. The verifier dispatch refuses to run a Judge turn under god mode rather than running one holding `bash`.~~ **SUPERSEDED 2026-09-20 (issue #761) — see §12.** What god mode actually does now: it sets the **global** tool policy to `allow` for every tool and removes global permission prompting. The per-agent policy still applies, and an agent is never granted a tool its own policy denies. The Judge's seed denies everything except its read-only set, so that set is what it holds under god mode too — there is nothing for the dispatch to refuse, and it no longer refuses. The closure this bullet asked for is delivered by the seed, not by a refusal.
...
**The Judge cannot run on a task.** A reason only an operator can fix (the Judge is misconfigured or not registered, ~~a god-mode refusal,~~ the Judge's output was cut off) ends the task Failed "The Judge could not run: <plain reason>", with no attempt used. A transient reason re-sends the same claim up to 3 times, with the reason visible on the task while it retries; after that the task ends Failed, again with no attempt used. *(Struck 2026-09-20, issue #761: there is no god-mode refusal any more, so it is no longer one of the reasons a task can fail this way. See §12.)*

**Amendment — 2026-09-15 (issue #710).** The founder's decision of 2026-09-15; the paragraph above stands as the record. Which Judge refusals only an operator can fix is now one classification, shared with a task's own worker turn (`pkg/agent/operator_only_turn_error.go::classifyOperatorOnlyTurnError`): the provider rejected the Judge's key, or no usable key is set (the provider answers 401 or 403); the Judge's provider sign-in has expired; its provider is not configured; it has no model; its model's context window is unknown; the Judge agent is on no workspace team; its working folder cannot be opened. The last two are new. Each is withheld at once, with no retry wait, and ends the task Failed "The Judge could not run: <the reason and how to fix it>", with no attempt used and no restart — the same rule as a worker turn refused for one of these reasons (ADR-043 §9). A temporary Judge failure — a rate limit, a network or provider outage, a stalled stream, a timeout, an error nothing recognises — still retries as described above, and the reason shown for it is plain wording ("the Judge's turn failed: its model returned an error"), never the provider's raw error text.
...
## 12. Amendment — 2026-09-20 (issue #761): god mode keeps every agent's own ceiling, and the Judge no longer refuses

**What changes for the operator.** Turn god mode on and plans keep moving. Before this, a finished plan task ended "failed" with an adjudication refusal, and everything waiting on it stayed blocked for good — issue #761. Now the Judge judges as usual, with exactly the same read-only reach it has when god mode is off.

**What god mode does, stated accurately.** God mode sets the **global** tool policy to `allow` for every tool and removes global permission prompting. That is the whole of its tool-policy effect. The per-agent policy still applies, and an agent is never granted a tool its own policy denies. A per-agent `ask` still asks. The merge rule is unchanged: strictest wins, `deny > ask > allow`.

**What was wrong.** D10's god-mode bullet described a short-circuit that returned `allow` for every tool before the per-agent policy was read at all. That threw away the per-agent layer along with the operator's global ceiling, which was never the intent — god mode is meant to lift the **operator's** restrictions, not an agent's own. `pkg/tools/compositor.go::resolveEffectivePolicyWith` now sets the global layer to `allow` and lets the normal merge run. The override stays non-destructive: neither policy map is written to, so switching god mode off restores the previous decision exactly.
...
**The caveat worth keeping.** God mode still removes the operator's global protections everywhere else — the kernel sandbox and outbound-network restriction are still switched off for every agent, and the global permission prompt is still gone. *[Corrected 2026-09-25: this list also named the shell guard as switched off. It is not — the shell's outside-workspace write refusal and operator deny rules still fire under god mode (`pkg/tools/shell.go::Execute` → `shell_path_guard.go::guardCommand`; see docs/security.md).]* This amendment narrows one claim about tool policy; it does not make god mode safe.

Code: `pkg/tools/compositor.go::resolveEffectivePolicyWith`; `pkg/agent/` (the removed refusal); `contracts/components/schemas/GoalStatusFrame.yaml` (the deprecated enum value).
