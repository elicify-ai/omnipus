...
- **FR-2 — Connection-as-instance:** channels MUST move from typed-singleton config to `map[string]ChannelInstanceConfig`, capped at 1 instance per type in v0.1.0, with credential-ref keys and a **modified** `ChannelEntry`/`ChannelConfigureRequest` contract (**these schemas already exist — this is a contract *modification*, not new** `[FACT: contracts/components/schemas/ChannelEntry.yaml + ChannelConfigureRequest.yaml; pkg/api/generated/openapi_types.gen.go]`); plus a Connectors UI and basic email (one mailbox). `[FACT: pkg/config/config.go:775-778 typed fields; pkg/channels/manager.go:582 initChannels if-ladder]`
  - **FR-2 scope corrections (grill round 1):** the migration is NOT just `initChannels` — the **13 factory constructors read `cfg.Channels.<Typed>` themselves** (`ChannelFactory(cfg *config.Config,…)`) and MUST be updated to read their instance config; extra readers (`emitGHSARemovalWarn`, `countEnabledChannels`, `config_old.go`) MUST migrate; the loop needs a **type→factory map** (`whatsapp` type → `whatsapp_native` factory); the closed **`ChannelId` enum gains `email`** + a transport value; cred-keying keeps `channelSensitiveFields` **type-keyed** (validate the *type* against the enum) while `channelCredKey` storage uses the **instance id**. `[FACT: pkg/channels/registry.go:15; pkg/gateway/rest.go:4693 channelSensitiveFields; openapi_types.gen.go:819-832 ChannelId]`
  - **FR-2 email dependency decision (grill round 1, Constraint #1):** there is **no pure-Go IMAP client in `go.mod`**. Decision: **add `emersion/go-imap` (pure-Go, no CGo)** for IMAP-inbound + stdlib `net/smtp` for outbound — a deliberate new **channel** dependency, in the same class as the existing channel deps (`whatsmeow`, `discordgo`, `telego`, `slack-go`). This is an accepted, documented Constraint-#1 exception scoped to the email channel (pure-Go, no security-path impact). `[FACT: go.mod has 0 imap deps]`
...
- **FR-5 — External-agent runners:** an `ExternalAgentRunner` interface MUST be bidirectional (events-out + control/permission-in), consent-routed, and resumable; universal transport = CLI + JSON streaming. **Claude Code + Codex have existing CLI providers to build on** `[FACT: pkg/providers/claude_cli_provider.go + codex_cli_provider.go]`; the **opencode driver is net-new** — there is no opencode runner in `pkg/` (`opencode-zen` is only a model-ref alias). `[FACT: pkg/agent/hook_process.go JSON-RPC/stdio; pkg/sandbox/hardened_exec.go]`
  - **FR-5 confinement + driver corrections (Spec-4 grill round 1):** (a) the existing `claude_cli`/`codex_cli` providers are **one-shot buffered** callers (`cmd.Run()`, `--dangerously-skip-permissions`), **not streaming** — the runner's bidirectional **streaming drivers are NEW** (the providers prove CLI-spawn+auth, not the event loop; `hook_process` is the *inverse* direction). (b) `hardened_exec` **only self-confines the gateway** (children inherit its Landlock domain unchanged — no per-child FS/seccomp API). **Decision (operator, post-spec review): NO new confiner primitive.** The external CLIs **self-sandbox** (Codex = Landlock/seccomp on Linux + Seatbelt on macOS; Claude Code = permission model) — v0.1.0 runs them **WITH their own sandbox ON** (drops the `--dangerously-skip-permissions`/`--dangerously-bypass-approvals-and-sandbox` flags the one-shot providers currently use) + **git-worktree isolation** + **consent-routing**; **native** sub-agents ride the **existing** Omnipus `workspace.shell` sandbox. Building a new per-child re-exec confiner to re-cage tools that already cage themselves was assessed as **over-engineering** and dropped. (c) the `executor` field is **config-only today (`SubagentsConfig`); it MUST be added to the agent/sub-agent CONTRACT** to cross the gateway/SPA boundary for the runner UI. `[FACT: hardened_exec_linux.go self-confine-only; SubagentsConfig absent from contracts/]`
...
- **FR-9 — Skills + self-improvement:** WIRE the existing stub tools (`system.skill.{install,remove,search,list}`) to the real `pkg/skills` engine; ADD `system.skill.create` + `system.skill.edit` (the authoring/self-improvement verb); default embedded set via `go:embed` + first-boot seed (`summarize · skill-authoring · plan · daily-briefing`); per-agent allowlist + progressive disclosure; skill writes consent-gated + versioned. `[FACT: pkg/sysagent/tools/skill.go — install/remove/search/list are wired to the real pkg/skills engine (wired since implementation; ADR grounding updated post-implementation); no create/edit]`
...
- **FR-12 — Integrations + auth:** Integrations provider-picker UI + composer mic (surface existing search/voice-in providers); single-user/one-password; sensitive settings = password re-type; Profile vs Settings; 3-step onboarding → auto-provision Mia·Assistant. `[FACT: pkg/tools/web.go:91 SearchProvider + pkg/voice/ Transcriber exist]`
  - **FR-12 consent correction (Spec-6 grill round 1, cross-cutting):** the "password re-type / consent" gate **does NOT exist today** — `RequireNotBypass` (`bypass_gate.go:35`) is a **dev-mode-bypass 503 guard** (no re-auth, no password), unrelated. **v0.1.0 MUST BUILD the consent primitive**, in two layers: (a) **HTTP-layer** sensitive settings → a **re-authentication check** (re-verify the one password); (b) **tool-layer** sensitive ops (skill `create`/`edit`, sensitive capability grants — Spec-3/6) → route through the **existing `ws_approval`** (`ToolApprovalRequest`→`ApprovalDecision`, the real tool-consent layer Spec-4 also uses) — an HTTP middleware **cannot** gate a tool `Execute()`. This correction applies to every spec that said "maps onto RequireNotBypass" (Spec-1/3/6). `[FACT: bypass_gate.go:35 returns 503; ws_approval.go:132 ApproveTool]`
...
| 9 | **Wire skill stubs to engine + add `system.skill.create/edit`**; embed default set; per-agent allowlist | `[FACT]` skill.go install/remove/search/list wired to pkg/skills (post-impl); create/edit absent | Keep stubs / no authoring verb — no self-improvement loop | **High** |
...
## 10. Decisions Addendum — post-spec operator Q&A (2026-06-13)

Five clarifications settled with the operator after all 6 specs PASSED:

1. **External runner drivers:** ship **all three** in v0.1.0 — Claude Code (clean), Codex (best-effort; JSON stream less stable), opencode (net-new driver). The `executor` field shape admits all.
2. **Sandbox approach:** **leverage the CLIs' OWN sandboxes** (Codex Landlock/seccomp; Claude Code permissions) — run them with their sandbox ON (drop `--dangerously-skip`) + git-worktree + consent-routing; native subs ride the existing `workspace.shell` sandbox. **No new re-exec confiner** (the earlier plan was over-engineering). See FR-5 (b).
3. **New dependencies approved:** `emersion/go-imap` (email IMAP-inbound, FR-2) + `bleve` v2 scorch (memory FTS, FR-7) — both pure-Go, accepted Constraint-#1 exceptions in the class of the existing channel deps.
4. **Inert delegation fields kept:** the full policy schema (`to·accept_from·modes·budget·depth`) ships now; `accept_from`+`budget` are present-but-inert (not enforced, not in UI) — foundation-first, so enabling enforcement later is no contract change.
5. **Memory design ratified:** the memory schemas (frontmatter, `counters.jsonl`, the 4-tier + 6-edge + bleve/MinHash architecture) are **validated and adopted from `memory-redesign-2026-05.md`** — it satisfies the operator's 5 requirements (self-improving · no-embeddings · simple · secure · graph). What was "superseded" in that doc was only the *naming* (Rooms→Workspaces) + roster, already reconciled in the specs. Per its **D2** + the operator's greenfield choice, **memory is greenfield — old `MEMORY.md` is not migrated** (optional best-effort import); Spec-5 FR-7.6 updated to match.
