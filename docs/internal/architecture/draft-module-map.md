...
**One class of test does break on a pure move: guard tests that read a source file by filename.** `pkg/agent` had ten of them reading `"loop.go"` as text to assert where a symbol lives or how many call sites it has. Two sliced `src[strings.Index(src, "func … windowTrim("):]`; after the move `Index` returned -1 and the whole test binary panicked. Before splitting any file, `grep -rn '"<file>.go"' pkg/<pkg>/*_test.go`. The fix that survives every future split: read all sibling sources of the unit together (`readLoopSourcesForTest` concatenates every `loop*.go`) and slice through a helper that fails the one test instead of panicking (`sliceFromMarkerForTest`). `rest.go` and `gateway.go` will have the same class of test; check first.
...
**Left in `loop.go` after the trial (2026-09-15): 65 functions, 7,612 lines.** Three are deliberate (`runTurn` 4,364, `abortTurn`, `typedTurnExit`: the `turn.go` move is its own step). Ten are the construct/run/stop set the `loop.go` row keeps (`NewAgentLoop` 429, `processMessage` 341, `Run` 239, `Close` 214, `ProcessScheduled` 148, `runAgentLoop` 141, and four small ones). The other 52 have no home in the tables above because they were added after 2026-09-12. Two clusters are obvious files; the founder should confirm the names:

| Proposed file | One-sentence job | Functions (lines) |
|---|---|---|
| `loop_truncation.go` | Detect a truncated model reply and decide whether to continue it | `evaluateTruncatedToolCallError` 87, `evaluateTruncatedSuccess` 86, `truncationContinuationEligible` 64, `preserveTruncatedAccumulator` 56, `flushContinuationAccumulator` 46, `stripContinuationChain` 27, `stripOrphanToolCallMarkup` 26, `orphanToolMarkupRepairMessage` 23, `continuationChainMessages` 13, `toolCallTruncationRepairMessage` 12, `truncationContinueMessage` 11, `isTruncatedFinishReason` 11, `sameContinuationChainMessage` 9 (~470) |
| `loop_toolname.go` | Suggest the nearest known tool name for an unknown one | `suggestUnknownToolName` 60, `suggestScore` 50, `toolOpEnum` 43, `levenshteinToolName` 22, `sameSegmentSet` 18, `segmentsCovered` 13, `minInt` 6 (~212) |

The remaining ~30 are small turn helpers (`sleepWithContext`, `isTransientStreamError`, `handleReasoning`, `hookAbortError`, `u9ToolExecSessionIDs`, the three `*Keys` provider helpers, log formatters). They belong with the `runTurn` extraction, not in a file of their own.

Trial result by file: `loop.go` 8,591; `loop_wire.go` 2,058; `loop_inbound.go` 879; `loop_window.go` 743; `loop_delegation.go` 619; `loop_accessors.go` 603; `loop_config.go` 468; `loop_session.go` 452; `loop_policy.go` 433; `loop_slash.go` 343; `loop_events.go` 294; `loop_browser.go` 253; `loop_search.go` 130; `loop_idle.go` 78; `goal_loop.go` 2,147 (was 1,816); `task_executor.go` 3,129 (was 2,696). Nothing new over 4,000; `loop_wire.go` and `goal_loop.go` sit just over the 2,000 warn line, and `task_executor.go` is back in the should-split band.
...
| Grandfather list | `scripts/budgets/files.txt` | One line per file: `path<TAB>lines`. Seeded from `size-budget-grandfather-2026-09-15.md` (the 10 files, by name and size). A PR that shrinks a file updates its number in the same commit; a PR may delete a line; no PR may add a line or raise a number. |
...
| Grandfather list | `scripts/budgets/functions.txt` | One line per function: `file<TAB>qualified name<TAB>lines`. Matched by exact file+name first, then by package directory+name, so a function that moves within a file or to a sibling file in the same package keeps its entry; a move to another package does not. Proven by self-check fixtures (g) and (h) and by running the gate against the `loop.go` trial split (exit 0). Seeded from `size-budget-grandfather-2026-09-15.md`, which names all 139 (70 Go production, 20 Go test, 13 TS production, 36 TS test) and, separately, the 70 warn-only components. Only shrinks. Components are not on it. |
| Self-check | `scripts/check-function-budget-selfcheck.sh` | Temp Go package and temp TS files with fixtures: a 241-line unlisted function must fail; a listed function one under its number must pass; one over must fail; a 121-line unlisted function must produce exactly one `WARN` line and exit 0; a 241-line PascalCase `.tsx` function returning JSX must produce a `WARN … component` line and exit 0; the same 241-line body renamed to `useThing` (a hook) must fail; a listed function moved to a sibling file in the same package must pass; the same function moved to another package must fail. |

### Wiring, three places, none optional

**Corrected 2026-09-15 after building it.** The repo already has `scripts/guards.sh`, which discovers every `scripts/check-*.sh`, runs its `-selfcheck.sh` or `.test.sh` companion first, and fails if any guard or companion fails or a guard has no companion. Both `pr.yml` (job `Tool-Error-From-Status Lint`) and `runci.sh` (`run_lint`) call only that script, and their comments forbid adding a second guard step (C-92). So the two budget gates are wired by existing, with no CI edit for the gates themselves.

| Where | What |
|---|---|
| `Makefile` | `make lint-budgets` runs the two self-checks then the two gates, for direct use. `make lint` reaches them through `lint-guards`, which runs `scripts/guards.sh`. |
| `.github/workflows/pr.yml` | No guard step added. The guard job gained what the function gate needs: Setup Go, Setup Node, and the shared `node_modules` artifact, because the TypeScript scanner requires the repo's `typescript` package. |
| `deploy/ci-worker/runci.sh` | No guard step added. `run_lint` now calls the stamp-aware `_e2e_ensure_deps` first so a lint-only worker run has `node_modules`. |
