...
| Grandfather list | `scripts/budgets/functions.txt` | One line per function: `file<TAB>qualified name<TAB>lines`. Keyed by name, not line number, so a function that moves within a file or to a sibling file in the same package keeps its entry. Seeded from `size-budget-grandfather-2026-09-15.md`, which names all 139 (70 Go production, 20 Go test, 13 TS production, 36 TS test) and, separately, the 70 warn-only components. Only shrinks. Components are not on it. |
