...
**Audit.** New event `tool.auto_approved`, decision `allow`, emitted once per call Auto ran, through `audit.EmitEntry`. The entry carries the agent and session as ordinary entry fields; its details are `{tool, class, reason, paths}`, where `class` is `runs`, `runs_if_args` or `mcp_not_destructive`, and `paths` (resolved locations, never file contents) appears only for RUNS-IF calls. Prompted or denied calls keep their existing `tool.policy.ask.*` rows.

**One dialog for a `bash` ask rule.** When `bash` resolves to plain `ask` (Auto off or no kernel sandbox) and the command matches an operator `{action: ask}` rule, the D3 verdict is settled inside the single upfront prompt (its arguments carry `adr092_kind: "rule_ask"` and a `note` naming the matched rule), and the approval is pinned with `tools.WithRuleAskSettled` so the tool skips its own second `requestRuleApproval`. D3 `deny` and D7/D8 still apply. "Always Allow" on that prompt records an exact grant over those request arguments; the next identical command reaches the same request, finds the grant in `CheckGrantOrRequestApproval`, and does not prompt again in that chat. Under Auto, or with a `bash` policy of `allow`, the ask rule is still asked by the tool itself (`requestRuleApproval`, arguments `{command, adr092_kind}`), with the same exact-grant behaviour.
