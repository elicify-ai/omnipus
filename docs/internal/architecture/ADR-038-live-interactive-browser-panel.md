...
A `browserlive` engine attaches to the session's chromedp context (`mgr.Session(sessionID)`), runs `page.StartScreencast(JPEG, quality≈60, maxW/H=1280/720, everyNthFrame)`, subscribes via `chromedp.ListenTarget` to `page.EventScreencastFrame`, forwards each frame to connected viewers and **acks it** (`page.ScreencastFrameAck`, off the event goroutine — inline ack deadlocks), and dispatches input via `input.DispatchMouseEvent`/`DispatchKeyEvent`/`InsertText`. **Screencast is repaint-driven, not fixed-FPS** — idle pages emit ~1 frame; the client renders a **synthetic cursor**, and liveness relies on the WS transport ping + `onclose` (an idle page legitimately sends no frames, so there is deliberately **no** separate app-level `browser_status` heartbeat — a state-carrying heartbeat would clobber the control indicator). One engine per (agent, session), reference-counted by connected viewers; `StopScreencast` when the last viewer detaches.

### D4 — Make `BrowserManager` reachable from the gateway
Today `BrowserManager` is a per-agent private field on `AgentLoop` (`al.browserMgr`), overwritten (and the prior one `Shutdown()`) on each agent's tool registration — there is **no** exported accessor and `al.browserMgr` is not reliably "the" manager. Fix: store managers **per agent** (`map[agentID]*BrowserManager`, populated in `registerSharedTools`; the replaced manager **is** `Shutdown()` on same-agent re-registration — see the post-review amendment below; dropping the Go reference alone would leak the Chromium subprocess) and add an exported `(*AgentLoop) BrowserManagerForAgent(agentID) (*browser.BrowserManager, bool)`. The gateway WS handler resolves the manager for the attached agent and calls `.Session(sessionID)`. Also fold `--disable-dev-shm-usage` into the ExecAllocator flags (container stability); add `--remote-allow-origins=*` only if screencast requires it on the pinned loopback debug port (verify empirically — the port stays loopback-only + Landlock-gated, never network-exposed).
...
The full 6× pr-review-toolkit + architect gate ran on the implementation; key resolutions folded back into the decisions above:

- **D1↔D5 session binding (CRITICAL, corrected).** The agent's browser tools all operate on one hardcoded tab (`browser.DefaultSessionID`, `"default"`). The live view therefore binds to **that** tab, not the client-supplied chat `session_id` — `browser_attach.session_id` is now context/correlation only (see `BrowserAttachFrame.yaml`). This is what makes the live view show the tab the agent actually drives and makes the D6 control-lock (`IsControlled("default")`) match the tools.
- **D4 hot-reload (BLOCKER, corrected).** The replaced `BrowserManager` **is** `Shutdown()` on same-agent re-registration (a config reload reruns `registerSharedTools`); only `Shutdown()`→`allocCancel()` kills the Chromium subprocess. An attached live viewer whose manager is replaced receives `browser_status(error)` (via a tab-death watcher) and re-attaches to the new manager — no orphaned process, no split-brain.
- **D6 trust model (made explicit).** Take-control is authorized for any authenticated connection (see D6). Per-owner authorization is a tracked follow-up, not a v1 blocker, and is consistent with the rest of the app.
- **Contract-first inbound validation.** The four client frames are now run through `ValidateInboundFrameJSON` on the browser WS (gated by `gateway.validate_inbound`), enforcing the schema's enum/`maxLength`/`modifiers` bounds server-side — parity with the chat WS.
- **Coordinate mapping** divides by the frame's `page_scale` (correct when `pageScaleFactor≠1`); client `mouse_move` is rAF-coalesced to stay under the server input rate limit; real CDP dispatch failures now surface a `browser_status(error)` instead of being swallowed.

Tracked follow-ups (non-blocking): per-session-owner take-control check for multi-user; true multi-viewer control display (viewer-scoped identity); delegated-sub-turn "Watch live" agent targeting.
