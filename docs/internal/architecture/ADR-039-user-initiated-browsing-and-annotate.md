...
## UAT amendments (2026-07-11, after 2 human-tester agents + a verification pass)

Two independent Playwright "human tester" agents (an impatient power user, a cautious first-timer) plus a rigorous verification pass drove the live preview build. Everything below was found and **fixed on `feat/browser`** (no deferrals), each re-verified live:

- **Blank annotate crop (critical, initially masked).** The crop attached to the chat was a blank white/black square; the feature only *appeared* to work because the D-B3 inspect text carried the answer. Root cause: the crop rect is computed in frame-metadata space (`Metadata.DeviceWidth/DeviceHeight`) but the screencast JPEG is captured with `WithMaxWidth(screencastMaxWidth=1280)` (`live.go`), so on a viewport wider than 1280 the decoded `<img>` is downscaled and the `drawImage` source rect read out of bounds → nothing drawn. Fix: new `scaleCropToImagePixels()` maps the source rect into the img's real natural-pixel space (unit-tested); verified live with a photo that has no DOM text (the vision model correctly described the cropped animal).
- **Launcher on a fresh chat:** "Open browser" errored on a zero-message chat; it now creates a session on open (D-A1 as intended).
- **Hand-to-agent page awareness:** the prefill hint now explicitly directs the agent to read the open page with its browser tools first (the shared tab always worked, but the agent hallucinated "no page open" off the old terse hint). The panel auto-closes on hand-off and is now **non-modal** so the chat behind stays interactive (the ADR-038 design intent; previously a Radix modal made it inert).
- **Pop-out consistency:** annotate is hidden in the pop-out (it has no chat to deliver to — removes a permanent "active chat changed" dead-end), mirroring the already-hidden "Hand to agent". Cross-window control state now agrees via a new server-computed **`controlled_by_other`** field on `BrowserStatusFrame` (broadcast to every other connection on take/release/detach) → the pop-out shows "Someone else is driving" instead of offering stealable control.
- **Polish:** the status pill reads "You're annotating" (not "Agent driving") in annotate mode; blocked/invalid-URL errors show friendly messages instead of raw Go strings (malformed-URL vs SSRF distinguished; the blocked-navigate banner clears on the next navigate); the D-B3 inspect text is reframed as *auto-detected context, image is source of truth* (was quoted verbatim as the literal region text); inspect can no longer 502 (dedicated 5s eval timeout + panic recovery to best-effort `ok:false`); `DialogContent` a11y description added.

**Pre-existing issues observed but out of ADR-039 scope (filed separately, NOT introduced here):** the chat composer's per-turn model override reverts to the default on every send (affects plain text too — a chat-composer bug), and an empty optimistic assistant bubble renders before every reply (general streaming-placeholder behaviour, not annotate-specific).
