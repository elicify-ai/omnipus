...
- **Status:** Proposed — *revised after grill-spec rounds 1–3* (reviews: `…-review.md` R1/R2, `…-review-round3.md` R3). R1: 3 CRITICAL + 8 MAJOR. R2: 1 CRITICAL (drift-enforcement mechanism) + 4 MAJOR. R3: **0 CRITICAL** + 6 MAJOR (scope-accuracy: factory count 13, `normalizeChannelMap` drop, `ResolvedRoute.Drop` carrier, disabled-agent drift, instance lifecycle, session-key instance-scoping). All folded in and re-verified against source. Convergence: 3C→1C→**0C**.
- **Date:** 2026-07-02
