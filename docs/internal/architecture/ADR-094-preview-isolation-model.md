...
4. **SSRF admission for the agent's panel.** The gateway-origin SSRF exception (`pkg/security/ssrf.go::isAllowedGatewayOrigin`) admits the Mode 1 label host class — `<label>.localhost` on the configured gateway port, label per normative 2's grammar, any path, only when the configured gateway host is exactly `localhost` — so the built-in browser can open the primary URL; the bare gateway host keeps ADR-073's `/preview/` path scope untouched. Full rule and negative-test set: the spec-review correction note at the top of this ADR.
