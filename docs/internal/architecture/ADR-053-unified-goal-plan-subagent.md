...
| # | Decision | Conf. | One-line rationale |
|---|---|---|---|
| **D1** | Child sessions are **ISOLATED-but-LINKED**: own durable `SessionID`; a typed `SessionMessage` channel is the only bridge; a curated context snapshot at spawn; **no shared transcript** (FR-6a dropped). | **High** | Clean isolation on S2/S3. Snapshot contents under-spec flagged (§8). |
| **D2** | Questions: a child asks its **PARENT**; the parent answers-or-escalates; only a direct session/plan owner asks the **human**, conversationally in chat (no per-question reply card). | **High** | Parent routes `correlation_id`; channel-portable; FE-3; g6. Answer-vs-escalate boundary flagged (§8). |
