...
**Accepted limitations (7-reviewer gate, documented rather than fixed):**
- **Tab identity is positional (`index`), not a stable id.** Both the `browser_tab_action` wire frame and the `browser_switch_tab`/`browser_close_tab` tools address a tab by its current 0-based `index`, which shifts when an earlier tab closes. A client acting on a *stale* index (read before a concurrent close/adopt landed) can therefore target a different tab than intended — the index is still in range, so there is no error. This is bounded in practice: the manager always bounds-checks the index (no panic/OOB), the live panel reconciles to every `browser_tabs` frame (so the human always sees current indices), the agent's own tool calls serialize, and the tool descriptions + the `tab.index` schema description explicitly instruct re-reading `browser_list_tabs`/`browser_tabs` immediately before acting. A stable per-tab id on the wire (and index-or-id addressing in the action frame/tools) is the correct full fix but is a contract redesign beyond this ADR's index-based scope — tracked as a fast-follow.
- **`browser_tabs` carries the active tab twice** (top-level `active_index` and per-tab `tabs[].active`). Both are always derived from the same `sessionEntry.activeIdx` in one snapshot pass so they cannot drift today; the FE consumes only `active_index`. The per-tab `active` field is redundant but harmless and retained for wire clarity.
