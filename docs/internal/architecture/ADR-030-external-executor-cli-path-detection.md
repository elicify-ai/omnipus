...
- **F-01 (CRITICAL) — validate hardening.** `POST /system/cli-validate` executes a caller-supplied path; it MUST be gated **`withAuth` (create-parity** with `createAgent` — operator decision "anyone who can add the assistant"; **not** an admin route, so the `RequireNotBypass`/e2e trilemma does not arise). It MUST apply a **dedicated** rate limiter, **reject a target that is not a regular executable whose basename matches the selected CLI** (`claude`/`codex`/`opencode`) before spawn, **cap concurrent in-flight validations**, and audit the executed-path event (`{cli, resolved_path, reason}`). Overrides NFR-3 / "must not audit" **for validation only** — detection stays unaudited. `[Decision — R3 C1: create-parity + basename constraint + concurrency + audit; resolves C1/M1/M2]`
