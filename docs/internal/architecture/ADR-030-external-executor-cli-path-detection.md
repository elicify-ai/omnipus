...
- **F-01 (CRITICAL) — validate hardening.** `POST /system/cli-validate` executes a caller-supplied path; it MUST be gated **`withAuth` (create-parity** with `createAgent` — operator decision "anyone who can add the assistant"; **not** an admin route, so the `RequireNotBypass`/e2e trilemma does not arise). It MUST apply a **dedicated** rate limiter, **resolve the target via `LookPath` then reject it if not a regular executable file** (→ `missing-binary`; **no basename/identity match** — superseded by F-03; resolve-first so the guard is authoritative over the actual spawn target and a `$PATH` bare-name override is not false-blocked), **cap per-caller in-flight validations**, **scrub gateway secrets from the probe env** (`sandbox.ScrubGatewayEnvForRunner`, with a hard `WaitDelay`), and audit the executed-path event as **`{cli, user, requested_path, resolved_path, reason}`**. Overrides NFR-3 / "must not audit" **for validation only** — detection stays unaudited. `[Decision — R3 C1 + reviewer-gate fixes: create-parity + regular-executable guard (resolve-first) + env-scrub + per-caller concurrency + audit-with-user; resolves C1/M1/M2 and gate findings MAJ-004 / audit-attribution / env-exfil]`
...
- **F-03 — handshake (operator decision, R3).** The handshake requires only that the binary **runs and returns a valid version-shaped response** — **no per-CLI name match** (the operator judged "answers something valid" sufficient). A mis-pointed but runnable binary is accepted. This **reverts** the Round-2 identity-matcher idea: `conntest` is **reused as-is** and `runner-test` is **unchanged**. `[Decision]`
