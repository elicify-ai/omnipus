...
- **F-01 (CRITICAL) — validate hardening.** `POST /system/cli-validate` executes a caller-supplied path; it MUST be gated `withAuth → RequireAdmin`, apply a **dedicated** rate limiter, **reject non-regular / non-executable targets before spawn**, **cap concurrent in-flight validations**, and audit the executed-path event (`{cli, resolved_path, reason}`). This overrides NFR-3 / the "must not audit" Non-Behavior **for validation only** — detection stays unaudited (no subprocess). `[Decision — closes F-01 fully incl. round-2 G2-01: RBAC + target constraint + concurrency cap, not just rate-limit+audit]`
...
- **F-03 — CLI identity.** The `--version` handshake MUST confirm the output identifies the expected CLI (per-CLI matcher), so a wrong binary (e.g. `/usr/bin/node`) yields `handshake-failed`, not a false pass. This **extends `conntest`** (dropping the earlier "reused verbatim" framing); `POST /agents/{id}/runner-test` also gains the identity check — an intended strict improvement. `[Decision]`
