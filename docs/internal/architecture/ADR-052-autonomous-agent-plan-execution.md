...
Implemented on `feature/plan-swimlane-board` (Wave 1 `4a772291`, Wave 2 `8ea0ba65`, fix wave `2224de75`/`51d69adb`; CI `go-test` ALL GREEN at `51d69adb`). Status stays **Accepted** — this is a completion record, not a decision revision. Four deliberate, verified deviations from this ADR surfaced during implementation; see the "Implementation notes (2026-07-21)" block in the spec ([autonomous-agent-plan-execution-spec.md](../specs/autonomous-agent-plan-execution-spec.md)) for the code-cited detail: the standalone-task §6.8 `▶ Play (re-run)` row split into separate Run (PATCH) / Play (restart, cancelled-only) actions; Stop additionally accepted on a cap-queued `approved` plan ("Stop wins"); the evidence-marker gate's first bare-claim rejection is free (only the second counts); and the `[goal:evidence]` instruction is taught to both native and external-CLI workers.
