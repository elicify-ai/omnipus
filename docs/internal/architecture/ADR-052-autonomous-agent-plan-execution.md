...
1. **Authorization = tool policy.** `create_plan`, `execute_plan` (+ linkage) are builtin tools resolved per-agent by the explicit seeded policy (Constraint #6). Seed (**operator, final 2026-07-21**): **Jim `allow`** for `create_plan`/`execute_plan`/`run_task` in his per-agent map; **every other seeded agent AND the global ceiling seed these tools as explicit `ask`**; nothing hardcoded — all seed data. `[FACT]` runtime resolution is **strictest-wins** (`deny > ask > allow`, `pkg/tools/compositor.go`), so the **default resolved posture is `ask` for EVERYONE — including Jim**: on a fresh install every plan execution raises an operator-approval prompt. This is deliberate (operator decision): the human-in-the-loop returns as an *approval prompt*, not an approval *gate* — and **full autonomy is one seed-edit away**: an operator flips the global ceiling entry to `allow`, and Jim (already `allow` per-agent) resolves autonomous while all other agents (explicit per-agent `ask`) still prompt. Grill F5 holds (ceiling entries explicit, never absent). No approval-agent, no approval flag, no code special-case. **Upgrade posture: moot** — no existing installations, no backwards compatibility on the v0.1.1 line; R3-3 closed.
...
- **One-way-door (security posture, sharpened — grill F10):** `[FACT]` standalone single-task execution is *already* autonomous today, so the marginal **new** autonomy is **multi-task DAG fan-out + agent-authored plans**, not "agents can now run code." Still, removing the human interlock shifts safety onto the guardrails + tool grants. Mitigation: the seed matrix (Jim per-agent `allow`; all others + the global ceiling explicit `ask`) + strictest-wins resolution means the **fresh-install default keeps a human approval prompt on EVERY plan execution, Jim included** — autonomy is an explicit operator opt-in (raise the ceiling to `allow`), which materially narrows this one-way-door; keep criteria-required, cap=16, idle=7d, attempts=3. Broadening `execute_plan` is a security decision; the grant UI should carry a **security affordance**, not read as an ordinary tool checkbox (grill F6).
...
Implemented on `feature/plan-swimlane-board` (Wave 1 `4a772291`, Wave 2 `8ea0ba65`, fix wave `2224de75`/`51d69adb`; CI `go-test` ALL GREEN at `51d69adb`). Status stays **Accepted** — this is a completion record, not a decision revision. Five deliberate, verified deviations from this ADR surfaced during implementation; see the "Implementation notes (2026-07-21)" block in the spec ([autonomous-agent-plan-execution-spec.md](../specs/autonomous-agent-plan-execution-spec.md)) for the code-cited detail: the standalone-task §6.8 `▶ Play (re-run)` row split into separate Run (PATCH) / Play (restart, cancelled-only) actions; Stop additionally accepted on a cap-queued `approved` plan ("Stop wins"); the evidence-marker gate's first bare-claim rejection is free (only the second counts); the `[goal:evidence]` instruction is taught to both native and external-CLI workers; and — a post-ship product-blocker fix, not a v1 design choice — the Judge's own verifier turn is resolved via System Agents being made IMPLICIT members of every workspace, since ADR-046 P1's literal `core_team`-membership gate is definitionally unsatisfiable for a System Agent (permanently barred from that roster). This modifies how ADR-046 P1's membership gate applies to System Agents specifically; it does not touch the gate for ordinary agents.
