...
- **Number verification:** `ADR-083` is free. Checked three ways: the directory listing tops out at `ADR-089`; `git log --all --diff-filter=A --name-only -- 'docs/internal/architecture/ADR-08*'` returns only ADR-080, ADR-081 and ADR-089 across every ref, past and present; and no `ADR-083` path exists on any branch. ADR-089's own header records why this check is not ceremony — ADR-081 was drafted as "ADR-069" and had to be renumbered after a merge.
...
- **Relates:** [ADR-067](ADR-067-omnipus-knowledge-base-and-render-first-preview.md) (the knowledge base, the render-first preview surface, and §10.3/§10.4/§10.7's isolation policies), [ADR-068](ADR-068-vault-records-typed-record-layer.md) (the typed record layer, `knowledge_edit`'s consolidation, FR-070b/c, FR-106), [ADR-089](ADR-089-rename-vault-to-knowledge-base.md) (naming)
