...
**Requirement 3 is the one that closes #618's actual defect.** `marshalWithinBudget` (`pkg/tools/result.go`) encodes with `encoding/json` — which escapes every control byte as `\u00NN`, always valid JSON — and then *measures the encoded result*, shrinking the caller-nominated fields **in priority order** until it fits under 1900 runes: it fully exhausts `shrinkable[0]` (repeatedly halving that one field down to a single rune if necessary) before touching `shrinkable[1]`, and so on, rather than halving every shrinkable field together in lockstep on each round. **[CORRECTION, round-2 review finding F4]** an earlier revision of this ADR described the lockstep-halving shape — that was the ORIGINAL implementation and it shipped a real defect: shrinking every field together on each round meant an oversized field near the end of the payload (e.g. an escape-heavy `reason`) could drag a short, fixed-literal field near the front (e.g. `PermissionDeniedPayload`'s model-facing `message` guidance) down to a near-empty fragment even though that field was never what made the payload oversized. Priority order fixes this: callers order `shrinkable` from "most likely to actually need shrinking" to "shrink only as a last resort" (see each producer's own call site, e.g. `PermissionDeniedPayload` orders `reason, tool, message`), so a fixed short field is never touched unless every field ahead of it has already been exhausted. Measuring the encoded size rather than clamping the input is separately load-bearing: `encoding/json` HTML-escapes `<`, `>` and `&` to six runes each and doubles `"` and `\`, all legal filename characters, so arithmetic on input runes understates the encoded size. A first version of this project's budgets made exactly that mistake and a path holding ~67 such characters overflowed a budget that looked safe. **Regression coverage for the priority-order property specifically** (not just the budget/escaping properties above): `pkg/tools/permission_denied_test.go`'s message-preservation assertion — reverting to lockstep halving fails it (see that test's own doc comment for the revert-proof).
...
**[CORRECTION, round-2 review finding F1]** Between this ADR's original acceptance and this correction, the union briefly regressed and was then repaired — recorded here so the history is honest rather than silently overwritten. Commit `dd36bc9a` observed correctly that the *original* `oneOf` double-matched every one of the seven `$ref`s against its own permissive first branch (`type: [object, array, string, number, boolean, 'null']` already matches any object) and was therefore schema-invalid against its own union — but it "fixed" that by switching the operator to `anyOf`. That over-corrected: under `anyOf`, the permissive branch always matches, so the union can **never reject anything**, including a malformed `PermissionDenied` missing `permanent`. That directly nullifies §7 item 2's own stated rationale for admitting `PermissionDenied`/`ToolAssemblyDuplicate` to the union — *"when the union is ever made executable, a member missing from it would be the silent-drop failure ADR-058 §7 item 4 warned about"* — because an `anyOf` over a universal branch can never be made executable in the first place, by construction, no matter what is added to or removed from it. The round-2 fix restores a genuine `oneOf`: the single permissive branch is split into an unconditional non-object catch-all (array/string/number/boolean/null — the JSON Schema `required` keyword does not apply to non-object instances, so no exclusion is needed there) and an object catch-all that excludes every reserved discriminator key the seven `$ref`s use (`_truncated`, `_marshal_error`, `_ref`, `error`) via `not: {anyOf: [{required: [...]}, ...]}`. With that split exactly one branch matches a plain scalar/array/opaque-object result, and exactly one matches a valid named family/sentinel shape; a payload carrying a reserved key but failing its own `$ref` (missing a required field, wrong `const`) now matches **zero** branches and is correctly rejected — proven by compiling this exact file with `santhosh-tekuri/jsonschema/v6` in `pkg/gateway/structured_failure_discriminator_coverage_test.go`'s through-frame tests (`TestStructuredFailureDiscriminators_ValidateThroughToolCallResultFrame`, `TestToolCallResultFrame_MalformedPermissionDenied_Rejected`, `TestToolCallResultFrame_MalformedFileExistsRefusal_Rejected`, `TestToolCallResultFrame_GenericResultShapes_StillValidate`). This does **not** change this section's own conclusion: the union is still documentary *in the generated Go/TS artifact types* (`Result any` / `z.unknown()`, unchanged by the operator swap — neither generator's oneOf/anyOf branch depends on which operator is used, only on whether a top-level `type` is present). What changed is that the **spec itself** is now a coherent, test-time-checkable discriminated union again, rather than one that can never fail regardless of what a future author adds to it.
