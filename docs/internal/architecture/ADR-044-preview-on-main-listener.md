...
Commit `37d246dc` closed a follow-on gap found by the full e2e suite: the legacy `/serve/` and `/dev/` back-compat preview prefixes (pre-dating this ADR's `/preview/` unification) had no handler registered on the main mux, so an unmatched request under either prefix fell through to the `"/"` SPA catch-all (`embed.go`'s `newSPAHandler`), which answers **any** unknown path with a `200` + `index.html`. That false `200` mattered specifically because the SPA's warmup probe for historical transcript replay (`IframePreview.tsx`, HEAD-fetch via `src/lib/preview-url.ts`'s `PREVIEW_PATH_REGEX`) treats any `2xx`/`3xx` status as "dev server ready" — so a dead legacy preview link was silently misreported as live instead of erroring.

The fix registers both `/serve/` and `/dev/` on the same main mux as `/preview/`, but pointed at a dedicated `handleLegacyPreviewRetired` responder (`pkg/gateway/rest.go`) instead of `HandlePreview` — the mux's longest-matching-subtree-prefix dispatch then outranks the `"/"` catch-all for anything under those two prefixes, without reviving them as real preview routes (no legacy token minted before this migration can still be valid; registrations are short-lived, max 24h). GET/HEAD/OPTIONS under either prefix now get a genuine `404`; a state-changing method (POST/PUT/PATCH/DELETE) is intercepted earlier by the CSRF middleware with a `403` instead, since `/serve/` and `/dev/` were deliberately left out of the CSRF exempt-prefixes set (only `/preview/` is exempt) — either outcome keeps the retired prefixes off the `200` SPA shell, which is the invariant this fix restores. Regression coverage: `pkg/gateway/rest_preview_legacy_gone_test.go`.
