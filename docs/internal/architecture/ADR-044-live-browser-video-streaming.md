...
- **Routing:** structural scope per the release strategy then in force (the v0.2/v0.3 release labels were retired 2026-09-25); supersedes the interim tuning levers recorded in the `browser-live-responsiveness` research note.
...
> 5. **Wire cleanups (M-10, m-3, O-1).** The `browser_screencast` message is **removed** from the contract (removal landed — `browser_screencast` is gone from `contracts/`; the v0.3 no-back-compat framing was retired 2026-09-25). **Correction (R5/F-02): it is NOT "dead"** — it is today's sole live-view transport (emitted `browser_ws.go:546`, consumed `browserLiveWs.ts:147`); its removal is the cutover of the only live-view path and **withdraws working JPEG live view from non-video-capable installs** (they get the unavailable state — the true A2-only cost), so it must not precede the video path being reachable on video-capable installs. The video-chunk envelope timestamp is **`ts:u64`** (not the non-standard `u48`). `browser_stream_bitrate` (ABR) is **deferred to v1.1** and is not in the v1 contract.
...
JSON control frames (`browser_status`, `browser_tabs`, `browser_control`, `error`) are unchanged. The dead JPEG `browser_screencast` message was **removed** from the contract (removal landed; R3/M-10 — no back-compat obligation; no runtime path selected it; the v0.3 label was retired 2026-09-25).
