...
**Decision.** Hydration may only **fill an empty archive**: the attach path checks the agent archive first and skips hydration when it has ≥ 1 line; the self-heal path keeps its existing emptiness condition. When hydration does run (a genuinely empty archive) it must preserve tool results: it **reconstructs tool calls from standalone `tool_call` entries** — each attached to the preceding assistant message of the same `turn_id`/agent as a `ToolCall` on that message, followed by a `role: "tool"` line carrying the entry's **D4-capped `result`** (the UI transcript's `tool_call` entries gain that bounded `result` field, written by the same choke point), so the rebuilt archive has one tool line per recorded call and is not lossy; until that transcript-shape change lands, a hydrated archive is marked `hydrated: true` in meta and recall by `tool_call_id` answers *"not available — session was rebuilt from the transcript"*. `SetHistory` must never reset `Skip` on an existing archive — greenfield: it refuses when the file is non-empty.

**Regression tests:** attach the same session twice → the archive file is byte-identical and `meta.skip` unchanged; attach to a session whose archive is empty → hydration runs once and includes tool results — **the fixture uses the real transcript shape (standalone `tool_call` entries, assistant entries with no `tool_calls`) and asserts one `role: "tool"` line per recorded call, attached to the right assistant message**; the ADR-028/FR-019 append-only invariant test gains an attach step.
