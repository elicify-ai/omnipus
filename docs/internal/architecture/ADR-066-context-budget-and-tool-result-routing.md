...
**D1 — extend `pkg/providers/capabilities`; no second catalog.** The catalog exists: an embedded seed of 78 models across 9 providers (`embed.go`), a checksum-verified `GHReleasePuller`, semver-aware refresh that cannot downgrade, a validated DTO → domain parse, and `Catalog.Resolve` → `resolveStrippedPrefix` that already maps `z-ai/glm-5.2` to the seed's `glm-5.2`. Its per-model keys are exactly `id`, `provider`, `input_modalities`, `resize_budget` — no context field (searched). **Add `context_window` and `max_output_tokens`**, bump `schema_version` 1.0.0 → 1.1.0, populate all 78, expose `resolvedModel.ContextWindow()`. Seed from OpenRouter's public catalog (420 models, every seeded vendor covered) via a **build-time** script, never a boot-time fetch. Treat catalog values as accurate-not-exact (`context_length` vs `top_provider.context_length` differ; observed 204,800 vs 198,000) and reserve a margin.

**D2 — resolution ladder.** Operator override (Settings, D9) → catalog → learned (D8) → conservative floor **with a WARN naming the model**. `maxTokens * 4` is retired. The two other flat `128000` fallbacks (`windowTrim`, model switch) consolidate onto the ladder — three paths currently give three answers. **`claude-cli` and `codex-cli` are exempt** (they manage their own context). `antigravity` is **not** exempt: it is plain HTTPS (`antigravityBaseURL = "https://cloudcode-pa.googleapis.com"`) **and** the seeded default model (`pkg/config/defaults.go` → `antigravity/gemini-3-flash`).

**D3 — unknown window ⇒ conservative and loud.** `Catalog.optimistic` assumes image support for unknown models; correct for modality, where optimism costs a retry. For a window, optimism costs a dead turn. The two policies diverge deliberately; this sentence is the record.
...
**Trigger** — operator-settable, shaped per the survey: `trigger = min(absoluteBudget, 0.9 × resolvedWindow)`; run down to a **target** below the trigger so it does not re-fire immediately; the most recent result is the **floor**. Absolute-primary because D1–D3 can be wrong (§1.1); the ceiling catches a model smaller than the absolute. Denominated in characters for the same reason as D4 — revisit when D8 supplies real `prompt_tokens`.
...
- D5's recall mark reaches the SPA inside a tool-result message. It is a candidate member of the ADR-060 family — it must not be hand-rolled with `fmt.Sprintf` (the `%q` finding) — but it is not a *failure*, which every current member is. Membership is left to the implementing commit to decide and record.
...
6. **Exempting `antigravity`.** Factually wrong; see D2.
