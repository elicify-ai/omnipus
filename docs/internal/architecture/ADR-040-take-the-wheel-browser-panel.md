...
**Risks / must-handle:** the implicit "first interaction takes the wheel" must not double-fire or race the lock acquisition; "Take over" must reliably pause the agent (reuse the proven cancel path); pin layout must not tear down the live WS on toggle; the omnibox search must stay behind the SSRF gate.
