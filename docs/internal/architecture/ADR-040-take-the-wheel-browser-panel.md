...
- **Status:** Accepted (build authorized 2026-07-12). **Amended 2026-07-16 (operator direction): D4's pin/overlay split is retired — the panel is ALWAYS docked side-by-side when open. The unpinned right-side Sheet overlay, the 📌 Pin/unpin control (D1's second header verb), the `ui` store's `browserPanelPinned`/`toggleBrowserPanelPinned`, and BrowserLiveView's `isPinned`/`onTogglePin` props were all removed. Fullscreen remains available via the Pop-out (`/#/browser-live`) tab, which is now always offered from the docked header. On phone-narrow viewports the docked column takes the full row (chat collapses) rather than overlaying.**
