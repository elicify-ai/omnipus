...
> ## ⚠️ Correction 2026-09-20 (issue #761) — god mode no longer overrides a per-agent `deny`
>
> **What changed for `list_jobs`: a System Agent stays denied `list_jobs` under god mode.**
> Everywhere below, this spec models god mode as a short-circuit that returns `allow` for every
> tool before either policy map is read. That is no longer true, and it was never the intent.
>
> **The accurate rule.** God mode sets the **global** tool policy to `allow` for every tool and
> removes global permission prompting. The normal merge then runs unchanged: the per-agent policy
> still applies, and an agent is never granted a tool its own policy denies. A per-agent `ask` is
> still an `ask`. Because the global side is never empty under god mode, the both-empty
> fail-closed branch can no longer be reached at all.
>
> **What that means for the rows below, one by one:**
>
> | Case | This spec says | Now |
> |---|---|---|
> | god mode on, agent map `deny` (every System Agent) | `allow` | **`deny`** — the per-agent seed wins |
> | god mode on, agent map `ask` | `allow` | **`ask`** |
> | god mode on, both maps absent | `allow` | `allow` — unchanged verdict, different route: the global side alone decides (`case a == "": return g`), not a short-circuit |
> | god mode on, agent map `allow` | `allow` | `allow` — unchanged |
> | the global `"list_jobs": "deny"` kill switch, god mode **on** | no effect | still no effect — god mode overrides the **global** layer, which is exactly what the kill switch is |
>
> Every `[VERIFIED: pkg/tools/compositor.go:175-177]` tag below points at the deleted
> short-circuit and is stale. The struck text is kept because the history matters. Recorded in
...
> 2. ~~**God-mode short-circuits the whole merge**~~ **God-mode changes the merge's global input**
>    *(headline corrected 2026-09-20, issue #761)*, and rev 1 missed it entirely (so did the
...
>    `off` neither map is consulted.~~ **Corrected 2026-09-20 (issue #761): the short-circuit is
>    gone. God mode sets the global side to `allow` and both maps are still read.** Dataset
>    "Tool-policy resolution" row 8 (*"both sides absent → deny + Error log"*) is still
>    **unreachable under god-mode**, because the global side is never empty while god mode is on;
>    rev 2 adds row 10 and an SC-008 god-mode clause. This is a coverage gap, not a wording quibble.
>
...
>
> **Corrected 2026-09-20 (issue #761) — the one-line rule now reads:** *god-mode sets the **global**
> side to `allow` for every tool and the merge then runs unchanged; the non-empty side wins; if both
> sides are empty it fails closed to `deny`; if both are non-empty, deny > ask > allow.* So a
> per-agent `deny` still denies under god mode, and the both-empty branch is unreachable while god
> mode is on. See the correction block at the top of this spec.
...
6. ~~**Given** sandbox `off` (god-mode), **When** policy is resolved for **any** agent including a System Agent, **Then** `list_jobs` is `allow` — `if cfg.GodMode { return config.ToolPolicyAllow }` short-circuits the merge before either map is consulted `[VERIFIED: pkg/tools/compositor.go:175-177]`. This is expected, is the documented behaviour of god-mode, and MUST be covered rather than left as an unmodelled branch.~~ **Corrected 2026-09-20 (issue #761).** Replaced by: **Given** sandbox `off` (god-mode), **When** policy is resolved for an agent with **no** per-agent `list_jobs` entry, **Then** it is `allow`, because god mode sets the global side to `allow` and the global side alone decides; **and When** policy is resolved for a **System Agent**, whose seed carries an explicit `deny`, **Then** it is still `deny`. Both branches MUST be covered.
...
- ~~When god-mode is enabled, `list_jobs` resolves to `allow` for every agent regardless of either policy map.~~ **Corrected 2026-09-20 (issue #761):** when god-mode is enabled, `list_jobs` resolves to `allow` for every agent that does not deny it in its own policy. A System Agent, whose seed denies it, still resolves `deny`.
...
- **God-mode enabled.** ~~Expected: `list_jobs` resolves `allow` for every agent, System Agents included, because the compositor short-circuits before consulting either map.~~ **Corrected 2026-09-20 (issue #761).** Expected: `list_jobs` resolves `allow` for every agent that has no `deny` of its own, because god mode sets the global side to `allow` and the merge then runs normally; **System Agents are the exception — their seeded `deny` still wins.** Behaviour of the tool itself is unchanged; only the policy verdict differs.
...
- **Contract**: coverage is OR-based per `(agent, tool)` — a global entry **or** a per-agent entry satisfies it `[VERIFIED: pkg/config/validate.go:459-464]`; resolution is ~~**god-mode floors everything at `allow`** `[VERIFIED: pkg/tools/compositor.go:175-177]`, else~~ **god-mode sets the global side to `allow` for every tool (corrected 2026-09-20, issue #761) and then** deny > ask > allow with the non-empty side winning and **only** both-empty failing closed `[VERIFIED: compositor.go:181-201]`.
...
| any System Agent, **god-mode on** | ~~`allow`~~ **`deny`** | **Corrected 2026-09-20 (issue #761).** God mode sets the global side to `allow`, then the normal merge runs; `systemAgentSeed`'s per-agent `deny` wins it (deny > ask > allow). The old short-circuit is deleted |
| any agent, **god-mode on**, both maps absent | `allow` | Verdict unchanged, route corrected (2026-09-20, issue #761): the global side is `allow` under god mode and the agent side is empty, so `case a == "": return g` returns `allow`. The both-empty `deny` branch is still never reached — the global side is never empty while god mode is on |
...
| 34 | `TestToolPolicy_FreshInstallPerAgentClass` | Integration | Scenario Outline: Seeded policy per agent class on a fresh install | Every row of the table, **including the god-mode rows — 10, 11 and 11a as corrected 2026-09-20 (issue #761); row 11's expected value flipped from `allow` to `deny` and any test pinned to the old value must be re-pinned** |
...
| 10 | **god-mode enabled**, both sides absent | **global side supplied by god mode** | `allow` — the agent side is empty, so the global side alone decides | Scenario Outline: Seeded policy per agent class | ⚠️ **Rev 2**, ~~`if cfg.GodMode { return ToolPolicyAllow }` precedes both `resolveFromMap` calls `[VERIFIED: pkg/tools/compositor.go:175-177]`~~ **corrected 2026-09-20 (issue #761): the short-circuit is deleted; god mode sets the global side to `allow` and the merge runs.** The verdict is unchanged, and row 8's both-empty branch is still unreachable while god mode is on |
| 11 | **god-mode enabled**, agent map `deny` | **per-agent deny survives god mode** | ~~`allow`~~ **`deny`** | Scenario Outline: Seeded policy per agent class | ~~Even a System Agent's explicit `deny` is floored to `allow` — this is god-mode's documented effect, not a defect~~ **Corrected 2026-09-20 (issue #761): the opposite is true.** God mode lifts the **operator's** global ceiling, never an agent's own policy. A System Agent's explicit `deny` still denies. **This row's expected value is inverted — any test pinned to it must be re-pinned** |
| 11a | **god-mode enabled**, agent map `ask` | **per-agent ask survives god mode** | `ask` | Scenario Outline: Seeded policy per agent class | **New 2026-09-20 (issue #761).** God mode removes **global** permission prompting; a prompt the agent's own policy asks for is still asked |
...
- **SC-008**: On a fresh install, effective policy for `list_jobs` is `allow` for Mia, Jim, Ava, Ray, Worker, every seeded specialist and every newly created custom agent; and `deny` for every System Agent. ~~**Under god-mode (sandbox `off`) it is `allow` for every agent including System Agents and including the both-maps-absent case**, because `if cfg.GodMode { return ToolPolicyAllow }` short-circuits before either map is read `[VERIFIED: pkg/tools/compositor.go:175-177]`.~~ **Corrected 2026-09-20 (issue #761): under god-mode (sandbox `off`) it is `allow` for every agent that does not deny it in its own policy — including the both-maps-absent case — and still `deny` for every System Agent**, because god mode sets the global side to `allow` and the normal merge then runs, deny > ask > allow. **With a global `sandbox.tool_policies` entry `"list_jobs": "deny"` and god-mode off, it is `deny` for every agent** — the supported kill switch.
...
- **Q**: Does the compositor "fail closed"? → **A**: **Only in the both-empty branch — one of four.** `case a == "": return g` returns the global verdict, which is `allow` for many high-blast-radius tools, and ~~`if cfg.GodMode { return ToolPolicyAllow }` short-circuits the whole merge before either map is read~~ **god mode sets the global side to `allow` for every tool, which makes the both-empty branch unreachable while it is on (corrected 2026-09-20, issue #761 — the short-circuit is deleted, and a per-agent `deny` still denies)**. Rev 1's bolded headline contradicted its own table cell two clauses later, and neither rev 1 nor its review modelled god-mode at all.
...
|---|---|---|---|
| §2 / D8 rationale | *"the real risk is silent permissive inheritance via `compositor.go:175-201`"* | Replace with the accurate four-branch rule **and add god-mode**: ~~`if cfg.GodMode { return ToolPolicyAllow }` short-circuits the entire merge, so under sandbox `off` neither policy map is consulted.~~ **Corrected 2026-09-20 (issue #761): god-mode sets the global side to `allow` for every tool and the merge then runs unchanged, so under sandbox `off` both maps are still consulted and a per-agent `deny` still denies.** Neither the ADR, nor rev 1, nor the spec review modelled this branch. | C2 as corrected in rev 2; corrected again for #761 |
