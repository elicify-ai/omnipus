...
**Status:** Approved (final fix round applied — founder decisions D10–D17 recorded and applied; all 10 MAJOR / 6 MINOR / 3 OBSERVATION findings from `provider-messages-spec-review-round2.md` fixed or founder-decided; zero CRITICAL findings. The resume-signal proposal is resolved — D16, none of the three options: an unattended run that hits `quota_billing`/`model_retired` simply fails like any other failure, no persistent blocked state. D17 fixes a self-contradiction in §7.4 the CHECK audit found (the fallback note's once-per-pair cap is per CHAT, not reset per turn — matches what GREEN already built). Only FQ-8, a landing-order default, remains open and does not block this approval.)
...
**D17 — The fallback note fires once per CHAT per (primary, fallback) pair, again
only when the pair changes — resolves a spec self-contradiction the CHECK audit
found between §7.4's prose and C-17/C-18/FB-4.** §7.4 (pre-D17) said in one sentence
both "at most once per session per pair until the pair changes" (matching C-17/C-18
and the FB-4 BDD scenario) AND, two clauses later, "a NEW turn inside the cooldown
window does announce" — directly contradicting the first clause for a same-pair
repeat. Founder ruling: **the once-per-CHAT-per-pair cap is correct — the "a NEW turn
… does announce" clause is deleted, not the cap.** This is exactly what backend GREEN
already built (the session-transcript-derived, restart-safe once-per-pair cap) —
CHECK found the code correct and the spec prose wrong; §7.4 is corrected to match the
code, not the other way around. No behavior changes; this is a spec-text fix so CHECK
and the 8-reviewer gate test the ruled behavior, not the withdrawn one.
