...
- **FR-4.5** **Sibling granting.** For each secret, walk `/` → secret. At each level, grant read (and execute) on every sibling **except** the entry on the path. Grant everything else whole at the top level.
  ...
  - **FR-4.5e** **Top-level creation is narrowed on Linux — a correction toward the intended policy, not a regression.** Agents default to `RestrictToWorkspace: true` and `pkg/fspolicy/carveout.go` already denies `agents/`, `workspaces/`, `entities/`, `master.key` and `credentials.json` at the application layer regardless of scope, so writing at the top level of `$OMNIPUS_HOME` was never intended agent behaviour; the kernel was simply broader than the policy above it.
  - **FR-4.5f** **The two policy layers still disagree in both directions (NOT fixed here, tracked separately).** The app layer denies `agents/` and `workspaces/` that the kernel grants; the kernel denies `config.json` and `cli.token` that the app layer does not. Because the kernel sandbox covers CHILD processes and the file tools run in-process, an agent running unrestricted can still reach the gateway bearer token via a file tool. Aligning the two lists is its own change.
  - **FR-4.5e-detail** Replacing the whole-tree `$OMNIPUS_HOME` grant with per-entry grants means the home DIRECTORY ITSELF is no longer granted, so a child can no longer create a new entry at the top level of `$OMNIPUS_HOME`, and a top-level entry created after a child spawned is unreachable by that child (the next spawn re-enumerates, so the window is one process lifetime). Writes inside already-granted subdirectories — `agents/`, `sessions/`, `skills/` — are untouched, which is where every agent write actually goes; the gateway creates the top-level layout, not its children. This is a real behavioural change from shipped code, where children inherit the whole-tree grant. It does NOT apply to macOS, where the home grant stays whole and the secrets are removed by explicit deny. Pinned by `TestExpandRulesExcluding_TopLevelCreationIsNarrowed` so it stays a known consequence rather than a later bug report.
