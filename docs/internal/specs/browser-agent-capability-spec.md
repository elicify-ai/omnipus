...
- **Revision 1 written at:** `077c5237` · **Revision 2 at:** `5a67157f` or later · **Revision 3 at:** `da6bb665` or later · **Revision 4 at:** `809555fc` or later — after the ADR was **consolidated and renumbered** (`22ceff6f`) and **D1.9a** landed (`809555fc`). The renumber is why every ADR citation in this document was re-derived rather than search-and-replaced; see **§0d**.
- **Status:** Draft for re-grill → implementation. **Three items need an operator ruling before the affected work starts** (§15 items 1, 7 and 8), one more than revision 3.
...
| 6 | **The lease is rescoped to what D1.9a left of it.** Tabs stay per agent; only the operator's tab is shared, so the lease is **agent-vs-agent on the operator's shared tab only** — agents on their own tabs do not contend. §4's tombstone, FR-035 and FR-038 say so. §3's membership rule is replaced with D1's real one (*lease iff `controlledResult`-gated*), not the registry derivation D1 discarded. | ADR **D1.9a** · consolidated grill **M1** |
...
| 8 | **"Take control on request" is specced in both shapes and flagged for ruling** (§12 A-20, §15 item 7). D1.9a leaves open whether it is an explicit tool or an implicit acquisition on first write to the operator's tab. The wording reads explicit; an explicit tool would change this spec's **tool count, tier fixtures and policy seeds**, so it cannot be left implicit-by-default. | ADR **D1.9a** (open) |
...
| `D1.1b` | Renderer bound, memory formula, admission gate | **D1.5** (sizing) + **D1.6** (site isolation is a floor, not a knob) | One old section became two. |
| `D1.1c` | LRU eviction, no error surface, soft cap with a `+1` hard ceiling | **D1.7** | — |
...
| — *(new, no old number)* | **Tabs stay per agent; the operator's tab is the shared one** | **D1.9a** | Landed 2026-09-01, after the consolidation. It **narrows the write lease** to agent-vs-agent on the operator's shared tab and leaves one question open that belongs to this spec's surface — §12 A-20. |
...
>
> **And the lease is now much smaller than when it was relocated (ADR D1.9a, 2026-09-01).** The operator ruled that **tabs stay per agent** — an agent opening a tab owns it, exactly as today — and that **only a tab the operator opens is shared**, visible to every agent on the workspace and drivable by an agent *on request*. Two agents working on their own tabs therefore **do not contend at all**, and the general "two writers, one browsing context" case the lease was designed for disappears. What is left is **agent-vs-agent on the operator's one shared tab**, which is where D1's lease now lives. D2's obligation is unchanged in shape and narrower in reach: the four action tools take it (FR-040 wires the human-control half; D1 wires the lease itself), and the two recovery/read verbs do not (FR-035, FR-038).

**Out of scope, and why:**
- **All of D1** — ownership, workspace re-keying, live-panel manager resolution, the workspace-less resolution rule, and the pool (cap, eviction, idle close, crash containment). Sibling spec. **Including the `CaptureSharedContext` default**, which revision 3 cited as an open `D1.0a` question: the consolidated ADR's **D1.3** records `CaptureSharedContext: true` as today's shipped default and **D1.4 replaces the isolation primitive outright**, which removes the question rather than answering it. Nothing in D2 depends on either, because the write lease is keyed by whatever browsing-context key D1 lands on and D2 never reads the key's value.
...
| `register.go` `EvaluateTool{executeEnabled: …}` | **read, must NOT be copied — and it is now also a live security question (§12 A-19)** | `browser_evaluate` is registered unconditionally but **runtime-gated** by `sandbox.browser_evaluate_enabled`, independent of tool policy. It is the one place "seventeen registered" is true and misleading at once. **None of the six may replicate this pattern** — see §5 (grill m1). **Re-verified at revision 4, with the fact ADR §6's open question does not carry:** the gate lives at `pkg/tools/browser/tools.go::EvaluateTool.Execute` (`:869-872`, *"This is the SOLE live gate"*), fed at construction from `config.Sandbox.BrowserEvaluateEnabled` (`pkg/config/sandbox.go:230-235`, *"Defaults to false"*, and **`pkg/config/defaults.go` seeds no value for it**, so a fresh install is off). Jim's seed grants `browser_evaluate: allow` (`pkg/coreagent/core.go:1058`) and the **global** seed grants it too (`pkg/config/defaults.go:282`). **So the control that actually disables arbitrary JS is not on the tool-policy surface at all** — an operator reading Agents → Tools, or the seeded policy table below, sees `allow` and sees no sign of the flag holding it shut. |
...
| Jim browser grant (`core.go` `case IDJim:`) | **modifies** | **Eleven** — the only agent granted `browser_evaluate` (`core.go:1058`, inline comment: *"operator-approved for Jim and stays runtime-gated by `sandbox.browser_evaluate_enabled` regardless of policy"*), and the only agent granted `serve_web: allow` (FR-030's whole problem). **⚠️ "Operator-approved" is true and its premise has changed (ADR §6, consolidated grill M11).** The approval was given when a browser was **per agent**. Under **D1.10** the browser is per workspace and carries the operator's live logins for **every site the workspace has visited**, so Jim's arbitrary JS now runs against those sessions. The ADR states this plainly and leaves it open: *"The grant predates the sharing model and has not been re-examined against it."* **This spec does not decide it** — §11(c) and §12 A-4 reason *from* the ten-allow/one-deny carve-out, and **§12 A-19 records that the carve-out's premise moved**. Routed to the operator at §15 item 8. |
...
//   defer release()
//
// SCOPE, AS OF ADR D1.9a (2026-09-01) — the lease is much narrower than when
// it was relocated. Tabs stay PER AGENT: an agent that opens a tab owns it,
// exactly as today. Only a tab the OPERATOR opens is shared with every agent
// on the workspace. So two agents working on their own tabs DO NOT CONTEND,
// and the general "two writers, one browsing context" case the lease was
// designed for is gone. What remains is agent-vs-agent ON THE OPERATOR'S ONE
// SHARED TAB. D2's shape is unchanged; its reach is not. D1's §14 is being
// rescoped in parallel and this comment must not run ahead of it.
//
...
- **D1.9a — tabs stay per agent, and the one thing it leaves open is on D2's surface.** The operator ruled (2026-09-01) that an agent owns the tabs it opens, exactly as today, and that only a tab the **operator** opens is shared with the workspace — visible to every agent on it, drivable by an agent *"on request"*. **What that does to D2:** nothing to the tool surface, and a real narrowing to the contention story (§3, §4). **What it leaves open, and D1.9a says so explicitly:** whether *"on request"* is an **explicit tool** an agent calls, or an **implicit acquisition** on first write to the operator's tab. **That is a tool-surface question, so it is this spec's to answer and the operator's to rule** — §12 A-20 specs both shapes, and §15 item 7 routes it. It is not cosmetic: the explicit shape adds a seventh new tool, which moves this spec's tool count, its Tier-3 fixture literals and its policy-seeding table (Hard Constraint #6 makes the last one a boot-abort risk, not a nicety).
...
- **Jim's `browser_evaluate` — surfaced here, decided nowhere (ADR §6 open question; consolidated grill M11).** Jim is the only agent granted arbitrary JS in the browser (`pkg/coreagent/core.go:1058`). Under **D1.10** that JS runs inside a browser holding the operator's live logins for **every site the workspace has visited**, and the ADR states plainly that *"the grant predates the sharing model and has not been re-examined against it."* **This spec reasons *from* that grant** — §11(c) and §12 A-4 both take the ten-allow/one-deny carve-out as settled precedent for how the roster treats arbitrary-code-adjacent verbs — so if the premise moves, two of this document's posture arguments move with it. **One fact the ADR's open question does not carry, and an operator needs:** the control that actually disables the tool is `sandbox.browser_evaluate_enabled`, read at `EvaluateTool.Execute` (`pkg/tools/browser/tools.go:869-872`), **not** tool policy — it defaults to `false` (`pkg/config/sandbox.go:230-235`, unseeded in `defaults.go`), and both the global seed (`defaults.go:282`) and Jim's seed say `allow`. **So an operator inspecting tool policy sees `allow` and cannot see the flag holding it shut, and an operator who turns the flag on for one reason turns it on for Jim on every workspace at once.** §12 A-19, §15 item 8. **D2 changes nothing here** — no new tool replicates the pattern (§5), and this document does not widen or narrow Jim's grant.
...
- AC5 (**FR-038**): **Given** a human holding the live view, **and** separately another tool holding the D1 write lease **on the operator's shared tab** (the only tab the lease reaches under **D1.9a**), **When** `browser_snapshot` runs, **Then** it returns the snapshot in both cases — it is never `{"deferred": true}`, because it is read-only and mutates nothing.
...
**40 live FRs** (FR-001…FR-041, FR-023 withdrawn), **62 live BDD scenarios** (S-01…S-65, S-52…S-54 withdrawn), **18 live user stories** (US-1…US-19, US-14 withdrawn). *Revision 4 adds FR-039/FR-040/FR-041, S-63/S-64/S-65 and US-19; nothing is renumbered.* The three withdrawn identifiers are tombstoned rather than renumbered — deliberately, so that this document, both grills of it and the D1 spec's §14 cross-reference all keep resolving. A tombstone is not a dangling row: it carries no test **because D2 asserts nothing there**, and it names where the requirement actually lives.
...
| **FR-035** `browser_handle_dialog` exempt from `controlledResult` **and** the D1 write lease — which under **D1.9a** reaches only the operator's shared tab | US-9/AC6-AC7 | S-30, S-31 (a+b) | `TestDialog_RecoversWhileHumanControls`, `_RecoversWhileCDPBlocked` (S-31a, writable now), `_RecoversWhileLeaseHeld` (S-31b, **after D1's `leaseWrite` lands** — only this one tests FR-035) (15) | ADR D2.3 "cannot be left wedged" / r1 C5 / **r2 M7**. **D1 §14.2 rule 3 must carry it — see §15 item 2.** |
...
**Counts, so a reader can check rather than trust:** 41 FR identifiers issued, 1 tombstoned (FR-023) → **40 live**. 65 scenario identifiers issued, 3 tombstoned (S-52…S-54) → **62 live**. 19 US identifiers issued, 1 tombstoned (US-14) → **18 live**. Every live scenario appears in the list above exactly once; every live FR appears in the table above exactly once.
...
- **FR-001 … FR-041** as tabulated in §9 (FR-023 tombstoned). All MUST.
...
**(c) `serve_web: allow` for Ray, Explorer and Researcher is a real posture change, argued (grill M4).** `serve_web` scaffolds and serves a web app from the sandbox. Granting it to three research-tier agents widens what they can do beyond reading. **Why it is nonetheless right:** (i) FR-019's whole purpose is to end #242's dead end, and a pointer to a tool the recipient resolves `deny` for is the same dead end one failed tool call further away; (ii) all three already hold `write_file` or its equivalent within their confinement, so the marginal capability is *serving* an already-writable file over the existing preview listener, not new write reach; (iii) the preview route is already token-authenticated and confined by `gateway.preview_enabled`. **The alternatives considered and rejected:** making the error message conditional on the caller's resolved policy (the `ValidateURL` seam has no access to the calling agent's policy map, so this needs plumbing that D2 does not otherwise justify) and saying "this may not be granted to you" in the message (honest, but leaves the capability gap the fix exists to close). **Note the existing ten-allow/one-deny shape of these agents' grants is the precedent this reasons from** — their browser grant is *not* uniform, it embeds a least-privilege judgement excluding `browser_evaluate`, and this decision makes the same kind of judgement explicitly rather than by group.
  ...
  > **⚠️ The precedent is real; its premise moved, and revision 3 reasoned from it without saying so (consolidated grill M11).** The ten-allow/one-deny carve-out was set when a browser was **per agent**. Under **D1.10** it is per workspace and carries the operator's live logins for every site the workspace has visited, and the ADR's own §6 records that **Jim's `browser_evaluate` grant "predates the sharing model and has not been re-examined against it."** So the carve-out this section and §12 A-4 both reason *from* is itself an open question upstream. **Nothing here decides it, and nothing here depends on the answer in a way that would change a seed value**: if Jim's grant is later revoked, the carve-out becomes *eleven*-deny for arbitrary JS rather than ten-allow/one-deny, and the least-privilege judgement this section applies to the five new verbs is unchanged — it argues from *"none of the five is arbitrary-code-adjacent"*, which is a property of the verbs, not of Jim. Recorded so the dependency is visible: **§12 A-19**, routed at **§15 item 8**.
...
| **A-11** | **The tool count in ADR §4 ("11 → 17").** | **No action *unless A-20 rules Shape A*.** Verified correct as it stands: 11 registered + 6 = 17. Recorded so it is not re-litigated. Note SC-002's caveat about what "registered" means for `browser_evaluate` and `browser_upload_file`. **Revision 4 dependency:** if §12 A-20 is ruled Shape A (an explicit take-control tool), the count becomes **18** and this row, SC-002, §11's seeded table, edit sites 1-7 and the Tier-3 fixture literals all move together. Flagged here rather than left for whoever finds the arithmetic wrong. |
...
| **A-19** | **Whether Jim keeps `browser_evaluate` under the sharing ruling** (new in revision 4; consolidated grill M11, ADR §6). | **OPEN — and deliberately not decided here.** ADR §6 asks it: under **D1.10** Jim's arbitrary JS runs against a browser carrying the operator's live logins for every site the workspace has visited, and *"the grant predates the sharing model and has not been re-examined against it."* **Why it is on this spec's map at all:** §11(c) and §12 A-4 both reason *from* the ten-allow/one-deny carve-out as settled precedent, so a reader is entitled to know the premise is under review. **One fact this spec adds that the ADR's question does not carry, and an operator needs before ruling:** the control that actually disables arbitrary JS is **not on the tool-policy surface**. `browser_evaluate` is registered unconditionally and gated at `EvaluateTool.Execute` (`pkg/tools/browser/tools.go:869-872`, *"This is the SOLE live gate"*) by `sandbox.browser_evaluate_enabled` — default `false` (`pkg/config/sandbox.go:230-235`; `pkg/config/defaults.go` seeds no value), while both the global policy seed (`defaults.go:282`) and Jim's seed (`core.go:1058`) say `allow`. **So an operator inspecting tool policy sees `allow` and no sign of the flag holding it shut; and an operator who enables the flag for any reason enables it for Jim on every workspace at once.** **D2 changes nothing here** — no new tool replicates the pattern (§5), and this spec neither widens nor narrows the grant. **Whose call:** the ADR's, as its own open question. Routed at §15 item 8. |
| **A-20** | **What *"take control on request"* looks like to an agent** (new in revision 4; ADR **D1.9a**, explicitly left open there). | **OPEN — needs a ruling before Stream B or E1 opens, because the two shapes differ in tool count.** D1.9a rules that an agent may drive the **operator's** shared tab *"on request"* and does not say what the request is. **Shape A — an explicit tool** (e.g. `browser_take_tab`). Reads as what the ruling says; makes the acquisition an auditable, deniable, per-agent event; and an agent that never calls it can never touch the operator's tab. **Cost, and it is not small for this document:** a **seventh** new tool. Surface growth becomes 11 → **18**, not 17 (§12 A-11 and SC-002 both move); it needs an explicit, literal policy entry for **every** seeded agent or **boot aborts** (Hard Constraint #6, §11's table and edit sites 1-7 all grow by a row); the Tier-3 fixture goes 62 → **69** and the arithmetic literal 87 → **94** (edit sites 9-10); and it needs its own FR, US, scenario and test. **Shape B — implicit acquisition on first write.** Zero new tools, zero seeding, zero fixture churn; the first leased write to the operator's tab acquires control. **Cost:** the grant becomes invisible — there is no moment an operator can deny, no policy entry to inspect, and "the agent took the wheel" is indistinguishable from "the agent did its job", which is the shape §1's own problem statement is about. **This spec does not choose.** The ruling's wording (*"take control **on request**"*) reads as Shape A; the ADR states the mechanism is a D2 tool-surface decision and belongs in its §6 until ruled. §15 item 7. |
...
**Three genuinely open rulings, up from one** — items **1**, **7** and **8**. Numbered 1–10 across all categories so nothing is lost between them; item 3 is now closed and retained as a closed row rather than deleted.

### Open — needs a decision before the affected work starts

1. **§12 A-3 — `browser_snapshot`'s manifest tier.** *(Revision 3 called this "THE one open item". Revision 4 adds two more — items 7 and 8 — so it is now one of three.)* Option A (Tier 3, zero production edits, but the "default way to read a page" is search-only) versus Option B (Tier 2, one production edit, one preview line's tokens on **every** turn, and an amendment to ADR-071's deliberately-sized previewed set). **Recommendation: Option B.**
...
**7. §12 A-20 — what *"take control on request"* is, in the tool surface** (new in revision 4). ADR **D1.9a** rules that tabs stay per agent and that an agent may drive the **operator's** shared tab *"on request"*, and explicitly leaves the mechanism open, calling it a D2 tool-surface decision. **Shape A — an explicit tool** (`browser_take_tab` or similar): matches the ruling's wording, makes the acquisition auditable and deniable per agent, and an agent that never calls it can never touch the operator's tab. **Shape B — implicit acquisition on first write** to that tab: no new tool, no seeding, no fixture churn — and no moment an operator can deny, no policy entry to inspect, and no way to tell "the agent took the wheel" from "the agent did its job".
   - **Why this cannot wait for implementation.** Shape A adds a **seventh** new tool, and in this codebase a tool is never just a tool: it needs an explicit, literal, wildcard-free policy entry for **every** seeded agent or **boot aborts** (Hard Constraint #6). Tool count 11 → **18**; §11's seeded table and edit sites 1-7 each grow a row; the Tier-3 fixture goes 62 → **69** and the arithmetic literal 87 → **94**; SC-002 and §12 A-11 both restate. Shape B changes none of it. **Deciding this after Stream E1 lands means redoing Stream E1.**
   - **This spec does not choose**, deliberately. The ruling's own words read as Shape A; both shapes are specced at A-20 so the ruling can be applied rather than re-derived.

**8. §12 A-19 — does Jim keep `browser_evaluate` under the sharing ruling?** (new in revision 4). **Strictly the ADR's question**, carried in its §6, and routed here because this document reasons from the carve-out it belongs to (§11(c), §12 A-4) and because **D2 supplies the fact the ADR's question is missing**: the control that actually disables the tool is `sandbox.browser_evaluate_enabled`, checked at `EvaluateTool.Execute` — **not** tool policy — so an operator inspecting policy sees `allow` and cannot see the flag holding it shut, and enabling that flag for any reason enables Jim's arbitrary JS against every workspace's signed-in browser at once.
   - **What it blocks: nothing in this spec.** No D2 FR, seed value or test changes on either answer; §11(c)'s argument is about the five new verbs and survives a revocation intact. Routed because a decision taken elsewhere, on incomplete facts, would be worse than one taken here with them.
...
2. **D1 §14.2 rule 3's exemption set is D1's to settle, and it is not settled.** *(Rewritten in revision 3 — round-2 **M1** was right that revision 2's version was stale, and the correct disposition is "downgraded", not "closed".)*
   ...
   **Re-verified against D1 at HEAD for revision 4: two of the three defects are FIXED, one survives, and a new obligation is added by D1.9a.**
   ...
   - **(c) D1 cites the wrong D2 requirement for the snapshot — STILL OPEN.** D1's US-9/AC4 still reads *"`browser_snapshot` (read-only, **D2 FR-018**)"* — which is the *values-by-default* requirement and says nothing about the lease. **Revision 3 created FR-038 precisely so there is a correct citation, and D1 has not taken it.**
     > **⚠️ Disambiguation, because these two documents have collided on a number.** **D1 has its own FR-038** — a `max_browsers` cap on the browser pool, unrelated to anything here. The citation D1 needs is therefore **"D2 FR-038"**, written with the document prefix, exactly as it already writes "D2 FR-035". A bare `FR-038` inside D1 means D1's own cap requirement and would be actively misleading. **This spec does not renumber to dodge the collision** — FR numbers here are cited by both grills and by D1, and the no-renumbering rule is what keeps every cross-reference resolving; the prefix is the fix.
   - **(d) NEW — D1 §14 must be rescoped to what D1.9a left of the lease.** The operator ruled on 2026-09-01 that **tabs stay per agent** and only the **operator's** tab is shared, so two agents on their own tabs do not contend and the lease engages **only on that one shared tab**. D1's §14 is written for the general "two writers, one browsing context" case that ruling removes. **D2 has already narrowed its own wording** (§3, §4, FR-035, FR-038, US-9/AC7, US-10/AC5, holdout 12) and must not run ahead of D1's revision. The ADR itself says the rescope is *"a much smaller change than either answer implied"* — the retry-vs-defer question D1 was answering now applies to one tab, not to every action tool.

   **Required of D1, then:** re-point the snapshot citation at **D2 FR-038**, and rescope §14 to the operator's shared tab per D1.9a. **D2's obligation is now complete on both sides of the boundary** — it states both exemptions (FR-035, FR-038) and, new in revision 4, **wires and tests the four inclusions (FR-040)**, which is what D1's own registry-driven test 18 reads to decide membership. *(Round-2 M7's ordering constraint is separate and lives in §3's Parallelization: D1's `leaseWrite` must exist before S-31b can be written, and S-31a covers the wedge-recovery half meanwhile.)*
...
**10. ADR-072 needs three edits this spec cannot make** (§0c, edit site 21). Two are **discharged** (the `types.go` line number; the D2.9 Mia/Ava cell). **Two are outstanding:** *(a)* **§3.3 criterion 22 and the §4 bullet still assert the ≤150 ms p95 budget SC-004 withdrew in full**, so an implementer reading the ADR will build a gate this spec has no test for; *(b)* **D2.11's "a `browser_snapshot` never appears there" is right for a parent-turn snapshot and wrong for a delegated one**. **And one structural edit that makes the others trackable: an ADR §9.5 for this document.** ADR §9 lists the D1 spec, #509, ADR-048 and root `CLAUDE.md` — **not this spec** — so every conflict recorded in §0c is currently invisible to an ADR reader. **All ADR owner's calls.**

---

**Next:** re-grill this revision; rule on **A-3** (routing it to ADR-071's ratifier if that is not the same person) and on **A-20** — **A-20 first**, because Shape A adds a seventh tool and therefore redoes Stream E1 if it arrives late. Then implement: **Stream E1 first** (names + policy, no behaviour, unblocked **once A-20 is ruled**), then A as critical path, then B/C/D/F in parallel, **E2 once A-3 is ruled**, then the 7-reviewer gate, then SC-001 as the headline demonstration (part 2 blocked on #659). **A-19 blocks nothing here** and is routed to the ADR.

**Four things a reviewer of revision 5 should check first**, because they are where revision 4 spent its judgement rather than merely its edits:
...
3. **§12 A-20** — the two shapes of *"take control on request"* differ by a whole tool, and this document deliberately declines to choose. Check that the cost side of Shape A is complete: if a seventh tool needs anything beyond a policy entry per agent, a metadata instance, a catalog name and two fixture literals, the estimate is wrong and Stream E1's ordering advice with it.
