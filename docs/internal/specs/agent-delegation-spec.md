...
- What happens when a child delegates to a grandchild (nested delegation)? Expected (resolved 2026-07-04, correcting an earlier mischaracterization): nested delegation already happens in production today — there is no "one level only" guard; the real limit is `SubTurn.MaxDepth` (`pkg/agent/subturn.go`, default 3, operator-configurable), which already permits multi-level chains. Grant inheritance MUST flow transitively down the chain: a grant the grandparent held is visible to the grandchild, not just the immediate parent. This already falls out of `Inherit`'s existing copy-at-spawn semantics (each `spawnSubTurn` call copies whatever the immediate parent's bucket *currently* holds — including anything the parent itself inherited — into the child's bucket), so no new inheritance logic is required; this spec adds the dedicated 3-level test (FR-D8) that was previously missing to prove it. **Re-verified 2026-07-04 (7-reviewer gate MAJ-001/MAJ-004 — see Clarifications):** both the "default 3" claim and the "no new logic required" claim were independently re-checked against the real code (not just re-asserted) and confirmed correct.
...
- Q: Should nested delegation (grandparent → parent → child) inherit grants transitively? -> A: Yes — in scope. Correction to an earlier assumption: there is no "one-level-only" delegation guard; nested delegation already happens today, bounded by `SubTurn.MaxDepth` (default 3, operator-configurable). Transitivity already falls out of `Inherit`'s existing copy-at-spawn semantics with no new inheritance code needed — this spec adds the missing dedicated test (FR-D8) proving it holds across a real three-level chain.
