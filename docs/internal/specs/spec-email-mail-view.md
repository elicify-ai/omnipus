...
| D35 | UI design step (founder) | Wave 1: frontend-lead builds the Mail panel, draft preview/edit, compose and signature editor as design-system stories with sample data (no backend), in the Library's look; founder gets screenshots (dark theme, desktop + narrow) plus a usability-heuristics review; founder approves or adjusts before the real frontend build | Founder chose recommended | Founder | 2026-09-26 |
| D36 | End-to-end tests (founder) | Playwright E2E runs Omnipus against a built-in fake IMAP/SMTP server so every main mail flow (read, edit agent draft, send with attachment, signature, Sent copy) is covered in a real browser on every CI run | Founder chose recommended | Founder | 2026-09-26 |
| D37 | UAT (founder, revised) | UAT runs on a separate instance built from the feature branch against a **local GreenMail test mail server in Docker** (real IMAP/SMTP, several test accounts that mail each other) — no live passwords, no overlap with the live instance; 2 uat-tester lanes + 2 independent uat-validators. After landing, one short live smoke test with Dmitri and Aisha on the live instance covers provider-specific behaviour | Founder: "can you use the local imap server for testing"; team-lead proposed GreenMail + live smoke | Founder | 2026-09-26 |
| D38 | Agent reads and read state (founder) | An agent reading a message with `read_message` marks it read (the mailbox belongs to that agent). The Mail panel shows a small **"read by agent"** tag on messages the agent read, so the human still sees what it handled (spec to define the mechanism, e.g. an IMAP keyword set on agent read, with graceful fallback when a server rejects keywords). Nothing is ever marked read automatically; a human opening a message in the panel marks it read | Founder chose recommended | Founder | 2026-09-26 |
