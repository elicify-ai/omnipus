...
- **Anthropic (the company)** keeps `id: anthropic`, `wire: anthropic`, no toggle. **Qwen Coding Plan** stays as-is (its only endpoint is anthropic-wire; `id: coding-plan-anthropic` — verify against catalog.go; if an openai-wire `coding-plan` protocol exists with a valid base, prefer it as primary with `anthropic_id: coding-plan-anthropic`).
