...
- **Task selection**: expands recurring-capable triggers **only for tasks the scheduler would arm** — the same predicate `OnTaskUpserted` applies before registering a job (`pkg/agent/task_trigger.go`, `task.Task.SeriesRetired`): not `surface: heartbeat`, AND (non-terminal OR terminal with a REPEATING trigger — `recurring`/`every` — whose series has not exhausted). A per-run `done`/`failed` status does not end a repeating series — the scheduler keeps such a task's next occurrence armed exactly as it would a non-terminal one (`task_trigger.go`'s `OnTaskUpserted`), so the calendar must keep rendering it too. Only a genuinely non-repeating trigger (`once`/manual) is omitted the instant it goes terminal; a truly exhausted RRULE series (COUNT/UNTIL) naturally yields zero occurrences from the expansion itself, not via this selection predicate. Heartbeat-surface tasks are omitted entirely regardless of status — the calendar never renders occurrences that will not fire. Defense-in-depth: the client drops any occurrence set whose `task_id` is absent from its fetched task list.
...
6. **Crash recovery & observability**: `Reconcile()` at boot re-arms the next future occurrence for every RRULE task `OnTaskUpserted` would arm — non-terminal, OR terminal-but-repeating with a non-exhausted series (see "Task selection" above) — this is the documented recovery for the crash window between fire and re-arm. In addition, a recovery sweep runs on a **dedicated ticker inside `TaskTriggerScheduler`, every 5 minutes** (no "existing maintenance cadence" exists to reuse — `Reconcile` is boot-only and `pkg/cron` has only the due-job tick loop). The sweep's predicate defines **armed** precisely: a registered job exists **and** (its `State.NextRunAtMS` is non-nil and ≥ now, **or** `State.Running` is true). The `Running` disjunct is load-bearing: `RunDueJobs` clears `NextRunAtMS` **before** dispatch (`pkg/cron/service.go:515-519`) and `State.Running` is set for the duration of the fire (`:613`) — so an **in-flight fire** (job present, `NextRunAtMS` nil, `Running` true, exit-path re-arm pending) is armed and MUST NOT be re-armed by the sweep, while the dead-entry shape the engine is known to produce (`rescheduleSkippedUnsafe` leaves an `at`-job entry with `NextRunAtMS` cleared and `Running` false) is a true orphan. Terminal-but-repeating is itself a CANDIDATE for the sweep, not a skip — a done/failed recurring task whose job was lost (e.g. a crash between `SpawnReset` and the terminal write landing) must be rescued exactly like a non-terminal one. Any task (terminal-but-repeating or non-terminal), non-exhausted, failing the predicate is re-armed (replace-by-task, rule 4) with a WARN — a recurring series must never be silently dead until restart. Exhaustion (COUNT/UNTIL) retires the job silently and legitimately, and is never resurrected by the sweep.
7. **`OnTaskUpserted` idempotency guard (7-reviewer gate on 1b3d4b3d)**: making a terminal-but-repeating task fall through to (re)registration is not itself sufficient — the registration must be a NO-OP when a job is already tracked for the task, its generation is unchanged, AND it is currently armed. Without this guard, a LATER completion notification (the async run's terminal status landing well after the fire's own exit-path re-arm already installed the correct job) would unconditionally replace an already-correct job: for `every`, re-anchoring `NextRunAtMS` to the completion's wall-clock time instead of the cron engine's own fire-time anchor (drift, compounding every cycle by however long the run took); for `recurring`, risking a silent occurrence skip if the replace computed a later "next after now" than the one already armed. Replacement still happens, correctly, when there is no live armed job (first registration, or boot `Reconcile` of a terminal-repeating task with a lost job) or the generation changed (a genuine trigger edit).
8. **Legacy invariance**: the `at`/`every`/`cron` translation for non-RRULE triggers is byte-for-byte unchanged (regression-tested; guards heartbeats).
