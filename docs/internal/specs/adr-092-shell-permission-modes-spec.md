...
**FR-034 — God Mode banner is a relocation, not a build (resolves M-12).** `GodModeActiveBanner` (`GodModeControl.tsx`) already exists, already red, already has a fetch-failure-safe `god-mode-status-unknown-banner` variant, and is already rendered at `GatewaySection.tsx:298`. This FR is: **move it to `AppShell`, app-wide**; preserve the fetch-failure `isError` state exactly (a naive move risks dropping it); correct its body text, which today says the "shell guard" is disabled — stale even before D6, doubly stale after (D6/FR-031 already corrects the Go comment; this FR extends that correction to this UI string). *[Amended 2026-09-25, founder decision: the app-wide banner is **removed**.]* *[Amended again 2026-09-25, founder decision — final indicator ruling, superseding the earlier amber suggestion: the God Mode indicators are RED when god-mode is on and INVISIBLE in every other state (off, still loading, unknown, fetch error, dev-mode bypass); there is no amber or "unknown" variant anywhere. Two indicators: a small red "God Mode" pill in the sidebar brand row (`layout/GodModeIndicators.tsx` — catalogued Badge, `error` variant, Phosphor `ShieldWarning`) and ONE corner dot (`god-mode-corner-dot`) rendered from the `AppShell` SHELL ROOT (outside `<main>`), at the left edge just below the 44px chrome-header band — clear of the sidebar-open hamburger's hit area, and above phone-width takeover panels that collapse and inert `<main>` — shown only while god-mode is on AND the sidebar is hidden, so every route is covered (Library, live-browser, admin chat have no sidebar button). The per-hamburger dots are deleted. Clicking either navigates to Settings → Gateway scrolled to and focused on the God Mode control (`?focus=god-mode`), including when /settings is already open on another tab; `?focus=god-mode` is one-shot — stripped from the URL after the first focus so a later Gateway-tab remount cannot re-steal focus. The existing chat-header `ChatModeBadge` is unchanged; Settings → Gateway `GodModeControl`'s toggle behavior is unchanged (the focus/scroll targeting is new). A god-mode fetch failure renders nothing, with no surface anywhere — the AppShell gateway-state fetch-error banner covers APP-STATE fetch failures only; under `dev_mode_bypass` the indicators render nothing and the god-mode query never fires.]*
