...
- **FR-084**: **Synthetic-error turn-abort floor** (resolves MIN-004). After **N consecutive synthetic-deny tool results within a single turn (N=8 default, configurable as `gateway.turn_synthetic_error_floor`)**, the loop MUST abort the turn with a system message `{role: "system", type: "turn_aborted", reason: "synthetic_error_loop"}` and emit audit `turn.aborted_synthetic_loop`. Counter resets per turn.
  > **SUPERSEDED by ADR-058** (`docs/internal/architecture/ADR-058-tool-denial-semantics.md`; `docs/internal/specs/adr-058-tool-denial-semantics-spec.md` FR-058-14). FR-084 is deleted in full — `gateway.turn_synthetic_error_floor`, `config.GatewayConfig.TurnSyntheticErrorFloor`, `defaultSyntheticErrorFloor`, `AgentLoop.syntheticErrorFloor`, `AgentLoop.recordSyntheticDeny`, `turnState.syntheticErrorCount`, all four call sites, the `synthetic_error_loop` abort stage, and `audit.EventTurnAbortedSyntheticLoop` no longer exist in the codebase (verified: `pkg/agent/loop_tool_denial_test.go`'s `TestADR058_FR084Identifiers_ResolveNowhereInSource` greps for exactly these identifiers and asserts zero hits outside a narrow historical-prose exception list). ADR-058's quarantine-at-first-permanent-denial design — an unconditional per-tool short-circuit after the first PERMANENT denial, backed by an aggregate per-turn denial budget of 10 (`turnDenialBudget`, `pkg/agent/tool_denial.go`) — made this floor's "N consecutive synthetic denials" counter a second, competing termination mechanism that could disagree with the first (spec §2's D4/FR-084 conflict); the amended decision deletes the competitor rather than reconciling the two. This bullet, the FR-084 row in the Traceability Matrix below, and the MIN-004 grill-finding entry are retained as historical record of the requirement that shipped and was later removed — do not implement against them.
...
| FR-084 | (Behavioral) | (Behavioral / synthetic-error floor) | TestAgentLoop_SyntheticErrorFloor_AbortsTurn (new integration) — **SUPERSEDED by ADR-058**: FR-084 deleted in full, this test was never implemented against it; see FR-084's entry above |
...
- **MIN-004**: Synthetic-error turn-abort floor (FR-084, default N=8). **SUPERSEDED by ADR-058** — FR-084 is deleted in full; see FR-084's entry in the Revision-6 additions above for the amended decision and rationale.
