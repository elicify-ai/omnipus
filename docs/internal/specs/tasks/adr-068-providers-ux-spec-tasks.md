...
- **FRs:** FR-037 (file + emitter deleted), FR-039 (`ProviderCatalogEntry.yaml` description — if not already removed by S67), FR-031 (SPA half of the `refresh-models` → entitlement move; wiring lands in T068-27), FR-011a (SPA fixtures).
...
### T068-14 — `codex-cli` (subprocess) and `openai-chatgpt` (saved-token HTTP): factory dispatch, sign-in start/status routes, expiry rule — **P0**
- **Files:** modify `pkg/providers/factory_provider.go` (under S67's protocol dispatch: `cli` + `cli_kind: codex` → `NewCodexCliProvider`; `openai-chatgpt` = `openai-compatible` with `NewCodexProviderWithTokenSource(CreateCodexCliTokenSource())`, no `cli_kind`), `pkg/providers/codex_cli_credentials.go` (`ReadCodexCliCredentials`: read-only, `account_id`, unverified JWT `exp` decode for display, else mtime + 1 h), `pkg/providers/factory_test.go`, `pkg/gateway/rest.go` (`POST /providers/{id}/sign-in` → `{method:"cli_login", command:"codex login", instructions}`; 400 `provider does not support sign-in` when `auth_methods` lacks `sign_in`; `GET /providers/{id}/sign-in/status` → `not_signed_in | signed_in | expired` + `account_label`/`expires_at`; GET `/providers` rows use it for `status`/`account_label`), `pkg/gateway/rest_sign_in_test.go` (new).
- **FRs:** FR-003 (construction half), FR-004 (catalog declares `auth_methods` — S67 data; this task consumes it), FR-007, FR-008, FR-009, FR-034 (backend states).
- **BDD:** OpenAI sign-in offers two named providers (persisted id `codex-cli` — server accepts both ids, no other spelling); Signed-in row shows account and Manage (server half incl. file bytes/mtime unchanged); Expired session routes to re-sign-in (server half: no refresh request leaves the gateway); Sign-in refused for a key-only provider; OpenAI device-code flow leaves no trace (`sign-in` response shape); Dataset "Sign-in status" rows 1–8.
- **Tests first:** TDD rows 4 `TestFactory_CliKindDispatch`, 15b `TestSignInStart_CLILoginOnly`, 20 `TestSignIn_RefusedForKeyOnly400`, 21 `TestSignInStatus_CodexCLI` (fresh `auth.json` → `signed_in` + `account_label`; JWT `exp` past → `expired`; stale mtime without JWT → `expired`; missing binary → `disconnected`; bytes + mtime unchanged; no outbound refresh).
- **Gate:** `go-test` (scope `./pkg/providers/ -run 'TestFactory_CliKindDispatch' ` then `./pkg/gateway/ -run 'TestSignIn'`).
- **Depends-on:** `T067-FACTORY` (the `cli`/`cli_kind` dispatch shape), T068-03, T068-06, T068-08 (row fields).
- **Size:** L.
- **DoD:** both ids construct the right type; status never writes `auth.json`; `refresh_token` is not on the wire or in any struct.
...
- **FRs:** FR-005 (sign-in control only where `auth_methods` has `sign_in`, pre-selected), FR-006 (`openai-chatgpt` label *"relies on OpenAI's stated tolerance, not its written terms"*; `codex-cli` default radio), FR-027 (copy *"Detected: <Region> — change"* / *"Region — change"*), FR-028 (control inside the panel; no extra step), Qualitative prohibitions (no Anthropic/Google sign-in anywhere; xAI key-only, no forward-looking copy).
...
- **Depends-on:** T068-20.
- **Size:** M.
- **DoD:** Anthropic/Google/xAI panels contain no sign-in control in the DOM; saving with OpenAI sign-in untouched persists `codex-cli`.
...
- *Backend, after T068-07/08:* T068-09 ∥ T068-11 ∥ T068-12 ∥ T068-13 (T068-09 and T068-11 share the `new_default` validator — one owner, named at integration); T068-10 after T068-09; T068-14 ∥ T068-13 once `T067-FACTORY` is in; T068-15 and T068-16 after T068-14; T068-17 last (needs `T067-ENTITLEMENT`).
