...
- **Amendment 2026-08-23 (A-CONTRACT):** commit `36801b44` already carries `ToolCall.content_state`, the `tool_result_projection` frame (`ToolResultProjectionFrame.yaml`), `ContextSettings*.yaml` (field names: `mcp_result_cap`, `builtin_success_cap`, `builtin_failure_cap`, `absolute_trigger_chars`, `ingest_bound_bytes`, `default_context_window`, `model_overrides`), `ContextWindowSource.yaml`, the `Agent.yaml`/`AgentUpdateRequest.yaml` window fields and the four `LLMError` copies with the shipped copy (spec §1.1). **Not carried, and therefore this task's remaining scope:** the ADR-060-family **recall-mark** and **argument-refusal** inline asyncapi schemas + their two `scripts/check-no-handwritten-wire-types.sh` register entries + `contract_test.go` rows. They ride in this follow-up contract commit (spec + generated diff atomic), never in T066-04's feature code. Note oapi-codegen emits per-parent `ContextWindowSource` copies (`AgentContextWindowSource`, …) — convert at the boundary.
