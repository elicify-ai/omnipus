...
| **MIN-108** | Inconsistency (source ADR) | ADR-072 D1.3 vs spec §2.1 | ADR-072 D1.3's key table still reads: *"Transcript session · `tools.ToolTranscriptSessionID(ctx)` · … · Used for: **Unattended delegated work — D1.2**"* — residue of the design D1.2 deleted. The spec correctly marks it "not used" (§2.1), so the ADR and its spec now disagree, and the ADR is the source of record. | Amend ADR-072 D1.3's table row to say the transcript session id is **not** a browsing key, per the superseding D1.2 ruling. |
