...
- **And** a local model whose projection has `window_unknown: true` renders *No context length* with a link whose href is the Settings → Context → Model overrides route pre-filled with `<provider>`/`<model>` (X-08)
...
| X-08 | Confirmed | **Applied (coordinator):** per-model `window_unknown: true` rendered on card and row expand with the Settings → Context → Model overrides pointer; status stays six. |
...
| X-37 (A68) | Confirmed | **Applied in ADR-068 §4/§9:** "Settings → Context" canonical; default-model control on Settings → Providers only. |
