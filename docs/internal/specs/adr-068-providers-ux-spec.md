...
## Amendment 2026-08-23 (A-CONTRACT) — BINDING on the points below

The A-CONTRACT commit (`36801b44`, ADR-067 T067-01) is the shipped shared contract. Where an older sentence in this document disagrees, this section wins.

- **`Agent.degraded_reason` / `needs_model` — exact wire shape (ADR-067 FR-024 is authoritative):** `Agent.degraded_reason` is an **optional string enum with the single value `needs_provider`** (ADR-067's field); `Agent.needs_model` is a **separate, required boolean**, derived by this spec's FR-014 rule. Both may be true; the `needs_provider` copy wins (FR-014, resolution #5). No combined enum, no `degraded` boolean.
- **`ProviderDeleteRequest.yaml`, `ProviderDeleteResponse.yaml`, `ProviderDependent.yaml`, `DefaultModelUpdateRequest.yaml` are already present** — written in the A-CONTRACT commit because `Provider.dependents` `$ref`s `ProviderDependent`. FR-035 no longer lists them as this spec's own files to create; T068-06 creates only `SignInStartResponse.yaml`, `SignInStatus.yaml`, `OnboardingProviderApiKey.yaml`, `OnboardingProviderSignIn.yaml` and its `openapi.yaml` paths (`DELETE /providers/{id}` and `GET/PUT /providers/default-model` also already exist — verify against `contracts/openapi.yaml` before editing).
- **FR-039 is satisfied:** `ProviderCatalogEntry.yaml`'s "never served from a live HTTP endpoint" description was replaced in A-CONTRACT; the file itself is deleted later by ADR-067 T067-02 / T067-13 once `catalog.go` and the 7 SPA importers stop using it (X-23).
- **`refresh-models`:** no such route ever existed in `openapi.yaml`; the Go handler and the SPA wrapper `refreshProviderModels` are removed by ADR-067 T067-10/T067-12 and this spec's T068-05 (X-23) respectively.
- **Providers-catalog envelope:** the origin marker is `served_from ∈ {embedded, pulled}` plus `stale`; the document's own `source` is free text. Use these names in the SPA catalog client (T068-18).
...
- **FR-006** *(amended 2026-08-23, ADR-068 §8b)*: The OpenAI pair MUST be offered as `openai-chatgpt` (**default** — in-app sign-in, no CLI needed; helper line *"Uses your ChatGPT plan's included usage"*) and `codex-cli` (helper line *"Drives the official Codex app; sign in inside it"*). The former *"relies on OpenAI's stated tolerance"* label MUST NOT appear.
- **FR-007** *(amended 2026-08-23)*: For `codex-cli` Omnipus MUST NOT write, refresh or proxy the vendor credential file. For `openai-chatgpt` the credential is **Omnipus's own**: the access + refresh token obtained by FR-044 MUST be stored only in the encrypted credential store under `openai_OAUTH` (never in `config.json`, never in `~/.codex/auth.json`); Omnipus MUST refresh it itself (FR-046). Reading `~/.codex/auth.json` MUST remain available only as the explicit **Import** action (FR-047), read-only: `tokens.access_token` + `tokens.account_id` only, bytes and mtime unchanged.
- **FR-008** *(amended 2026-08-23)*: `POST /providers/{id}/sign-in` MUST return 400 `{"error":"provider does not support sign-in"}` for providers whose catalog row lacks `sign_in`; MUST be rate-limited like the auth endpoints; and MUST return `{method:"cli_login", command, instructions}` for `codex-cli` / `github-copilot` and `{method:"device_code", verification_url, user_code, device_auth_id, expires_at, interval_seconds}` for `openai-chatgpt` and (once `sign_in` is on its row) `xai`. `device_auth_id` is an opaque server-side handle; the vendor's device code and any PKCE verifier MUST NOT leave the gateway.
- **FR-009** *(amended 2026-08-23)*: `GET /providers/{id}/sign-in/status` MUST return one of `not_signed_in | pending | signed_in | expired` with `account_label` and `expires_at` when known. For `device_code` providers the state comes from the stored `<id>_OAUTH` entry (`expired` = access token past `exp` AND refresh failed/absent); for `cli_login` providers the rule of the previous text applies (JWT `exp` unverified, else the 1 h file-age rule). `pending` is returned only while a device-code session started by FR-008 is open and unapproved.

**D13a — in-app device-code sign-in (ADR-068 §8b, added 2026-08-23)**
- **FR-044**: `POST /providers/{id}/sign-in/poll` with `{device_auth_id}` MUST perform at most one vendor poll per call and return `{state: pending | signed_in | expired | denied}`; on `signed_in` the gateway MUST have stored `<id>_OAUTH` before responding. Unknown or expired `device_auth_id` → 404. A device-code session MUST expire server-side at the vendor's `expires_at` (ceiling 15 min) and MUST be single-use.
- **FR-045**: The SPA sign-in dialog (Settings → Providers row and onboarding step 3, same component) MUST show the verification link (opens in a new tab, `rel="noopener"`), the user code in a copyable, visually prominent field, a live status line, and MUST poll `/sign-in/poll` at `interval_seconds` (never faster; back off on `slow_down`) until `signed_in | expired | denied`, then render the matching end state with a *Try again* for the two failures. Closing the dialog MUST stop polling. The dialog MUST meet the spec's 11 WCAG constraints (focus trap, Escape closes, code announced via `aria-live="polite"`).
- **FR-046**: The `openai-chatgpt` token source MUST read `openai_OAUTH` from the credential store and, when the access token is within 5 min of `exp` or a request returns 401, refresh it once with the stored refresh token and persist the result; a failed refresh MUST surface as `LLMError` code `needs_provider` with attribution `user` and message *"Your ChatGPT sign-in expired — sign in again under Settings → Providers"* (never a silent turn exit, ADR-066 D7).
- **FR-047**: `POST /providers/openai-chatgpt/sign-in/import` MUST copy `tokens.access_token` + `tokens.account_id` from `~/.codex/auth.json` into `openai_OAUTH` (no refresh token is imported; such a session ends at `exp`) and MUST leave the file's bytes and mtime unchanged; 404 `{"error":"no codex login found"}` when absent. The SPA offers it as a secondary *Use my existing Codex login* link in the dialog only when the server reports the file exists.
- **FR-048**: `DELETE /providers/{id}/sign-in` (sign out) MUST delete `<id>_OAUTH` (NotFound = success), emit audit `provider.signed_out`, and the row MUST return to `not_signed_in`. `DELETE /providers/{id}` step 3 MUST delete `<id>_OAUTH` alongside `<id>_API_KEY`.
- **FR-049**: The identical dialog, routes and token source MUST work for `xai` when its catalog row carries `sign_in`; the xAI device-code endpoints and client id MUST be configuration (`OMNIPUS_XAI_CLIENT_ID` or the catalog row's `oauth` block), never code constants, and with no client id configured the row MUST stay key-only with no sign-in control in the DOM. Anthropic and Google rows MUST never gain `sign_in` (Qualitative prohibition; ADR-068 §8b decision 4).
- **FR-050** *(added 2026-08-24, T068-14 implementation finding)*: `POST /providers/{id}/sign-in`, `GET .../sign-in/status`, `POST .../sign-in/poll`, `POST /providers/openai-chatgpt/sign-in/import`, and `DELETE .../sign-in` MUST be reachable without an authenticated session ONLY while onboarding is incomplete (`onboardingMgr.IsComplete() == false`) — the same `onboardingDone` gate `POST /providers/{id}/test` already uses, and the same reasoning CLAUDE.md documents for bare `GET /providers` staying pre-auth (onboarding step 3, FR-036, needs a working sign-in flow before any admin account exists to authenticate as). Once onboarding completes, all five routes revert to the contract's normal `adminWrap` posture (401 unauthenticated, 503 under dev-mode bypass) — a post-onboarding caller with no session cannot start or complete a sign-in, poll an open session, import a Codex login, or sign a provider out.
...
**§6/§7 — wire**
- **FR-035**: Every new or changed wire shape MUST be defined in `contracts/` first and consumed only via generated types; `make verify-contracts` MUST pass. **Ownership (X-26): S67 owns and commits the contract FILE edit for every shared schema** — `Provider.yaml`, `Agent.yaml`, `ProbeProviderRequest.yaml`, `DefaultModel.yaml`, `EntitlementResponse.yaml`, the `LLMError` set in all four copies, and the `pkg/gateway/inboundschemas/` copies (cross-spec Q1) — including the values this spec defines (`signed_in`, `expired`, `needs_model`, `model_unassigned`); this spec owns their semantics and copy and consumes generated types only; handlers that cannot yet compute a required field emit its zero value. **S67 lands first, then this spec.** This spec's own (unshared) schemas it still writes: `SignInStartResponse.yaml`, `SignInStatus.yaml`, `OnboardingProviderApiKey.yaml` + `OnboardingProviderSignIn.yaml`, and the `openapi.yaml` paths it adds — **Amendment 2026-08-23 (A-CONTRACT):** `ProviderDeleteRequest.yaml`, `ProviderDeleteResponse.yaml`, `ProviderDependent.yaml` and `DefaultModelUpdateRequest.yaml` were written in the A-CONTRACT commit (`36801b44`; shared, because `Provider.dependents` `$ref`s `ProviderDependent`) and are **already present** — this spec consumes their generated types and does not recreate them. *(Superseded list kept for traceability:* `ProviderDeleteRequest.yaml`, `ProviderDeleteResponse.yaml`, `ProviderDependent.yaml`, `DefaultModelUpdateRequest.yaml`, `SignInStartResponse.yaml`, `SignInStatus.yaml`, `OnboardingProviderApiKey.yaml` + `OnboardingProviderSignIn.yaml`, and the `openapi.yaml` paths it adds.*) The complete list (MAJ-014; items already shipped by A-CONTRACT are marked †): `ProviderDeleteRequest.yaml`†, `ProviderDeleteResponse.yaml`†, `ProviderDependent.yaml`†, `DefaultModel.yaml`†, `DefaultModelUpdateRequest.yaml`†, `SignInStartResponse.yaml`, `SignInStatus.yaml`, `EntitlementResponse.yaml` (shared with ADR-067), `Provider.yaml` (`status` six values, `auth_method`, `account_label`, `dependents`, `backs_default`, `updated_at`), `Agent.yaml` (`needs_model`, coordinated with ADR-067's `degraded_reason` in one edit), `OnboardingProviderApiKey.yaml` + `OnboardingProviderSignIn.yaml` with the inline `oneOf`+`discriminator` in `openapi.yaml` (ADR-034), `ProbeProviderRequest.yaml` (one shape `{id, auth, api_key?, model?, api_base?, protocol?}` — X-04), `ProbeProviderResponse.yaml` (`probed_model`), `LLMError` `model_unassigned` in `LLMError.yaml` + `LLMErrorReplay.yaml` + asyncapi inline `LLMError` + `LLMErrorReplay` (X-01), `DefaultModel.window_source` → `$ref ContextWindowSource.yaml` (S66-owned, X-06), and the `openapi.yaml` paths `DELETE /providers/{id}`, `GET/PUT /providers/default-model`, `POST /providers/{id}/sign-in`, `GET /providers/{id}/sign-in/status`.
