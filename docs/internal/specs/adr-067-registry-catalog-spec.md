...
- **FR-024** Every new or changed wire type MUST be defined in `contracts/` first and consumed only via generated types: `ProvidersCatalog`, `CatalogProvider`, `CatalogModel`, `EntitlementResponse`; `ProbeProviderRequest.id` (enum → string, `maxLength: 64`, plus optional `protocol`); `Provider.protocol` and `ProviderUpdateRequest.protocol`; `Provider.status` — one enumeration shared verbatim with the ADR-068 spec: `connected | disconnected | error | unknown-provider | signed_in | expired` (this spec defines `unknown-provider`; ADR-068 defines `signed_in`/`expired`); `Agent.yaml` coordinated edit — this spec adds `degraded_reason` (enum, currently the single value `needs_provider`, optional) beside ADR-068's `needs_model: boolean` (always present, derived); both may be true and `needs_provider` wins in copy (FR-031, ADR-068 FR-014); `ModelCapabilities` deleted.
