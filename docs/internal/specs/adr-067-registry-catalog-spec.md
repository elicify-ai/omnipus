...
4. **Given** a provider absent from models.dev but shipped by Omnipus (`ollama`, `vllm`, `litellm`, `codex-cli`, `openai-chatgpt`, `shengsuanyun`, `volcengine`, `avian`, `mimo`), **When** the job publishes, **Then** it appears in the document from the local provider file with the same shape as registry providers. `custom` is **not** a row (F-07/F-08).
...
3. **Given** an id not in the catalog with `endpoint` and `protocol`, **When** probed, **Then** the probe runs against `endpoint` as a custom row; without both, 400 (F-07). **Given** an `id` longer than 64 characters, **Then** 400 by schema (`maxLength`).
...
| `codex-cli`, `codexcli` | `codex-cli` (subprocess) and `openai-chatgpt` (HTTP) per ADR-068 MAJ-013, protocol `cli` |
| (new) `custom` | factory case, not a row (FR-035) |
...
When `{id: my-proxy, endpoint: …, protocol: openai-compatible}` → probe runs there; without `endpoint` or `protocol` → 400; a 65-character `id` → 400 by schema.
...
| 18 | `TestEmbeddedSnapshot_LocalProvidersPresent` | Unit | local-file providers | 9 ids present; `custom` absent |
...
| 31b | `TestContract_AgentDegradedReason_Provider_Protocol` | Unit | (schema) | `Agent.degraded_reason` enum and `Agent.needs_model` boolean both present; `Provider.protocol`, `ProviderUpdateRequest.protocol`; `Provider.status` enum equals exactly `connected,disconnected,error,unknown-provider,signed_in,expired`; `EntitlementResponse` shape |
...
| 37 | `TestRestProviders_Entitlement_PerProtocol` | Integration | entitlement outline (6 rows); intersects; 422 | per-protocol call; 409 for cli/custom; cache keyed by sha256, evicted on refresh `[A-13]` |
...
- **FR-012** The factory MUST dispatch on protocol only (`openai-compatible`, `anthropic`, `google`, `ollama`, `cli`, plus the `custom` case); the base URL comes from the catalog row (explicit `api_base` in config wins); keys come from the credential store, never from the row's `env`. `case google` constructs the HTTP provider at the row's Gemini OpenAI-compatible URL with Bearer auth — the same transport as today's `google` case `[A-7]`.
...
- **FR-024** Every new or changed wire type MUST be defined in `contracts/` first and consumed only via generated types: `ProvidersCatalog`, `CatalogProvider`, `CatalogModel`, `EntitlementResponse`; `ProbeProviderRequest.id` (enum → string, `maxLength: 64`, plus optional `protocol`); `Provider.protocol` and `ProviderUpdateRequest.protocol`; `Provider.status` — one enumeration shared verbatim with the ADR-068 spec: `connected | disconnected | error | unknown-provider | signed_in | expired` (this spec defines `unknown-provider`; ADR-068 defines `signed_in`/`expired`); `Agent.yaml` coordinated edit — this spec adds `degraded_reason` (enum, currently the single value `needs_provider`, optional) beside ADR-068's `needs_model: boolean` (always present, derived); both may be true and `needs_provider` wins in copy (FR-031, ADR-068 FR-014); `ModelCapabilities` deleted. **Shared contract artefacts (aligned with the ADR-066 and ADR-068 specs, 2026-08-22):** `EntitlementResponse.yaml` (this spec's entitlement response); `Provider.status` is **exactly** `connected | disconnected | error | unknown-provider | signed_in | expired` — one enumeration, listed verbatim here and in ADR-068's FR-038 (this spec defines `unknown-provider`; ADR-068 defines `signed_in`/`expired`); `Agent.degraded_reason` enum `[needs_provider, needs_model]` sits beside ADR-068's derived `Agent.needs_model`, precedence **needs_provider wins** in copy; the default model is the **pair** `agents.defaults.default_model` per ADR-068's `DefaultModel.yaml` / `DefaultModelUpdateRequest.yaml` — `agents.defaults.model_name` does not exist (ADR-068 CRIT-001) and nothing in this spec reads or writes it; per-model context-window overrides live **only** in ADR-066's `ContextSettings.yaml` (`model_overrides`) — this spec defines no competing override.
...
- **FR-035** `custom` MUST be a factory case, not a catalog row: a provider config whose id is not in the catalog is accepted iff it carries `api_base` and `protocol ∈ {openai-compatible, anthropic}`, becoming a custom row keyed by that id (several coexist); otherwise `ErrUnknownProvider`. `PUT /providers/{id}`, the onboarding probe and CLI onboard MUST apply the same rule (F-07/F-08/F-09).
