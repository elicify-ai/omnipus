...
| F-11 | Yes — `Agent.yaml` has no `degraded` field | **Apply.** FR-024 now lists `Agent.degraded_reason` (enum `[needs_provider, needs_model]`, both defined here), `Provider.protocol`, `ProviderUpdateRequest.protocol`, `Provider.status += unknown-provider`, `ProviderEntitlement`, `ProbeProviderRequest.id` `maxLength: 64`. DS-5 row 18. |
...
| Unasked Q4 | — | Override re-clamp on refresh is ADR-066 D2's rule; this spec only guarantees `Window()` reflects the new catalog value immediately. |
| Unasked Q8 | — | Refresh does not rebuild provider instances; a changed `api` takes effect at the next agent reload (ADR §8b). |
| Unasked Q9 | — | `GET /providers` returns configured rows **with** `models[]` filled from the catalog (US-9.AC1); the SPA does not join. |
...
| Cross-doc | — | `Provider.status += unknown-provider` explicit in FR-024; the entitlement route is `POST /api/v1/providers/{id}/entitlement` (single name); the no-trace proof (SC-010) asserts only the absence of a canonical id, never the echoed user id. |
...
Then `claude-x: entitled`, `claude-y: not entitled`, `brand-new-model: limits unknown`; stub counter stays 1; after a catalog refresh the counter becomes 2.
...
| 31b | `TestContract_AgentDegradedReason_Provider_Protocol` | Unit | (schema) | `Agent.degraded_reason` enum, `Provider.protocol`, `ProviderUpdateRequest.protocol`, `Provider.status` `unknown-provider` present in generated types |
...
- **FR-024** Every new or changed wire type MUST be defined in `contracts/` first and consumed only via generated types: `ProvidersCatalog`, `CatalogProvider`, `CatalogModel`, `ProviderEntitlement`; `ProbeProviderRequest.id` (enum → string, `maxLength: 64`, plus optional `protocol`); `Provider.protocol` and `ProviderUpdateRequest.protocol`; `Provider.status += unknown-provider`; `Agent.degraded_reason` enum `[needs_provider, needs_model]` (both values defined in this change; ADR-068 consumes `needs_model`); `ModelCapabilities` deleted.
