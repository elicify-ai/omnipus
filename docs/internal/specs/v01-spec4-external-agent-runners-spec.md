...
| `pkg/sandbox/hardened_exec.go` + the `workspace.shell` sandbox | **reuse for NATIVE subs** | native sub-agents ride the existing Omnipus sandbox; external CLIs use their OWN sandbox — **no new confiner** (operator decision). *Amended 2026-07-05 (issue #488): only codex still self-sandboxes — claude and opencode now run fully permission-bypassed with no boundary of their own; see FR-5.3.* |
...
**US-5 — Own-process, kernel-confined, worktree-isolated (P0, security).** 1. **Given** an external run, **When** it starts, **Then** it is its own process **under the external CLI's OWN sandbox** (Codex = Landlock/seccomp on Linux + Seatbelt on macOS; Claude Code = its permission model — Omnipus does NOT disable it via `--dangerously-skip`), in a **git worktree** (or isolated temp dir) isolated from the main tree. **Native** sub-agents run under the **existing Omnipus `workspace.shell` sandbox**. 2. **Given** a hostile agent, **Then** the tool's own sandbox + the worktree confine it (no new Omnipus confiner needed). *(Historical acceptance criteria as originally written — both premises are now superseded, see FR-5.3's amendments: worktree isolation was removed by ADR-032, and as of 2026-07-05 (issue #488) Claude Code no longer runs under its own permission model as a boundary — only codex still self-sandboxes.)*
...
- **FR-5.3 (operator decision — leverage the tools' own sandboxes, NO new confiner):** the external CLIs **self-sandbox** (Codex = Landlock/seccomp on Linux + Seatbelt on macOS; Claude Code = permission model + bash sandbox). v0.1.0 MUST therefore: (1) **run external CLIs WITH their own sandbox ENABLED** — i.e. **DROP the `--dangerously-skip-permissions` / `--dangerously-bypass-approvals-and-sandbox` flags** the existing one-shot providers use (those *disable* the tool's sandbox); (2) **git-worktree-isolate** each run (isolated temp dir if not a repo) — Omnipus's cheap FS boundary; (3) **route the CLI's permission prompts to Omnipus consent** (deny-by-default). **Native sub-agents** (no external sandbox) ride the **EXISTING Omnipus `workspace.shell` sandbox** — no new primitive. A **startup reaper GCs orphaned run dirs** (M-4). The CLI's **credentials are passed via the env-allowlist**. **There is NO new re-exec confiner primitive** — confinement comes from the tool's own sandbox (external) + the existing sandbox (native). **Amended by ADR-032 (2026-07):** external-CLI runs now execute directly in the delegate's workspace (worktree isolation removed); see ADR-032 for rationale and the residual-risk discussion. **Further amended 2026-07-05 (operator decision, issue #488):** claude's `--permission-mode acceptEdits` middle ground (chosen here specifically to avoid disabling Claude Code's own sandbox) is reversed — the driver now passes `--dangerously-skip-permissions` unconditionally, matching opencode (which already ran fully bypassed), because a TTY-only permission prompt has no reachable approval UI in a headless run and stalls forever. **claude and opencode now run fully permission-bypassed with no tool-level gate of their own; codex is unaffected and retains a real one** — its `--sandbox workspace-write` flag is an OS-level Landlock/seccomp confiner, not merely a permission prompt, and continues to run unconditionally as before, independent of any CLI's permission-bypass posture. Issue #488 tracks whether Omnipus should build its own confiner for the claude/opencode runner children now that neither retains an enforced tool-level gate of its own.
...
**Success Criteria**
- **SC-1:** `verify-contracts` exits 0 (CI). · **SC-2:** build + typecheck exit 0 (CI authority; local scoped/Linux-gated). · **SC-3:** an external run is its own process, Landlock/seccomp-confined, worktree-isolated (verified on Linux). **Amended by ADR-032 (2026-07):** external-CLI runs now execute directly in the delegate's workspace (worktree isolation removed); see ADR-032 for rationale and the residual-risk discussion. · **SC-4:** a mid-run permission-request routes to consent; no-handler → deny. · **SC-5:** the connection test validates without running work. · **SC-6:** `remote-a2a` is schema-valid + dispatch-rejected. · **SC-7:** a run over the timeout is terminated. · **SC-8:** `executor=native` sub-agents are unchanged (no regression).
