...
- **FR-024** — There MUST be one budget for both owner kinds, defaulting to **20** (`config.DefaultGoalMaxRounds`).
  ...
  - **Amendment 2026-09-15 (issue #710):** an error only an operator can fix does not use a task attempt. A task run whose worker turn is refused because the provider key is rejected or missing (the provider answers 401 or 403), the provider sign-in has expired, the agent's provider is not configured, the agent has no model, the model's context window is unknown, the agent is on no workspace team, or the agent's working folder cannot be opened ends the task Failed at once, with the reason and how to fix it — no attempt used, no restart. Temporary errors (a rate limit, a network or provider outage, a stalled stream, a timeout, an unrecognised error) still fail the run and restart the task, up to its attempt limit. See ADR-043 §9.
