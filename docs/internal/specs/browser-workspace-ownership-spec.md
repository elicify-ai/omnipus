...
25. **The operator's tab is taken by acting on it, and never while the operator is at the wheel.** An agent that acts on the workspace-owned tab becomes its driver with no acquisition step, no extra tool and no extra permission — *unless a human currently holds the live-view control lock*, in which case the agent defers with ADR-038 D6's existing reason and acquires nothing (D1.9b ruling 1; FR-070, FR-071).
...
- The system must **not** give implicit acquisition of the operator's tab a **surface**. No `browser_take_control` tool, no policy entry, no wire field, no result key announcing a transfer. An agent becomes the driver by executing a `controlledResult`-gated tool against `TabOwnerWorkspace()` and by nothing else (D1.9b ruling 1, FR-070).
...
| 97 | `TestTabs_TwoSessionsDoNotMerge` / `TestTabs_AgentSwitchWithinASessionSeesTheSameTabs` / `TestTabs_WorkspaceOwnedSetIsVisibleToAll` | Unit | FR-080 | **Three assertions, and the middle one is the ruling.** (a) Two **sessions** on one manager: two distinct `sessionEntry` values, neither's `tabs` slice containing the other's `tabEntry`, and a turn in S2 refused when it tries to switch to, drive or close S1's tab. (b) **One session, two agents:** Mia opens a tab, the turn is re-run as Jim on the **same** `transcriptSessionID`, and Jim's `browser_list_tabs` returns that tab and can drive it — *this is the assertion that is RED on a D1.9a build, and it is ADR §1.1's own conversation.* (c) The workspace-owned entry appears in both sessions' results, labelled. **Red today** in the sense that matters: written against a re-keyed manager with no `TabOwner`, all three fail |
...
| `IsControlled(ws:W) == false`; agent acts on `TabOwnerWorkspace()` | the call proceeds; the agent is the driver; no acquisition tool exists in the registry | FR-070 |
...
- **FR-001 … FR-079** as enumerated in §9. All MUST. **Counts: 104 rows in the §9 matrix, 10 of them withdrawn tombstones (FR-011, FR-012, FR-014a, FR-038, FR-038a, FR-039, FR-046, FR-049, FR-055, FR-056), so 94 live FRs** — re-counted mechanically against the matrix, not carried forward. Movement in **this** revision (the D1.9b pass) is stated first, then the D1.5c/D1.5d pass's, then D1.5b's, then the D1.5a subtraction pass, then revision 4's, each amending the one below it:
