...
| Linux with an unreadable `/proc/meminfo` (e.g. gVisor) | `ok=false` — **not** the fabricated 2 GiB the `fallbackTotalRAMBytes/2` path yields (`meminfo_linux.go:16`); the pool refuses. *The "and `autoDetectMaxParallel` still consumes the fallback, deliberately" half of this row is deleted with that function (FR-067); what the agent gate does here is E-8's open branch, and this row asserts nothing about it* | FR-065 |
