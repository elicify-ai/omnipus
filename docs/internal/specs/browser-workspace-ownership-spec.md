...
**Five are live** (E-1, E-3, E-4, E-5, **E-8**); **E-2, E-6 and E-7 are ruled and kept struck
through** so a reader arriving from an earlier review sees each answered rather than dropped. **E-6
was the most consequential of them** — it asked whether the one control that remains can run at all
on two of the three supported platforms — and **ADR D1.5b (2026-09-01) answers it: the macOS reader
is in scope and must be written; Windows is foreseen and explicitly NOT in scope. See §0.6a.**
...
offered rather than choosing between them.** See §0.6b. **E-8 is new, and it is E-7's successor
rather than its replacement:** the rulings that dissolved E-7 did so by reaching *further* into
agent concurrency, not less far, so what needs an answer is no longer *"which shape"* but *"who
ratifies a cross-domain change a browser ruling authorised."*
...
| **E-8** | **Who ratifies an agent-concurrency change that a BROWSER ruling authorised — and what happens on a host the gate cannot measure?** Two parts, and the second is the sharper one. **(a) Ratification.** FR-067/068/069 delete a shipped constant, change a public accessor's signature at **five** call sites across `pkg/agent` and `pkg/gateway` (`task_executor.go:250-260`, `subturn.go::getSubTurnConfig`, `admission.go:216-218`, `rest_performance.go:71-78`, `:149-154`), and change a wire field's meaning (`contracts/components/schemas/PerformanceSettings.yaml:27-29`). None of that is browser work, and no browser ruling can approve it. **(b) The unmeasurable host, which is NOT simply FR-065 applied twice.** FR-065's rule — *availability undeterminable ⇒ refuse* — is right for a browser: a refused browse costs one tool call. Applied to agent admission it refuses **every** agent turn on Windows, and on any Linux host that falls through to the 4 GB fallback (gVisor and other `/proc`-less sandboxes, `meminfo_linux.go:16`). That is not a degraded platform; it is a gateway that cannot run a turn. **The operator has not been shown that consequence**, because it was ruled in a browser context where the same words mean something an order of magnitude smaller. | **Recorded, NOT decided.** FR-068's measurable-host behaviour is fully specified and testable; **its unmeasurable branch is written as un-ratified and must not be implemented in either direction on this spec's authority** — neither refusing (which stops the product) nor admitting (which is the silent-no-op FR-065 exists to forbid). Three shapes are available and this spec ranks but does not choose: **(i)** the agent path keeps a conservative floor on an unmeasurable host — the existing floor of 2 is the obvious candidate and it is a *floor*, not a computed default, so D1.5d's objection does not reach it; **(ii)** refuse, and declare Windows/gVisor **unsupported for agents**, which is a product decision far above this spec; **(iii)** refuse only *growth* past the floor, which is (i) stated as a gate. **Nothing here blocks the browser work** — see §0.6b's stream split: the browser FRs and FR-067/068/069 share exactly one artefact (FR-065's accessor), which the browser owns and writes either way. | **Operator + agent-concurrency's owner** — a cross-domain scope question, not a spec detail |
...
- **AC17 (an unmeasurable host refuses, and says why — NEW): Given** a host where availability cannot be determined — the accessor returns `ok=false`, whether because the platform has no reader (Windows) or because Linux fell through to its 4 GB fallback constant (`meminfo_linux.go:16`, e.g. an unreadable `/proc/meminfo` under gVisor) — **When** a browser launch or a tab open is requested, **Then** it is **refused**, the refusal names memory (FR-053, FR-063), and the reason *"memory availability cannot be determined on this platform"* is logged **once**, not per call. **The pool never treats an unmeasurable host as empty.** *A gate that answers "room available" because it has nothing to read reports success while admitting without limit — the exact false-green shape this project documents, and the one thing D1.5a's own text forbids.* (FR-065.)
...
| 83 | `TestPool_UnmeasurableHostRefusesToGrow` / `TestManager_UnmeasurableHostRefusesTabOpen` / `TestPool_UnmeasurableRefusalIsLoggedOnce` | Unit (fake accessor) | FR-065, FR-053, FR-063 | **The assertion is REFUSAL, not the absence of a crash.** With the accessor returning `ok=false`: a launch is refused naming memory, a tab open is refused with `memory_pressure`, and the run **FAILS** if either succeeds. The third test asserts the explanation is logged **exactly once per process**, and **fails** on both zero log lines and one-per-call. **Fixture inputs must include Linux's fallback path** (`/proc/meminfo` unreadable ⇒ `fallbackTotalRAMBytes/2` = a fabricated 2 GiB, `meminfo_linux.go:16`), not only the Windows platform case — otherwise the test proves nothing about the shape that actually reads as *"launch away"* |
...
**One regression is OUTSIDE `pkg/tools/browser`, and it is the one most likely to be missed (D1.5b).** `pkg/config/meminfo_other_test.go` is gated `//go:build !linux` (`:1`) and tests `readMemAvailableBytes() == 0` (`:38-41`) and `readCgroupMemoryAvailableBytes() ok == false` (`:61-64`). **FR-064 makes the first assertion FALSE on Darwin**, so the file's build constraint must narrow to `!linux && !darwin` **in the same commit as `meminfo_other.go`'s** — otherwise the package stops compiling on macOS, and this repo's Linux-only CI would not notice. **The second assertion stays true everywhere** (cgroups are Linux-only) and must be preserved for Darwin by test 82's file, not dropped. `pkg/config/parallel_clamp_test.go:111-120` derives its expectation from `availableRAMBytes()` itself, so it follows the accessor rather than pinning a value — **it must keep doing that, and must not be "fixed" by hardcoding 2**, which would freeze the very default §0.5 E-7 is escalating.
...
| Linux with an unreadable `/proc/meminfo` (e.g. gVisor) | `ok=false` — **not** the fabricated 2 GiB the `fallbackTotalRAMBytes/2` path yields (`meminfo_linux.go:16`); the pool refuses. *`autoDetectMaxParallel` still consumes the fallback, deliberately* | FR-065 |
...
- **FR-001 … FR-066** as enumerated in §9. All MUST. **Counts: 90 rows in the §9 matrix, 10 of them withdrawn tombstones (FR-011, FR-012, FR-014a, FR-038, FR-038a, FR-039, FR-046, FR-049, FR-055, FR-056), so 80 live FRs** — re-counted mechanically against the matrix, not carried forward. Movement in **this** revision (the D1.5b addition pass) is stated first, then the D1.5a subtraction pass, then revision 4's, each amending the one below it:
...
- **SC-014 (gates).** `gofmt -l . | wc -l` = 0; `golangci-lint run --build-tags=goolm,stdjson` exit 0; CI `go test -tags goolm,stdjson -count=1 ./...` exit 0; `govulncheck` 0 vulnerabilities; `npm run typecheck` exit 0; `npx vitest run` exit 0. **Not sufficient on its own:** SC-012, SC-012a, SC-019, SC-020, SC-021 and **SC-023** are separate and none is satisfied by a green CI run. **SC-023 is the sharpest case:** CI here is Linux-only, so it never compiles, let alone runs, FR-064's `//go:build darwin` tests — a fully green CI is compatible with a Darwin reader that returns nonsense. Note also that `golangci-lint` caps findings at 3 per message by default — read `docs/internal/false-green-patterns.md` before reporting a clean lint.
...
- **§0.5's escalations need answers, and the list has now moved three times.** **E-2 is RULED** by ADR D1.5a — the pressure gate is a hard stop, the cap is soft, and with the cap deleted only the hard stop remains — so the implementation no longer has to avoid picking, and test 72 asserts the case revision 4 deliberately left unasserted. **E-6 is RULED** by ADR D1.5b: revision 5 escalated that the live-memory gate had **no signal at all** on macOS or Windows (`readMemAvailableBytes` returns 0 by design, `pkg/config/meminfo_other.go`), so on two of three supported platforms the only admission control that exists could not run — and D1.5a had deleted the counter that used to bound the pool there. **The operator ruled: write the macOS reader (in scope, FR-064); refuse rather than admit where availability is undeterminable (FR-065); foresee Windows but do not scope it, and declare the gap in code, release notes and config documentation (FR-066).** **Five remain live**, all of them the operator's or a downstream document's: **E-1** the "take control on request" verb (D2 surface), **E-3** `browser_snapshot`'s reachability under D1.9a (D2), **E-4** the three ADR §6 rows (ADR owner), **E-5** whether a per-tab headroom floor is needed (operator / G-1's Linux pass), and **E-7 — new, and a consequence of E-6's own ruling rather than a fresh question — writing the Darwin reader moves `performance.max_parallel_agents`' DEFAULT on macOS from 2 to 2000** (`autoDetectMaxParallel`, `pkg/config/config.go:614-618`, is `readMemAvailableBytes`' one existing consumer). **Operator ruling required on the consumer set, not on the reader:** let the default move with a release-note line, or give the browser gate its own accessor so the concurrency default keeps its floor until sized deliberately. Nothing in E-7 blocks FR-064, FR-065 or FR-066.
