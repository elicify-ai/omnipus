...
- **Round-1 ADR review folded in:** `docs/internal/architecture/ADR-075-workspace-scoped-browser-sessions-review.md`.
...
- **Round-5 spec review folded in (2026-09-02):** a final grill before implementation planning, returned **in-session and not written to disk** — cite it as *"round-5, 2026-09-02, returned in-session"*, and do not go looking for a review file. Verdict: **every finding is a sweep-completeness failure on ONE ruling (D1.9c), not a design failure** — the ruling was swept through the prose, ACs, scenarios, FR rows and tests, and **not** through §3.1's interface and §14.1's signatures, the two normative code artefacts an implementer types from. 18 findings (5 CRITICAL / 8 MAJOR / 5 MINOR, plus four drifted citations). All dispositioned in **§17.5**; **one (m-505) is rejected on a mechanical recount** and says so there.
- **ADR D2.9a folded in (2026-09-02, commit `0be8988ac`):** *"a registered tool with no seeded policy aborts boot"* is **FALSE** — the repair backfills an explicit `deny` before validation runs, so a missing seed is a **silent per-agent deny**. Nothing here relied on the abort; the consequence is that co-landing means **one atomic commit**, never a sequence (§0.7).
- **Amends:** **ADR-043 D1** (one shared Chrome for the process — *this spec replaces it with a pool*), **ADR-043 D2** (per-agent CDP browser context — *replaced by per-workspace Chrome profiles*) and **ADR-043 D3** (live-view binding). Read ADR-043 first; D1 has the largest blast radius of anything in ADR-075.
...
> ADR-075 **D1.5e**'s deployment matrix describes the gVisor row in the same
...
**Solution (ADR-075 D1).** Move ownership from the **agent** to the **workspace**:
...
// the operator ruling recorded in ADR-075 D1.7: "there is no 'pool full' error
...
// error, and under ADR-075 D1.5a it names MEMORY rather than a cap: memory
...
```go
// pkg/tools/browser/pool.go (new) — ADR-075 D1.4 (mechanism), D1.5a (memory is
...
//
// PATHS — the layout is ADR-075 D1.8's, which is FLAT ("ws-<id>", one
...
//
// and ADR-075 D1.5a deletes every term of it: max_browsers and
...
| **A10** | **Operator of record for the rulings.** | **RESOLVED — and the citation is corrected.** The attribution is in ADR-075's **header block**, not in any D-section, and it is unenumerated: *"Decider for every ruling in this document: Daniel Piatkowski (operator)."* Revision 3 quoted a longer sentence that named six sections, four of which the consolidation deleted — a quotation the ADR no longer contains. The header form is broader and survives renumbering, which is why nothing broke. See SC-015. §17 M3. |
...
**SC-015 is satisfied** — ADR-075's **header block** (not D1.1a, which no longer exists) names Daniel Piatkowski as decider for every ruling in that document, in an unenumerated form that survives renumbering.
