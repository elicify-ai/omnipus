...
- **Given** a host whose availability accessor returns `ok=false` and an empty pool
- **When** the first browser launch is requested, **Then** it **succeeds**, and the first tab in it opens
- **And when** a **second** launch is requested, **Then** it is refused naming memory; **and when** a **second** tab is requested, **Then** it is refused with `memory_pressure`
- **And** the explanation is logged **once per process**, not per call
- *Both directions are required. Refusal-only is green on a build that refuses everything — which takes browsing away from gVisor, a **supported** Linux deployment (§0.9) — and admit-only is green on a build with no gate. This is the boundary §13 holdout 24 and test 93 already assert; before this pass, AC17 and test 83 asserted the opposite one (round-4 C-401).*

<!-- superseded phrasing of this scenario's heading retained above; original steps follow -->
