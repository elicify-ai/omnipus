...
**Five are live** (E-1, E-3, E-4, E-5, **E-7**); **E-2 and E-6 are ruled and kept struck through**
...
scope and must be written; Windows is foreseen and explicitly NOT in scope. See §0.6a.** **E-7 is
new, and it is E-6's own consequence rather than a fresh question:** writing the reader D1.5b
mandates also changes a shipped default outside the browser, and this spec may not decide that on
a browser ruling's authority.
...
| **E-7** | **Writing the Darwin reader moves `performance.max_parallel_agents`' DEFAULT on macOS from 2 to 2000.** `readMemAvailableBytes` has one existing consumer — `autoDetectMaxParallel` (`pkg/config/config.go:614-618`) — which sizes the default concurrency cap as `availableRAMBytes() / bytesPerAgent` (3.5 MB, `:608`), clamped by `clampParallel` (`:557-566`). Today macOS reads `0`, so the default is the floor of **2** (`autoDetectFloorParallel`, `:558`). With FR-064's reader on the reference 32 GB Mac (~8.5 GB available) it computes ≈2300 and clamps to `physicalConcurrencySafetyCeiling` = **2000** (`:586`). **This is a shipped-behaviour change well outside the browser, arriving as a side effect of a browser-capacity fix**, for every operator who never set the key explicitly. | **Recorded, NOT decided — and not silently absorbed** (§0.6a). Two shapes, and this spec may not choose: **(a)** let the default move and carry one release-note line — arguably the *correct* number, since the floor of 2 exists only because there was no signal and `meminfo_other.go:30-33` says exactly that; or **(b)** give the browser gate its own accessor so `autoDetectMaxParallel` keeps the floor of 2 on macOS until the number is sized deliberately for that workload. **Nothing here blocks FR-064, FR-065 or FR-066** — the reader is written either way; only which consumers see it is open. **What is NOT available is shipping it unremarked:** that is the ADR-037 "saved, changed nothing" anti-pattern inverted — *changed something nobody asked to change, and said nothing.* | **Operator** — a scope question about a non-browser default, not a spec detail |
