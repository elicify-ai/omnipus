...
- **Status:** Draft **v3** (2026-09-05; ADR-080 compatibility note added 2026-09-07). Two grill rounds complete per project standard: r1 (4C/11M — blocking model refuted, rebuilt on the durable park) corrected in v2; r2 (2C/8M — resume contract misdescribed, v1 channel hole) corrected here; the one flagged sign-off (§9.1, channel phasing) was RESOLVED by operator interview #5 — specs complete, zero open sign-offs.
- **ADR-080 compatibility (no change to this tool):** ADR-080 extends the goal-compile CLEAR branch (adds a restated `definition`, per-criterion `judgment`, and a `dod[]`) and feeds workspace instructions into the compile/Judge. It does **not** touch the AMBIGUOUS branch (`clarity == ambiguous → questions[]`) that drives this card, nor the tool's park/resume mechanics — the goal-compile consumer (§0 note, US-3 goal-loop gate, Test 12) is unchanged. The only downstream effect: after a card is answered, the *resumed* compile now also emits `definition`/`dod` on its clear result (transparent to the card, which only ever renders the `questions[]` it was given). No wire, park, or resume change.
...
- **Role:** general-purpose agent tool; first consumer is ADR-074 D4a's goal-compilation clarifying question — now **wired and active** (ADR-079 D3): on a web owner session the goal-compile clarify emits a card through the registry. NOTE the goal-compile is an **engine-invoked provider call, not an agent tool call**, so that consumer creates its pending set via `Registry.CreatePending` **directly** and the originating `/goal` turn ends normally — there is **no `ParksTurn`** for it. This is a sanctioned second creation path: `ParksTurn` drives the seam only for the TOOL's own `Execute` (§0.1); resume for BOTH paths runs through `Submit → dispatchResume` (§0.2), which is independent of `ParksTurn`. The one-per-routing-session guard (§0.9) still holds — a goal-compile card and a tool-invoked card cannot coexist on one session; the second `CreatePending` loses with `ErrAlreadyPending` and the goal path falls back to plain chat.
...
| 12 | E2E: goal clarifying question through the **active** card (ADR-079 D3), stubbed LLM — web owner session compile with `clarity:ambiguous` → engine `CreatePending` (no `ParksTurn`) → card renders, composer blocks → answers submit → `Submit`→`dispatchResume` → resume message parsed by the goal loop into the resumed compile; **channel-origin goal compile → plain-chat question, no card** (askuser US-5 permanent fallback); delegated-child compile falls back to message_parent (o-R2-3); a tool-invoked card already pending → goal `CreatePending` returns `ErrAlreadyPending` → goal falls back to plain chat | E2E |
