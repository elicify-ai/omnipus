...
4. **`[goal:evidence]` taught to both dispatch kinds.** The worker-prompt block teaching the `[goal:evidence]` marker (FR-035) is emitted BEFORE the `dispatchesExternalCLI(t.AgentID)` early-return inside `TaskExecutor.buildPrompt` (`pkg/agent/task_executor.go`; cited by function name rather than a line range — this file is under concurrent edit by other fix-wave agents), so both native (Main/Subagent, which also has the `update_task` tool escape hatch) and external-CLI (`subagent_3p`, which has no tool escape hatch — the marker is its ONLY path to satisfy the gate) workers receive the instruction.
5. **Judge workspace resolution: implicit membership, not a refusal bypass (2026-07-21 post-ship fix, product blocker).** A fresh-install smoke test found the Judge's verifier turn (FR-011) was refused before it started — ADR-046 P1's workspace-membership gate requires literal `core_team` membership, but System Agents are permanently barred from that roster (`pkg/gateway/rest_workspaces.go`'s `validateCoreTeamMembers`), so the check was definitionally unsatisfiable. Operator direction: "make the judge a member of every workspace, keep it simple." System Agents are now treated as IMPLICIT members of every workspace at the single place membership is decided (`pkg/workspace`'s `isImplicitMember`, consulted by `FindForAgent`/`FindForAgentPreferring`) — `resolveTurnWorkDirOrRefuse` (`pkg/agent/workspace_reroot.go`) calls that resolver uniformly for every agent, no per-identity branch beforehand. `JudgeCriteriaInput.WorkspaceID` (unchanged) is the "preferring" selector that picks WHICH workspace a given adjudication roots in; with no selector threaded, the Judge falls back to the same sorted-first tie-break any ordinary multi-membership agent gets; the one remaining agent-home fallback fires only when genuinely no workspace exists at all yet (pre-onboarding). This implicit-membership rule keys on System-Agent IDENTITY (`coreagent.IsSystemAgentID`), not the verifier ROLE — a future custom/per-agent verifier (a non-System agent assigned the verifier role, FR-040) will need either ordinary roster membership or a generalization of this rule; design for that before multiple verifiers ship (architect F3).
6. **`inspect_session` ceiling seeds `allow`, not `deny` (fix-wave finding #2, architect F2 half 1).** FR-027 as originally drafted said `inspect_session` "stays ceiling-`deny` (verifier-role only)". The shipped code does the OPPOSITE: `pkg/config/defaults.go`'s `sandbox.tool_policies` seeds `inspect_session: allow` at the global ceiling (`defaults.go:410`). This is deliberate, not a regression — under strictest-wins (`deny > ask > allow`, `pkg/tools/compositor.go:resolveEffectivePolicyWith`), a ceiling `deny` would OVERRULE the Judge's own seeded per-agent `allow` (`coreagent.systemAgentSeed`'s `IDJudge` case) and resolve the Judge itself to `deny` — exactly the landed defect this fix-wave repaired. The ceiling `allow` only raises the upper bound an agent's own policy can be granted up to; it does not by itself grant the tool to anyone. The actual "deny for everyone except the Judge" posture is delivered entirely by EXPLICIT PER-AGENT `deny`: every seeded core/subagent-tier agent's fully-enumerated `denyAllThenOverride` map already denies `inspect_session` by default (`pkg/coreagent/core.go`'s `coreAgentSeed`), and Worker's sparse `tightenGlobalCeiling` map now carries an explicit `inspect_session: deny` override too (`core.go:493`) — without it, Worker would silently inherit the ceiling's `allow`. A CUSTOM (non-seeded) agent has no such per-agent override and the ceiling itself is `allow`, so its real protection against calling `inspect_session` is NOT the policy layer — it is the fail-closed, engine-set verifier-session ctx scope-lock (FR-033's target-session lock): even an agent that somehow resolves to `allow` for `inspect_session` cannot read a session outside the engine-set target set. **Flag for the ADR-052 owner/lead (not corrected here — `pkg/config` is outside this spec-edit's ownership):** `defaults.go`'s own inline comment at the seed site (lines 385-406) documents this same mechanism but reads as though the backfill-to-explicit-deny repair (FR-006) alone covers `inspect_session` for an unlisted agent; it does not call out that the ceiling being `allow` (not `deny`) means that repair provides no deny-by-default for `inspect_session` specifically, and that the ctx scope-lock is the actual last line of defense for a CUSTOM agent — the comment should be tightened to say so.
...
| `allStaticToolNames` / `buildKnownBuiltinToolNames` / defaults ceiling | **modifies** | register the new tools + explicit `ask` (the three plan tools) / `allow` (`inspect_session`) at the ceiling; per-agent explicit `deny` (not the ceiling) enforces the resolved deny-for-all-except-Judge posture on `inspect_session` — `coreagent/core.go:279`, `config/validate.go`, `config/defaults.go` (M4, see Implementation note 6) |
...
| Approve gate (`handlePlanApprove` FR-084) | `execute_plan` and the SPA ▶ must route here; PUT must not reach `approved`/`running`. **As shipped, `execute_plan` (`PlanExecuteTool.Execute`, `pkg/tools/plan.go`) re-implements the same criteria/DoD gate rather than calling `handlePlanApprove` directly — two documented INTENTIONAL differences remain (FR-030 empty-plan reject; broader idempotent-no-op state handling on an already-running/approved plan), flagged in that file's own doc comment for a Wave-2 shared-gate extraction folding both handlers onto one internal `pkg/plan` gate function (tracked: #531).** |
...
| 32 | `TestVerifierE2EEval` | **e2e-eval** (NEW category — CI worker e2e gate) | (eval scenarios) | real-LLM end-to-end: verifier verdicts correct on met / unmet / subjective cases; runs on the ci-omnipus e2e gate, not the unit suite. **Deliberately carries no dedicated `behavior`-kind criterion case** — that rung resolves deterministically off the session tool-call log with zero LLM verifier involvement (FR-034), so it is covered by unit Test 22 (`TestBehaviorCriteriaScanner`), not this LLM eval. As implemented (`tests/e2e/verifier-eval.spec.ts`), Test 32's 4 cases are all subjective/prose criteria adjudicated by the real verifier LLM: 3 adversarial DS-8 rows (spoofed evidence, prompt injection, leniency pressure) + 1 genuine-completion control. |
...
- **FR-027 (M4/R3-2)**: `create_plan`, `execute_plan`, `run_task`, `inspect_session` MUST be registered in `allStaticToolNames` (`coreagent/core.go:279` — a seeded-override referencing an unregistered name **panics boot** via `validateOverrideKeys`, which is exactly why the seeded verifier set uses the REAL catalog names `read_file`/`list_directory`, GS-01), in `buildKnownBuiltinToolNames` (`[FACT]` `gateway.go:693`), and given explicit **`ask`** entries at the `defaults.go` global ceiling for the three plan tools (sparse Worker map inherits; interview 2026-07-21). **`inspect_session` ceiling seeds `allow`, as built (corrects this FR's earlier "ceiling stays `deny`" draft — see Implementation note 6 for the full inversion + code cites):** under strictest-wins (`deny > ask > allow`), a ceiling `deny` would overrule the Judge's own seeded `allow` and resolve the Judge itself to `deny` — the ceiling `allow` only raises the upper bound an agent's own policy can be granted up to. The resolved "deny for everyone except the Judge" posture on `inspect_session` is delivered by EXPLICIT PER-AGENT `deny` on every non-Judge seeded agent (`denyAllThenOverride`'s fully-enumerated deny-by-default, plus an explicit override in Worker's sparse `tightenGlobalCeiling` map), never by the ceiling itself; a CUSTOM (unlisted) agent's real protection against calling `inspect_session` is therefore NOT the policy layer but the fail-closed, engine-set verifier-session ctx scope-lock (FR-033) — the ceiling being `allow` rather than `deny` means the boot-time backfill-to-deny repair (FR-006) is not what's stopping it. **Resolved posture note:** under strictest-wins the `ask` ceiling governs `create_plan`/`execute_plan`/`run_task` for everyone by default (incl. Jim); raising it to `allow` is the single autonomy knob (FR-005). Coverage is then guaranteed by backfill-to-deny + the seeds (FR-006). The `seedSystemAgents` invariant is **redefined** (R3-2): from "all-deny re-enforced every boot" to **"exactly the seeded tool set re-enforced every boot"** — the Judge's seeded set = `read_file`, `list_directory`, `inspect_session` allow, everything else deny (`core.go:1226-1233`).
