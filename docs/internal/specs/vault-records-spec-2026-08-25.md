...
- **FR-070** The system MUST expose exactly nine tools: `record_schema`, `record_query`, `record_explain`, `record_write`, `record_relate`, `record_log`, `record_view_write`, `record_validate`, `record_view_import`.
- **FR-071** Tool names MUST contain no dots.
- **FR-072** `record_schema` MUST return a compact textual schema, not a JSON schema object.
- **FR-073** `record_explain` MUST report what a query would return and which properties it could not evaluate, without evaluating it.
