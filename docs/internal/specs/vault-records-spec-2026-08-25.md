...
- **FR-004d** **NEW, Draft 10 (ADR-068 D24.6). A `relation`'s `to:` becomes OPTIONAL — the mechanism `person` already has (FR-004b), generalised.** With `to:` declared, FR-034's target-type check applies in full. Without it, only the link SHAPE is validated and comparison is R-8 identity, exactly as an untargeted `person`. *Why: importing the founder's vault, FR-034's mandatory `to:` rejected **17 of 34** inferred schemas outright — the source system has no target-type concept, and forcing the importer to guess a plurality target manufactures declarations nobody made.* The person/relation distinction becomes declared intent, which FR-004b had already made it in all but name. FR-004b's *"MUST be refused without it"* clause for `relation` is superseded by this line.
...
- **FR-018b** **NEW, Draft 10 (ADR-068 D24.1). ViewDef schema_version 2: the view format adopts the find grammar.** The product spoke two filter languages — `knowledge_find`'s `VaultFilterNode` tree (`all`/`any`/`not`, the ten SQL operators) and the view format's flat AND-only `RecordFilter` list in the vocabulary ruling R-B retired — with a bridge (`pkg/records/view_find_bridge.go`) that refuses `contains` and `via` leaves outright. Seven filter groups in the founder's vault use disjunction; none was expressible as a view. ViewDef v2:
  ...
  - **`type` is OPTIONAL.** An untyped view queries every note in scope (4 of the founder's 18 bases scope purely by folder, spanning types). A property reference in an untyped view is valid if ANY in-scope record type declares the name; records of types that do not declare it hold it ABSENT (FR-007's three-state model renders the mixed population coherently). A name NO type declares is still refused with the valid names listed (FR-024, unchanged).
  ...
  - **`formulas` and `summaries` top-level keys** — FR-140s and FR-150s below.
  - **Version 1 views stay readable** (translated to v2 semantics on load); writes emit v2; files on disk are never rewritten on read. `SupportedViewVersion` becomes the set {1, 2}.
...
- **FR-130** The system MUST expose **thirteen virtual properties** under the reserved `file.` namespace, usable in every property position (filter leaf, sort, group_by, properties, summary target, formula operand) and evaluated by the ONE comparator under R-1..R-13 like any declared property: `file.name`, `file.path`, `file.folder`, `file.ext` (text); `file.mtime`, `file.ctime` (date instants); `file.size` (integer, bytes); `file.tags` (many text, stored fully qualified — `a/b`); `file.links`, `file.embeds`, `file.backlinks` (many link); `file.properties` (many text — the note's frontmatter key set; the full map is a formula operand); `file.file` (a formula/display operand only — never a filter target). A date-only literal compared against an instant means the start of that day, UTC; stated here so two implementers cannot pick two readings.
- **FR-131** **Storage is split by what each value needs, and NONE of it changes what SQL decides.** `file.name`/`path`/`folder`/`ext` are DERIVED in Go from `Candidate.Path` — no storage. `file.mtime`/`ctime`/`size` are three new columns on `notes`, written at index time. `file.tags` is a new `note_tags` child table (frontmatter and inline); `file.links`/`embeds` a new `note_links` child table (target, embed flag), from the same body parse the link graph performs. The index schema version bumps and the store rebuilds itself (`NeedsFullIndex`, FR-020a). The child-table join for `note_tags`/`note_links` is the `<child>.note_id = <parent>.note_id` shape AC-8.10 already admits by its stated reason; **the AC-8.10 guard is EXTENDED to the new emitted statements as a specification event** — the trip-wire working as designed, not routed around. `propindex.Selector` keeps exactly its three fields; no `file.*` predicate reaches SQL, ever.
- **FR-132** **`file.backlinks` is DERIVED, never stored** — FR-032's precedent. It is computed in Go by streaming `note_links` within scope, the exact pattern `Store.Relations` already uses so reachability is decided in Go. It is declared expensive in the tool description, as Obsidian's own documentation declares it.
...
- **FR-135** **Ruling R-A is UNTOUCHED by all of the above, and the control that proves it is extended, not weakened.** No `file.*` predicate reaches SQL; every comparison runs in the Go comparator over values the candidate stream delivered; `propindex.Selector` keeps EXACTLY its three fields (the growth guard of D16.6a's watch-point stays armed). AC-8.10's allow-list gains the new tables' admitted join shape and the three new `notes` columns as SELECTED columns only — never as predicate operands — and `TestQuery_NoComparisonIsDelegatedToSQL` MUST pass against the extended statements before anything else in W7 is accepted (the same gate C-5 set for W1).
...
- **FR-146** An expression is bounded at **64 nodes / depth 8** — FR-023c's numbers, deliberately reused — and refused above either, naming which. A formula's per-candidate cost multiplies against B1 exactly as filter leaves do; the A-14 measurement gains a formula rung at both bounds. `now()`/`today()` are snapshotted ONCE per query, so one response is internally consistent.
...
- **FR-150** The system MUST support **all fifteen** summary functions, per view via the `summaries` map (property → summary) and per group when grouped: Average, Min, Max, Sum, Range, Median, Stddev, Earliest, Latest, Checked, Unchecked, Empty, Filled, Unique, plus the existing `count`. Domain rules: Range is max − min in the number AND date domains (a date range renders as a duration); Earliest/Latest are the date domain's extremes; Checked/Unchecked count `true`/`false` over `checkbox` properties (FR-004c); Empty/Filled count absent/present over ANY type. The aggregate op enums (`RecordAggregate.yaml`, `VaultFindAggregate.yaml`) grow accordingly, contract-first.
- **FR-151** **Two computational classes, and the bounds hold.** *Streaming (O(1) accumulator):* Sum, Min, Max, Average (exact sum + count), Range, Earliest, Latest, Checked, Unchecked, Empty, Filled, count — and Stddev, which streams exactly via count, sum and sum-of-squares in rational arithmetic, rounding only at the final square root. *Population (one COLUMN buffered):* **Median and Unique only.** They hold the summarised property's values — never rows — for the survivor set, which B2 already caps at 10,000 on row queries and B1 at 50,000 on the aggregate-only path (worst case ≈50,000 decimal strings: low single-digit MB). FR-066b is amended in place; A-14/W1 measures Median and Unique at both bounds.
...
| FR-004c, FR-004d, FR-007a, FR-030a | US-1, US-4 | edge | `TestFilter_AbsentIsDistinctAndIncludedByNegation` (extended); `TestComparisonTruthTable` (checkbox + `""` rows); `TestRelation_InverseIsDerivedAndSurvivesRename` (raw-text wikilink fixture) — **NEW, Draft 10** |
...
| 88 | `TestView_V2TreeExpressesDisjunction` | unit | FR-018b — **NEW, Draft 10.** A v2 view whose filter is `{any: […]}` loads, validates and returns the union; a v1 view still loads; `group_by` direction is honoured and its absence defaults stated |
| 89 | `TestView_UntypedViewSpansTypesViaAbsence` | integration | FR-018b, FR-018d — an untyped, folder-scoped view returns records of several types, holds undeclared properties ABSENT, and a declared-but-empty type renders "0 records, complete" |
| 90 | `TestFileMeta_EvaluatedInGoNeverInSQL` | unit | FR-130, FR-131, FR-135's guard extension — every `file.*` comparison passes through the comparator; the AC-8.10 recorder shows the new tables queried ONLY by the admitted join shape; `Selector` still has exactly three fields (the existing growth guard) |
| 91 | `TestFileMeta_CtimeAbsentWhereBirthtimeUnknown` | unit | FR-133 — on a platform without birth time, `file.ctime IS NULL` matches and the problem list names unknown-creation-time; nothing substitutes the inode-change time |
| 92 | `TestFormula_ParseFailureIsWriteTimeRefusal` | unit | FR-140 — an unparseable expression is refused at `knowledge_configure` time naming the position; a hand-edited view file with a broken formula is REJECTED at load; no formula error is reachable through `knowledge_find` |
| 93 | `TestFormula_AbsencePropagatesAndArithmeticIsExact` | unit | FR-144, FR-145, R-14/R-15 — absent operand yields absent; `1/3` compared at declared scale; division by zero is a named problem, never zero; no float64 appears on the path (FR-013's assertion reused) |
| 94 | `TestSummaries_AllFifteenWithDeclaredPrecision` | integration | FR-150..FR-153 — each summary over a fixture with a hand-computed expected value; Average/Median/Stddev labelled with their scale; Stddev's label says population |
| 95 | `TestSummaries_BoundRefusalReturnsNoSummary` | integration | FR-154 — a query refused at B1/B2 carries NO summary value; a median over a truncated set is unconstructable, asserted by the refusal path not by inspection |
| 96 | `TestImport_UntranslatableFilterDisablesNeverBroadens` | integration | FR-105, FR-106 — a base with one untranslatable filter clause imports DISABLED and applying it is refused naming the clause; row count of every ENABLED imported view is ≤ the original's over the same fixture (the broadening assertion, the one that matters) |
...
**AC-8.4c** — the mutation table grows four rows: remove R-14's propagation (a formula over an
absent operand must stop yielding a value, not zero); remove R-15's declared-scale boundary;
remove R-16's refusal; remove R-17's equality-only disposition. Each must kill a named cell.
...
| **W7** | **NEW, Draft 10 (ADR-068 D24) — Bases parity, in dependency order:** *(a)* the model rulings — FR-004c, FR-004d, FR-007a, FR-030a — with their truth-table and mutation rows, because everything later evaluates through them; *(b)* ViewDef v2 — FR-018b..FR-018d — contract-first; *(c)* file metadata — FR-130..FR-135 — with the index schema bump and the AC-8.10 guard extension in the SAME change; *(d)* summaries — FR-150..FR-155; *(e)* formulas — FR-140..FR-147, FR-213..FR-216 — last, because they consume all of the above; *(f)* the importer re-run — FR-104..FR-108 | **The exit criterion is the number that started this:** the founder's 18 `.base` files re-imported; every `untranslated` entry names either an enabled-with-annotation-loss or a DISABLED view, **zero broadened views** (test 96's row-count assertion over the real fixture), and the clean count is reported against the 0-of-18 baseline. A-14's measurement extended to a formula rung and to Median/Unique at both bounds. `TestQuery_NoComparisonIsDelegatedToSQL` still green with the extended guard — the R-A control outliving the largest capability addition since it was written is the whole point of having it |
