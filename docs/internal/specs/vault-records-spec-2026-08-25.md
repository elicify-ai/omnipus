...
- **Implements:** [ADR-068](../architecture/ADR-068-vault-records-typed-record-layer.md) **revision 12** (2026-08-30)
...
- **Date:** 2026-08-25; **revised 2026-08-30** to ADR-068 revision 12
- **Branch:** `feat/library-improvements`
- **Status:** **Draft 11** (2026-08-31), applying **four founder rulings** and the findings of **two independent reviews of Draft 10** (one line-level, one systemic; systemic verdict REVISE). Rulings: **every note's properties are indexed, typed or not** (FR-021e — a note whose file says `status: open` must never answer TRUE to `status IS NULL`); **the fifteen summary functions extend `aggregates` under ONE name** (no `summaries` wire key — parity is capability, not key names); **relation `to:` stays effectively required** — FR-004d's general relaxation is WITHDRAWN and the importer INFERS the target from what the links actually point at (FR-104a); and **"set to empty" and "remove property" are two explicit write actions** (FR-041a). Review must-fixes resolved in place: stat metadata staleness through the hash-equal sync skip (FR-136, new work, costed), the formula cost recomputation and caps (FR-146 — the honest worst case was 204.8M steps, now bounded at 16M), formula cycles (FR-148) and static typing (FR-143a, R-18), the cross-format view broadening (no operator is ever substituted on the operator's behalf, FR-018b), the AC-8.10 column-half extension (FR-135, test 39a), FR-105's oracle, FR-151's honest memory math and the **B3 column-buffer bound**, backlink scope + bound (FR-132), `file.properties` storage (via FR-021e), the view `layout` field so a cards view cannot import "clean" while silently becoming a table (FR-109), and the Draft 10 test-numbering collision (rows renumbered 90–101). **What Draft 11 changes is tabled immediately below; the Draft 10 block follows it.** *Draft 10 (2026-08-30) implemented ADR-068 **D24 — Obsidian Bases parity** (founder ruling: *parity IS the goal*), earned by importing the founder's real vault: 757 notes, 18 `.base` files, **10 converted with named losses, 8 refused, ZERO clean**.* New requirement families: **FR-004c/FR-004d** (the `checkbox` type; relation `to:` optional), **FR-007a** (empty string is absent), **FR-018b..FR-018d** (the view format: one filter grammar, a grouping direction, optional `type`, valid empty views), **FR-030a** (raw text decides a wikilink), **FR-104..FR-108** (the import contract under parity), **FR-130..FR-135** (file metadata), **FR-140..FR-147** (formulas), **FR-150..FR-155** (summaries), and §8's **R-14..R-17** as **FR-213..FR-216**. **What Draft 10 changes is set out immediately below; the Draft 9 block follows it.** *Draft 9 (2026-08-28) recorded the operator's ruling on **FR-113's substitution rule** — what a ranking evaluation is allowed to conclude when the human-graded query set it requires does not exist — together with the **anti-rigging clause** that ruling depends on and the **worked example** that clause was earned by. **What Draft 9 changes is set out immediately below; the Draft 8 block follows it.** *Draft 8 (2026-08-28) recorded three operator rulings raised by the Stage 1 implementers who hit them while building the read path — **AC-8.10's `COUNT(*)` exception, the child-table join generalisation, and FR-022e's `LIKE` carve-out** — together with three defects in AC-8.10's own wording that the same reading exposed; the Draft 8 block sits immediately below the Draft 9 one.* *Draft 7 (2026-08-28) closed the ten under-specifications the UAT author found in ADR-068 revision 8 / Draft 6 (`uat-vault-records-2026-08-28.md`). Draft 6 followed grill pass 2.* Revision 2 followed round-2 review (BLOCK: 8 critical, 21 major); revision 3 realigned to ADR-068 revision 5; revision 4 realigned to ADR-068 revision 6 — a sixth tool (`knowledge_configure`), two named blast-radius criteria, a specified freshness token, a platform posture, a W0 wave, and byte budgets. Revision 5 followed grill pass 1 (BLOCK: 19 critical, 41 major, 17 minor) and three operator rulings that delete work. **Revision 6 follows grill pass 2 (BLOCK: 11 critical, 42 major, 14 minor — `vault-records-spec-2026-08-25-review-round6.md`) and one further operator ruling: Unicode case folding is REQUIRED.**


**What Draft 11 changes. Four founder rulings first, then the two reviews' must-fixes — each
resolved in place or explicitly costed as new work (revision 6's standard: a mechanism either
exists and is cited, or is specified as new work with what it costs, never named as though it
already worked).**

| # | Ruling / finding | Outcome |
|---|---|---|
| **R1** | Founder: index properties for EVERY note, typed or not — *"can you not just fix them, use common sense"* | **FR-021e** (new): property rows are built from frontmatter regardless of schema; a note saying `status: open` NEVER answers TRUE to `status IS NULL`. FR-018b's untyped-view resolution REWRITTEN — Draft 10 said undeclared-on-that-type meant absent, which was the Opus 1.6 defect by construction. The importer also writes inferred `type:` into untyped notes where the shape matches, reported per note (FR-104b) |
| **R2** | Founder: summaries are ONE name | **FR-150 rewritten**: the nine missing functions EXTEND `aggregates`; there is NO `summaries` wire key. Obsidian's `summaries`/view-summary keys TRANSLATE at import into `aggregates` entries. Parity is capability, not key names |
| **R3** | Founder: the importer infers `to:`; checking stays ON | **FR-004d WITHDRAWN** as a standing relaxation — `relation` still refuses without `to:` (FR-004b and AC-4b.3 stand unchanged; the person/relation distinction and D5's derived inverse are RESTORED — the Draft 10 relaxation had quietly killed both). **FR-104a** (new): the importer declares `to:` from what the links resolve to (unanimous, or a stated supermajority with the minority reported); where it genuinely cannot tell, it declares `text`, says so, and names the one-line `knowledge_configure` fix |
| **R4** | Founder: empty-vs-remove is the agent's explicit choice | **FR-041a** (new): `set_empty` and `remove` are two distinct write actions; a bare `""` through `set` on a non-text property is refused asking which was meant |
| **F1** | mtime/ctime/size freeze at the last CONTENT change — the hash-equal sync skip updates the manifest's stat and nobody else's (`pkg/knowledge/index.go::SyncWith`, the `opts.Deep` skip); attachments freeze at first index forever | **FR-136** (new work, costed): a hash-equal, stat-changed file gets a METADATA-ONLY index update — no re-parse, no bleve write, no manifest ordering change; attachments refresh from the walk's own stat, never opened (FR-039a). Test 99 |
| **F2** | Formula cost miscounted: 64 leaves × 64-node formulas × 50,000 candidates = **204.8M** steps against the 3.2M the bound defines; nothing capped formulas per view | **FR-146 rewritten**: formulas are MEMOIZED once per candidate; per-view budget of **16 formulas, 64 nodes each, 256 formula nodes total**; defined worst case **50,000 × (64 + 256) = 16M steps** — five times FR-023c's 3.2M, stated as such, measured by A-14's new formula rung, and the caps come down if W-measurement says so (A-14's existing posture). Constraint table row added |
| **F3** | Formula cycles hang: `a: formula.b + 1` / `b: formula.a + 1` passes every Draft 10 check | **FR-148** (new): the reference graph is checked at write AND load; a cycle is refused naming its path — the same active-set guard `pkg/records/frontmatter.go` already applies to YAML aliases, applied where it was omitted. Test 100 |
| **F4** | A formula's type and arity were undefined, and R-1 makes the failure silent | **FR-143a + R-18/FR-217**: every formula has ONE static type and arity, inferred and validated at write/load; `if()` branches must agree (or one be absent); the comparator sees a declaration, never a per-record surprise. Test 100 |
| **F5** | Draft 10's load path TRANSLATED the old flat format's `contains` to `LIKE '%…%'` — broadening, automatically, in the revision that made broadening the one prohibited thing (`view_find_bridge.go`'s own header refuses exactly this) | **FR-018b rewritten**: no automatic operator substitution, ever. A file in the old format kept its own semantics VERBATIM until EXPLICITLY migrated through `knowledge_configure`, which refuses a lossy rewrite naming the faithful alternatives. **Superseded 2026-08-31** (founder ruling, ADR-068 D24.1 amendment): the old format is DELETED, so there is one operator vocabulary and no mapping step in which a `contains` could be widened. The prohibition stands where a rewrite can still happen — `knowledge_configure` |
| **F6** | `file.properties` had a description and no storage; the child-table join strategy was unspecified (the obvious three-way LEFT JOIN is D16.6's fan-out again); `file.backlinks` had no bound and view-local scope | `file.properties` reads FR-021e's rows (real storage). **FR-131**: each child table is streamed by its OWN statement under the shared narrowing — the `Tasks`/`Relations` pattern — never joined together. **FR-132**: backlinks are computed over the CALLER'S WORKSPACE SCOPE (not the view's filter — matching Obsidian's vault-wide semantics without crossing FR-062), with a mid-scan abort above **200,000 edges**, named refusal, remedy stated |
| **F7** | Sonnet: AC-8.10's guard was "extended" on the construct half only — the value-column denylist never gained the new columns. The exact two-thirds omission D16.6a blocked on one revision ago | **FR-135 rewritten + test 39a's column half amended in place**: `mtime`/`ctime`/`size` join the no-predicate column list; `note_tags.tag`, `note_links.target`, `note_links.embed` join the value-column denylist. Test 92 |
| **F8** | Sonnet: FR-105's "never more rows than the original" had no oracle | **FR-105 gains its oracle**: enforcement is STRUCTURAL (every clause either has a mapping from FR-104's normative table — each mapping proven row-set-faithful by its own test — or the view disables); the fixture assertion compares against HAND-DERIVED expected row sets committed as data at fixture-authoring time, never recomputed from the importer's own output. Test 98 |
| **F9** | Sonnet: FR-151's memory math took the friendliest case — a `many` property at 20 elements is 1,000,000 values; 1 KB text at B1 is 50 MB | **FR-151 rewritten with a THIRD named bound, B3**: the population-class column buffer refuses above **100,000 values or 8 MB**, whichever first — a mid-scan abort with count and remedy, `near.go`'s actual pattern. Test 101 |
| **F10** | Opus + coordinator: an Obsidian CARDS view imports as a table with no loss recorded, and scores CLEAN under W7 | **FR-109** (new): the view format gains `layout` (`table`, `cards`; extensible); the importer reads each view's `type`; a layout the product does not render imports with the loss NAMED as an annotation loss (FR-106) — W7's clean count cannot be green over an undetected loss |
| **F11** | Eight contradictions — two rules in force at once | Closed in place: FR-004's "exactly seven" (already amended Draft 10; the two REMAINING live instances fixed — §10a's PropertyDef rewrite row now says eight); AC-4b.3 vs FR-004d (resolved by R3's withdrawal); FR-004b's distinction sentence (true again); D5's inverse (alive again); ADR §4.1/§4.2/§4.3 (revision 13 blocks added there); duplicate test numbers 88/89 (renumbered 90–101) |
| **F12** | Sonnet: a closed grammar against a drifting upstream spec | FR-143 pins the grammar to the Bases syntax reference **as fetched 2026-08-30** (source and date recorded); a construct outside the snapshot is refused/reported as undocumented (FR-107's label), and updating the snapshot is a spec revision, not a code change. **Exercised once: PIN REVISION 1 (2026-09-01)** adopted the Date type field family whole and recorded, rather than inherited, the duration-model divergence found in the same fetch — see FR-143 |

**What Draft 10 changed. All of it is ADR-068 D24 — the founder ruled that Obsidian Bases parity
is the target design, and a real-vault import measured the gap: of the founder's 18 `.base`
files, not one translated clean.** The four documented capability gaps (file metadata, formulas,
nine of fifteen summaries, display config) were joined by four the import run alone exposed: the
view format has **no boolean tree at all** (the find grammar has one; views do not), `group_by`
carries **no direction** on the wire (24 real occurrences flattened), **folder-scoped views
spanning record types are inexpressible** (4 of 18 bases), and six bases reference
forward-provisioned types with zero notes, which the loader refuses. Two data-model collisions
surfaced with them: real vaults write `completed: ""` to mean unset (52 findings), and unquoted
`- [[Target]]` parses as a nested YAML list, silently (66 findings).

| # | Change | Outcome |
|---|--------|---------|
| **1** | Views speak a retired filter vocabulary and cannot express OR | **The view format adopts the find grammar** (FR-018b): one `all`/`any`/`not` tree, the ten SQL operators, everywhere. Grouping keys gain a `direction`; `type` becomes optional (untyped, folder-scoped views); a declared type with zero records is a **valid empty view** (FR-018d) |
| **2** | `file.inFolder()` — the clause the founder personally hit — inexpressible; no file metadata at all | **Thirteen `file.*` virtual properties** in a reserved namespace (FR-130..FR-135), typed, evaluated by the ONE comparator; the four file methods are translations to ordinary leaves, never grammar |
| **3** | No formulas | **Parsed at write/load, stored as source, evaluated in Go per candidate** (FR-140..FR-147). `knowledge_find` still parses no text; nothing reaches SQL; ruling R-A untouched |
| **4** | Six aggregate spellings short of fifteen summaries; `avg` deliberately absent | **All fifteen** (FR-150..FR-155), two computational classes; Median/Unique buffer ONE COLUMN under the existing B1/B2 bounds (FR-066b amended, in place); precision now DECLARED, which satisfies the no-`avg` objection instead of arguing with it |
| **5** | Checked/Unchecked presuppose a boolean the seven types lack | **`checkbox` is the eighth type** (FR-004c) — equality only, absent as the third state |
| **6** | The model meets real vaults badly: `""`-as-unset, unquoted wikilinks, mandatory `to:` | **Three rulings** (FR-007a, FR-030a, FR-004d): empty string is ABSENT for non-text types; the RAW TEXT decides a wikilink; relation `to:` optional — shape-only validation without it, exactly `person`'s mechanism |
| **7** | FR-101 treats every loss as acceptable if reported | **The broadening prohibition** (FR-105): an untranslatable filter clause imports the view DISABLED, never broadened — an imported view must NEVER return more rows than its original. Under parity a residual loss is a **defect to file** (FR-107) |

**What Draft 9 changed. All of it is FR-113 — the gate that decides whether the D21.3 ranking fusion
ships — and the shape of the problem is one this document has hit before: a requirement that
specified ONE branch of a decision and stayed silent on the branch reality actually took.** FR-113
required a query set *"written by a human, reviewed by a second"* with graded 0/1/2 judgements. **No
such set exists, and authorship cannot be manufactured.** The implementer raised that before building
rather than substituting a weaker set silently, and built a self-generated known-item harness in the
open. **The two are not interchangeable, and the ruling is about what each is allowed to CONCLUDE.**

| # | Change | Outcome |
|---|--------|---------|
| **1** | FR-113 had one branch; the evaluation has two | **TWO NAMED BRANCHES** (§4 Retrieval and ranking, FR-113). **Branch A — human-graded (0/1/2) — is authoritative in both directions.** **Branch B — a substituted self-generated known-item set — is ONE-DIRECTIONAL: it MUST veto a fusion that regresses and MUST NOT authorise one that improves.** Under Branch B the fusion ships **BUILT, MEASURED and DEFAULTED OFF**, ablation table committed as an artifact |
| **2** | FR-112's "ships or does not ship" has two states; FR-113 now has three | **THE THIRD STATE IS NAMED** (§4, FR-112): **default on** (Branch A cleared), **built and off** (Branch B, no harm), **not shipped at all** (either branch, regression). Revision 5's wording would have forbidden the very artifact that lets a future graded set decide this in an afternoon rather than a wave |
| **3** | Nothing forbade rigging the corpus | **AN ANTI-RIGGING CLAUSE, RECORDED WITH ITS REASONING** (§4, FR-113). Recency and backlink degree MUST be statistically independent of ground-truth selection, **asserted by a test rather than claimed in prose** — because without it *the cheapest way to make the backlink signal look good is to seed the corpus so the answer is always a hub*, and nothing in revision 5 forbade it |
| **4** | The clause needs the incident that earned it | **THE WORKED EXAMPLE IS RECORDED** (§4, FR-113): the first generated corpus produced **520 notes and indexed 430** — a non-injective name pairing let **90 notes silently overwrite each other with no error anywhere** — and skewed the independence property itself (ground truth **6.10** mean backlinks against a corpus mean of **3.63**). **The generalisable line: the fixture silently lost 17% of itself and still looked fine.** A corpus that quietly shrinks produces metrics computed over a population nobody described, and every number downstream is then confidently wrong |
| **5** | Which branch is LIVE was left to inference | **STATED IN PLACE** (§4, FR-113, *Current status*): **Branch B. Built, measured, defaulted off** — `FusionEnabledByDefault = false`. Flipping the constant is not how it is turned on; a Branch A query set is. **§7 test 28 asserts the constant and the verdict agree in BOTH directions**, so neither can outrun the other |

**Plus four defects in the proposed wording and in revision 5's, found by reading them against the
harness that was actually built rather than against themselves.**

| # | Defect | Outcome |
|---|--------|---------|
| **6** | *"MUST veto a fusion that regresses"* is unquantified — **the same unfalsifiability revision 5 removed from the other branch, reintroduced in the new one** | **QUANTIFIED** (§4, FR-113, Branch B): two hard failures against their own predecessor rungs — BM25F must not fall below plain BM25, the fusion must not fall below BM25F — mean nDCG@10, no tolerance beyond floating-point noise |
| **7** | The one-directional rule was derived from the query-independence premise **alone**, which reaches signals 3 and 4 and **does not reach signals 1 and 2** — the two most likely to move the number | **TWO REASONS, SPLIT BY SIGNAL** (§4, FR-113, Branch B). Query-independent signals: uninformative by construction. Query-dependent signals: the **task** is wrong, not the corpus — a known-item task is not the graded-relevance task. **The rule is unchanged; the reason for half of it is** |
| **8** | The threshold's baseline **contradicted between documents**: FR-113 said "plain BM25F", ADR-068 D21.3 said "plain BM25" | **RESOLVED TO THE STATUS QUO — plain, unweighted BM25** (§4, FR-113; ADR-068 D21.3 revision 11). **BM25F is itself part of the change under test**, so deciding against BM25F alone silently grants the field weighting for free. Both columns are reported; the status quo decides |
| **9** | *"the improvement must hold at the 10th percentile"* reads two ways — "improves by 0.03 there too" or "does not regress there" | **PINNED to the second** (§4, FR-113). Leaving it open put the ship/no-ship boundary itself in the hands of whoever wrote the assertion. Also: the metric's **grade scale** is now stated, because under binary relevance nDCG@10 degenerates to `1/log2(rank+1)` and MRR to `1/rank` — two views of one rank, not two confirmations |

**And one honest limit, recorded because the opposite belief is the dangerous one** (§4, FR-113): the
shipped independence guard is a deliberately loose two-sided **1.75×** bound, and **the incident's own
skew was 1.68× — inside it.** The independence assertion as tuned would NOT have caught the corpus
that motivated it; the **duplicate-path and indexed-count assertions** are what caught it. Both
controls are required and neither substitutes for the other.
...
document — and the defect was that no requirement said so, or that the answer sat somewhere a
reader would not look.** That is a different failure from an open design question and it is cheaper
to fix, but it is not less dangerous: an implementer who cannot find the answer invents one, and an
answer that exists in `pkg/records` and nowhere else is one refactor from being lost. **Three of the
ten were closed AGAINST the ruling proposed to this author**, with evidence, and those are marked so
the disagreement is auditable rather than silent.

| # | Under-specification | Outcome |
|---|---|---|
| **1** | `person` has no target type (UAT C-7) | **DECIDED — `to:` is OPTIONAL** (**FR-004b**). Neither offered reading taken; the code already did it. ADR-068 D3's prose was the wrong part, D2's example was right |
| **2** | Unknown schema keys ungoverned (C-8.4) | **WRITTEN DOWN** (**FR-005a**) — `checkDeclaredKeys` already refuses by name and lists the permitted set; three shipped guards cited |
| **3** | Bare `LIKE` unspecified (F-2.4) | **DECIDED — it is exactly `=`** (**FR-022b**). The rule was already in FR-022b as "anchored"; **§4.1.2's operator table, which is the description a caller reads, was silent — that was the real defect** |
| **4** | Empty `IN` list (F-3.4) | **CONFIRMED — refused** (**FR-022d**, which already ruled it). **The refusal STRING was missing** from §4.1.2's table beside its `LIKE` sibling; added |
| **5** | Cross-type ordering comparisons (F-5.5) | **DECIDED, and the proposed ruling is CORRECTED** (**FR-022e**). The literal *is* interpreted in the property's declared type — that instinct is adopted — but one that **cannot** be so interpreted is **REFUSED**, not silently retyped. Retyping a JSON `2` into the text `"2"` is the coercion R-1 forbids in its own words |
| **6** | Templates have no location or format (I-8.3) | **SPECIFIED from the shipped implementation** (**FR-047a**) — `.omnipus-vault/templates/`, ordinary note files, a closed four-token pass. Plus a citation-namespace fix: `template.go`'s `FR-100..FR-102` are **ADR-067's**, and this document's FR-100 is the `.base` importer |
| **7** | Nothing puts the audit log in front of a human (I-6.4, K-5.3) | **REQUIREMENT ADDED** (**FR-077a**) — **not** machine-only. The surface exists: Settings → Security → Audit Log (`AuditLogViewer.tsx`). **Refusals are audited**, stated once generally instead of per tool |
| **8** | Trash convention "still design work" | **CORRECTED — it is NOT pending, and marking it pending would have deleted six ruled behaviours.** A consolidating table under FR-048 maps all six questions to FR-048 / FR-048a / FR-048b / FR-038a. §11's W4 wording, which read as "not yet written down", is fixed |
| **9** | Health view has no route (N-2) | **NAMED** (**FR-025c**) — `/library/health?workspace=<id>`, `/library` becoming a layout route on the `agents.tsx` precedent already in this tree |
| **10** | ADR/spec drift on the candidate caps (F-16) | **ADR FIXED to match this document** — ADR-068 D15.5b showed one 10,000 bound *"counted before retrieval"*; **10,000 is the survivor count and cannot be counted before retrieval**. B1/B2 now appear in both |
...
**What revision 3 changed, in one paragraph.** The agent surface is six new tools — shipped as
`knowledge_*` names, not `vault_*` as this document originally called them, once the naming
decision moved the six replacements onto the prefix the nine retiring tools vacate — split by
**blast radius**, not nine `record_*` tools and not the nine (different, RETIRING) `knowledge_*`
tools shipping today (§4.1). Storage is resolved: bleve keeps text, a derived SQLite properties
index holds typed
...
saved-view authoring leave `knowledge_edit` for a control plane of its own, **`knowledge_configure`**
...
| FR-070 | nine `record_*` tools | five new tools *(superseded by revision 4: **six**)* | §6, §7 test 18, SC-011 |
| FR-072 | compact text for `record_schema` | compact text for **every** result of all five *(superseded by revision 4: **all six**)* | §6, §7 |
| FR-073 | a `record_explain` **tool** | an `explain` **flag** on `knowledge_find` | §6, §7 |
...
| FR | Was | Is | Cited by |
|---|---|---|---|
| FR-016 | new record type is an op of `knowledge_edit` — *"changes no existing note's meaning"* | an op of **`knowledge_configure`**; the premise was **false** under ADR-068 D1 — a new schema file converts every pre-existing note declaring that type | ADR-068 D15.6, D23.3; §4.1.6, US-12, §7 test 37 |
| FR-017 | change/delete existing type is an op of `knowledge_restructure` | an op of **`knowledge_configure`** | ADR-068 D15.6; §4.1.5, §4.1.6 |
| FR-018 | saved view is an op of `knowledge_edit` | an op of **`knowledge_configure`** | ADR-068 D15.6 |
...
| FR-070 | **five** tools | **six** tools — `knowledge_configure` added | §6, §7 test 18, SC-011 |
...
existing-record-type change in a cascading tier. *(Revision 4: that tier is now `knowledge_configure`,
not `knowledge_restructure` — the citation stands, the destination moved.)*
...
| `pkg/coreagent/core.go:357-483` | **98** names | the static builtin catalog the six new tools join (`knowledge_describe`, `knowledge_find`, `knowledge_read`, `knowledge_edit`, `knowledge_restructure`, `knowledge_configure` — the `knowledge_` prefix, not `vault_`, per the naming decision) and the nine RETIRING `knowledge_*` names leave — FR-070a, FR-078. *(Revision 3 said 102. Re-counted over the composite literal at `core.go:358-482`: **98 quoted identifiers, 98 unique, 9 of them the retiring `knowledge_*`**. ADR-068 D15.0 corrects the same miscount.)* |
...
1. ~~There is **no SQLite**.~~ **Superseded by ADR-068 D16.2.** ADR-067 rejected SQLite for
   **search** (its A2), and for search that was right — scorch is better at text. For **records**
   ...
   Markdown (FR-020a).
2. Tool names contain **no dots** — `knowledge_find`, not `vault.find`. A §7 invariant test asserts
   zero dots across builtin tool names. *(The audit **event** names `knowledge.note.edit` and
   `knowledge.note.rename` carry a dot deliberately and are not tool names — FR-071.)* **Revision 5
...
| Retired | Replaced by |
|---|---|
| `knowledge_search` | `knowledge_find` (`words`) |
| `knowledge_graph` | `knowledge_find` (`near` + `hops`) |
| `knowledge_tasks` | `knowledge_find` (`kind: task`) |
| `knowledge_create` | `knowledge_edit` (`create`) |
| `knowledge_link` | `knowledge_edit` (`link`) |
| `knowledge_set_property` | `knowledge_edit` (`set_property`, now scalar **and list**) |
| `knowledge_append_section` | `knowledge_edit` (`append_section`) |
| `knowledge_move` | `knowledge_restructure` (`move`) |
| `knowledge_rename` | `knowledge_restructure` (`rename`) |
| — *(no equivalent today)* | `knowledge_read`, `knowledge_describe`, `knowledge_configure` |
...
| `knowledge.Scope` | **called** | unchanged; all **six** new `knowledge_*` tools resolve through it (`Scope.WorkspaceID`, `.Roots`, `.Contains`, `.Truncated`) |
...
An agent filters and groups records with `knowledge_find` and receives, in one response, both the
...
1. **Given** a note found by `knowledge_find`, **When** it is read, **Then** the response carries a
   version token an immediately following `knowledge_edit` accepts.
...
1. **Given** an agent with `knowledge_configure`, **When** it creates a new record type, **Then** it
   ...
   so (ADR-068 D15.6).
2. **Given** an agent with `knowledge_edit` but **without** `knowledge_configure`, **When** it creates or
   changes any record type or saved view, **Then** it is refused, naming `knowledge_configure` as the
   tool that would allow it. *(Revision 3 put creation in `knowledge_edit`, so this posture was not
   expressible at all — the defect ADR-068 D15.6 corrects.)*
3. **Given** an operator who sets `knowledge_configure: allow`, **When** an agent changes a type,
   **Then** it succeeds — a conservative default is not a prohibition (FR-019a).
4. **Given** an agent with `knowledge_configure: allow` and `knowledge_restructure: deny`, **When** it
...
| Windows, two processes allocating IDs | collision possible (`fileutil.WithFlock` is a documented no-op — `pkg/fileutil/flock_windows.go`). **NOT healed by reconcile** — reconcile heals a lost counter, it cannot un-write two records that already share an identifier. Detected and reported by `knowledge_describe check_integrity`, which names both files and refuses to choose. Accepted, inherited from ADR-054 §5, not introduced here |
...
- When an operation would write bytes into a file the caller did not name (**C-A**), it is
  **only** reachable through `knowledge_restructure`, and the cascade is reported in counts.
- When an operation would change what already-existing files **mean** — their validity, their
  ...
  through `knowledge_configure`, and the count of records whose validity changes is reported.
...
| Formula budget | **64 nodes / depth 8 per formula; 16 formulas and 256 formula nodes per view; MEMOIZED once per candidate** (FR-146). **NEW, Draft 11** — unbounded formulas made the defined worst case 204.8M steps; the caps bring it to **50,000 × (64 + 256) = 16M**, measured by A-14's formula rung, and the caps come down if the measurement says so |
| Column-buffer bound (B3) | **100,000 buffered values or 8 MB, whichever first**; a population-class aggregate (Median, Unique) aborts MID-SCAN above it with the count and the remedy named (FR-151). **NEW, Draft 11** — a value count is not a record count: a `many` property at 20 elements over B1 is 1,000,000 values |
| Backlink derivation | **200,000 link edges within the caller's scope**; mid-scan abort, named refusal, remedy = narrow the scope (FR-132). **NEW, Draft 11** |
...
| Agent tool count | exactly **6** new `knowledge_*` names; **0** of the nine RETIRING `knowledge_*` names; catalog **98 → 95** |
...
| Response budget | **BYTES, not tokens**: ~200–320 bytes/hit, ~4,000 default, **16,000 hard cap**; `minimal` ~80 bytes/hit. **The cap is allocated, not first-come** — mandatory header and `NEXT` are reserved first, then problems to their own clamp, then rows (FR-127c). Scoped to `knowledge_find`, `knowledge_describe` and `knowledge_configure`; `knowledge_read` has its own budget (FR-072a) |
...
- **FR-004c** **NEW, Draft 10 (ADR-068 D24.5). The EIGHTH property type: `checkbox`.** The system MUST support a `checkbox` property type holding YAML `true`/`false` (the strings `true`/`false`, folded, parse; anything else is non-conforming). **Absent is the third state** — D3.2's "days I did not meditate" example finally described a native type. Defined operators: `=`, `<>`, `IN`, `IS NULL`, `IS NOT NULL`; the ordering operators are refused naming the remedy (R-13's pattern). FR-004's count sentence is amended: the count is now **eight**, and this line is the record of when it changed. §8's truth table MUST gain `checkbox` rows and AC-8.4b a mutation that kills one.
  - **AC-4c.1** — `{ type: checkbox }` accepts `true`, `false`, absent; refuses `yes` as non-conforming naming the accepted spellings. **AC-4c.2** — `{checkbox-prop, <, true}` is refused naming the operators defined for the type.
- ~~**FR-004d**~~ **WITHDRAWN, Draft 11 (founder ruling 3) — one draft after it was written, before any code implemented it.** *Draft 10 made `relation`'s `to:` optional so the importer would not have to guess target types. Two reviews showed the relaxation cost more than the import problem it solved: it made FR-004b's "the ONLY thing distinguishing `person` from `relation`" false (the two types collapsed into one mechanism with two names), and it silently killed D5's derived inverse for every untargeted relation — the inverse is derived by knowing which type points back, so an untargeted relation's inverse was reachable in the design and dead in effect.* **The ruling: `to:` stays required on `relation` (FR-004b and AC-4b.3 stand, unchanged), and the IMPORT problem is solved at the importer** — it INFERS the target type from what the links actually resolve to and writes it into the schema it emits (FR-104a); where the evidence is genuinely mixed it says so and declares `text` with the remedy named. Target-type checking stays ON for the vault's standing posture.
...
- **FR-007a** **NEW, Draft 10 (ADR-068 D24.6). For every NON-TEXT type, the empty string IS the absent state.** A value that is `""` (after trimming) on a `date`, `enum`, `integer`, `decimal`, `relation`, `person` or `checkbox` property resolves to **StateAbsent** — `IS NULL` matches it, FR-008 re-includes it under negation, and NO finding is raised. *Why: real vaults write `completed: ""` to mean "unset" — the founder's own does, and 52 findings (46 not-a-date, 4 enum, 2 wikilink) traced to treating a value that visibly is not there as a value that failed.* No non-text type has any reading of `""` as a value, so nothing is lost. For **`text`**, `""` remains a PRESENT empty string. In a `many` property, empty elements resolve as absent elements. A welcome consequence: Obsidian's idiomatic `field != ""` ("is set") translates mechanically to `IS NOT NULL`.
  - **AC-7a.1** — a `date` property holding `""` matches `IS NULL`, does not match `IS NOT NULL`, and raises no finding. **AC-7a.2** — a `text` property holding `""` matches `IS NOT NULL`. **AC-7a.3** — the truth table gains the `""` column for every non-text type and AC-8.4b a mutation that kills it.
...
**What it is NOT is an operation of `knowledge_edit`, and revision 3 had that wrong.** Policy resolves
on the tool **name** alone (FR-070c), so putting schema authoring inside `knowledge_edit` made the
posture *"this agent may edit notes freely, but may not redefine what a note is"* **inexpressible**
— while revision 3's own FR-083 told an operator that restricting `knowledge_restructure` protected
schemas. It did not: an agent holding `knowledge_edit: allow` could create arbitrary record types.
A control described as working while it does not is the failure class this document exists to
remove. **Every schema and view operation therefore lives in `knowledge_configure`** (ADR-068 D15.6).
...
`knowledge_restructure` is C-A. `knowledge_configure` is C-B. `knowledge_edit` is neither, and its one
...
|---|---|---|---|---|
| `set_property`, `append_section`, `replace_body` | `knowledge_edit` | no | no | tier 4, uncontested |
| `create` | `knowledge_edit` | **yes, `.seq`** | **YES — and revision 4 never asked** | **tier 4 by STATED EXCEPTION.** C-A is answered by FR-036a. **C-B is answered YES and the exception is new here:** FR-033 reports a relation whose target does not exist as a validation finding, so creating a note at a path an existing record links to **silences that finding on a note the agent never named** — its validity changes. That is C-B by the identical move that reclassified `create_record_type`. **It stays in tier 4 anyway**, and the ground is *bounded, monotone repair*: the only meaning-change a `create` can produce is turning a dangling relation into a resolved one, which is a **strict improvement in exactly one direction**, affects only notes that already named this path, and is undone by trashing the note. `create_record_type` is the opposite on all three counts — it can invalidate hundreds of notes, reaches notes that named nothing, and its cascade is invisible in the diff. **The exception is bounded by that reasoning and does not generalise**: an operation whose meaning-change can make an existing note *worse* is C-B and belongs in `knowledge_configure`. |
| `link` | `knowledge_edit` | no — writes the source only (FR-030) | **YES — and revision 4 answered only C-A** | **tier 4 by STATED EXCEPTION.** Revision 4's defence (*"it looks like a two-file operation and is a one-file operation"*) answers C-A and nothing else. C-B: FR-032 makes the inverse **derived**, so after the link `knowledge_find near="Acme"` and "the company's related deals" return a different answer for a note nobody named — that is *how a query renders an existing file*. **It stays in tier 4 on the same bounded-monotone-repair ground as `create`**, plus one that is specific to it: the derived inverse is **a view of the source file**, not a state of the target, so the target's meaning is unchanged and only its *rendering under a derived query* moves. **An operator who wants to withhold this withholds `knowledge_edit`**, and FR-079 requires `knowledge_edit`'s description to say that linking changes what queries return about the target. |
| `rename`, `move` | `knowledge_restructure` | **YES** — inbound wikilinks in N notes are rewritten | yes, consequentially | tier 5, uncontested. These are the genuine C-A cases. |
| `trash` | `knowledge_restructure` | **NO** — it moves **one** file and FR-048 explicitly does **not** repair inbound links, so it writes bytes into no file the caller did not name | **YES** — it breaks N existing notes' relations without writing them | **PLACEMENT CONFIRMED IN TIER 5, and revision 4's reasoning REPLACED.** Revision 4 said *"Recoverability and blast radius are different axes"*, which answers a question nobody asked and never applies either criterion. Under the criteria as written, **`trash` is C-B and FR-070b would send it to `knowledge_configure`.** It stays in `knowledge_restructure`, and the ground is stated rather than assumed: **`knowledge_configure` is the CONTROL PLANE — it writes `.omnipus-vault/` and nothing else** (see FR-070e), and `trash` moves a **note**. Putting a note-destroying operation behind the schema tool would mean an operator who grants type authoring also grants deletion, which is a worse posture than the one being fixed. **So the tier is not "C-A only"** — revision 4's framing of `knowledge_restructure` as *"the C-A tier"* is withdrawn: it is **the tier for operations that reach beyond one named note**, of which C-A is the common but not the only shape. |
| every `knowledge_configure` op | `knowledge_configure` | no — one file, inside `.omnipus-vault/` | **YES** | tier 6, uncontested |
...
the grill's O-1, and it is right.)* **Writing into `.omnipus-vault/` is `knowledge_configure`; writing a
...
**`<vault>/.omnipus-vault/trash/<timestamp>/<path>`**, and `trash` is a **`knowledge_restructure`**
operation; FR-048a's `restore` reads out of the same directory and writes back into the vault. So
`knowledge_restructure` writes into `.omnipus-vault/` **on every trash**, §4.1.5's *"`knowledge_configure`
...
> `.omnipus-vault/views/`.** Writing into either is `knowledge_configure`. Two paths inside
> `.omnipus-vault/` are **named non-control-plane** and are written by other tools:
>
> | Path | Written by | Why it is not control plane |
> |---|---|---|
> | `.omnipus-vault/records/<type>.seq` | `knowledge_edit` (FR-036a) | An identifier counter is bookkeeping about records, not a definition of one. **It is inside `records/` and is the one genuinely awkward case** — m-9's note stands |
> | `.omnipus-vault/trash/**` | `knowledge_restructure` (FR-048, FR-048a) | Trash is note storage that happens to live out of the way. It defines nothing and changes no note's validity |
>
...
because `.seq` sits inside the control-plane directory, `knowledge_edit: allow` + `knowledge_configure: deny`
...
- **FR-019a** No policy default may be presented as a prohibition. `knowledge_restructure` **and `knowledge_configure`** are seeded `ask` across the base roster, and an operator MAY set either to `allow` — including for schema changes. A conservative default is not a ban (ADR-068 D23.5).
- **FR-018b** **NEW, Draft 10 (ADR-068 D24.1); AMENDED 2026-08-31 — THERE IS EXACTLY ONE VIEW FORMAT AND IT CARRIES NO VERSION NUMBER (founder ruling; the flat AND-only predecessor is DELETED, not versioned around — no back-compat is owed and no view file predates this tooling). The view format adopts the find grammar.** The product spoke two filter languages — `knowledge_find`'s `VaultFilterNode` tree (`all`/`any`/`not`, the ten SQL operators) and the view format's flat AND-only `RecordFilter` list in the vocabulary ruling R-B retired — with a bridge (`pkg/records/view_find_bridge.go`) that refuses `contains` and `via` leaves outright. Seven filter groups in the founder's vault use disjunction; none was expressible as a view. The view format:
  - **`filters` is ONE `VaultFilterNode` tree.** Obsidian's `and`/`or`/`not` map 1:1 to `all`/`any`/`not`. The seven-op `RecordFilter` vocabulary is retired from the view surface; `contains` translates as an escaped `LIKE '%…%'`, `via` as the request-level join shape. FR-023c's 64-leaf/depth-8 bound applies to a view's tree identically.
  - **`group_by` entries are `{property, direction: asc|desc}`** — the bare name list dropped 24 real direction declarations silently.
  - **`type` is OPTIONAL.** An untyped view queries every note in scope (4 of the founder's 18 bases scope purely by folder, spanning types). **REWRITTEN, Draft 11 (founder ruling 1 — the Draft 10 rule was Opus finding 1.6 by construction):** property resolution in an untyped view is by NAME, over FR-021e's rows, which exist for every note. A note that CARRIES the key — declared by its type or not — holds its value: it is parsed in the domain the view resolves the name to (the declaring type's, when exactly the in-scope types that declare it agree), a value that does not parse in that domain is NON-CONFORMING and reported (R-4), and only a note NOT carrying the key at all is ABSENT. *A note whose file says `status: open` must never answer TRUE to `status IS NULL` — the ruling, verbatim in effect.* Two in-scope types declaring one name with DIFFERENT types refuse the query naming both declarations (FR-024's posture — loud, never a silent domain split). A name no in-scope type declares resolves in the TEXT domain over the raw values.
  - **Display config:** a `property_config` map (property → `display_name`, first) joins the existing display-order list. Obsidian's rule is kept verbatim: display names are never used in filters or formulas. The engine never reads this key.
  - **`layout` (NEW, Draft 11 — FR-109):** which rendering the view asks for (`table`, `cards`; extensible enum). The engine never reads it; the SPA does; the importer MUST carry it (an Obsidian cards view that silently became a table scored "clean" — the green-over-undetected-loss failure this document is written against).
  - **A `formulas` top-level key** — FR-140s below. *(Draft 10's `summaries` key is REMOVED before it shipped — founder ruling 2, FR-150: the fifteen functions extend `aggregates` under the one existing name.)*
  - **A VIEW IS NEVER BROADENED ON THE OPERATOR'S BEHALF — no operator is ever substituted. REWRITTEN, Draft 11 (review finding F5); STRENGTHENED 2026-08-31:** *Draft 10 said a file in the old flat format loads "translated", with `contains` becoming `LIKE '%…%'` — which is BROADENING (`labels contains "in"` matches the element `in`; `LIKE '%in%'` matches `indoor`, `printing`, `min`), applied automatically, in the revision that declared broadening the one prohibited thing; `pkg/records/view_find_bridge.go`'s own header refuses exactly this substitution by name.* Draft 11 withdrew the translation and kept such a file verbatim — listed by `knowledge_describe`, not servable through `knowledge_find`, the reason named — until EXPLICITLY migrated through `knowledge_configure`.

    **The 2026-08-31 amendment deletes the old format**, which makes the hazard structurally absent rather than guarded: a view carries ONE filter tree in ONE operator vocabulary, so there is no mapping step left in which a `contains` could be widened into a `LIKE`. This also retires the standing cost Draft 11 accepted — there is no second operator vocabulary for the comparator to keep alive and no migration list for the health view to empty. **The prohibition itself still binds** and is still enforced where a rewrite can still happen: `knowledge_configure` REFUSES a rewrite that would change the row set, naming the faithful alternatives (`=` for whole-element membership where folding is acceptable — stated, because folding widens case; `LIKE` where a pattern is actually wanted). **No operator-to-operator map may be reintroduced into the view→find bridge; that is the shape of the defect.**
  - **Every wire change is contract-first** (Hard Constraint #8): `ViewDef.yaml` and its dependents change first, both generated trees land in the same commit, `make verify-contracts` exits 0.
- **FR-018c** **NEW, Draft 10 (ADR-068 D24.2). `file.` and `formula.` are RESERVED property namespaces.** Both are valid in every property position of a view or find request; a schema declaring a property whose name begins with either prefix is REFUSED at write time naming the reservation, and an existing one is reported at load. *This is D8's namespacing move, not domain vocabulary — the names are fixed by the system the way SQL's grammar is fixed by SQL (D0 is not breached).*
- **FR-018d** **NEW, Draft 10 (ADR-068 D24.1). A declared record type with ZERO records is a VALID, EMPTY view** — the response carries the ordinary completeness verdict ("0 records, complete"), which is distinguishable from the silent empty FR-024 exists to prevent. *Six of the founder's eighteen bases reference forward-provisioned types written ahead of the data.* `RejectViewUnknownType` still fires for a type NO schema declares — that is drift, not provisioning.
...
- **FR-020g** The agent-facing and human-facing views of index state MUST NOT diverge. `knowledge_find`'s header (FR-020c) and the snapshot FR-020f requires MUST be computed from **one** source; a test asserts the two agree for the same collection at the same generation.
- **FR-020h** **NEW, revision 4 (ADR-068 D16.2a). On a build without SQLite, the record layer MUST REFUSE BY NAME. It MUST NOT return an empty result, and it MUST NOT fail to build.** `modernc.org/sqlite` cannot compile on three targets, each already documented in the tree: `linux/mipsle` softfloat (`pkg/gateway/channel_matrix.go:20-22`), `netbsd/*` (`channel_matrix.go:23-25`) and `freebsd/arm` (`channel_matrix.go:26-28`). Both existing consumers are gated against exactly that set — `pkg/channels/whatsapp_native/whatsapp_native.go:1` and `channel_matrix.go:1`. Three consequences are normative:
  ...
  - **The degradation is a build-tagged stub that registers and refuses**, following `pkg/channels/whatsapp_native/whatsapp_native_stub.go`. `knowledge_read` and the plain-text half of `knowledge_find` keep working, because they resolve through bleve. Typed filters, relation joins, grouping and aggregation MUST each return a refusal naming the platform. **An empty result here would be the exact failure this specification exists to remove**, arriving from the build system.
- **FR-021** **REVERTED to its original meaning, revision 5 (operator ruling R-A). The spec was right the first time.** Filtering, grouping, joining and aggregation MUST be evaluated **in Go**, by the comparator that implements §8's R-1..R-13, over the candidate set the properties index returns. **The properties index NARROWS; the comparator DECIDES.** *Revision 3 moved this into the properties index ahead of ADR-068, and ADR-068 D16.2b then made "the properties index answers every typed predicate" normative. Both are now reversed. §8.1 records the eleven SQLite-semantics defeats this deletes and why deleting the class beats mitigating it.* **Three consequences are normative:**
  - **What SQLite still answers:** set-membership over indexed columns — `record_type = 'deal'`, `path` prefix within scope, `kind = 'task'`, and the **child-table `note_id` join** that assembles a record's rows (for a relation, the join that yields **reachability**). None of these is a comparison governed by R-1..R-13. *(Draft 8 corrects two column names here — the shipped schema declares `record_type` and keys every child table on `note_id` — and generalises the join from relations to child tables; see AC-8.10.)* FR-020's "select candidates without materialising documents that cannot match" is unchanged.
  - **What moves back to Go:** every operator in FR-022b's vocabulary, every grouping, every aggregate, **every SORT**, and the `many`-arity fan-out. **Sorting is named explicitly, revision 6 (review round 6, M-7): revision 5's list omitted it**, and four other places then assumed SQLite did it (FR-021b's *"at `ORDER BY` time"*, FR-066b's `GROUP BY` result set, SC-007's temp-b-tree budget, and R-5 / FR-010 / §4.1.2's *"SQLite's own ordering"*). `sort` is a first-class `knowledge_find` parameter and **ordering IS comparison** — it is governed by R-1, R-4, R-5, R-7 and R-13 like every other comparison, and an omission from this list is how a ruling gets quietly half-applied. **FR-066b's one-page-in-memory rule therefore binds harder, not softer** — a Go pass over a candidate stream must stream, and it is asserted by measured peak RSS at the cap, not by inspection.
...
- **FR-021e** **NEW, Draft 11 (founder ruling 1, verbatim intent: *"can you not just fix them, use common sense"*). The properties index holds EVERY note's frontmatter, typed or not.** `BuildNoteRows` (`pkg/records/propindex/rows.go`) currently returns early when no schema matches, so an untyped note contributes a bare `notes` row and no property rows — and a note whose file says `status: open` would answer TRUE to `status IS NULL` in any untyped view. That early return is removed: for a note with no matching schema (and for keys a matching schema does not declare), every frontmatter key is stored as a RAW property row — raw scalar text in `v_raw`/`v_text`, present/absent state, no declared type. Interpretation stays a QUERY-time question (FR-018b's untyped-view resolution rule); storage is unconditional. **Costs, stated:** `note_props` grows from records-only to the whole vault (the founder's vault: 755 notes instead of ~730 typed ones — trivial; a 50,000-note vault at ~10 keys is ~500k rows, well inside SQLite's comfort); the index schema version bumps with the same rebuild FR-131 already forces; and R-A is untouched — raw rows are streamed and decided in Go like every other row, and no new predicate shape is admitted.
- **FR-022** **`knowledge_find`** MUST accept a structured filter object; the system MUST NOT accept a text query language (ADR-068 O-3, unchanged in substance — only the tool name moved).
...
- **FR-025a** **NEW, revision 5 (operator ruling R-G). Bad values MUST be reported on BOTH surfaces, and the two MUST agree.** One surface is not a substitute for the other, and the reasoning for each is different:
  - **In the answer** — every `knowledge_find` response carries the skipped/excluded block (FR-025, FR-026), **so an agent never reports a total as complete when it is not.** This surface exists for the machine, at the moment of the wrong answer.
  ...
  - **The two MUST be computed from one source and MUST agree.** A test asserts that the set of problems a `knowledge_find` reports for a scope is a **subset** of what the health view reports for that scope, and that a record absent from the health view never appears in a query's problem list. *(Subset, not equality, and the asymmetry is deliberate: FR-020c1's residual hole means a query compares only what it returned, so the health view is the broader of the two by construction. Requiring equality would be requiring the query to do the sweep.)*
...
- **FR-030a** **NEW, Draft 10 (ADR-068 D24.6). The RAW SOURCE TEXT decides what a wikilink is.** An unquoted `- [[Target]]` inside a block list parses as a *nested YAML flow sequence* — `[[…]]` is legal YAML list syntax — and the mis-parse is silent until validation (the root cause of most of 66 wrong-shape findings in the founder's vault). A value MUST be accepted as a wikilink **if and only if its raw source text is exactly wikilink syntax** (`[[…]]`, with the usual `|display` and `#heading` forms), whatever shape YAML parsed it into. The decision reads the operator's own bytes, never the parser's accident, so a genuine nested list (`[[a, b]]`, raw text with a comma) can never be mistaken for a link. D14 is unchanged — the file is never rewritten on read; D5.1's quoting convention remains what OUR writes produce.
  - **AC-30a.1** — a note holding unquoted `- [[Target]]` resolves the relation, raises no wrong-shape finding, and round-trips the file byte-identical. **AC-30a.2** — `related: [[a, b]]` (a real flow list) is NOT treated as a link.
...
- **FR-041a** **NEW, Draft 11 (founder ruling 4). "Set to empty" and "remove property" are TWO write actions, chosen explicitly — never one overloaded behaviour.** The property-write surface offers `set` (a value), **`set_empty`** (writes `""` — for `text`, a present empty string; for any non-text type the response states that the property will READ as absent under FR-007a, so the agent knows what it just wrote), and **`remove`** (deletes the key from the frontmatter). A bare `""` arriving through `set` on a NON-text property is REFUSED asking which of the two was meant, naming both — because on those types the two actions produce indistinguishable reads (both absent) and distinguishable files, and an agent that did not choose should not have a file shape chosen for it.
  - **AC-41a.1** — `set_empty` on a `date` property writes `key: ""` and the write response says the property now reads as absent. **AC-41a.2** — `remove` deletes the key; the file is byte-identical elsewhere (FR-041). **AC-41a.3** — `set` with `""` on a `date` property is refused naming `set_empty` and `remove`; on a `text` property it is an ordinary value write.
- **FR-042** A write violating the schema MUST be rejected with the expected shape named, leaving the file unmodified.
- **FR-043** **SCOPED, revision 5.** A **`knowledge_edit`** write against an existing file MUST carry ADR-067 D14's version token; a stale token MUST be refused and the refusal audited. *Previously unqualified, which contradicted FR-018a (which exempts `knowledge_configure`) and §4.1.5's own parameter table. **The two exceptions are named here rather than left implicit:** `knowledge_configure` (FR-018a, AC-C3) and `knowledge_restructure` (§4.1.5, AC-X3, ADR-068 AC-15.5d) declare no `expect_version`, on the same ground — a single-file token cannot honestly guard a cascade. `knowledge_edit`'s `create` is a third case and needs no token, there being no prior version to compare against.*
- **FR-043a** **NEW, revision 5 — `knowledge_configure` had NO concurrency control at all, and the refusal it relied on is a check-then-write race.** FR-018a's argument for removing `expect_version` is sound as far as it goes, but it removed the protection a token *can* give and put nothing in its place: **two agents concurrently issuing `create_record_type company` both pass the "already declared" check and both write `.omnipus-vault/records/company.yaml` — a silent lost update on the file that defines the type system, with no error, no detection, and an audit trail showing two successful writes and no anomaly.** FR-003's duplicate-type protection does not help, because `create_record_type` writes one path per type so the collision is *within* one file rather than across two; and FR-037's cross-process flock covers `.seq` only. The requirement is:
...
- **FR-044** Every mutating new `knowledge_*` tool MUST emit an audit entry per ADR-067 D19, named per FR-077.
...
- **FR-048a** **NEW, revision 5 — a soft-delete with no restore is a hard delete.** FR-048 justifies the trash path by *"preserving the path so a restore is unambiguous"*, and **no tool in §4.1 has a restore operation**. Three requirements:
  - **`restore` is an operation of `knowledge_restructure`**, taking the trashed path and restoring it to its original relative location. It is C-B by the reasoning of FR-070d (it revalidates relations on notes nobody named) and C-A-adjacent by nothing, but **FR-070e's mechanical rule decides it: it writes a note, so it is not `knowledge_configure`** — and it belongs with `trash` for the same reason `trash` does.
  - **Retention and purge:** trashed notes are purged after **30 days**, matching the session-retention default's shape, and the trash directory's size is reported by `knowledge_describe`. Without a purge the trash grows without bound and the "soft" in soft-delete costs disk forever.
...
- **FR-060** Every one of the **six** new `knowledge_*` tools MUST resolve through the calling agent's workspace scope: agent → workspace → `AllowedMountRoots(home, workspaceID)` → records within those roots. Records are a **stronger** read primitive than search — typed fields, relations and aggregation across them — so the boundary ADR-067 D7 drew applies with no exception.
...
- **FR-070b** **The tool boundary is the policy boundary. REVISED, revision 4: there are TWO criteria, not one.** `knowledge_edit` MUST write **only** the file named in its `path` argument, plus `.seq` on `create` (FR-036a). `knowledge_restructure` is the only tool permitted to **write bytes into** a file the caller did not name (**C-A**). `knowledge_configure` is the only tool permitted to **change what existing files mean** — their validity, their type, or how a query renders them — **without writing them** (**C-B**). A reviewer decides an operation's tool by asking both questions in order. *Revision 3 stated one criterion and, following ADR-068 revision 5, applied it under two incompatible readings — bytes to keep `link` in the per-file tier, meaning to push a schema change into the cascading one — and then presented the result as evidence the criterion generalises. **ADR-068 D23.3 withdraws that claim in revision 6.** A criterion that needs two readings is two criteria; the design is not weaker for having two, the document was weaker for pretending it had one.* Additive-versus-destructive remains explicitly **not** a criterion — `set_property` overwrites and stays in `knowledge_edit`.
- **FR-070c** An operation enum **within** one tool is permitted and carries no policy meaning, and **the seeded default for a tool MUST be chosen for the WIDEST operation it grants, not the most common one**. Policy resolves on the tool **name** only — `resolveToolPolicyAtExec(ts *turnState, toolName string, …) string` (`pkg/agent/loop.go:12418`) takes no arguments and no operation discriminator — so every operation reachable from one tool MUST be equally acceptable to an operator who granted that tool. *(Revision 4: this is why FR-016..FR-018 move out of `knowledge_edit`. It is also why `knowledge_edit`'s description must name `replace_body` — its widest remaining operation — rather than `set_property`, its most common one; see FR-079.)*
- **FR-071** Tool names MUST contain no dots. *(Audit event names are not tool names: FR-077's `knowledge.note.edit` / `knowledge.note.rename` carry a dot deliberately and are out of this requirement's scope.)*
- **FR-072** **MEANING WIDENED, revision 3 (ADR-068 D22.1); extended to six in revision 4.** **Every** result from **all six** tools MUST be rendered to the model as compact text, never as a JSON document. *Previously: `record_schema` alone. Cited by §6 and §7 test list.* **Revision 5 narrows one over-broad consequence:** "never a JSON document" is a rule about the **envelope**, not about the bytes. `knowledge_read` returns a note's body, and a note may legitimately contain a fenced JSON code block; §7 test 25's "asserts no JSON document is emitted" MUST NOT be applied to `knowledge_read`'s body content, or the test forbids reading an ordinary note.
- **FR-072a** **NEW, revision 5.** FR-127's budget is scoped to `knowledge_find`, `knowledge_describe` and `knowledge_configure`. **`knowledge_read` has its own budget and its own truncation contract**, because a note is not a result set: its `max_bytes` default of 40,000 (§4.1.3) is **2.5× FR-127's 16,000-byte hard cap**, and revision 4 stated both without scoping either, so the two read as a contradiction. `knowledge_read`'s contract is: **`max_bytes` bounds the note content**, truncation is stated in the header and never silent, and the version token, the typed-frontmatter block and the refusal strings are **outside** the bound and always emitted. A note larger than `max_bytes` is truncated with `section` named as the remedy — never refused, because refusing to read a large note leaves the agent with `read_file` and no scope check (US-9).
- **FR-073** **MEANING CHANGED, revision 3 (ADR-068 D15.3).** `explain` MUST be a **boolean argument of `knowledge_find`**, not a tool. With `explain: true` the system MUST report what the query would select, which properties it could not evaluate and why, and MUST NOT evaluate the query. *Previously: a `record_explain` tool. Cited by §6 and §7.*
- **FR-074** `knowledge_read` MUST return the note's **version token** in every successful response. Obtaining a token MUST NOT require sending a write that is expected to fail. *(Today the only source of a token is `*ConflictError` from a refused `EditNote` — `pkg/knowledge/author.go:701`; `EditNoteRequest.ExpectVersion` (`author.go:566`) refuses an empty token too.)*
- **FR-075** **WIDENED, revision 4 (ADR-068 D15.3).** `knowledge_describe` MUST accept an integrity sweep (`check_integrity: true`) reporting **duplicate identifiers** (FR-039), **unresolvable and mistyped relations** (FR-033, FR-034), **unresolved ordinary wikilinks and orphan notes across the whole vault**, and **rows in the properties index with no note behind them** (FR-020c), and MUST name every offending path. *The wikilink and orphan half is not optional and not a nicety: `knowledge_graph`'s `unresolved` and `orphans` cover ordinary wikilinks vault-wide (`pkg/knowledge/tools.go:809-811`), **most notes in a vault are not records**, and FR-070a retires that tool — so without this widening a vault-wide broken-link report would have no home in the new surface at all.*
...
- **FR-080** Every one of the **six** new `knowledge_*` tools MUST have an explicit, literal, wildcard-free policy entry for **every** seeded agent — all ten `coreagent.SeedConfig` creates (`mia`, `jim`, `ava`, `ray`, `worker`, `planner`, `explorer`, `researcher`, `judge`, `plansupervisor`), not only the four base agents. `worker`'s map is sparse, and in a sparse map **absence grants**.
- **FR-080a** **EXTENDED, revision 4.** The seeded posture MUST be: `knowledge_describe` / `knowledge_find` / `knowledge_read` **allow** for Mia, Jim, Ava and Ray and **deny** for workers and the rest; `knowledge_edit` **ask** for Mia and Ray, **allow** for Jim and Ava, **deny** elsewhere; `knowledge_restructure` **ask** for all four base agents and **deny** elsewhere; **`knowledge_configure` ask for all four base agents and deny elsewhere**. Reads are `allow` roster-wide because a prompt in front of a read that `read_file` already permits protects nothing and trains the operator to click through the prompts that do.
...
- **FR-112** **CONDITIONAL, revision 5 — this was a MUST that FR-113 may forbid, which is not a requirement.** Ranking MUST fuse four signals with **Reciprocal Rank Fusion** **if and only if FR-113's comparison clears its stated threshold; otherwise the default ranking is plain BM25F and this FR does not ship.** **AMENDED, Draft 9 — that sentence has only TWO outcomes and FR-113 now has three.** Under FR-113's **Branch B** (a substituted known-item measurement, which is the live branch today) the fusion ships **BUILT, MEASURED and DEFAULTED OFF**: the code exists, is exercised by §7 test 28, and is reachable only through `RankOptions.Fusion`, whose zero value is off. *(The distinction is not bookkeeping. "Does not ship" as revision 5 wrote it would have forbidden the very artifact that lets a future graded query set decide the question in an afternoon rather than a wave — and would have made an implementer who built it anyway look non-compliant. The three states are: **DEFAULT** on Branch A's cleared threshold; **BUILT, OFF** on Branch B's no-harm result; **NOT SHIPPED AT ALL** on either branch's regression.)* *(Revision 4 stated FR-112 unconditionally and FR-113 as a gate that could forbid it, so the two requirements contradicted each other in the ordinary case where the fusion does not win. The conditionality now lives in FR-112's own text, where an implementer reads it.)* The signals are: BM25 over weighted fields (BM25F-style, title and name above body), exact/prefix name match, recency, and backlink degree. RRF operates on **ranks, not scores**, so no cross-signal normalisation is required — normalising a BM25 score against a recency score against a degree count is a tuning problem with no principled answer, and RRF removes the question instead of answering it badly.
- **FR-113** **REVISED, Draft 9 — revision 5 specified ONE branch of this gate, and the query set that branch requires does not exist.** *(Revision 5 made an unfalsifiable gate falsifiable, which was the right fix and is kept whole below. What it did not anticipate is the ordinary case: an implementer reaches W2, finds no human-authored graded query set, cannot manufacture authorship, and must choose between measuring nothing and measuring something else. Revision 5 said nothing about what "something else" is allowed to CONCLUDE — which is the entire question, because a substituted measurement permitted to authorise is worse than no measurement at all: it manufactures the confidence the gate existed to withhold. The implementer who hit this raised it before building rather than substituting silently, which is the handling this document wants and could not have relied on.)* The fusion MUST ship behind a measured comparison, and the comparison is specified so that it can fail:
  - **Metric: nDCG@10.** Chosen over MRR because the vault case is "give me the right handful", not "give me the one right answer", and nDCG rewards getting a good set into the visible page rather than only the top hit. **Where relevance is BINARY — one ground-truth note per query — nDCG@10 degenerates to `1/log2(rank+1)` and MRR to `1/rank`: two views of one rank, not two independent confirmations of a result, and any report printing both MUST say so.** *(Draft 9: revision 5 named the metric without naming the grade scale it is computed over, so "the same metric" silently meant two different things across the two branches below.)*
  - **Corpus: the §7 fixture corpus**, **committed rather than generated per run**, holding at least **500 notes** across at least **four record types**.
  - **Query set: at least 30 queries per evaluated condition**, committed alongside the corpus, **each with its relevance judgement fixed BEFORE either ranking was run.** *(Draft 9 replaces revision 5's flat "30 queries" with "at least 30 per condition", which is what a fixture evaluating more than one condition actually commits.)*
  - **Baseline: the STATUS QUO — plain, unweighted BM25 — is the baseline that decides shipping.** *(Draft 9 resolves a contradiction BETWEEN documents rather than a gap in one: revision 5 worded the threshold against "plain BM25F" and ADR-068 D21.3 worded it against "plain BM25". The difference is not pedantic — **BM25F field weighting is itself part of the change under test**, so measuring the fusion only against BM25F silently grants the field weighting for free. Both columns MUST be reported; the one that decides is the comparison against the ranking real users have today. ADR-068 D21.3 is amended to match in revision 11.)*
  - **Threshold: mean nDCG@10 must improve by at least 0.03 absolute**, and the fusion's **10th percentile of the per-query distribution MUST NOT fall below the baseline's** — i.e. the fusion must not win on average by badly damaging a minority of queries. *(Draft 9 pins the second clause. Revision 5 wrote "the improvement must hold at the 10th percentile", readable either as "improves by 0.03 there too" or as "does not regress there". **The second is meant**, and leaving it to the reader put the ship/no-ship boundary itself in the hands of whoever implemented the assertion.)*
  - **Branch A — HUMAN-GRADED judgements (0/1/2), written by a human and reviewed by a second.** The measurement is **authoritative in BOTH directions**: the fusion ships as the default **if and only if** the threshold above is met.
  - **Branch B — SUBSTITUTION.** Where no human-graded set exists, a **SELF-GENERATED KNOWN-ITEM set MAY be substituted** — sample a sentence from a note, reduce it to the query a person would type, and the ground truth is the note the sentence came from — **and its verdict is ONE-DIRECTIONAL: it MUST veto a fusion that regresses, and it MUST NOT authorise one that improves.** Under substitution the fusion ships **BUILT, MEASURED and DEFAULTED OFF**, with the ablation table committed as an artifact.
    - **What "regresses" means is stated numerically, because leaving it open would reintroduce exactly the defect revision 5 removed from the other branch.** Two hard failures, each against its own predecessor rung so a difference is attributable: **BM25F must not fall below plain BM25**, and **the full fusion must not fall below BM25F** — mean nDCG@10, evaluated on the verdict condition, with no tolerance beyond floating-point noise. Field weighting ships on established IR rather than on this eval, but established IR is not a licence to make our own corpus worse, and a known-item eval is fully competent to detect that it has.
    - **The judge is unbiased in the one way that matters:** relevance is established by **PROVENANCE**, not by lexical overlap with any ranker's notion of similarity, so no ranking under test had a hand in deciding what the right answer is. That is what makes the veto sound even though the authorisation is not.
    - **Two reasons the verdict cannot run the other way. They cover DIFFERENT signals and both are needed.** (i) The **query-independent** signals — recency and backlink degree — are by construction uninformative about which note is ground truth (the corpus is built that way; see the anti-rigging clause below), so they can only add noise: a flat or positive result says they **did no harm**, and can say nothing else about them. (ii) For the **query-dependent** signals — field weighting and the exact/prefix name prior — the reason is different, and it is the **task** rather than the corpus: **a known-item task is not the graded-relevance task**, and binary provenance judgements are not the 0/1/2 judgements the 0.03 threshold was calibrated against. *(Draft 9 splits these deliberately. The proposed wording derived the whole one-directional rule from the query-independence premise alone — but that premise reaches signals 3 and 4 and does not reach signals 1 and 2, leaving the two signals most likely to move the number unaccounted for by the stated reason. **The RULE is unchanged; the reason for half of it is.**)*
  - **ANTI-RIGGING — the corpus MUST be constructed so that recency and backlink degree are statistically independent of ground-truth selection, and that independence MUST be ASSERTED BY A TEST rather than claimed in prose.** Without this clause the cheapest way to make the backlink signal look good is to seed the corpus so that the answer is always a hub, measure the backlink signal, and discover that it works — and **nothing in revision 5 forbade it.** The clause is recorded with its reasoning because a rule whose reason is not written down is a rule the next person deletes as ceremony.
    - **The worked example is not hypothetical — it happened while this harness was built. The fixture silently lost 17% of itself and still looked fine.** The first generated corpus produced **520 notes and indexed 430**: generated entity names collided, because the pairing `first[i%10] + second[(i/2)%6]` is **not injective**, so **90 notes silently overwrote each other on disk, with no error anywhere.** It also skewed the very independence property the corpus exists to have — ground truth showed **6.10 mean backlinks against a corpus mean of 3.63**, because the notes that survived the collisions were disproportionately hubs. It was fixed with an injective pairing plus a hard duplicate-path assertion.
    - **The generalisable rule is the one worth keeping: a corpus that quietly shrinks produces metrics computed over a population nobody described, and every number downstream is then confidently wrong.** The note count is not bookkeeping — it is the denominator of every figure in the ablation table, and the one quantity no reader of that table will think to question.
    - **So the fixture MUST assert its own size in TWO places, and both are cheap.** The generator MUST refuse a **duplicate note path** (`pkg/knowledge/rank_eval_corpus_test.go` — *"the corpus would silently lose a note"*), and the harness MUST fail when **indexed count is below generated count** (`pkg/knowledge/rank_eval_test.go::newEvalHarness` — *"an eval over a partial index measures nothing"*). One catches the collision at construction, the other catches every other way a note fails to reach the index.
    - **And the honest limit of the independence assertion itself, recorded because the opposite belief is the dangerous one.** The shipped guard (`TestRankEval_GroundTruthIsNotPrivileged`) is a deliberately **loose two-sided bound**: ground-truth mean backlink degree and mean age must stay within **1.75×** of the corpus mean, in both directions — loose on purpose, because thirty samples from a heavy-tailed degree distribution genuinely wander and a guard that cries wolf gets disabled. **But the incident's own skew was 6.10 / 3.63 = 1.68×, which is INSIDE that bound.** The independence assertion as tuned would therefore **not** have failed the corpus that motivated it; the **duplicate-path and indexed-count assertions are what caught it**. Both controls are required, neither substitutes for the other, and anyone tightening or loosening one should know which of them actually fired. *(The ratio arithmetic is ours, computed from the reported figures against the shipped constant; the tolerance and its rationale are the harness's own.)*
  - **Decision: the architect records the verdict**, with the run and the per-query table attached. A result inside the threshold is a **no-ship as the default** for FR-112, not a judgement call.
  - **CURRENT STATUS, stated so no reader has to infer which branch is live: BRANCH B, as of Draft 9 (2026-08-28).** No human-graded query set exists. The fusion is **BUILT, MEASURED against a self-generated known-item set, and DEFAULTED OFF** — `FusionEnabledByDefault = false` (`pkg/knowledge/rank.go`), and `RankOptions`'s zero value is BM25F weighting with no fusion, so a caller who has not thought about ranking gets the configuration the evidence supports rather than the one that is most interesting. **Flipping that constant is not how the fusion is turned on; producing a Branch A query set is, and the ablation must be re-run against it.** Branch A has never been exercised. **The code and the evidence are held together by an assertion rather than by this paragraph** — §7 test 28 fails if `FusionEnabledByDefault` and the measured verdict ever disagree in either direction, so a future graded set that clears the bar cannot leave a stale constant behind it, and a flipped constant cannot outrun its evidence.
  - **Holdout scenario 11's human judgement over 20 queries is a DIFFERENT and weaker criterion and is not this one.** It stays in §9 as a sanity check; it MUST NOT be substituted for this measurement, and §9 is explicitly not for use during development. **In particular it is not Branch A** — 20 ungraded queries judged during a holdout exercise is neither the size nor the grade scale Branch A requires.
  - **FR-112, SC-018 and §7 test 28 are written against this**, because as revision 4 wrote them they recorded a comparison without asserting anything about it, and as revision 5 wrote them they admitted only Branch A's binary outcome.
...
- **FR-128** **CORRECTED, revision 5 — the number is right for what it measures and wrong for what it was used to argue.** Each tool description MUST fit ~150 tokens (FR-079). **Six** tools at that budget is **~900 tokens of DESCRIPTION PROSE** — and that is **not** the surface's standing per-turn cost, because the whole parameter schema ships with it on every request (FR-079's correction, verified at `pkg/tools/registry.go:557-560`). The ~900 is a **floor**. The comparison against ~2,700 for nine `record_*` plus nine `knowledge_*` is still directionally right — eighteen prose budgets against six — but **it MUST NOT be quoted as the affordability argument** until W5 measures the serialised definition set. *(ADR-068 D15.6 cites "~750 for the surface as a whole", which is the **five**-tool figure and is stale as well as measuring the wrong thing; the ADR's revision 7 corrects both.)* *(The sixth tool's whole standing price is those ~150 tokens — ADR-068 D22.8. Tool **count** costs selection accuracy; tool **descriptions** cost tokens on every turn forever, for every agent holding the tool, whether or not it is ever called. `knowledge_configure` is selected only when an agent is authoring a type or a view, an intent distinct enough that it competes with nothing — the tools that hurt selection are near-synonyms.)*

### File metadata (ADR-068 D24.2) — NEW, Draft 10

- **FR-130** The system MUST expose **thirteen virtual properties** under the reserved `file.` namespace, usable in every property position (filter leaf, sort, group_by, properties, summary target, formula operand) and evaluated by the ONE comparator under R-1..R-13 like any declared property: `file.name`, `file.path`, `file.folder`, `file.ext` (text); `file.mtime`, `file.ctime` (date instants); `file.size` (integer, bytes); `file.tags` (many text, stored fully qualified — `a/b`); `file.links`, `file.embeds`, `file.backlinks` (many link); `file.properties` (many text — the note's frontmatter key set, **read from FR-021e's rows, which exist for every note since Draft 11** — a real storage source, not a description; the full map is a formula operand); `file.file` (a formula/display operand only — never a filter target). A date-only literal compared against an instant means the start of that day, UTC; stated here so two implementers cannot pick two readings.
- **FR-131** **Storage is split by what each value needs, and NONE of it changes what SQL decides.** `file.name`/`path`/`folder`/`ext` are DERIVED in Go from `Candidate.Path` — no storage. `file.mtime`/`ctime`/`size` are three new columns on `notes`, written at index time **and refreshed by FR-136 when content does not change but stat does**. `file.tags` is a new `note_tags` child table (`note_id`, `elem`, `tag`; frontmatter and inline); `file.links`/`embeds` a new `note_links` child table (`note_id`, `elem`, `target`, `embed`), from the same body parse the link graph performs. **The assembly strategy is NAMED, because the obvious one is D16.6's fan-out defect returning (Draft 11, review finding F6): each child table is streamed by its OWN statement under the shared narrowing `WHERE` — the exact `Store.Tasks`/`Store.Relations` pattern — and assembled per note in Go. `note_props`, `note_tags` and `note_links` are NEVER joined to each other; a multi-child join is a cross-product no parent-anchored de-duplication repairs.** The index schema version bumps and the store rebuilds itself (`NeedsFullIndex`, FR-020a). `propindex.Selector` keeps exactly its three fields; no `file.*` predicate reaches SQL, ever.
- **FR-132** **`file.backlinks` is DERIVED, never stored — and, Draft 11, it is SCOPED and BOUNDED.** Derivation: computed in Go by streaming `note_links`, the `Store.Relations` pattern (FR-032's derived-inverse precedent). **Scope: the CALLER'S WORKSPACE SCOPE (FR-060), not the view's own filter** — Obsidian's backlinks are vault-wide, and "vault-wide within the caller's scope" is the closest semantics that does not leak across FR-062's boundary. **Bound: the edge stream aborts MID-SCAN above 200,000 edges, a named refusal carrying the count and the remedy (narrow the scope).** *Draft 10 cited `near.go` as precedent while shipping no cap, no abort and no refusal — the three things `near.go` actually has; the citation now describes the mechanism.* It is declared expensive in the tool description, as Obsidian's own documentation declares it.
- **FR-133** **`file.ctime` is the file's BIRTH time where the platform records one; where it does not, the value is ABSENT** — flagged under FR-007's rules, never silently substituted with the POSIX inode-change time that shares the name. An honest absence over a plausible wrong answer, which is this document's standing trade.
- **FR-134** **The four file methods are TRANSLATIONS, never grammar** — the structured-object rule (O-3) admits no function-call syntax in a filter. Normatively: `file.hasTag(x)` ⇒ `{any: [{file.tags, =, x}, {file.tags, LIKE, x/%}]}` (hierarchy-aware, pattern escaped); `file.inFolder(x)` ⇒ the same shape over `file.folder` (the folder and its descendants); `file.hasLink(x)` ⇒ `{file.links, =, x}`; `file.asLink()` lives in the formula layer (FR-147).
- **FR-135** **Ruling R-A is UNTOUCHED by all of the above, and the control that proves it is extended on BOTH halves — Draft 10 extended the construct half only, which is the exact two-thirds omission D16.6a blocked on one revision ago (Draft 11, review finding F7).** No `file.*` predicate reaches SQL; every comparison runs in the Go comparator over values the candidate stream delivered; `propindex.Selector` keeps EXACTLY its three fields (the growth guard of D16.6a's watch-point stays armed). The extension, concretely: **(a) construct half** — the per-child-stream statements for `note_tags`/`note_links` join the allow-list under the admitted `<child>.note_id = <parent>.note_id` shape; **(b) COLUMN half** — test 39a's no-predicate column list gains **`mtime`, `ctime`, `size`** (metadata a predicate may never mention — a stat comparison is a comparison, and it is the Go comparator's), and the value-column denylist gains **`note_tags.tag`, `note_links.target`, `note_links.embed`**. `TestQuery_NoComparisonIsDelegatedToSQL` MUST pass against the extended statements AND the extended column lists before anything else in W7 is accepted (the same gate C-5 set for W1).
- **FR-136** **NEW WORK, Draft 11 (the highest-consequence review finding): stat metadata must survive the hash-equal sync skip, or `sort: file.mtime desc` — the commonest Bases view there is — silently rots.** The deep-sync skip (`pkg/knowledge/index.go::SyncWith`, the `opts.Deep` branch: *"hash the note and skip it if the bytes are identical after all"*) already updates the MANIFEST's `Size`/`ModTimeNanos` on a hash-equal skip — and would leave the properties index's new stat columns at the last content change. `git checkout`, rsync, `touch`, an iCloud resync: all change stat without content, with no error anywhere. **The requirement:** on ANY sync path that skips re-indexing because content is unchanged, if the walk's stat differs from what the `notes` row holds, the store performs a **metadata-only UPDATE** of `mtime`/`ctime`/`size` — no re-parse, no bleve write, no manifest-ordering change (D16.5's write ordering is untouched because no content hash moves). **Attachments refresh through the same path from the walk's own stat** — stat is not open, so ADR-067's FR-039a holds — which also un-freezes the Draft 10 design's frozen-at-first-index attachment sizes. **Cost, stated:** one stat comparison per walked file against values the walk already holds, plus one stat-columns UPDATE per drifted file; a full-vault `touch -R` worst-cases at one UPDATE per note, bounded by vault size, not query size. Asserted by test 99 through the recorder (no re-parse statements on a metadata-only refresh), not by inspection.

### Formulas (ADR-068 D24.3) — NEW, Draft 10

- **FR-140** **The formula parser lives in the WRITE path, and only there.** `knowledge_configure` and the importer parse an expression at write time and REFUSE one that does not parse, naming the position and reason — it is never stored. The view loader re-validates on load (a hand-edited file is re-checked; `LoadViews`' existing posture), rejecting with the `RejectView*` pattern. **`knowledge_find` accepts no text expression anywhere**; a query reaches a formula only as `formula.<name>`, a reference to something already validated. Both halves of O-3 hold on the query path — no text language, no parser — and a parse failure can never become an empty result, because an unparseable formula never exists at query time.
- **FR-141** A view stores each formula as its SOURCE TEXT (human-diffable, comparable against the Obsidian original), under the top-level `formulas:` map (name → expression).
- **FR-142** **Evaluation happens in Go, per candidate, after narrowing.** A formula value is a typed value computed from the candidate's streamed values; used in a filter, sort, group or summary it goes through the ONE comparator like any property value, and comparisons INSIDE an expression delegate to that same comparator (§8's single-implementation rule). Nothing reaches SQL; `Selector` is untouched; AC-8.10's role is unchanged.
- **FR-143** The expression grammar is Obsidian's documented surface and is CLOSED: arithmetic `+ - * / % ( )`, boolean `! && ||`, the comparison operators, the function set `if`, `toFixed`, `mean`, `round`, `date`, `today`, `now`, `format`, `list`, `link`, `icon`, `contains`, the `.time()`/`.date()`/`.hour` accessors, and the four file methods. A name outside the set is refused BY NAME at parse time listing the supported set (FR-024's posture applied to functions). **PINNED, Draft 11 (review finding F12 — a closed grammar against a drifting upstream spec is otherwise a moving target nobody notices moving):** "documented" means the Bases syntax reference **as fetched 2026-08-30** from `https://obsidian.md/help/bases/syntax`; the snapshot's source and date are recorded here, a construct outside it is refused and reported as UNDOCUMENTED (FR-107's label), and adopting a newer snapshot is a SPEC REVISION with its own diff — never a silent code change.

- **FR-143 — PIN REVISION 1, 2026-09-01: the Date type field family.** This is the first exercise of FR-143's own revision clause, and it is recorded here in full because the clause is worthless if a pin can move without a diff. Two implementers in a row REFUSED to add `.year` and `.month` on FR-143 grounds and both were right to; the founder ruled that the accessors should be adopted, and that what had been forbidden was doing it quietly.
  - **Snapshot adopted:** the Bases **functions** reference, `https://obsidian.md/help/bases/functions`, **as fetched 2026-09-01**. The 2026-08-30 pin named the *syntax* reference, which states only that "many other fields and functions are available on date objects" and defers to the functions page; the functions page is where the Date fields are actually enumerated, so this revision names both pages as the pinned surface rather than one.
  - **What was added — the "Date type › Fields" table, WHOLE, all seven rows:** `date.year`, `date.month`, `date.day`, `date.hour`, `date.minute`, `date.second`, `date.millisecond`, every one typed `number`. Only `.hour` had previously been transcribed. The table is adopted entire and not two rows at a time: a snapshot taken piecemeal is not a snapshot, and the seven are one receiver, one result type and one meaning (extract a calendar component from a date). Each is a WHOLE number — declared scale 0, since there is no month 3.5.
  - **What was excluded, and why nothing was:** the ruling asked which of a family to adopt and why the rest are left out. There is no rest. The reference's Date table has exactly seven rows and defines no `.week`, `.quarter`, `.dayOfWeek` or `.dayOfYear`; those exist in other expression languages, which is why they are refused BY NAME rather than by accident.
  - **Singular is a date field, plural is a duration total** — the reference's own distinction, now load-bearing in both directions. `.second` is the seconds place of a clock reading (0–59); `.seconds` is a whole span expressed in seconds. Reading the wrong one is a typed REFUSAL naming the receiver, never a plausible wrong number.
  - **What was NOT adopted from the same fetch, deliberately.** The 2026-09-01 fetch shows upstream has RESTRUCTURED its duration model: the "Duration Type" field table that FR-143's 2026-08-30 snapshot supplied is absent from both pages, and "Date arithmetic" now says subtracting two dates yields "the millisecond difference" — a number, not a duration object. Adopting that would DELETE `.days`, `.hours`, `.minutes`, `.seconds` and `.milliseconds` and change what `dateA - dateB` MEANS. That is a behavioural revision rather than an addition, it breaks sixty-five working uses in the founder's own vault, and it is outside this ruling. **The duration model therefore stays pinned at 2026-08-30, the divergence is recorded here as known and dated, and it is owed its own decision.** A future revision that takes it up must weigh the vault breakage explicitly; it must not be inherited silently by the next person who re-fetches the page.
  - **The pin stays enforceable.** `pkg/records/formula_date_field_test.go`'s `TestFormula_AccessorSurfaceIsPinned` asserts the parenless accessor set EXACTLY — not "contains" — so the next addition, however obviously correct, fails a test that names this clause and the procedure. The wall the two refusing implementers hit is still there; only its position moved.
  - **Scope, honestly stated:** this unblocks the *grammar* half of `Deals::Closing This Month` (`date(close_date).year == today().year`) and nothing else. That view stays DISABLED, because its filter is an EXPRESSION and a filter leaf holds a property, an operator and a literal — an expression reaches a leaf only as a `formula.<name>` the base declares. The import counts are unchanged by this revision: CLEAN=10, WITH-LOSSES=8, REFUSED=0, 69 views, 3 disabled.
- **FR-143a** **NEW, Draft 11 (review finding F4 — R-1 made the failure silent). Every formula has ONE static type and ONE arity, inferred and validated at write/load — never a per-record surprise.** The type is determined by the expression's structure: literals and functions have declared result types, property operands have their declared types, and **`if()`'s branches must agree in type (or one branch be absent)** — `if(c, 1, "x")` is REFUSED at validation naming both branch types. Arity is single unless built by `list()` or a `many` operand. The comparator sees the formula's declaration exactly as it sees a schema property's; without this, a formula returning `decimal` on some records and `text` on others compares FALSE under R-1's different-domains rule with NO problem reported — a silently wrong answer wearing a type system. Carried as **R-18/FR-217** with truth-table and mutation coverage.
- **FR-144** **Arithmetic is exact and FR-013's no-binary-float rule survives it.** Internal arithmetic runs over exact rationals; a result crosses into display or comparison at a DECLARED scale — `toFixed`/`round` where the author said so, otherwise scale 10, round-half-even — and a rounded value is labelled as rounded. `%` is defined over integers only, refused otherwise naming `round()`. Division by zero yields an ABSENT result plus a named problem, never a silent zero. (§8 R-15.)
- **FR-145** **Absence propagates.** An arithmetic or function step over an absent operand yields absent (R-2's discipline extended); `if()` treats an absent condition as false; a non-conforming operand is a reported problem and an excluded record, never a value (R-4 unchanged). (§8 R-14.)
- **FR-146** **REWRITTEN, Draft 11 (review finding F2 — Draft 10's cost claim was wrong by a factor of ~64).** *Draft 10 said a formula "multiplies against B1 exactly as filter leaves do." It multiplies against B1 AND the leaf count: 64 leaves × 64-node formulas × 50,000 candidates = **204.8M** steps over exact rationals, against the 3.2M that FR-023c's numbers were chosen to define — and nothing capped formulas per view at all.* The recomputed contract:
  - **Memoization is REQUIRED:** each distinct formula is evaluated ONCE per candidate and its result reused across every leaf, sort key and aggregate that references it. Per-candidate cost is therefore **additive** (leaves + total formula nodes), not multiplicative.
  - **Three caps, refused above each naming which:** one formula ≤ **64 nodes / depth 8** (FR-023c's numbers); a view ≤ **16 formulas**; a view's formulas ≤ **256 nodes in total**.
  - **The defined worst case: 50,000 × (64 + 256) = 16M evaluation steps** — five times FR-023c's 3.2M, stated as such rather than hidden, of which up to 256 per candidate are rational-arithmetic operations. The A-14/W measurement gains a formula rung at both bounds AT these caps, and **the caps come down if the measurement says so** — A-14's existing posture; they do not get absorbed. Both figures join the constraint table.
  - `now()`/`today()` are snapshotted ONCE per query, so one response is internally consistent.
- **FR-147** **Presentation values are not comparable.** `link()`, `icon()`, `asLink()`, `format()` produce display values; the comparator refuses a comparison over one with the reason named. (§8 R-16.)
- **FR-148** **NEW, Draft 11 (review finding F3 — an unspecified HANG, the worst severity a spec defect has).** `a: formula.b + 1` / `b: formula.a + 1` parses clean, passes every Draft 10 bound, and recurses forever. **The formula REFERENCE GRAPH is validated at write AND at load: a cycle is REFUSED naming its path** (`a → b → a`). The guard is the active-set walk this package already applies to YAML aliases — `pkg/records/frontmatter.go`'s `b.active[n.Alias]` check exists precisely because self-referential structures hang evaluators — applied where Draft 10 omitted it. Cross-formula references also COUNT toward FR-146's 256-node view total (a referenced formula's nodes counted once, since memoized), so a reference chain cannot smuggle cost past the caps.

### Summaries (ADR-068 D24.4) — NEW, Draft 10

- **FR-150** **REWRITTEN, Draft 11 (founder ruling 2: "summaries: ONE NAME").** The system MUST support **all fifteen** summary functions **as new ops of the EXISTING `aggregates` surface** — there is NO `summaries` wire key, on a view or anywhere else; Draft 10's proposed key is removed before it shipped. The fifteen, per view and per group when grouped: Average (`avg`), Min, Max, Sum, Range, Median, Stddev, Earliest, Latest, Checked, Unchecked, Empty, Filled, Unique, plus the existing `count`. Domain rules: Range is max − min in the number AND date domains (a date range renders as a duration); Earliest/Latest are the date domain's extremes; Checked/Unchecked count `true`/`false` over `checkbox` properties (FR-004c); Empty/Filled count absent/present over ANY type. The op enums (`RecordAggregate.yaml`, `VaultFindAggregate.yaml`) grow accordingly, contract-first. **Obsidian's top-level `summaries` key and per-view summary map TRANSLATE at import into `aggregates` entries** — parity is capability, not key names.
- **FR-151** **REWRITTEN, Draft 11 (review finding F9 — the Draft 10 memory estimate was computed on the friendliest case).** *Two computational classes.* *Streaming (O(1) accumulator):* Sum, Min, Max, Average (exact sum + count), Range, Earliest, Latest, Checked, Unchecked, Empty, Filled, count — and Stddev, which streams exactly via count, sum and sum-of-squares in rational arithmetic, rounding only at the final square root. *Population (one COLUMN buffered):* **Median and Unique only.** They hold the summarised property's VALUES — and a value count is NOT a record count: a `many` property at 20 elements over B1's 50,000 candidates is **1,000,000 values**, and 1 KB text values at B1 are **50 MB** — the "low single-digit MB" Draft 10 stated was the scalar-decimal case presented as the general one. **The bound is therefore its own, named refusal — B3, the column-buffer bound: a population-class aggregate aborts MID-SCAN above 100,000 buffered values or 8 MB of buffered bytes, whichever first**, with the count reached and the remedy (narrow the filter, or aggregate a scalar property) — the cap/abort/refusal triple `near.go` actually implements. B3 joins FR-064's table and the constraint table as a third bound alongside B1 and B2; FR-066b is amended in place; A-14/W measures Median and Unique AT B3's caps.
- **FR-152** **Precision is DECLARED, which reverses the recorded no-`avg` ruling by satisfying its own objection.** `RecordAggregate.yaml` refused `avg` as "a number whose precision nobody declared." Average, Median (even count) and Stddev are computed exactly in rationals and RENDERED at a declared scale — the property's declared scale + 2, round-half-even, overridable — with the response labelling the value as rounded. The contract's description is superseded and MUST be rewritten in the same commit that adds the ops.
- **FR-153** **Stddev is POPULATION standard deviation, stated in the response label.** Obsidian's documentation does not say which theirs is; ours declares its definition rather than guessing a match, and the importer records the divergence risk by name (FR-107) — never silence.
- **FR-154** **No summary is ever computed over a truncated set.** A bound that refuses returns NO summary (FR-066's never-partial rule). A median quietly taken over whatever fit is the exact failure this document exists to remove, restated here because Median is the first summary for which it is tempting.
- **FR-155** A summary over a property whose type does not define it (Stddev over text, Checked over a date) is REFUSED naming the summaries defined for the type — FR-024's posture, applied to summaries.
...
- **FR-104** **NEW, Draft 10 (ADR-068 D24.7). The parity translation table is NORMATIVE.** Every construct in Obsidian's documented Bases grammar has a translation target in this specification — the `file.*` properties and methods (FR-130..FR-134), formulas (FR-140s), the fifteen summaries (FR-150s), `and`/`or`/`not` as `all`/`any`/`not`, `groupBy` direction, top-level `properties` as display config (FR-018b) — and the importer MUST translate all of it. The measured baseline this exists to close: the founder's 18 bases, **10 converted with named losses, 8 refused, ZERO clean**; the target is 18 of 18.
- **FR-104a** **NEW, Draft 11 (founder ruling 3). The importer INFERS a relation's `to:` from what the links actually resolve to, and writes it into the schema it emits** — `to:` stays required on `relation` (FR-004b, unchanged; FR-004d is withdrawn). The inference rule, stated so two importers agree: targets resolving UNANIMOUSLY to one type declare it; a **supermajority (≥ 2/3 of resolved targets)** declares the majority type WITH the minority targets reported by name (they will surface as FR-034 findings, which is correct — they ARE type mismatches, now visible); below that, or with nothing resolved, the property is declared `text`, the report SAYS a relation could not be typed, and it names the one-line `knowledge_configure` edit that fixes it once the operator decides. *Observed live: 30 relation properties needed exactly this; the run guessed plurality (`contact.related` → `to: task` on a 2-of-5 plurality) because nothing told it where guessing had to stop.*
- **FR-104b** **NEW, Draft 11 (founder ruling 1, second half). Untyped notes are not left stranded: the importer infers and WRITES `type:` where the shape matches.** For each note carrying no `type:` key, the importer compares its frontmatter key set against the schemas it inferred; a match writes `type:` into the note's frontmatter — an ordinary FR-041-governed edit, reported per note in the import report. No match: the note stays an ordinary note, fully indexed (FR-021e) and reachable through untyped views — stated so "left as is" is a recorded outcome, not an accident. *The founder's vault: 27 such notes.*
- **FR-105** **The broadening prohibition — the one import rule that admits no exception.** An imported view MUST NEVER return MORE rows than its original while looking correct. An untranslatable expression in any **row-set-affecting** position (a filter, anywhere in its tree) imports the view **DISABLED**: a new `disabled` field on `ViewDef`, the expression preserved verbatim in `untranslated` (FR-101), and APPLYING the view refused naming the expression — never imported with the clause dropped. *The standing example: a base filtering `type == "decision"` AND NOT `inFolder("99-Temp")` AND NOT `inFolder("00-Inbox")` — drop the folder clauses and the view quietly includes every scratch note.* **The ORACLE, named (Draft 11, review finding F8 — a prohibition without an oracle is a hope):** enforcement is STRUCTURAL — every clause either has a mapping in FR-104's normative translation table, each mapping carrying its own row-set-faithfulness test, or the view disables; "never more rows" follows by induction over clauses, not by counting anything at import time. The fixture-level assertion (test 98) compares imported views against **hand-derived expected row sets committed as data at fixture-authoring time** — derived from the base's documented semantics by a person, never recomputed from the importer's own output, which would be the importer grading itself.
- **FR-106** An untranslatable expression in an **annotation** position (display config, a summary) cannot change which rows appear; the view imports ENABLED with the loss declared in `untranslated`, per FR-101.
- **FR-107** **Under parity, every residual `untranslated` entry is a DEFECT to file, not an accepted loss — and the import report MUST say so in those words.** FR-101's neutral "reported" posture is superseded for anything the documented grammar covers; it survives only for genuinely undocumented behaviour, which the report labels as such (the Stddev definition gap of FR-153 is the standing example).
- **FR-108** **A filter literal is schema evidence.** A base filtering `realm == "personal"` proves `personal` is a permitted value even when no sampled note carries it; the importer's enum inference MUST count literals appearing in the base's own filters, or the run refuses a view against the very schema it inferred (observed live: 2 views lost clauses this way).
- **FR-109** **NEW, Draft 11 (the card-view finding — a green number over an undetected loss is the failure this whole surface is written against).** The view format carries a **`layout`** field (`table`, `cards`; extensible enum, contract-first); the importer MUST read each Obsidian view's `type` and carry it. A layout the product does not yet render imports with the loss NAMED as an annotation loss (FR-106) — so an Obsidian cards view can never become a table silently and score CLEAN under W7's exit criterion. The engine never reads `layout`; the SPA does.
...
### 4.1.1 `knowledge_describe` — READ, orientation and integrity
...
- **AC-D2** — `knowledge_describe` with an unknown `record_type` is refused with the declared type
...
### 4.1.2 `knowledge_find` — READ, the one retrieval path
...
| Third hop | `hops=3 exceeds the limit of 2; run a second knowledge_find from one of these results` |
...
| No manifest entry (FR-020c) | `DEAL-0221: no indexed note at Deals/old.md — the row is orphaned; run knowledge_describe check_integrity` |
| Typed query on a SQLite-less build (FR-020h) | `typed filters are unavailable on linux/mipsle: this build has no properties index. Plain-word search and knowledge_read still work` |
...
### 4.1.3 `knowledge_read` — READ, a note or one section of one
...
- **AC-R1** — a successful `knowledge_read` carries a version token that `knowledge_edit` accepts
  unchanged (FR-074).
...
### 4.1.4 `knowledge_edit` — WRITE, one named file
...
`knowledge_configure` operations — FR-016, FR-018. Revision 3 placed them here on the ground that a
...
| Condition | Message |
|---|---|
| Stale token (FR-043) | `Deals/Acme.md changed since you read it; you have v1:ab12…, current is v1:cd34… — knowledge_read it again and re-apply` |
...
| Cascading op sent here (C-A) | `rename cascades to notes you did not name; use knowledge_restructure` |
| Schema or view op sent here (C-B) | `create_record_type changes what existing notes mean; use knowledge_configure` |

- **AC-E1** — after any successful `knowledge_edit`, **exactly one file has a changed mtime and hash — except on a `create` of a record, where exactly TWO do** (the note and `.seq`, FR-036a), and on no operation more than two. *(Revision 6, m-6: revision 5 stated the one-file rule unqualified and repaired it in AC-E2a two bullets later, while §7 test 49 asserts "exactly **two** paths change on a **record** `create`" and "exactly ONE on a non-record create". The exception is folded into AC-E1's own text, because an unqualified criterion is what a test gets written to.)*
  This is FR-070b as a test, and it is the tier's definition.
- **AC-E2** — `create_record_type` or `write_view` sent to `knowledge_edit` is refused, naming
  `knowledge_configure` (FR-016, FR-018). An agent holding `knowledge_edit: allow` and
  `knowledge_configure: deny` **cannot** create a record type — the posture revision 3 claimed and
  did not deliver.
...
### 4.1.5 `knowledge_restructure` — WRITE, cascades
...
> AC-X3 in the same section asserted *"`knowledge_restructure` declares **no** `expect_version` on any
> cascading op that cannot honour it"* — and **every** operation of `knowledge_restructure` is a
...
> contradicted FR-018a's exemption for `knowledge_configure` and now this table too.

*(Revision 4: `edit_record_type` and `delete_record_type` are **removed from this tool** and are
`knowledge_configure` operations — FR-017. They cascade in **meaning** (C-B), not in bytes (C-A), and
...
writes a note, and `knowledge_configure` writes only `.omnipus-vault/`.** Putting a note-destroying
...
37 asserted that every schema and view op is refused by `knowledge_restructure` naming `knowledge_configure`
— a behaviour this section never specified.*

| Condition | Message |
|---|---|
| A schema or view op sent here (C-B) | `create_record_type changes what existing notes mean; use knowledge_configure` |
| A one-file note edit sent here | `set_property writes one note; use knowledge_edit` |
| Version token supplied (§4.1.5, AC-X3) | `knowledge_restructure takes no expect_version: a single-file token cannot guard a rename that rewrites inbound links in notes you did not name. Re-read with knowledge_read and re-send` |
| `restore` of a path not in the trash (FR-048a) | `no trashed note at Deals/Acme.md; knowledge_describe reports the trash contents` |
...
- **AC-X2** — an operator policy of `knowledge_edit: allow` + `knowledge_restructure: deny` permits
  `set_property` and refuses `rename` in the same session (FR-083).
- **AC-X3** — `knowledge_restructure` declares **no** `expect_version` on any cascading op that cannot
...
### 4.1.6 `knowledge_configure` — WRITE, cascades in meaning
...
`knowledge_edit` cannot be granted or withheld on its own.

**It writes one file** — a schema or a view — **and reinterprets many notes.** That is C-B, and it
is the opposite shape to `knowledge_restructure`, which writes many files and reinterprets none.
...
| A cascading-in-bytes op sent here (C-A) | `rename writes notes you did not name; use knowledge_restructure` |
| A one-file note edit sent here | `set_property writes one note; use knowledge_edit` |
| Version token supplied (FR-018a) | `knowledge_configure takes no expect_version: a single-file token cannot guard a change to every note declaring this type. Re-read with knowledge_describe and re-send` |
...
- **AC-C2** — an agent holding `knowledge_edit: allow` and `knowledge_configure: deny` **cannot** create,
...
- **AC-C3** — `knowledge_configure` declares no `expect_version` parameter; a test asserts its absence
...
- **AC-C4** — `delete_record_type` reports how many records revert to ordinary notes, and a
  subsequent `knowledge_find type=<deleted>` is **refused** naming the declared types, never returning
  an empty result (FR-024).
- **AC-C5** — every `knowledge_configure` call emits a `knowledge.configure` audit entry carrying the
...
## 4.2 The `knowledge_find` response — a literal worked example, normative
...
NEXT
  page       knowledge_find cursor="c2FnZTI"
  narrow     knowledge_find type=deal filter=(status = 'open' AND arr >= 100000)
  widen      knowledge_find near="Companies/Acme Ltd.md" hops=2 words="pricing" type=<any>
  fix        knowledge_read path="Deals/DEAL-0052.md"   then knowledge_edit set_property arr=50000
```
...
… 9 more
NEXT  knowledge_find cursor="c2FnZTI"  |  knowledge_find detail=standard
```
...
NEXT  knowledge_find type=company words="prospect"  |  knowledge_describe record_type=company
```
...
- **AC-P5** — a `knowledge_configure` response renders its cascade-in-meaning counts in the same
  block position `knowledge_restructure` renders its cascade-in-bytes counts: after the rows, before
...
- **SC-001** The two-hop question from ADR-068 §1.2 is answered by one `knowledge_find` call with no hand-maintained state and no regular expression.
...
- **SC-011** **CORRECTED, revision 4.** The static builtin catalog contains exactly **six** new `knowledge_*` names and zero of the nine retiring `knowledge_*` names, and `allStaticToolNames` has **95** entries. *(Revision 3 said five and 98. 98 is the count **today**, before any change — re-counted at `pkg/coreagent/core.go:358-482`: 98 quoted identifiers, 98 unique, 9 `knowledge_*`. After the swap: 98 − 9 + 6 = 95.)*
- **SC-012** **WIDENED, revision 4.** All three write policies are independently settable, proven by test in one session: `knowledge_edit: allow` + `knowledge_restructure: deny` permits a property write and refuses a rename; `knowledge_edit: allow` + `knowledge_configure: deny` permits a property write and refuses `create_record_type`.
...
- **SC-018** **AMENDED, Draft 9 — revision 5's criterion admitted only FR-113's Branch A, and Branch B is the live branch.** The FR-112 fusion is compared against **plain, unweighted BM25 — the status quo — and against BM25F, both columns reported**, on the committed fixture corpus and the committed query set (≥ 30 queries per condition), measured as **nDCG@10**. **Branch A (human-graded 0/1/2):** the fusion ships **as the default if and only if** mean nDCG@10 improves by ≥ 0.03 absolute AND the fusion's 10th percentile does not fall below the baseline's (FR-113). **Branch B (substituted known-item set):** the fusion ships **built, measured and defaulted off** when it does not regress, and **does not ship at all** when it does; it can never ship as the default on this evidence. **In both branches the code's default MUST be asserted to agree with the measured verdict**, so the constant and the evidence cannot drift apart unnoticed. A result inside the threshold is a **no-ship as the default for FR-112**, recorded with the per-query table. Separately, AC-8.6's set-invariance is asserted **over a fixture in which the two orders demonstrably differ** — a run where they do not differ fails, because it proves the fixture did not exercise the change.
- **SC-019** An agent obtains a version token via `knowledge_read` and completes a write with zero failed writes in between.
- **SC-020** A client mounting the collection panel after indexing completed renders the completed state, and it matches the freshness `knowledge_find` reports for the same collection.
- **SC-021** A `knowledge_find` response over a partial answer places its completeness verdict on line 1, and a test asserts block order `header → rows → totals → problems → next`.
...
| FR-104..108 | US-7 | 7.1–7.3 | `TestImport_UntranslatableFilterDisablesNeverBroadens` — **NEW, Draft 10** |
| FR-004c, FR-007a, FR-030a | US-1, US-4 | edge | `TestFilter_AbsentIsDistinctAndIncludedByNegation` (extended); `TestComparisonTruthTable` (checkbox + `""` rows); `TestRelation_InverseIsDerivedAndSurvivesRename` (raw-text wikilink fixture) — **NEW, Draft 10** *(FR-004d withdrawn, Draft 11)* |
| FR-021e | US-2 | 2.1 | `TestView_UntypedViewResolvesUndeclaredProperties` — **NEW, Draft 11 (founder ruling 1)** |
| FR-041a | US-4 | 4.2 | AC-41a.1..3 under `TestWrite_ByteIdenticalOutsidePatchedSpan`'s fixture — **NEW, Draft 11 (founder ruling 4)** |
| FR-136 | US-2 | 2.1 | `TestFileMeta_StatRefreshSurvivesHashEqualSkip` (test 99) — **NEW, Draft 11** |
| FR-143a, FR-148, FR-217 | US-2 | 2.1 | `TestFormula_CyclesAndTypesRefusedAtWrite` (test 100) — **NEW, Draft 11** |
| FR-151's B3 | US-2 | 2.2 | `TestAggregates_ColumnBufferBoundRefusesNamed` (test 101) — **NEW, Draft 11** |
| FR-104a, FR-104b, FR-109 | US-7 | 7.1–7.3 | import-report assertions inside `TestImport_UntranslatableFilterDisablesNeverBroadens`'s fixture run — **NEW, Draft 11** |
| FR-018b..018d | US-2 | 2.1 | `TestView_V2TreeExpressesDisjunction`; `TestView_UntypedViewSpansTypesViaAbsence` — **NEW, Draft 10** |
| FR-130..135 | US-2 | 2.1 | `TestFileMeta_EvaluatedInGoNeverInSQL`; `TestFileMeta_CtimeAbsentWhereBirthtimeUnknown` — **NEW, Draft 10** |
| FR-140..147, FR-213..216 | US-2 | 2.1 | `TestFormula_ParseFailureIsWriteTimeRefusal`; `TestFormula_AbsencePropagatesAndArithmeticIsExact` — **NEW, Draft 10** |
| FR-150..155 | US-2 | 2.2 | `TestSummaries_AllFifteenWithDeclaredPrecision`; `TestSummaries_BoundRefusalReturnsNoSummary` — **NEW, Draft 10** |
...
| 22 | `TestTools_EditWritesOnlyNamedFile` | integration | FR-070b — snapshots every file's hash, runs each `knowledge_edit` op, asserts exactly one changed |
...
| 28 | `TestRank_FusionMeetsNDCGThreshold` | integration | FR-113, SC-018, AC-8.6 — **AMENDED, Draft 9; revision 5 specified only Branch A's assertion.** Measures **nDCG@10** over the committed corpus and query set, reports the **five-rung ablation ladder** (bm25 → bm25f → +name → +recency → +backlinks) as an **artifact**, and asserts four things: **(a)** BM25F does not regress plain BM25; **(b)** the full fusion does not regress BM25F — the Branch B veto, a hard failure in both cases; **(c)** FR-113's ≥ 0.03 / 10th-percentile threshold is **evaluated and reported**; **(d)** the outcome of (c) **agrees with `FusionEnabledByDefault`** in both directions — a cleared threshold with the default still off FAILS, and a default turned on without a cleared threshold FAILS. **(d) is the assertion that matters**: it is what stops the shipped default and the evidence for it from drifting apart silently. AC-8.6's set-equality is asserted **over a fixture whose two orders demonstrably differ** — a run in which they do not differ FAILS |
| 28a | `TestRankEval_GroundTruthIsNotPrivileged` | unit | FR-113 anti-rigging clause — **NEW, Draft 9.** Asserts that the verdict condition's ground truth is statistically ORDINARY: mean backlink degree and mean age within a stated two-sided bound of the corpus mean. **Its limits are part of the requirement** — the bound is loose by design, and FR-113 records that the incident which motivated the clause would have passed it. Paired with, never a substitute for, the duplicate-path and indexed-count assertions |
| 28b | `TestRank_EvalHasHeadroom` | unit | FR-113 — **NEW, Draft 9.** Fails when the fixture could not have detected a difference: a baseline scoring near 1.0 makes every rung report the same number, and five identical rows read as a careful null result when they are a corpus with no discriminating power. **A "no difference" verdict from a fixture with no headroom is the most expensive kind of false green**, because it looks exactly like evidence |
...
| 32 | `TestIndexes_SourceHashDivergenceIsIncomplete` | integration | FR-020c — **tests divergence, not rebuild, in BOTH directions.** Writes a row, re-indexes the modified note into bleve only (SQLite write suppressed), asserts the returning `knowledge_find` reports `complete: false` with the record named and staleness as the reason; then the symmetric case. A rebuild-only criterion would pass with the mitigation absent. Also asserts an orphaned row (no manifest entry) and an empty hash are both flagged, never assumed fresh |
...
| 37 | `TestSchemaAuthoring_LivesInVaultConfigure` | integration | FR-016, FR-017, FR-018 — **REPLACES revision 3's `TestSchemaAuthoring_TierPlacement`, which asserted the wrong placement.** Every schema and view op is refused by `knowledge_edit` **and** `knowledge_restructure`, naming `knowledge_configure`, and accepted by `knowledge_configure`. Creating a new type over a vault already holding notes of that type asserts the conversion count and the newly-failing records (AC-C1) |
...
| 41 | `TestToolPolicy_ThreeWriteTiersAreIndependent` | integration | FR-083, SC-012 — `knowledge_edit` / `knowledge_restructure` / `knowledge_configure` set independently; the `edit: allow, configure: deny` case asserts `create_record_type` is refused |
...
| 48 | `TestWritePath_RateLimited` | integration | FR-067 — `knowledge_edit`, `knowledge_restructure` and `knowledge_configure` each 429 with `Retry-After`. **Asserts the limiter is reached at all**, because today no write `Execute` consults one |
...
| 90 | `TestView_TreeExpressesDisjunction` | unit | FR-018b — **NEW, Draft 10; RENUMBERED, Draft 11; RENAMED 2026-08-31 with the one-format amendment.** A view whose filter is `{any: […]}` loads, validates and returns the union; a grouping direction is honoured and its absence defaults stated |
| 91 | `TestView_UntypedViewResolvesUndeclaredProperties` | integration | FR-018b, FR-018d, FR-021e — **REWRITTEN, Draft 11 (founder ruling 1):** an untyped, folder-scoped view returns records of several types; a note whose file SAYS `status: open` does NOT match `status IS NULL`, whether or not its type declares `status`; a truly missing key is absent; two in-scope types declaring one name with different types REFUSE the query naming both; a declared-but-empty type renders "0 records, complete" |
| 92 | `TestFileMeta_EvaluatedInGoNeverInSQL` | unit | FR-130, FR-131, FR-135's guard extension — every `file.*` comparison passes through the comparator; the AC-8.10 recorder shows the new tables queried ONLY by dedicated per-child streams under the admitted join shape (never a multi-child join); no predicate mentions `mtime`/`ctime`/`size` or any new value column; `Selector` still has exactly three fields |
| 93 | `TestFileMeta_CtimeAbsentWhereBirthtimeUnknown` | unit | FR-133 — on a platform without birth time, `file.ctime IS NULL` matches and the problem list names unknown-creation-time; nothing substitutes the inode-change time |
| 94 | `TestFormula_ParseFailureIsWriteTimeRefusal` | unit | FR-140 — an unparseable expression is refused at `knowledge_configure` time naming the position; a hand-edited view file with a broken formula is REJECTED at load; no formula error is reachable through `knowledge_find` |
| 95 | `TestFormula_AbsencePropagatesAndArithmeticIsExact` | unit | FR-144, FR-145, R-14/R-15 — absent operand yields absent; `1/3` compared at declared scale; division by zero is a named problem, never zero; no float64 appears on the path (FR-013's assertion reused) |
| 96 | `TestAggregates_AllFifteenWithDeclaredPrecision` | integration | FR-150..FR-153 — each of the fifteen ops over a fixture with a hand-computed expected value; Average/Median/Stddev labelled with their scale; Stddev's label says population; there is NO `summaries` key anywhere on the wire (founder ruling 2) |
| 97 | `TestAggregates_BoundRefusalReturnsNoValue` | integration | FR-154 — a query refused at B1/B2/B3 carries NO aggregate value; a median over a truncated set is unconstructable, asserted by the refusal path not by inspection |
| 98 | `TestImport_UntranslatableFilterDisablesNeverBroadens` | integration | FR-105, FR-106 — a base with one untranslatable filter clause imports DISABLED and applying it is refused naming the clause; every ENABLED imported view's row set equals the hand-derived expected set COMMITTED WITH THE FIXTURE (FR-105's oracle — expectations derived from the base's documented semantics at fixture-authoring time, as data, never recomputed from the importer's own output) |
| 99 | `TestFileMeta_StatRefreshSurvivesHashEqualSkip` | integration | FR-136 — **NEW, Draft 11 (the Opus staleness finding).** `touch` a note (content unchanged), deep-sync, and `file.mtime` answers the NEW time; an attachment's size changes on disk and `file.size` follows without the file being opened (FR-039a); the refresh is a metadata-only UPDATE — the recorder shows no re-parse, no bleve write |
| 100 | `TestFormula_CyclesAndTypesRefusedAtWrite` | unit | FR-143a, FR-148, R-18 — `a: formula.b + 1` / `b: formula.a + 1` is refused at write time naming the cycle path; `if(c, 1, "x")` is refused as mixed-type; a stored formula's declared type is static and the comparator sees exactly one declaration |
| 101 | `TestAggregates_ColumnBufferBoundRefusesNamed` | integration | FR-151's B3 — a Median/Unique over a `many` property whose value population exceeds the cap aborts MID-SCAN with a named refusal carrying the count and the remedy; peak RSS at the cap measured, not inspected (near-`near.go` pattern, this time with the cap it was cited for) |
...
concurrently (test 57); a `knowledge_find` running concurrently with `SyncWith` under `-race`
...
narrows. `record_type = 'deal'`, `path` prefix within scope, `kind = 'task'`, and the **child-table
`note_id` join** that assembles a record's rows — for a relation, the join that yields
*reachability* — these are set-membership questions over indexed columns, they are
...
### 8.1a-bis — Draft 10: four new rules for formulas and the eighth type (R-14..R-17, carried as FR-213..FR-216)

The formula layer and the `checkbox` type extend the rule table rather than growing a second
one. Each rule below gets truth-table cells and an AC-8.4b mutation that kills a named cell,
exactly as R-1..R-13 have; a rule without a mutation is prose, and prose does not survive
refactors.

- **R-14 / FR-213 — absence propagates through expressions.** Any arithmetic or function step
  over an absent operand yields ABSENT; `if()` treats an absent condition as false and is the
  one sanctioned way to give absence a value. R-2's discipline, extended one layer up.
- **R-15 / FR-214 — exact rationals inside, declared scale at the boundary.** No binary float
  exists anywhere in evaluation (FR-013). A value crosses into display or comparison at a
  declared scale (`toFixed`/`round`, else scale 10, round-half-even), labelled as rounded.
  Division by zero is an absent result plus a named problem. `%` is integer-only.
- **R-16 / FR-215 — presentation values do not compare.** `link()`, `icon()`, `asLink()`,
  `format()` produce display values; a comparison over one is refused with the reason named.
- **R-17 / FR-216 — `checkbox` compares by equality only.** `true`/`false` (folded spellings),
  absent is the third state, `""` is absent (FR-007a), ordering operators refused naming the
  remedy (R-13's pattern).
- **R-18 / FR-217 — a formula has ONE static type and arity, declared before any record is
  touched (Draft 11, FR-143a).** Inferred from the expression's structure at write/load;
  `if()` branches must agree in type or one be absent. The comparator receives the declaration
  exactly as it receives a schema property's — without this, a formula whose type varied per
  record would compare FALSE under R-1 with no problem reported, a silent wrong answer wearing
  a type system.

**AC-8.4c** — the mutation table grows five rows (Draft 11 adds the fifth): remove R-14's
propagation (a formula over an absent operand must stop yielding a value, not zero); remove
R-15's declared-scale boundary; remove R-16's refusal; remove R-17's equality-only disposition;
remove R-18's static-type validation (a mixed-type `if()` must stop parsing). Each must kill a
named cell.
...
11. Run the same corpus through plain BM25 and through the FR-112 fusion; have a human judge the
    ...
    committed query set measured as **nDCG@10** against a stated threshold; **this holdout MUST
    NOT be substituted for it**, and §9 is explicitly not for use during development.
    **Draft 9 adds the reason it is not Branch A either:** 20 ungraded queries judged during a
    holdout exercise is neither the size nor the 0/1/2 grade scale FR-113's Branch A requires, so
    running it cannot move the fusion off Branch B's defaulted-off outcome.
...
13. Build for `linux/mipsle` and confirm the binary builds, the gateway boots, `knowledge_read` and
    plain-word `knowledge_find` work, and every typed filter refuses **naming the platform** rather
...
| ~~A-9~~ | **RESOLVED 2026-08-28, in this spec's favour.** Revision 3 read ADR-068 D20's two listings of `trash` as: the **primitive and its convention** land in W4, the **`knowledge_restructure` operation** exposing it lands in W5, and said that if the ADR meant otherwise the spec should be corrected rather than reconciled. **ADR-068 revision 6's D20 adopts exactly that split in its own words** — *"the convention is W4 design work, the operation is W5"* — and names the failure it avoids: shipping part of a tier-5 tool before W5 defines the tool or seeds its policy. FR-048 is unchanged and is now the ADR's reading too. |
| **A-10** | **PARTLY RESOLVED, revision 4, and narrowed.** The half that was a real defect is fixed: `create_record_type` no longer sits inside `knowledge_edit` at all — it is `knowledge_configure` (FR-016), so granting `knowledge_edit` no longer silently grants the type system. **The residual is genuine and stays LIVE:** granting `knowledge_edit` still grants `replace_body` as well as `set_property`, and an operator reading the tool name will not infer that. FR-070c and FR-079 now require every tool description to name its **widest** operation rather than its most common one — `replace_body` for `knowledge_edit`, `delete_record_type` for `knowledge_configure`, `trash` for `knowledge_restructure`. **Needs:** a review that the six shipped descriptions actually do this, at W5, not an assertion here that they will. |
...
| **W5** | FR-016..FR-019a, FR-043a, FR-048a's `restore` operation, FR-049a, FR-070..FR-084 (incl. **FR-077a**), FR-090..FR-092, FR-127..FR-128 | **FR-049a's migration scan reports EMPTY.** FR-079's serialised six-tool definition set is **measured in bytes** and written back into FR-128. A-10's description review |
| **W6** | FR-020f, FR-025a's health view, **FR-025b**, **FR-025c** (the `/library/health` route and the `/library` layout split), FR-100..FR-103 | FR-025a's two surfaces agree **within one workspace scope** (FR-025b), asserted by test 58 |
| **W7** | **NEW, Draft 10; REVISED, Draft 11 (ADR-068 D24 rev 13) — Bases parity, in dependency order:** *(a)* the model rulings — FR-004c, FR-007a, FR-030a, **FR-021e** (every note's properties indexed), **FR-041a** (explicit empty-vs-remove) — with their truth-table and mutation rows, because everything later evaluates through them *(FR-004d is withdrawn — `to:` stays required and the importer infers it, FR-104a)*; *(b)* the view format — FR-018b..FR-018d, **FR-109's `layout`** — contract-first; *(c)* file metadata — FR-130..FR-135 with the index schema bump and BOTH halves of the AC-8.10 guard extension in the SAME change, **plus FR-136's metadata-only refresh** (new work — without it `sort: file.mtime desc` rots silently); *(d)* aggregates — FR-150..FR-155 with **B3**; *(e)* formulas — FR-140..FR-148, FR-213..FR-217 — last, because they consume all of the above; *(f)* the importer re-run — FR-104..FR-109 incl. **FR-104a/FR-104b** | **The exit criterion is the number that started this:** the founder's 18 `.base` files re-imported; every `untranslated` entry names either an enabled-with-annotation-loss or a DISABLED view, **zero broadened views** (test 98's assertion against hand-derived expected row sets committed with the fixture — FR-105's oracle), **every view's `layout` carried** (FR-109 — a cards view rendered as a table is a NAMED loss, never clean), and the clean count reported against the 0-of-18 baseline. A-14's measurement extended: the formula rung AT FR-146's caps, Median/Unique AT B3's caps, and test 99's stat-refresh assertion. `TestQuery_NoComparisonIsDelegatedToSQL` still green with BOTH guard halves extended — the R-A control outliving the largest capability addition since it was written is the whole point of having it |
