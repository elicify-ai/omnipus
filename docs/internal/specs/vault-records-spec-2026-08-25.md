...
| **F5** | Draft 10's v1→v2 load TRANSLATED `contains` to `LIKE '%…%'` — broadening, automatically, in the revision that made broadening the one prohibited thing (`view_find_bridge.go`'s own header refuses exactly this) | **FR-018b rewritten**: a v1 view keeps v1 semantics VERBATIM; views using `contains`/`via` stay exactly as today (listed by describe, not servable by find, reason named) until EXPLICITLY migrated through `knowledge_configure`, which refuses a lossy rewrite naming the faithful alternatives. No automatic operator substitution, ever |
...
| # | Change | Outcome |
|---|--------|---------|
| **1** | Views speak a retired filter vocabulary and cannot express OR | **ViewDef v2 adopts the find grammar** (FR-018b): one `all`/`any`/`not` tree, the ten SQL operators, everywhere. `group_by` gains `direction`; `type` becomes optional (untyped, folder-scoped views); a declared type with zero records is a **valid empty view** (FR-018d) |
...
- **FR-018b** **NEW, Draft 10 (ADR-068 D24.1). ViewDef schema_version 2: the view format adopts the find grammar.** The product spoke two filter languages — `knowledge_find`'s `VaultFilterNode` tree (`all`/`any`/`not`, the ten SQL operators) and the view format's flat AND-only `RecordFilter` list in the vocabulary ruling R-B retired — with a bridge (`pkg/records/view_find_bridge.go`) that refuses `contains` and `via` leaves outright. Seven filter groups in the founder's vault use disjunction; none was expressible as a view. ViewDef v2:
  ...
  - **Version 1 views stay readable UNDER THEIR OWN SEMANTICS — nothing is translated. REWRITTEN, Draft 11 (review finding F5):** *Draft 10 said v1 loads "translated to v2 semantics", with `contains` becoming `LIKE '%…%'` — which is BROADENING (`labels contains "in"` matches the element `in`; `LIKE '%in%'` matches `indoor`, `printing`, `min`), applied automatically, in the revision that declared broadening the one prohibited thing; `pkg/records/view_find_bridge.go`'s own header refuses exactly this substitution by name.* A v1 view evaluates with its recorded v1 operators, verbatim. A v1 view using `contains` or `via` stays precisely as it is today — listed by `knowledge_describe`, not servable through `knowledge_find`, the reason named — until EXPLICITLY migrated: `knowledge_configure` rewrites it to v2 on request, and REFUSES a rewrite that would change the row set, naming the faithful alternatives (`=` for whole-element membership where folding is acceptable — stated, because folding widens case; `LIKE` where a pattern is actually wanted). Writes emit v2; files on disk are never rewritten on read; `SupportedViewVersion` becomes the set {1, 2}. The standing cost is stated: the comparator keeps v1's operator set alive for as long as v1 views exist, and the health view lists unmigrated v1 views so the operator can retire them deliberately.
...
| 90 | `TestView_V2TreeExpressesDisjunction` | unit | FR-018b — **NEW, Draft 10; RENUMBERED, Draft 11** (Draft 10 issued 88/89 twice — 88 is the RSS benchmark, 89 the identifier-collation test). A v2 view whose filter is `{any: […]}` loads, validates and returns the union; a v1 view still loads UNDER ITS OWN SEMANTICS; `group_by` direction is honoured and its absence defaults stated |
