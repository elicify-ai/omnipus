...
> [!NOTE]
> The `gateway` profile only serves the webhook handlers and health endpoints on the gateway port, so it does not expose generic REST chat endpoints such as `/chat` or `/a2a`.
