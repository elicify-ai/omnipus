...
Last reviewed: 2026-09-26 — issue #900 squad-delegation rules; goal-loop squad briefs; hotfix lane
...
| Confirm before destructive or outward-facing actions | State the action and get the founder's go-ahead before: pushing to a **shared** branch (landing on the integration branch above all), opening/closing PRs or issues, posting comments, deleting files, resetting state. Commits and pushes to your **own working branch** need no confirmation — that is a standing grant; so is opening and merging a hotfix-lane PR while the integration branch is red (section 9) |
...
## 9. Change delivery: three sizes, the hotfix lane, and the 8-reviewer gate

Every change is sized before it is dispatched. Urgent is not a size and not the hotfix lane — it is small or standard, moved to the front of the dispatch queue.
...
| Hotfix (the fourth path) | Fix-only repair of a red integration branch, or a live bug the founder marks "hotfix" (founder decision 2026-09-26) | Reproduce first → fix → the whole affected-area test set locally → PR merged at once by you (standing founder yes while red; a live-bug hotfix gets a one-line yes) → you watch the post-merge run until green → reviews after the merge. Full lane: `omnipus-planning-orchestration` knowledge/parallel-planning.md §4a |
