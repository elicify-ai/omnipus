...
Last reviewed: 2026-09-26 — issue #900 squad-delegation rules
...
| Leading a feature-size squad itself (dispatching and reviewing its specialists) | No — every feature-size squad gets its own `squad-lead` from the first dispatch; you never lead a squad yourself. If starting one needs a model choice or another trade-off, ask the founder at squad start, never silently lead it instead (full rule: `omnipus-planning-orchestration` §2) | `squad-lead`, one per squad |
