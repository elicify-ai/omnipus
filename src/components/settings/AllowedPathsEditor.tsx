...
function ReadOnlyBadge() {
  ...
  return (
    <span className="relative inline-block">
      <button
        type="button"
        className="inline-flex items-center rounded px-1.5 py-0.5 text-[length:var(--type-caption-size)] font-mono border border-[var(--color-border)] bg-[var(--color-surface-2)] text-[var(--color-muted)] cursor-default"
      ...
      {tip && (
        <span
          id="ro-tip"
          role="tooltip"
          className="absolute bottom-full left-0 mb-1 z-50 w-64 rounded border border-[var(--color-border)] bg-[var(--color-surface-1)] px-2 py-1.5 text-[length:var(--type-caption-size)] text-[var(--color-muted)] shadow-lg pointer-events-none"
        >
        ...
        {paths.map((p, i) => (
          <div key={i} className="flex flex-col gap-0.5">
            <div className="flex items-center gap-2 rounded border border-[var(--color-border)] bg-[var(--color-surface-2)] px-2 py-1.5">
              ...
              <ReadOnlyBadge />
              {restartedRows.has(i) && (
                <span className="inline-block rounded px-1.5 py-0.5 text-[length:var(--type-caption-size)] border border-yellow-500/40 bg-yellow-500/10 text-yellow-400">
                  restart required
                </span>
              )}
              <button tabIndex={0}
            ...
            {rowErrors[i] && (
              <p className="text-[length:var(--type-caption-size)] text-[var(--color-error)] pl-2">{rowErrors[i]}</p>
            )}
          </div>
        ))}
      </div>
