...
describe('Board/List recurring exclusion (FR-011, US-3)', () => {
  ...
  it('tags and other fields on a surviving non-recurring task are unaffected', () => {
    const onceTaskWithTags = makeTask({
      id: 'once-tagged',
      trigger: { type: 'once', config: { at_ms: 5000 } },
      tags: ['urgent'],
      priority: 1,
    })
    const visible = userVisibleTasks([onceTaskWithTags])
    expect(visible).toEqual([onceTaskWithTags])
  })
})
