 ...
 *
 * The component has a useEffect that redirects projectId === 'inbox' to the
 * real default project UUID:
 *
 *   useEffect(() => {
 ...
 *
 * Scenarios tested:
 *   1. Redirect to default project — projectId='inbox', data has is_default=true project → navigate called
 *   2. No redirect when projects loading — projectId='inbox', empty data → navigate NOT called
 *   3. No redirect when no default project — projectId='inbox', list without is_default → navigate NOT called
 *
...
// Mock the full API layer — only fetchProjects matters for redirect tests;
...
vi.mock('@/lib/api', async (importOriginal) => {
  ...
  return {
    ...
    fetchAgents: vi.fn(),
    projectsQueryKeys: actual.projectsQueryKeys,
...
import { fetchProjects, fetchBoardTasks, fetchMilestones, fetchAgents } from '@/lib/api'
...
function renderScreen(projectId: string) {
...
/**
 * Build a minimal Project fixture from the generated OpenAPI type.
 */
...
beforeEach(() => {
  ...
  vi.mocked(fetchBoardTasks).mockReturnValue(new Promise(() => {}))
  vi.mocked(fetchMilestones).mockReturnValue(new Promise(() => {}))
  vi.mocked(fetchAgents).mockReturnValue(new Promise(() => {}))
})
...
describe('ProjectDetailScreen — inbox redirect', () => {
  it('navigates to default project UUID when projectId is "inbox" and is_default project exists', async () => {
    // BDD: Given projectId='inbox' and fetchProjects returns a project with is_default=true
    // When the component mounts and the projects query resolves
    // Then navigate is called with the default project's id and replace: true
    //
    // Traces to: ProjectDetailScreen.tsx line 54

    const defaultProject = makeProject({ id: 'abc-123', name: 'Inbox', is_default: true })
    vi.mocked(fetchProjects).mockResolvedValue([defaultProject])
  ...
  it('navigates to the correct UUID (not a hardcoded value) when default project has different id', async () => {
    // Differentiation test: two different default project IDs → two different
    // navigate calls.  A hardcoded response would return the same id regardless
    // of what fetchProjects returns.
    //
    // Traces to: ProjectDetailScreen.tsx line 54
    ...
    await waitFor(() => {
      expect(mockNavigate).toHaveBeenCalledWith(
        expect.objectContaining({ params: { projectId: 'xyz-999' } }),
      )
    })

    // Must NOT have been called with the id from the previous test scenario.
    expect(mockNavigate).not.toHaveBeenCalledWith(
      expect.objectContaining({ params: { projectId: 'abc-123' } }),
    )
  })

  it('does NOT call navigate when projectId is NOT "inbox"', async () => {
    // Content test: the guard `if (projectId !== 'inbox') return` means navigate
    // must never be called for a real UUID.
    // Traces to: ProjectDetailScreen.tsx line 50

    const realProject = makeProject({ id: 'real-uuid-001', name: 'My Project', is_default: false })
    vi.mocked(fetchProjects).mockResolvedValue([realProject])
...
describe('ProjectDetailScreen — no redirect while projects still loading', () => {
  it('does NOT call navigate when projects query has not resolved yet (empty default)', async () => {
    // BDD: Given projectId='inbox' and fetchProjects is pending (data defaults to [])
...
describe('ProjectDetailScreen — no redirect when no default project in list', () => {
  it('does NOT call navigate when projects list has entries but none is is_default', async () => {
    // BDD: Given projectId='inbox' and fetchProjects returns projects without is_default=true
    // When the component mounts
    ...
    //
    // Rejection test: a project list without is_default set must not trigger a
  ...
  it('does NOT call navigate when is_default is explicitly false on all projects', async () => {
    // Persistence test: even with multiple projects all having is_default=false,
