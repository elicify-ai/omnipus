...
//
// The `a` renderer is NOT in this static map because it is built per-render
// via createLinkRenderer (shared with the finalized renderer). Since
// ADR-044 (preview-on-main-listener), `/preview/` shares the SPA's own
// origin — there is no operator-configured preview host/port to read from
// /api/v1/about anymore, so links render exactly as returned (no rewrite,
// no about-info fetch). All other renderers come from this shared memoized map.
...
const staticMarkdownComponents = memoizeMarkdownComponents({
  ...
  // "no rewrite" case in markdown-shared.tsx — this can now be a static,
  // module-level component instead of rebuilt per-render from a query result).
