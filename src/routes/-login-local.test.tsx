// -login-local.test.tsx — upstream's local (core-edition) password login
// screen's own test suite, restored unchanged from the engine merge base
// (engine/PIN ENGINE_COMMIT, commit 184d724773789513a4a7fd404596115ea4ec55cf's
// src/routes/-login.test.tsx) and repointed at -login-local.tsx (ADR-0010 WP6:
// login.tsx is now a mode dispatcher, not this screen).
//
...
beforeAll(async () => {
  const { LoginLocal } = await import('./-login-local')
  LoginComponent = LoginLocal
})
