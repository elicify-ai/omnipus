...
function LandingPage() {
  return (
    <div className="min-h-screen bg-[var(--color-primary)] text-[var(--color-secondary)]">
      {/* Hero */}
      <section className="flex flex-col items-center justify-center min-h-screen px-[var(--space-4)] py-20 text-center gap-[var(--space-5)]">
        ...
        <div className="max-w-2xl">
          <h1 className="mb-[var(--space-3)]">
            <Wordmark className="text-5xl md:text-7xl leading-tight" />
          </h1>
          {/* US-6: Tagline per brand guidelines */}
          <p className="text-[var(--color-accent)] font-headline text-xl md:text-2xl font-bold mb-[var(--space-3)]">
        ...
        {/* US-6: Forge Gold CTA */}
        <div className="flex flex-col sm:flex-row gap-[var(--space-3)] mt-[var(--space-3)]">
          <a tabIndex={0}
            href="https://github.com/omnipus-ai/omnipus"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-[var(--space-2)] px-[var(--space-4)] py-[var(--space-2-5)] rounded-lg bg-[var(--color-accent)] text-[var(--color-primary)] font-headline font-bold text-base hover:bg-[var(--color-accent-hover)] transition-colors"
          >
            <GithubLogo size={20} />
            View on GitHub
          </a>
          <a tabIndex={0}
            href="/#/"
            className="inline-flex items-center gap-[var(--space-2)] px-[var(--space-4)] py-[var(--space-2-5)] rounded-lg border border-[var(--color-border)] text-[var(--color-secondary)] font-medium text-base hover:bg-[var(--color-surface-2)] transition-colors"
          >
            Open App
          </a>
        </div>
      ...
      <section className="px-[var(--space-4)] py-20 max-w-6xl mx-auto">
        ...
        <p className="text-[var(--color-muted)] text-center text-lg mb-[var(--space-8)] max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[var(--space-4)]">
          {FEATURES.map(({ Icon, title, description }) => (
            <Card key={title}>
              <CardContent className="flex flex-col gap-[var(--space-3)] pt-[var(--space-4)]">
      ...
      {/* Footer */}
      <footer className="border-t border-[var(--color-border)] px-[var(--space-4)] py-[var(--space-6)]">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-[var(--space-3)]">
          <div className="flex items-center gap-[var(--space-2-5)]">
            <img src={OmnipusAvatar} alt="Omnipus" className="h-8 w-8" />
            <Wordmark />
          </div>
          <div className="flex items-center gap-[var(--space-4)] text-[length:var(--type-body-compact-size)] text-[var(--color-muted)]">
            <a tabIndex={0}
              href="https://github.com/omnipus-ai/omnipus"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-[var(--space-1)] hover:text-[var(--color-secondary)] transition-colors"
            >
              <GithubLogo size={16} />
              GitHub
            </a>
