// -login-local.tsx — upstream's local (core-edition) password login
// screen, restored unchanged from the engine merge base (engine/PIN
// ENGINE_COMMIT, commit 184d724773789513a4a7fd404596115ea4ec55cf's
// src/routes/login.tsx) and moved to a per-mode file (ADR-0010 WP6).
//
// Reachable only when the running binary's identity.mode is 'local'
// (core edition) — chosen by login.tsx's dispatcher. POST /api/v1/auth/login
// is not registered at all in platform mode, so this screen's one API call
