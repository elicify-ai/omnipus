...
// Production modules index tokens dynamically (`values[id]`). That keeps every
// property of the object they import, so importing `tokens` / `resolvedTokens`
// ships the whole catalog. These slices are the only maps the app actually
// indexes; the full maps stay for the library entry and for tests.
const STATUS_TOKEN = (id) => id.startsWith('component.status.')
const STATUS_RESOLVED = (id) => id.startsWith('color.status.') || id === 'color.surface.page' || id === 'color.text.on-status-fill'
const MOTION_RESOLVED = (id) => id.startsWith('motion.loading.')

function sliceLines(lines, predicate) {
...
function constExport(name, lines) {
...
export function generateArtifacts(sources) {
  ...
  return {
    ...
    typescript: [
      ...
      constExport('tokens', tokenLines),
      '',
      constExport('resolvedTokens', resolvedLines),
      '',
      'export type TokenId = keyof typeof resolvedTokens',
      '',
      constExport('statusTokens', sliceLines(tokenLines, STATUS_TOKEN)),
      '',
      constExport('statusResolvedTokens', sliceLines(resolvedLines, STATUS_RESOLVED)),
      '',
      constExport('motionResolvedTokens', sliceLines(resolvedLines, MOTION_RESOLVED)),
      '',
    ].join('\n'),
  }
}
