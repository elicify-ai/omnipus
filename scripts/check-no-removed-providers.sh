...
# pkg/gateway/spa/ is the go:embed staging directory: a byte copy of the Vite
# output (dist/spa/), gitignored, regenerated by `make build`. "No trace" is a
# property of the SOURCE (spec CRIT-003) and this directory contains none — it
# contains minified third-party bundles whose contents nobody in this repo
# wrote or can edit.
#
# It is skipped rather than allow-listed because it is not a historical record
# and must never appear in scripts/no-removed-providers.allow (which is a list
# of files people wrote on purpose). On a fresh GitHub checkout the directory
# does not exist at all, which is exactly why this was invisible until the
# guard was wired into the shared CI worker, whose checkout has run a build.
#
# A real trace can never hide here: anything in this directory is overwritten
# on the next build from src/, and src/ IS scanned.
is_build_artifact() { case "${1#./}" in pkg/gateway/spa/*) return 0 ;; *) return 1 ;; esac; }
