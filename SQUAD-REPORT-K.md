...
- `ca7199ad1` — `test(browser): update two tests for the loud-fallback installer behavior` *(commit-message wording says "two tests" but the diff actually changes three test files — the commit subject is a one-sentence summary; the message is now immutable history, the body of the commit is the accurate record. Documented in the A1-A4 advisory-remediation note below.)*
...
- `edd087361` — `docs(squad): GLM-remediation report corrections (26010a4d6 receipts)`
- *(next GLM A1-A4 advisory remediation — A1 stale doc comments, A2 misleading `live-view requires` parenthetical, A3 stale JPEG wording in WARN-BROWSER-001/003/005, A4(a) trailing newline, A4(b) cosmetic dead manifest entry, A4(c) commit list + immutable-history note)*
...
> **GLM A1-A4 advisory remediation (forthcoming commit):** four small advisories on the prior commits — all text-only or fixture-cosmetic, no code path changes.
> 1. **A1** — `pkg/tools/browser/installer.go` `EnsureChromium` and `selectDownloadBuild` doc comments still described the silent-fallback contract. Rewritten to describe the current loud-error behavior with a one-sentence citation of the founder ruling.
> 2. **A2** — `pkg/tools/browser/installer.go` `EnsureChromiumBuild`'s two loud error sites said `live-view requires the full %q build` even when the requested build WAS the headless-shell (the parenthetical is only true for the full build). Now gated on `build.downloadID == cftFullChromeDownloadID` so the hint only appears when the full build was actually requested.
> 3. **A3** — `cmd/omnipus/internal/doctor/command.go` `WARN-BROWSER-001`, `WARN-BROWSER-003`, and `WARN-BROWSER-005` all said the panel "will fall back to JPEG screenshots" — factually wrong post-ADR-061 (the JPEG screencast fallback is deleted; WebRTC is the only live-video path). Reworded to name the real behavior: a persistent not_capable error with the real reason and a Retry. The same correction was applied to the `Long` doc comment for the `doctor` command and the `checkBrowserVideoCapability` function's header comment.
> 4. **A4** — (a) Trailing newline added to `tests/e2e/fixtures/webrtc-debug.ts`. (b) The cosmetic dead `cftFullChromeDownloadID` entry in `TestPreprovision_BrokenPATH_EmptyInstallRoot_Downloads` was removed; the test forces headless-shell via the `selectDownloadBuildGOOS` seam and never asks for the full build, so the extra entry was unreachable config. (c) This report's commit list now reflects ALL commits on the branch (was missing `03d5c1526`, `26010a4d6`, and `edd087361`); the `ca7199ad1` commit-message's "two tests" wording is inaccurate (the diff actually changes three test files) but the commit message is now immutable history and is NOT rewritten.
> Receipts: gofmt clean; targeted Go test receipt (`pkg/tools/browser` + `cmd/omnipus/internal/browser`); vitest receipt (`npx vitest run src/lib/browserWebRTC.test.ts` → 72 tests pass, exit 0). NO chroot run needed (comment/string-only + fixture cosmetics; the chroot receipt from `26010a4d6` still stands).
