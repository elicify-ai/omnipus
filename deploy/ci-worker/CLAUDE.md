...
5. **Disk-full false-RED (`ENOSPC`).** Each e2e shard's `OMNIPUS_HOME` is ~400 MB and
   there are 24 shards; until 2026-09-11 they were hardcoded to `/tmp`, which shares the
...
**E2E gate (Playwright).** The `e2e` gate (and the `e2e` step inside `all`) builds the SPA + gateway binary once, then **fans the Playwright suite out across the shards defined in `tests/e2e/shards.json`** — the SAME plan `.github/workflows/pr.yml` uses (both consume `scripts/e2e-shards.sh`), so the two CI surfaces can never drift. Each shard boots its OWN isolated gateway (own port `6060`–`6083`, own `OMNIPUS_HOME`, own `credentials.json`, own auth + skip-manifest files), completes onboarding via the public API, runs its slice of the matrix with `--reporter=list --output=$E2E_DIR/e2e-<shard>-results`, and is torn down via a per-shard `trap RETURN`. A whole-run interrupt reaps survivors by **exact pid** from `$E2E_DIR/e2e-shard-*.gwpid` — never pkill-by-pattern (self-kill risk). Concurrency is capped **per shard kind**: render-bound shards fly at most 2 at a time (a proven-safe finding — a 5-wide run failed 17 specs that a 1-wide re-run passed 16/17, CPU starvation flaking timing-sensitive specs; that cap is not caution and must not be raised on a whim), while `llm-*` shards fly at most 4 (they are mostly I/O-bound, parked on OpenRouter HTTP, and do not compete for the cores Chromium needs). Shards marked `solo` in `tests/e2e/shards.json` drain every in-flight shard and run alone in the foreground — they are listed LAST in the plan because a solo shard blocks the launcher while it runs, so an early solo idles every concurrent slot. Wall clock is therefore bounded by the concurrent phase (the 2-wide render blocks, ~4 specs each after the 2026-09-16 rebalance, overlapping the 4-wide LLM shards) plus the strictly serial chain of solo shards — not by any single 15-spec shard, and not "all shards at once". `scripts/e2e-shards.sh check` runs first and **FAILS the gate** if any `tests/e2e/*.spec.ts` is unassigned (it would silently never run) or the plan references a deleted spec. Per-shard PASS/FAIL is printed at the end, and each failing shard's full log (`$E2E_DIR/e2e-shard-<name>.log`) is dumped.
