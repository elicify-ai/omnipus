 ...
 * independent GET, and `test.skip()`s with a clear reason if it does not,
 * per this task's skip-with-reason convention — it does NOT silently pass.
 */
 ...
 * model_params (for restoration) on success, or `null` if the override did
 * not take — the caller must `test.skip()` in that case rather than
 * proceed against an agent that will never truncate.
 */
...
test(
  ...
  async ({ page }) => {
    ...
    if (!jim) {
      test.skip(true, 'BLOCKED: could not resolve an agent named "Jim" via GET /api/v1/agents — cannot target the override.')
      return
    }
    ...
    if (forceResult === 'unsupported') {
      test.skip(
        ...
        'BLOCKED: PUT /api/v1/agents/{id} model_params.max_tokens did not round-trip through an ' +
          'independent GET on this build (config.AgentConfig.ModelParams persistence is outside ' +
          'this agent\'s scope — pkg/gateway/rest.go, pkg/config/config.go). Cannot force a real ' +
          'provider truncation without it; skipping per the repo\'s skip-with-reason convention ' +
          '(see e.g. calendar.spec.ts) rather than asserting against a turn that will never ' +
          'truncate.',
      )
      return
    }
