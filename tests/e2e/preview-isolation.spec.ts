...
test.describe('ADR-067 preview isolation — seven egress vectors, retries: 0', () => {
  ...
  test('11a — embedded in the product-shaped iframe, zero of seven vectors reach the second origin', async ({ page }) => {
    // Recording keeps the request log warm for debugging; the assertions below
    // read the DOM/probe results directly, so the handle itself is unused.
    void recordBrowserRequests(page.context());
