...
test("live browser view streams genuinely playing video with real audio and realtime input", async ({
  ...
  // ADR-090 deliberately does not give any built-in both serve_web and the
  // browser surface. This integration test therefore uses a disposable custom
  // agent with only that exact test bundle, exercising the supported custom
  // configuration path without widening Jim's orchestrator policy.
  ...
  await test.step("evidence: video pixels genuinely change over time (not frozen/black)", async () => {
    ...
    while (
      luminance(firstFrame.top) < FIRST_FRAME_READY_LUMINANCE &&
    ...
    if (luminance(firstFrame.top) < FIRST_FRAME_READY_LUMINANCE) {
      const capturedUrl = await addressBar
        .inputValue()
        .catch(() => "(unavailable)");
