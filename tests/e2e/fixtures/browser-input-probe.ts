...
import { apiFetch } from "./conformance-helpers";
import {
  assistantMessages,
  browserLiveFrame,
  browserLivePanel,
...
type ProbeWindow = Window & {
  __omnipusSoak?: Probe;
  __omnipusLatency?: Diagnostics;
};

async function installLatencyDiagnostics(
  page: Page,
  videoOnly = false,
): Promise<void> {
  await page.addInitScript((disableAudio) => {
    ...
    window.RTCPeerConnection = class extends NativePeer {
      ...
      addTransceiver(
        ...
        if (disableAudio && kind === "audio") {
          audioOverrides++;
          return super.addTransceiver(trackOrKind, {
            ...init,
            direction: "inactive",
          });
        }
    ...
    const observeSend = (route: string, payload: unknown, send: () => void) => {
      ...
      if (current && typeof payload === "string") {
        ...
        try {
          ...
          /* Non-JSON transport messages carry no input timing. */
        }
        if (
          frame &&
          (frame.kind === "mouse_down" || frame.kind === "mouse_up")
        ) {
          entry = { route, kind: frame.kind, attemptedAt: performance.now() };
          if (
            typeof frame.capture_id === "string" &&
            /^[a-f0-9]{64}$/.test(frame.capture_id)
          )
            entry.captureId = frame.capture_id;
          if (
            typeof frame.capture_generation === "number" &&
            Number.isSafeInteger(frame.capture_generation) &&
            frame.capture_generation > 0
          )
            entry.captureGeneration = frame.capture_generation;
      ...
      try {
        ...
        if (entry) entry.succeeded = false;
        throw error;
      } finally {
        if (entry) entry.returnedAt = performance.now();
      }
    };
    const wsSend = WebSocket.prototype.send;
    WebSocket.prototype.send = function (data: unknown) {
      observeSend("websocket", data, () => Reflect.apply(wsSend, this, [data]));
    };
    const dcSend = RTCDataChannel.prototype.send;
    RTCDataChannel.prototype.send = function (data: unknown) {
      observeSend("datachannel", data, () =>
        Reflect.apply(dcSend, this, [data]),
      );
    };
    const fields = [
      "localCandidateId",
    ...
    (window as ProbeWindow).__omnipusLatency = {
      begin(count) {
        current = { count, armedAt: performance.now(), sends: [] };
        clicks.push(current);
      },
      pointer(at) {
        if (current) current.pointerAt = at;
      },
      presented(metadata) {
        if (current) current.presentation = metadata;
      },
      async snapshot(label) {
        ...
        for (const [index, pc] of peers.entries()) {
          ...
          for (const receiver of pc.getReceivers()) {
            ...
            snapshot.receivers.push({
              pc: index,
              kind: receiver.track.kind,
              jitterBufferTarget: settings.jitterBufferTarget ?? null,
              playoutDelayHint: settings.playoutDelayHint ?? null,
            });
          }
          try {
            ...
            snapshot.errors.push(`peer${index}: ${String(error)}`);
          }
        }
        stats.push(snapshot);
      },
...
async function captureRTCStats(page: Page, label: string): Promise<void> {
  await page.evaluate(async (name) => {
    await (window as ProbeWindow).__omnipusLatency!.snapshot(name);
  }, label);
}

function receiverDeltas(evidence: LatencyEvidence | null) {
  const first = evidence?.stats[0],
    last = evidence?.stats.at(-1);
  ...
  return last.rows
    ...
    .map((row) => {
      const before = first.rows.find(
        (value) => value.pc === row.pc && value.id === row.id,
      );
      ...
      const averageMs = (total: string, count: string) => {
        const seconds = delta(total),
          samples = delta(count);
        return seconds !== null && samples !== null && samples > 0
          ? (seconds * 1000) / samples
          : null;
      };
      return {
        pc: row.pc,
        id: row.id,
        intervalMs: last.at - first.at,
        jitterBufferMs: averageMs(
          "jitterBufferDelay",
          "jitterBufferEmittedCount",
        ),
        jitterBufferTargetMs: averageMs(
          "jitterBufferTargetDelay",
          "jitterBufferEmittedCount",
        ),
        decodeMs: averageMs("totalDecodeTime", "framesDecoded"),
        processingMs: averageMs("totalProcessingDelay", "framesDecoded"),
        framesDecoded: delta("framesDecoded"),
...
async function installVideoProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    if (!video || !surface || !(video.srcObject instanceof MediaStream))
      throw new Error("Real live-video stream unavailable");
    const stream = video.srcObject;
    const track = stream.getVideoTracks()[0];
    if (!track || typeof video.requestVideoFrameCallback !== "function")
      throw new Error("Decoded video frame callbacks unavailable");
    const locatorCanvas = document.createElement("canvas");
    const locatorContext = locatorCanvas.getContext("2d", {
    ...
    if (!locatorContext || !gridContext)
      throw new Error("Decoded pixel sampling unavailable");
    gridContext.imageSmoothingEnabled = false;
    type Geometry = Pick<
      Decoded,
      "left" | "top" | "width" | "height" | "canvasWidth" | "canvasHeight"
    ...
    const locate = (): Geometry | null => {
      ...
      locatorCanvas.height = Math.max(
        1,
        Math.round((384 * video.videoHeight) / video.videoWidth),
      );
      locatorContext.drawImage(
        video,
        0,
        0,
        locatorCanvas.width,
      ...
      const pixels = locatorContext.getImageData(
        0,
        0,
        locatorCanvas.width,
        locatorCanvas.height,
      ).data;
      let left = locatorCanvas.width,
        right = -1,
        top = locatorCanvas.height,
        bottom = -1;
      ...
      for (let y = 0; y < locatorCanvas.height; y++) {
        ...
        for (let x = 0; x <= locatorCanvas.width; x++) {
          ...
          if (!pink && run >= 0) {
            ...
            if (x - run >= locatorCanvas.width * 0.2) {
              left = Math.min(left, run);
      ...
      return {
        ...
        width: right - left + 1,
        height: bottom - top + 1,
        canvasWidth: locatorCanvas.width,
        canvasHeight: locatorCanvas.height,
    ...
    const sample = (): Decoded | null => {
      ...
      if (
        !geometry ||
        geometry.videoWidth !== video.videoWidth ||
        geometry.videoHeight !== video.videoHeight
      )
        geometry = locate();
      ...
      gridContext.drawImage(
        video,
        (left + width * 0.1) * scaleX,
        (top + height * 0.1) * scaleY,
        width * 0.8 * scaleX,
        height * 0.55 * scaleY,
        0,
        0,
      ...
      const read = (size: number) => {
        let value = 0;
        for (let i = 0; i < size; i++) value += bits[offset++] * 2 ** i;
        return value;
      };
      return {
        count: read(16),
    ...
    const onFrame: VideoFrameRequestCallback = (callbackTime, metadata) => {
      if (expected && started !== null && latency === null) {
        ...
        if (decoded?.nonce === expected.nonce) {
          if (decoded.firstError)
            error = `Fixture event order mismatch at event ${decoded.firstError}`;
          if (
            decoded.count === expected.count &&
            decoded.hash === expected.hash &&
            decoded.last === expected.last &&
            decoded.held === expected.held &&
            decoded.firstError === 0
          ) {
            ...
            const detail: Record<string, number> = {
              ...
              sampleStarted,
              sampleFinished,
              sampleCostMs: sampleFinished - sampleStarted,
            };
            ...
            for (const key of [
              ...
              if (typeof available[key] === "number")
                detail[key] = available[key] as number;
            }
    ...
    (window as ProbeWindow).__omnipusSoak = {
      ...
      arm(value) {
        ...
        latency = null;
        error = null;
        (window as ProbeWindow).__omnipusLatency?.begin(value.count);
      },
      latency: () => latency,
      error: () => error,
    };
    ...
    .poll(
      async () => {
        decoded = await page.evaluate(() =>
          (window as ProbeWindow).__omnipusSoak!.sample(),
        );
        if (decoded?.nonce === expected.nonce && decoded.firstError !== 0)
          throw new Error(
            `Unexpected event order at ${decoded.firstError}; actual=${JSON.stringify(decoded)}`,
          );
        return decoded
          ? {
              count: decoded.count,
              ...
              last: decoded.last,
              held: decoded.held,
              firstError: decoded.firstError,
              nonce: decoded.nonce,
            }
          : null;
      },
      { timeout: 3_000, intervals: [16, 32, 50] },
    )
    .toEqual(expected);
  return decoded!;
}
    ...
    (element, geometry) => {
      ...
      return {
        x:
          left +
          ((geometry.left +
            geometry.width * (geometry.zone === 0 ? 0.25 : 0.75)) /
            geometry.canvasWidth) *
            v.videoWidth *
            scale,
        y:
          top +
          ((geometry.top + geometry.height * 0.87) / geometry.canvasHeight) *
            v.videoHeight *
            scale,
      };
    },
...
function directFixturePreparation(
  preview: string | undefined,
  directory: string | undefined,
  origin: URL,
  workspaceWork: string,
): { preview: URL; directory: string } | undefined {
  if (preview === undefined && directory === undefined) return undefined;
  if (!preview || !directory)
    throw new Error(
      "BROWSER_PROBE_PREVIEW_URL and BROWSER_PROBE_FIXTURE_DIR must be supplied together",
    );
  ...
  if (
    ...
    throw new Error(
      "Direct fixture preview must use the designated runtime preview origin without credentials, query or fragment",
    );
  if (!path.isAbsolute(directory))
    throw new Error("BROWSER_PROBE_FIXTURE_DIR must be absolute");
  const resolved = fs.realpathSync(directory);
  if (
    !resolved.startsWith(workspaceWork + path.sep) ||
    !fs.statSync(resolved).isDirectory()
  )
    throw new Error(
      "Direct fixture directory must be strictly inside the selected workspace work directory",
    );
  const index = path.join(resolved, "index.html");
  if (fs.lstatSync(index, { throwIfNoEntry: false })?.isSymbolicLink())
    throw new Error("Direct fixture index must not be a symbolic link");
  return { preview: url, directory: resolved };
}

/**
 * Create a throwaway chat agent with the two capabilities this probe combines.
 * ADR-090 intentionally separates serve_web (General Purpose) from browser
 * tools (Jim/Mia/Ava), so built-in agents must not be widened just to stage a
 * test. Custom agents remain the sanctioned way to grant a purpose-built
 * capability bundle.
 */
export async function createBrowserProbeAgent(
  page: Page,
  nonce: number,
): Promise<string> {
  const name = `Browser Probe ${nonce}`;
  const created = await apiFetch<{ id: string; warning?: string }>(
    page,
    "POST",
    "/api/v1/agents",
    {
      type: "Main",
      name,
      soul: "You are a browser integration test runner. Make the exact requested tool calls and report the exact requested marker.",
    },
  );
  if (!created.ok)
    throw new Error(
      `createBrowserProbeAgent: POST /agents failed ${created.status}: ${created.raw}`,
    );
  if (created.body.warning)
    throw new Error(
      `createBrowserProbeAgent: agent reload failed: ${created.body.warning}`,
    );
  ...
  if (!tools.ok)
    throw new Error(
      `createBrowserProbeAgent: GET tools failed ${tools.status}: ${tools.raw}`,
    );
  const fullPolicies = {
    ...(tools.body.config?.builtin?.policies ?? {}),
    ...policies,
  };
  const overrideNames = Array.from(
    new Set([...(tools.body.override_names ?? []), ...Object.keys(policies)]),
  );
  const updated = await apiFetch<{ warning?: string }>(
    page,
    "PUT",
    `/api/v1/agents/${agentId}/tools`,
    {
      revision: tools.body.revision,
  ...
  if (!updated.ok)
    throw new Error(
      `createBrowserProbeAgent: PUT tools failed ${updated.status}: ${updated.raw}`,
    );
  if (updated.body.warning)
    throw new Error(
      `createBrowserProbeAgent: tool reload failed: ${updated.body.warning}`,
    );
  ...
  >(page, "GET", "/api/v1/workspaces");
  if (!workspaces.ok)
    throw new Error(
      `createBrowserProbeAgent: GET workspaces failed ${workspaces.status}: ${workspaces.raw}`,
    );
  const workspace =
    workspaces.body.find((w) => w.is_default) ??
    workspaces.body.find((w) => !w.status || w.status === "active");
  if (!workspace)
    throw new Error("createBrowserProbeAgent: no active workspace");
  const current = await apiFetch<{ revision?: string; core_team?: string[] }>(
    page,
    "GET",
  ...
  if (!current.ok)
    throw new Error(
      `createBrowserProbeAgent: GET workspace failed ${current.status}: ${current.raw}`,
    );
  const saved = await apiFetch(
    ...
    `/api/v1/workspaces/${workspace.id}`,
    {
      ...
      core_team: Array.from(
        new Set([
          ...(current.body.core_team ?? workspace.core_team ?? []),
          agentId,
        ]),
      ),
    },
  ...
  if (!saved.ok)
    throw new Error(
      `createBrowserProbeAgent: PUT workspace failed ${saved.status}: ${saved.raw}`,
    );
  return name;
}

export async function runBrowserInputProbe(
  page: Page,
  testInfo: TestInfo,
  mode: "soak" | "latency" | "latency-video-only",
): Promise<void> {
  const targetTimingEnabled = process.env.BROWSER_PROBE_TARGET_TIMING === "1";
  const timingHoldMs = Number(
    process.env.BROWSER_PROBE_TARGET_TIMING_HOLD_MS ?? 0,
  );
  if (
    !Number.isSafeInteger(timingHoldMs) ||
    timingHoldMs < 0 ||
    timingHoldMs > 300_000 ||
    (timingHoldMs > 0 && !targetTimingEnabled)
  )
    throw new Error(
      "Target timing hold requires timing enabled and duration0..300000ms",
    );
  testInfo.setTimeout(
    (mode === "soak" ? 32 * 60_000 : 8 * 60_000) + timingHoldMs,
  );
  ...
  const plans = Array.from({ length: CLICK_COUNT }, (_, i) => {
    ...
    return mode === "soak"
      ? plan
      : { ...plan, events: plan.events.slice(0, 3) };
  });
  ...
  const timing: {
    ...
    mixedElapsedMs?: number;
  } = {};
  const viewerClockBrackets: Array<{
    label: string;
    before: number;
    wallMs: number;
    after: number;
  ...
  page.on("websocket", (socket) => {
    ...
    const record = (direction: string, payload: string | Buffer) => {
      ...
      try {
        frame = JSON.parse(payload.toString()) as Record<string, unknown>;
      } catch {
        return;
      }
      if (
      ...
      wire.push({
        ...
        captureId: frame.capture_id as string | undefined,
        generation: frame.capture_generation as number | undefined,
        offerId: frame.offer_id as number | undefined,
        state:
          frame.type === "browser_webrtc_state" &&
          (frame.available === false || frame.active === false)
            ? "error"
            : (frame.state as string | undefined),
      });
    };
    socket.on("framereceived", (frame) => record("received", frame.payload));
    socket.on("framesent", (frame) => record("sent", frame.payload));
    socket.on("close", () =>
      wire.push({ at: Date.now(), direction: "closed", type: "socket_closed" }),
    );
  });
  ...
  try {
    if (targetTimingEnabled && !diagnostic)
      throw new Error("Target timing is a latency diagnostic only");
    await page.goto(origin.href);
    const response = await page.request.get(
      new URL("/api/v1/workspaces", origin).href,
    );
    ...
    if (!workspace || !/^[a-zA-Z0-9_-]+$/.test(workspace.id))
      throw new Error("No safely addressed active workspace");
    const relative = `browser-soak-${nonce}-${Date.now()}`;
    const workspaceWork = fs.realpathSync(
      path.join(runtimeHome, "workspaces", workspace.id, "work"),
    );
    if (!workspaceWork.startsWith(runtimeHome + path.sep))
      throw new Error("Runtime work directory escapes the isolated home");
    const direct = directFixturePreparation(
      process.env.BROWSER_PROBE_PREVIEW_URL,
      process.env.BROWSER_PROBE_FIXTURE_DIR,
      origin,
      workspaceWork,
    );
    ...
    const probeAgentName =
      process.env.BROWSER_PROBE_AGENT_NAME ||
      (await createBrowserProbeAgent(page, nonce));
    await selectAgent(
      page,
    ...
    if (direct) {
      ...
      const served = await page.request.get(previewURL.href, {
        timeout: 15_000,
        maxRedirects: 0,
        headers: { "Cache-Control": "no-cache" },
      });
      expect(served.ok(), "existing preview serves the fresh fixture").toBe(
        true,
      );
      expect(
        await served.text(),
        "served fixture contains this run’s exact event plan and nonce",
      ).toContain(
        `const expected=${JSON.stringify(expectedEvents)}, nonce=${nonce};`,
      );
      ...
      await expect
        .poll(
          () =>
            browserLiveVideo(page).evaluate(
      ...
      await chatInput(page).fill(
        `Prepare this browser test yourself. You may first use ToolSearch to load exactly serve_web and browser_navigate. Then call serve_web with path "${relative}" and no command, followed by browser_navigate to exactly its returned preview URL. Do not click, type, or use unrelated tools. Only after both tools succeed, reply with exactly SOAK_READY_${nonce} and nothing else. If any step fails, reply SOAK_SETUP_FAILED with the reason and never include the success marker.`,
      );
      ...
      const reply = assistantMessages(page)
        .last()
        .locator(":scope > div > div.text-sm.leading-relaxed")
        .locator(
          ":scope > p, :scope > h1, :scope > h2, :scope > h3, :scope > h4, :scope > h5, :scope > h6, :scope > ul, :scope > ol, :scope > blockquote, :scope > hr, :scope > pre, :scope > .overflow-x-auto, :scope > .rounded.overflow-hidden",
        );
      await expect
        .poll(async () => (await reply.allTextContents()).join("\n"), {
          timeout: 240_000,
        })
        .toMatch(new RegExp(`SOAK_READY_${nonce}|SOAK_SETUP_FAILED`));
      await expect(page.locator('[data-testid="stop-btn"]')).not.toBeVisible({
        timeout: 60_000,
      });
      await expect(reply).toHaveText([`SOAK_READY_${nonce}`], {
        timeout: 5_000,
      });
      console.log(`[${label}] setup ready`);
      ...
      await expect(initialAddress).toHaveValue(/\/preview\//, {
        timeout: 30_000,
      });
      previewURL = new URL(await initialAddress.inputValue());
      if (
        previewURL.origin !== origin.origin ||
        !previewURL.pathname.startsWith("/preview/")
      )
        throw new Error(
          "Fixture did not use the designated runtime preview origin",
        );
    }
    ...
    await expect
      .poll(
        () =>
          browserLiveVideo(page).evaluate(
            (el) => (el as HTMLVideoElement).readyState,
          ),
        { timeout: 45_000 },
      )
      .toBeGreaterThanOrEqual(2);
    ...
    const answer = wire
      .filter((e) => e.type === "browser_webrtc_answer")
      .at(-1);
    if (!answer?.captureId || !answer.generation || !answer.offerId)
      throw new Error(
        "Missing exact negotiated capture/generation/offer identity",
      );
    const baselineCapture = wire
      .filter((e) => e.direction === "received" && e.captureId && e.generation)
      .at(-1);
    ...
    const assertContinuous = async () => {
      expect(
        await page.evaluate(() =>
          (window as ProbeWindow).__omnipusSoak!.continuity(),
        ),
        "original video stream/track must remain live",
      ).toBe(true);
      for (const event of wire.slice(baselineWire)) {
        expect(event.type, "original browser socket closed").not.toBe(
          "socket_closed",
        );
        expect(event.type, "unexpected viewer renegotiation").not.toBe(
          "browser_webrtc_offer",
        );
        expect(event.type, "unexpected replacement answer").not.toBe(
          "browser_webrtc_answer",
        );
        expect(
          ["lost", "recovering", "unrecoverable", "error"].includes(
            event.state || "",
          ),
          "unexpected lifecycle failure",
        ).toBe(false);
        if (event.captureId)
          expect(event.captureId).toBe(baselineCapture.captureId);
        if (event.generation)
          expect(event.generation).toBe(baselineCapture.generation);
      }
    };
    if (mode === "soak") {
      ...
      while (performance.now() - idleStart < PHASE_MS) {
        await waitUntil(
          page,
          Math.min(idleStart + PHASE_MS, performance.now() + 5_000),
        );
      ...
      console.log(
        `[browser-soak] idle complete: ${timing.idleElapsedMs.toFixed(1)}ms, ${idleSamples.length} unchanged decoded checkpoints`,
      );
    ...
    console.log(
      `[${label}] input phase started: ${CLICK_COUNT} clicks, ${expectedEvents.length} exact events, target${mixedDuration}ms`,
    );
    for (let i = 0; i < CLICK_COUNT; i++) {
      ...
      await page.evaluate(
        (expected) => (window as ProbeWindow).__omnipusSoak!.arm(expected),
        state,
      );
        ...
        .poll(
          async () => {
            const error = await page.evaluate(() =>
              (window as ProbeWindow).__omnipusSoak!.error(),
            );
            if (error) throw new Error(error);
            return page.evaluate(() =>
              (window as ProbeWindow).__omnipusSoak!.latency(),
            );
          },
          { timeout: 3_000, intervals: [16, 32, 50] },
        )
    ...
    const p95 = ordered[Math.ceil(0.95 * CLICK_COUNT) - 1];
    console.log(
      `[${label}] input complete: ${timing.mixedElapsedMs.toFixed(1)}ms, ${latencies.length} clicks, ${state.count}/${expectedEvents.length} exact events, p95=${p95.toFixed(3)}ms, target<=${MAX_P95_MS}ms`,
    );
    await persistScreenshot(page, testInfo, "after-mixed.png");
    expect(pageErrors, "uncaught viewer errors").toEqual([]);
    expect(
      p95,
      "100-click decoded-video p95, including trusted viewer input dispatch",
    ).toBeLessThanOrEqual(MAX_P95_MS);
  } finally {
    const ordered = [...latencies].sort((a, b) => a - b);
    const p95Ms = ordered.length
      ? ordered[Math.ceil(0.95 * ordered.length) - 1]
      : null;
    ...
    if (diagnostic) {
      try {
        ...
        diagnostics = await page.evaluate(() =>
          (window as ProbeWindow).__omnipusLatency!.evidence(),
        );
      } catch (error) {
    ...
    if (targetTimingEnabled) {
      try {
        await bracketViewerClock("after-input");
      } catch {
        viewerClockError = "Viewer clock read failed";
      }
    }
    ...
    const evidenceName =
      mode === "soak"
        ? "soak-evidence.json"
        : videoOnly
          ? "latency-video-only-evidence.json"
      ...
      JSON.stringify(
        {
          ...
          targetTimingEnabled,
          targetTimingExtraction: targetTimingEnabled
            ? "pending authorized browser_evaluate after measurement"
            : "disabled",
          viewerClockBrackets: viewerClockBrackets.map((clock) => ({
            ...clock,
            wallMinusMonotonicBounds: localClockOffsetBounds(clock),
          })),
          viewerClockError,
          timingCaveats: [
            "canvas command completion is not physical paint",
            "cross-clock offsets require pre/post bracket bounds; never assume equal time origins",
            "target timing adds synchronous performance reads and array writes; sampling overhead is retained separately",
          ],
          diagnostics,
          diagnosticsError,
          rtcDeltas,
    ...
    console.log(
      `[${label}] evidence persisted: ${file}; clicks=${latencies.length}, p95=${p95Ms === null ? "unavailable" : p95Ms.toFixed(3)}ms`,
    );
    if (diagnostic)
      console.log(
        `[${label}] receiver deltas: ${JSON.stringify(rtcDeltas)}; statsError=${diagnosticsError ?? "none"}`,
      );
    ...
    if (
      ...
      try {
        ...
        controlReleaseError =
          "Postmeasurement control release was not confirmed";
      }
      const releaseFile = testInfo.outputPath("target-timing-release");
      const holdStarted = performance.now();
      fs.writeFileSync(
        testInfo.outputPath("target-timing-ready.json"),
        JSON.stringify({
          nonce,
          releaseFile,
          controlReleaseError,
          maxHoldMs: timingHoldMs,
          note: "Postmeasurement only; extract snapshot chunks0/100/200 before writing nonce to releaseFile",
        }),
        { flag: "wx" },
      );
      console.log(
        `[${label}] target timing extraction ready: nonce=${nonce}, releaseFile=${releaseFile}`,
      );
      let released = false;
      while (performance.now() - holdStarted < timingHoldMs) {
        if (
          fs.existsSync(releaseFile) &&
          fs.readFileSync(releaseFile, "utf8").trim() === String(nonce)
        ) {
          released = true;
          break;
        }
        await page.waitForTimeout(
          Math.min(
            500,
            Math.max(0, timingHoldMs - (performance.now() - holdStarted)),
          ),
        );
      }
      fs.writeFileSync(
        testInfo.outputPath("target-timing-hold.json"),
        JSON.stringify({
          nonce,
          released,
          elapsedMs: performance.now() - holdStarted,
          measuredWindowUnaffected: true,
        }),
      );
      ...
      if (!released)
        console.log(
          `[${label}] target timing hold expired; extraction remains unverified`,
        );
    }
    if (diagnostic && latencies.length === CLICK_COUNT) {
      expect(
        diagnosticsError,
        "latency instrumentation must remain readable",
      ).toBeNull();
      expect(
        diagnostics?.clicks,
        "one outbound/presentation trace per click",
      ).toHaveLength(CLICK_COUNT);
      for (const click of diagnostics!.clicks) {
        expect(
          click.pointerAt,
          "trusted viewer pointer timestamp",
        ).toBeGreaterThan(0);
        expect(
          click.sends.map((send) => send.kind),
          "exact native outbound click pair",
        ).toEqual(["mouse_down", "mouse_up"]);
        expect(
          click.sends.every((send) => send.succeeded),
          "outbound sends must succeed",
        ).toBe(true);
        expect(
          click.presentation?.sampleFinished,
          "matching decoded frame timestamp",
        ).toBeGreaterThan(0);
      }
      expect(
        rtcDeltas.length,
        "real video receiver statistics must be available",
      ).toBeGreaterThan(0);
      if (videoOnly) {
        expect(
          diagnostics!.audioOverrides,
          "experiment must intercept actual audio negotiation",
        ).toBeGreaterThan(0);
        const audioTransceivers = diagnostics!.stats
          .flatMap((snapshot) => snapshot.transceivers)
          .filter((value) => value.kind === "audio");
        expect(
          audioTransceivers.length,
          "observe the disabled audio transceiver",
        ).toBeGreaterThan(0);
        for (const transceiver of audioTransceivers) {
          expect(
            transceiver.direction,
            "audio must remain explicitly inactive",
          ).toBe("inactive");
          expect(
            ["inactive", null],
            "no negotiated sending or receiving audio",
          ).toContain(transceiver.currentDirection);
        }
        const audioReports = diagnostics!.stats
        ...
        for (const report of audioReports)
          expect(
            ...
            "video-only experiment received audio packets",
          ).toBe(0);
      } else {
        expect(
          diagnostics!.audioOverrides,
          "normal diagnostic must not alter audio negotiation",
        ).toBe(0);
      }
    }
  }
