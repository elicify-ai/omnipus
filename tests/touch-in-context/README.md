...
**Page-settle timing is not fully reliable — some `controlsChecked: 0` entries are measurement gaps, not real findings.** `settle()` (`specs/in-context-touch.spec.ts`) polls the interactive-control count after navigation and returns once it holds steady for two consecutive 300ms polls (never on a stable-at-zero plateau, capped at 5s). This is the third iteration; the first two were measurably worse (documented in the function's own doc comment: a flat 800ms wait undercounted 33/140 route checks at zero controls; an "at least one control exists" gate undercounted 83/140, because it fired on the app shell's persistent sidebar before route-specific fetched content — e.g. `_app/agents`'s agent cards — had rendered). The current version measured **42 of 140 route checks (30%) at `controlsChecked: 0`** in the baseline run this README ships with. Some of these are almost certainly genuine (e.g. `__root`, one of three inventory entries that all resolve to path `/`, is a router layout shell with no content of its own by design — TanStack Router's root route just renders an `<Outlet />`). Others — `_app/browser-live` showing zero controls in **all five environments**, and several list-backed screens (`_app/policies`, `_app/profile`, `_app/tasks`, `_app/usage`, `_app/sessions.$sessionId`) showing zero in three or more — are more likely still content that hadn't finished an async fetch within the 5s budget, especially under this run's machine load (15 concurrent agents sharing the worktree). Both readings are stated as **inferred**, not measured: nobody watched these routes render in a headed browser to confirm which explanation applies to which route.
...
3. Poll the SAME selector `measurePage()` itself uses (exported as `INTERACTIVE_SELECTOR` from `lib/measure.ts` specifically to keep "ready" and "counted" from drifting apart again) until the count is stable for two consecutive 300ms polls, never accepting a stable-at-zero plateau — 42/140 zero, the version this suite ships with.

**Not tried, and why:** waiting for a specific per-route "this content has loaded" signal (e.g. a data-testid the fetch resolves into) would likely close most of the remaining gap, but that means a per-route-surface readiness map — real design work, not a one-line fix — and the lead's instruction for this lane was to stop tuning and deliver once the current run finished. A future lane extending this suite should either build that map or accept `controlsChecked: 0` as inherently ambiguous between "genuinely empty" and "not yet measured" and treat it as a prompt for a follow-up headed-browser check, not as a clean pass.
