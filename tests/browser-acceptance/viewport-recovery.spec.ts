...
test('one six-second native renderer resize stall recovers without Retry', async ({ page }, info) => {
  ...
  try {
    ...
    // The observer retains transient failures: eventual readiness cannot erase
    // an earlier failure, and the test never clicks Retry or resends the arm.
    await page.evaluate(() => {
    ...
    observations.push({ checkpoint: 'recovered', at: new Date().toISOString(), state: resized });
    ...
    await ready();
    expect(await page.evaluate(() => (window as unknown as { __resizeFailures: string[] }).__resizeFailures)).toEqual([]);
    ...
    const output = info.outputPath('viewport-recovery-evidence.json');
    fs.writeFileSync(output, JSON.stringify({ provenance, testWorkspace: browserTestWorkspacePath, nonce, deliberateResizeStallMs: 6000, observations, failures, errors, final: await sample(page).catch(() => null), routes: await routeEvidence(page).catch(() => null) }, null, 2));
    await info.attach('viewport-recovery-evidence', { path: output, contentType: 'application/json' });
