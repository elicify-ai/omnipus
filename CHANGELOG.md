...
- **Adding an agent to a workspace's team gives it that workspace's live logins.** The workspace browser stays signed in to every site anyone has signed into on it, so an agent you add can act as whoever the workspace is signed in as — including on scheduled and background turns nobody is watching. Only add agents you would trust with those accounts. The team picker says this at the moment you add someone, and removing the agent takes the access away again immediately.
- `tools.browser.idle_close_ttl` — **seconds**, default `900` (15 minutes) — how long a workspace's whole browser may sit with no tabs, no viewer and nothing running before the Chrome process is closed. Its profile stays on disk, so the workspace is still signed in when it comes back. There is no way to switch this off: `0` and any negative value both mean "use the default", never "never close".
- `tools.browser.cache_trim_interval` — **seconds**, default `3600` (1 hour) — how often *closed* workspace profiles are swept for disposable browser cache. Cookies, saved passwords, Local Storage, Session Storage, IndexedDB and a site's own Cache Storage are never touched. Both keys take effect on a settings save, without a restart.
...
- **The audit log now redacts credentials.** The production audit logger never switched redaction on, so a key or token typed into a `bash`, `web_serve` or `environment_setup` command was written to the audit log verbatim and shown by the audit-log viewer. Recognised credential formats (including passwords in `scheme://user:password@host` addresses) in the command, parameters and details of each tool-call and shell audit entry, and in the fields of structured audit event records, are now replaced with `[REDACTED]` before the entry is written, as are values in those parameters, details and fields whose field name marks them as secret; it is always on, with no setting to disable it. Gateway log lines about a failed audit write that include the command now carry it redacted, and the web_serve preview token is no longer logged. Email addresses are kept, so mail recipients and Message-IDs are still recorded in full. **Audit logs written before this fix are not rewritten and may contain credentials typed into shell commands — rotate those credentials if that matters for your installation.** (#914)

### Fixed

- **An agent removed from a workspace's team loses that workspace's browser immediately.** A chat remembers the workspace it was opened in, and that label outlived the team roster it was written from. The live browser panel re-checked membership and fell back to a workspace the agent really was on; the agent's own browser tools took the stale label at face value with no membership check at all. Two consequences, and the second is the serious one. You watched one browser while the agent drove another, with nothing on either side reporting a problem — and an agent that was **not on a workspace's team got that workspace's browser, and therefore its live logins and cookies**. Reachable by removing an agent from a team mid-conversation. Membership is now re-checked on every resolution, in the one place both the panel and the tools resolve through, so the two cannot disagree. A chat's workspace is a *preference* — it picks between the workspaces an agent is on, and can never reach one it is not on. An agent on no team is refused a browser outright, by both.
- **`tools.browser.cache_trim_interval` now really does take effect on a settings save.** It was documented as needing no restart and needed one. The value reached the browser pool on reload and stopped there: the sweep ran on a timer armed once, at startup, so an operator lowering the interval because their disk was filling saw the setting accepted, saw the new value reported back, and got the old hourly behaviour. The running sweep now re-reads the interval and follows a change within about fifteen seconds — including when the new, shorter interval has already elapsed, in which case it sweeps almost immediately instead of waiting out the old one.
- **The `tools.browser.lease_wait` clamp warns again when you change the value.** The warning that fires when a configured lease wait is lowered against `page_timeout` was throttled to once per process, and the function meant to re-arm it on a config reload was never called by anything. An operator who read the warning, chose a different still-too-large value and saved got silence — the same silence someone with a perfectly good value gets. It is now throttled per configured value pair, so any edit to either key warns again.
