...
- **The audit log now redacts credentials.** The production audit logger never switched redaction on, so a key or token typed into a `bash`, `web_serve` or `environment_setup` command was written to the audit log verbatim and shown by the audit-log viewer. Recognised credentials and secret-named fields are now replaced with `[REDACTED]` before every entry is written; it is always on, with no setting to disable it. Email addresses are kept, so mail recipients and Message-IDs are still recorded in full. **Audit logs written before this fix are not rewritten and may contain credentials typed into shell commands — rotate those credentials if that matters for your installation.** (#914)
