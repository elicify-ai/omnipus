...
Wrap an error with `fmt.Errorf("<what failed>: %w", err)` when it crosses a package or interface boundary; pass `err` through bare inside a package; never format an error with `%v` — only `%w` keeps `errors.Is`/`errors.As` working through the chain (enforced by `wrapcheck` in `.golangci.yaml`; `scripts/wrapfix` does the mechanical rewrite). Two rules that keep wraps correct: a wrap message must add information the reader lacks (the operation that failed, not the name of the function they already called), and only wrap a non-nil error — `fmt.Errorf("x: %w", nil)` manufactures a failure out of a success, so a wrap goes under an `if err != nil` guard, never on a bare tail `return err`. Context errors (`ctx.Err()`, `context.Cause`) are the sanctioned passthrough: compared with `==` at ~17 production sites, so they are ignored via wrapcheck's settings, not wrapped.
