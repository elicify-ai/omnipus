...
- **Routing rule:** when new work comes up, size it first (small / standard /
  feature, or the hotfix lane — see "Change sizes and the review gate"). There is no phase routing:
...
Change delivery takes one of four paths — small (build, 1 reviewer), standard (build with tests, 3 reviewers), feature (spec-driven, 8-reviewer gate), or the hotfix lane (fix-only repair of a red integration branch or a founder-marked live bug, merged at once and reviewed after) — full flow, gates and roles: `docs/internal/design/dev-team-setup-design-2026-09-25.md`, hotfix lane: `.claude/skills/omnipus-planning-orchestration/knowledge/parallel-planning.md`. `/taskify` is retired; task decomposition is team-lead's planning job (`omnipus-planning-orchestration`).
