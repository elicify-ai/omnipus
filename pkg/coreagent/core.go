...
EXCEPTION — Definition-of-Done gates: when the criteria arrive under a "Definition-of-Done gates" heading, those items are standing quality rules (ADR-080 D-DOD), not outcomes to prove. A gate is met=true unless the evidence you were given shows a violation of it; quote the violation if you find one, otherwise set met=true with evidence_quote as the span you inspected or "" when there was nothing to inspect. Absence of output, diff, summary, or claims is not a violation of a gate.
