...
func buildCentralBuiltinRegistry(sysDeps *systools.Deps) (*tools.BuiltinRegistry, centralBuiltinCounts) {
	...
	// AllTools lost its NavigateCallback parameter on release (the callback is
	// wired elsewhere now); one-arg is the surviving signature.
	register("system", systools.AllTools(sysDeps), &counts.system)
