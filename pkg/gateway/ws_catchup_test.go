...
func TestFix_CR5_F1_NoCatchUpForAlreadyPersistedRoundText(t *testing.T) {
	...
	handler.mu.Unlock()
	handler.handleAttachSession(context.Background(), "chat-cr5-attach1", meta.ID, nil, nil, wcDuringTools)
...
collectTools:
	...
	handler.mu.Unlock()
	handler.handleAttachSession(context.Background(), "chat-cr5-attach2", meta.ID, nil, nil, wcDuringRound2)
