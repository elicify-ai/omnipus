...
func TestCreateAgent_TypeOmitted_DefaultsToCustom(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodPost, "/api/v1/agents",
		strings.NewReader(`{"name":"Typed Omit"}`))
...
func TestCreateAgent_TypeCustom_Explicit(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/api/v1/agents",
		strings.NewReader(`{"name":"Typed Custom","type":"custom"}`))
	r.Header.Set("Content-Type", "application/json")
	api.HandleAgents(w, r)
...
func TestCreateAgent_TypeWorker_AllowsNonNativeExecutor(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/api/v1/agents",
		strings.NewReader(`{"name":"Worker External","type":"worker","executor":{"kind":"external-cli","cli":"codex"}}`))
	r.Header.Set("Content-Type", "application/json")
	api.HandleAgents(w, r)
...
func TestCreateAgent_NonWorker_RejectsNonNativeExecutor(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/api/v1/agents",
		strings.NewReader(`{"name":"Bad Custom","type":"custom","executor":{"kind":"external-cli","cli":"codex"}}`))
	r.Header.Set("Content-Type", "application/json")
	api.HandleAgents(w, r)
...
func TestUpdateAgent_DoesNotChangeType(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/api/v1/agents",
		strings.NewReader(`{"name":"Sticky Custom","type":"custom"}`))
	r.Header.Set("Content-Type", "application/json")
	api.HandleAgents(w, r)
...
// TestCreateAgent_Worker_RejectsNonEmptyToList proves the worker-leaf
// invariant is enforced at create time: a worker created with a
// delegation_policy that has a non-empty `to` list is 400. The guard lives
// in buildDelegationPolicy (called with selfIsWorker=true for a worker
// create) so it does not need a new code path — just a regression test
// that exercises it through the new type="worker" entry point.
func TestCreateAgent_Worker_RejectsNonEmptyToList(t *testing.T) {
	...
	assert.Equal(t, http.StatusBadRequest, w.Code, "body: %s", w.Body.String())
	assert.Contains(t, w.Body.String(), "delegation leaf",
		"the rejection must name the worker-leaf rule")
}
