...
func TestCentralBuiltinRegistry_ContainsGeneralAndSystemTools(t *testing.T) {
	...
	assert.Equal(t, 41, systemCount,
		"systools.AllTools must produce exactly 41 system tools (see sysagent/tools/registry.go)")
	...
	total := reg.Count()
	assert.Greater(t, total, 41,
		"central BuiltinRegistry must contain more than 41 tools after adding general builtins (SC-101)")
...
func TestCentralBuiltinRegistry_NoDoubleCountSystemTools(t *testing.T) {
	reg := tools.NewBuiltinRegistry()
	for _, tool := range systools.AllTools(nil, nil) {
		if err := reg.RegisterBuiltin(tool); err != nil {
			t.Logf("skipping system tool %q (err: %v)", tool.Name(), err)
		}
	}
	assert.Equal(t, 41, reg.Count(),
		"system tools registered once must produce exactly 41 entries — no double-count (Issue #350)")
}
...
//	Then the response is a JSON array with more than 41 entries,
//	And the array contains entries for exec and system.agent.list.
//
...
func TestHandleToolsRegistry_WithCombinedRegistry(t *testing.T) {
	...
	assert.Greater(t, len(entries), 41,
		"GET /api/v1/tools must return more than 41 entries with the combined registry (SC-101)")
