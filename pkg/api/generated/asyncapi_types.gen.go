...
type ToolCallResultFrame struct {
	...
	// ADR-057 FR-012/FR-013. Present iff it differs from session_id. Class (a) (FR-089): the child turn's own session id when this frame crosses the wire from a delegated child.
	ProducingSessionId *string `json:"producing_session_id,omitempty"`
	// Tool return value. Any JSON type or null (null is the contract for error frames). Sentinels TruncatedResult, MarshalErrorResult, and ToolResultRef are alternative shapes. anyOf, not oneOf: branch 1's permissive `type: [object, array, ...]` already matches any object, so every structured-failure $ref below (itself an object) always matches BOTH branch 1 and its own $ref — oneOf demands exactly one match and would make every one of these payloads schema-invalid against its own union. This union exists to catalogue the shapes a reader may encounter, not to make branch membership mutually exclusive, so anyOf (at least one match) is the correct operator here. See ADR-060 D6 — validated in neither generated artifact today regardless (TS: z.unknown(); Go: any), so this is a spec-correctness fix, not a behavior change.
