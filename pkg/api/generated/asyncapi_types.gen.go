...
type BrowserWebRTCOfferFrame struct {
	...
	Sdp string `json:"sdp"`
	// The client's chat session id. Exactly as in BrowserAttachFrame, it RESOLVES the browsing context under ADR-072 FR-017 — the gateway reads this session's own workspace_id server-side and targets that workspace's browser and its single capture session. It is still echoed back on browser_webrtc_answer / browser_webrtc_state so the client can match them to this offer. A session that names no workspace degrades to the agent's own unambiguous membership, and to FR-033's refusal when the agent is on more than one workspace. agent_id is no longer the binding key.
	SessionId string `json:"session_id"`
	Type      string `json:"type"`
}
