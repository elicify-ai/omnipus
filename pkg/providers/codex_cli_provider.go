...
func (p *CodexCliProvider) Chat(
	...
	for _, msg := range messages {
		if len(msg.Media) > 0 {
			return nil, fmt.Errorf("codex CLI transport does not support image input; choose a native vision-capable provider")
		}
	}
	if p.command == "" {
...
func (p *CodexCliProvider) parseJSONLEvents(output string) (*LLMResponse, error) {
	...
	for scanner.Scan() {
		...
		case "turn.completed":
			if event.Usage != nil {
				// PromptTokens = plain (uncached) input; CacheReadTokens = cached input.
				// CachedInputTokens are tokens served from cache (read), not newly cached (write).
				total := event.Usage.InputTokens + event.Usage.CachedInputTokens + event.Usage.OutputTokens
				usage = &UsageInfo{
