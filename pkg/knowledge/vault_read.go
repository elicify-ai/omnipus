// Omnipus — ADR-068 D15.3 / spec §4.1.3: vault_read, a note or one section of
// one. THE LOGIC HALF — the tool ADAPTER is in tools.go.
//
// Same split as vault_describe.go, and the same guard enforces it:
...
//
// FR-074: every successful vault_read carries a version token vault_edit
...
//     FR-020h — vault_read keeps working there precisely because its token
...
// ReadDefaultMaxBytes is FR-072a's default bound on a vault_read response's
// body/section content. 2.5x FR-127's 16,000-byte hard cap for vault_find,
...
// the same device vault_describe's describeSectionOrder uses.
...
// ReadLink is one resolved or unresolved wikilink, in vault_read's own
...
// ReadData is everything one vault_read response is rendered from — the
// projection source, exactly as DescribeData is for vault_describe. Render
...
// vault_describe's check_integrity about what is wrong with a value). A key
