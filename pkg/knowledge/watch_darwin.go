...
func (dw *darwinWatcher) addTree(startRel string, out chan<- fsEvent, stop <-chan struct{}) error {
	...
	return filepath.WalkDir(startAbs, func(path string, d fs.DirEntry, err error) error {
		...
		rel, ok := watchRel(dw.root, path)
		if !ok {
			// Unconvertible for the reason watchRel documents: not possible
			// for a WalkDir-produced path, skipped rather than aborting the
			// whole walk if it somehow ever did.
			return nil
		}
		if rel == "." {
			rel = ""
