// Omnipus — tests for knowledge_edit's argument-envelope hygiene (code review B
...
// TestKnowledgeEditAppendSection_UnknownArgument_Refused is code review B
...
func TestKnowledgeEditAppendSection_UnknownArgument_Refused(t *testing.T) {
...
// TestKnowledgeEditReplaceBody_MissingBody_Refused is code review B finding 7's
...
// byte-identical.
func TestKnowledgeEditReplaceBody_MissingBody_Refused(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
// TestKnowledgeEditReplaceBody_ExplicitEmptyBody_StillDeletes proves the
...
// refused.
func TestKnowledgeEditReplaceBody_ExplicitEmptyBody_StillDeletes(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
// TestKnowledgeEditCreate_UnknownArgument_Refused proves the check is not
// scoped to a single op — a stray field on create is caught the same way.
func TestKnowledgeEditCreate_UnknownArgument_Refused(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
