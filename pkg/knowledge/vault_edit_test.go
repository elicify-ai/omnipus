// Omnipus — tests for vault_edit (ADR-068 D15.3, spec §4.1.4): byte
...
// veTool builds vault_edit over a fresh fixture's deps.
...
// TestVaultEdit_ByteIdentical_AwkwardFile is the exact demonstration the
...
// value.
func TestVaultEdit_ByteIdentical_AwkwardFile(t *testing.T) {
...
func TestVaultEdit_SetProperty_ListOpAddAndRemove(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
func TestVaultEdit_Create_FromTemplate(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
func TestVaultEdit_Create_WithFrontmatterMapAndSchemaValidation(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	veSchema(t, root)
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
func TestVaultEdit_StaleToken_RefusedWithBothTokensNamed(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
	...
	if !strings.Contains(res.ForLLM, "vault_read") {
		t.Fatalf("refusal must name the remedy (re-read with vault_read): %s", res.ForLLM)
	}
...
func TestVaultEdit_MissingExpectVersion_Refused(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
	a4Note(t, root, "Note.md", "---\nstatus: draft\n---\nBody.\n")
...
func TestVaultEdit_UnknownProperty_NamesDeclaredOnes(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	veSchema(t, root)
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
func TestVaultEdit_EnumValueNotDeclared_NamesPermittedValues(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	veSchema(t, root)
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
func TestVaultEdit_WrongArity_NamesTheFix(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	veSchema(t, root)
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
func TestVaultEdit_WrongType_NamesExpectedShape(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	veSchema(t, root)
	deps, _ := a4Deps(home)
	tool := veTool(deps)
...
// TestVaultEdit_OrdinaryNote_NoSchemaBound proves FR-005: a note with no
// declared type (or none the vault's schemas recognise) is unconstrained —
// any property name and value is accepted.
func TestVaultEdit_OrdinaryNote_NoSchemaBound(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
...
func TestVaultEdit_CrossTierOps_RedirectByName(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
	a4Note(t, root, "Note.md", "---\nstatus: draft\n---\nBody.\n")
	...
	for _, c := range cases {
		...
		if !res.IsError {
			t.Fatalf("op %q must be refused by vault_edit", c.op)
		}
...
func TestVaultEdit_UnknownOp_NamesSupportedSet(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
	a4Note(t, root, "Note.md", "---\nstatus: draft\n---\nBody.\n")
...
// TestVaultEdit_NeverTouchesAFileTheAgentDidNotName is the blast-radius
...
func TestVaultEdit_NeverTouchesAFileTheAgentDidNotName(t *testing.T) {
	home, ws, root := a4Fixture(t, "kb")
	deps, _ := a4Deps(home)
	tool := veTool(deps)
