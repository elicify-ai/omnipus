// Omnipus — tests for knowledge_read (spec §4.1.3, AC-R1..AC-R3, FR-072, FR-072a,
// FR-074, US-9).
// License: MIT
...
func TestReadTool_RealRenderedReadWithByteCount(t *testing.T) {
	...
	out := resultText(res)
	t.Logf("rendered knowledge_read response, %d bytes:\n%s", len(out), out)
...
func TestReadTool_ReadThenEditWithZeroFailedWrites(t *testing.T) {
	...
	// This is the ENTIRE point: the token knowledge_read handed back is consumed
	...
	writeRes, werr := EditNote(OSLinkFS(), c, EditNoteRequest{
		...
		Edits:         []NoteEdit{AppendSection("Follow-up", "Added via the token knowledge_read returned.")},
		ExpectVersion: token,
	})
	require.NoError(t, werr, "the version token knowledge_read returns must be accepted by knowledge_edit's write path unchanged (AC-R1)")
...
// design decision the file header documents: the token knowledge_read returns is
...
func TestReadTool_VersionTokenIsIndependentOfPropindexSourceHash(t *testing.T) {
	...
	assert.Equal(t, want, token, "knowledge_read's token must be knowledge.ComputeVersionToken over the exact bytes returned")
...
// TestReadTool_SymlinkedPathIsRefusedNotFollowed is the knowledge_read half of
...
// records.ParseFrontmatter provably disagree; knowledge_read must follow the
