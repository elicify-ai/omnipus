...
// ---------------------------------------------------------------------------
// D17 / FR-070 / FR-071 — every knowledge tool that EXISTS is seeded
// ---------------------------------------------------------------------------

// TestKnowledgeToolPolicy_EveryImplementedToolIsSeededForEveryAgent closes the
// loop the other D17 tests cannot.
//
// pkg/coreagent and pkg/gateway both assert their hardcoded nine-name literals
// against each other and against the catalog. All of those literals could agree
// perfectly while the tool this package actually REGISTERS is called something
// else — the seed would then cover a name nobody offers, and the real tool
// would arrive with no policy entry at all.
//
// The failure that produces is silent, which is why it needs a test rather than
// a review: config.RepairIncompleteToolPolicyCoverage runs BEFORE
// config.ValidateToolPolicyCoverage on the load path, and it backfills an
// unknown tool to ToolPolicyDeny with a WARN. Boot succeeds. The tool is dead.
// One log line is the entire evidence (FR-071).
//
// So this test derives the names from the tool objects themselves — the same
// objects a registry would be handed — and requires, for every seeded agent:
// a per-agent entry, a global ceiling entry, and a RESOLVED posture matching
// D17's matrix for the four base agents.
func TestKnowledgeToolPolicy_EveryImplementedToolIsSeededForEveryAgent(t *testing.T) {
	implemented := RetrievalToolNames()
	require.NotEmpty(t, implemented, "this test measures nothing if no tool is implemented")

	cfg := config.DefaultConfig()
	require.True(t, coreagent.SeedConfig(cfg), "expected a fresh seed on a fresh DefaultConfig()")
	require.NotEmpty(t, cfg.Agents.List)
	...
	for _, name := range implemented {
		_, hasGlobal := cfg.Sandbox.ToolPolicies[name]
		assert.Truef(t, hasGlobal,
			"%q is implemented in pkg/knowledge but has NO global ceiling entry in "+
				"pkg/config/defaults.go — Constraint #6 admits no default, and a missing "+
				"ceiling that is tighter than a per-agent seed silently overrules it", name)

		for _, ac := range cfg.Agents.List {
			require.NotNilf(t, ac.Tools, "seeded agent %q carries no tools config at all", ac.ID)
			got, hasEntry := ac.Tools.Builtin.Policies[name]
			assert.Truef(t, hasEntry,
				"agent %q has NO explicit policy entry for the implemented tool %q. This does "+
					"not abort boot: the load path repairs before it validates and backfills "+
					"deny with a WARN, so the tool ships silently dead (FR-071)", ac.ID, name)
			assert.NotEmptyf(t, string(got), "agent %q's entry for %q must be a literal posture", ac.ID, name)
		}
	}

	// D17's matrix, restated here rather than read out of the seed: retrieval
	// is "allow" for all four base agents. Asserted on the RESOLVED policy,
	// because a global ceiling tighter than the per-agent seed silently wins
	// under strictest-wins — the defect pkg/config/defaults.go records four
	// previous instances of.
	for _, agentID := range []string{
		string(coreagent.IDMia), string(coreagent.IDJim),
		string(coreagent.IDAva), string(coreagent.IDRay),
	} {
		var policies map[string]config.ToolPolicy
		for _, ac := range cfg.Agents.List {
			if ac.ID == agentID && ac.Tools != nil {
				policies = ac.Tools.Builtin.Policies
			}
		}
		require.NotNilf(t, policies, "base agent %q must be seeded", agentID)
		for _, name := range implemented {
			resolved := tools.ResolveEffectivePolicy(&tools.ToolPolicyCfg{
				Policies: policies, GlobalPolicies: globalPolicies,
			}, name)
			assert.Equalf(t, "allow", resolved,
				"ADR-067 D17 seeds the retrieval tools 'allow' for all four base agents; "+
					"agent %q resolves %q for %q", agentID, resolved, name)
		}
	}
}

// TestKnowledgeToolPolicy_SeedIsTheSourceNotTheDenyBackfill is FR-071/AC-17.2
// measured against the IMPLEMENTED tool names, in two halves.
//
// (a) A freshly seeded configuration backfills zero entries for them.
// (b) POSITIVE CONTROL: delete one seeded pair (both sides of the OR — coverage
// is satisfied by the global ceiling OR the agent's own map) and the repair
// returns exactly that one, backfilled to DENY. Without (b), half (a) passes
// for a config in which the tool is not in the known-tools universe at all,
// because both the validator and the repair return nothing for a name they do
// not know.
func TestKnowledgeToolPolicy_SeedIsTheSourceNotTheDenyBackfill(t *testing.T) {
	implemented := RetrievalToolNames()
	known := make(map[string]struct{}, len(implemented))
	for _, n := range implemented {
		known[n] = struct{}{}
	}

	t.Run("a seeded config backfills nothing", func(t *testing.T) {
		cfg := config.DefaultConfig()
		...
		assert.Emptyf(t, repaired,
			"the SEED must be the source of every knowledge posture, not the deny-backfill; got %d", len(repaired))
	})

	t.Run("positive control: one deleted pair comes back as exactly one gap", func(t *testing.T) {
		cfg := config.DefaultConfig()
		require.True(t, coreagent.SeedConfig(cfg))

		victimTool := implemented[0]
		victimAgent := string(coreagent.IDMia)

		_, hadGlobal := cfg.Sandbox.ToolPolicies[victimTool]
		require.Truef(t, hadGlobal, "%q must have a global ceiling entry to delete", victimTool)
		...
		require.Truef(t, hadAgent, "(%s, %s) must be seeded per-agent to delete", victimAgent, victimTool)
		delete(victim.Tools.Builtin.Policies, victimTool)

		repaired := config.RepairIncompleteToolPolicyCoverage(cfg, known)
		require.Lenf(t, repaired, 1,
			"exactly the deleted pair must come back; zero means this control measures nothing "+
				"and the half above is vacuous (got %d)", len(repaired))
		...
		assert.Equal(t, config.ToolPolicyDeny, victim.Tools.Builtin.Policies[victimTool],
			"the repair backfills to DENY — which is why a forgotten knowledge tool ships with "+
				"the feature dead rather than failing boot (FR-071)")
	})
}
