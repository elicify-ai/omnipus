...
// TestSteerRecordClassifier_Row* exercise every row of landing order I-8's
// classification table (originally one t.Run per row inside a single
// TestSteerRecordClassifier_AllEightRows — split into independent top-level
// test functions, same bodies, same assertions, to keep the function under
// the founder's 240-line function-size budget,
// scripts/budgets/functions.txt).

func TestSteerRecordClassifier_Row1_NoRecordMetaHasNoParentAndNotDelegateType_IsOrdinaryRoot(t *testing.T) {
...
func TestSteerRecordClassifier_Row2_RecordPresentSteeredByNilOriginPresentMetaAgrees_IsOrdinaryRoot(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
...
func TestSteerRecordClassifier_Row3_RecordPresentValidSteeredByMetaAgrees_IsSteered(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
...
func TestSteerRecordClassifier_Row3b_ThreeLevelChainWalksToTheRealRoot_IsSteered(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
...
func TestSteerRecordClassifier_Row4_NoRecordMetaHasParent_IsDamagedChild(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
	steerer := newMetaSession(t, unified, session.SessionTypeChat, "")
	child := newMetaSession(t, unified, session.SessionTypeDelegate, steerer)
...
func TestSteerRecordClassifier_Row5_RecordPresentSteeredByNilOriginPresentMetaHasParent_IsDamagedChildEdgeLost(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
	steerer := newMetaSession(t, unified, session.SessionTypeChat, "")
	child := newMetaSession(t, unified, session.SessionTypeDelegate, steerer)
	if err := lifecycle.Persist(&session.LifecycleRecord{
...
func TestSteerRecordClassifier_Row6_RecordPresentSteeredByNilOriginAbsentDelegateType_IsLegacyDelegate(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
	steerer := newMetaSession(t, unified, session.SessionTypeChat, "")
	child := newMetaSession(t, unified, session.SessionTypeDelegate, steerer)
	if err := lifecycle.Persist(&session.LifecycleRecord{
...
func TestSteerRecordClassifier_Row7_RecordUnreadable_IsUnreadable(t *testing.T) {
...
func TestSteerRecordClassifier_Row8_SteeredByPresentButMetaDisagrees_IsInvalidEdge(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
	steerer := newMetaSession(t, unified, session.SessionTypeChat, "")
...
// a cycle, an unknown ancestor, and a wrong root — the three tests below
// exercise each, not just the local field/meta-agreement check above.

func TestSteerRecordClassifier_Row8b_CycleInTheAncestorChain_IsInvalidEdge(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
...
func TestSteerRecordClassifier_Row8c_UnknownAncestor_IsInvalidEdge(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
...
func TestSteerRecordClassifier_Row8d_WrongRoot_IsInvalidEdge(t *testing.T) {
	lifecycle, unified := newTestClassifierStores(t)
	root := newMetaSession(t, unified, session.SessionTypeChat, "")
	mustPersistOrdinaryRoot(t, lifecycle, root, session.OriginKindChat)
	mid := newMetaSession(t, unified, session.SessionTypeDelegate, root)
