...
//
// The distinction from the unexported detectFileClass (used internally for the
// step-5 offload guidance line's PRESENTATION-NOUN class, e.g. "document" for
// a PDF) is deliberate and load-bearing: a PDF is presentation-class
// "document" but media-category "file". Both are correct in their own
// vocabulary; only this one is legal on the wire. DO NOT reuse detectFileClass
// for a wire-format field — conflating the two previously shipped a BLOCKER:
// "document" is not in the contract enum, the SPA's strict Zod schema
// rejected the whole payload, and chat history failed to load entirely with
// "Backend response failed validation" after any document upload (found in
// live UAT, 2026-07-29).
