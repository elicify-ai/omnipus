...
func (rr *agentLoopRunTurnResponse) callLLMWithRetries() agentLoopRunTurnResponseFlow {
	rr.rq.ri.rf.callLLM = func(messagesForCall []providers.Message, toolDefsForCall []providers.ToolDefinition) (*providers.LLMResponse, error) {
		return rr.rq.ri.rf.rt.callProvider(messagesForCall, toolDefsForCall)
	}

	rr.rq.ri.rf.response = nil
	rr.rq.ri.rf.err = nil
	maxRetries := 2
	...
	tryPDFTextFallback := func() bool {
		return rr.rq.ri.rf.tryPDFTextFallback()
	}
	...
	synthesizeImageRejection := func(pe *ProviderError, rejectionErr error) bool {
		return rr.rq.ri.rf.synthesizeImageRejection(pe, rejectionErr)
	}

	for retry := 0; retry <= maxRetries; retry++ {
		rr.rq.ri.rf.response, rr.rq.ri.rf.err = rr.rq.ri.rf.callLLM(rr.rq.ri.rf.callMessages, rr.rq.ri.rf.providerToolDefs)
		...
		if repaired, ok := rr.rq.ri.rf.rt.al.evaluateTruncatedToolCallError(rr.rq.ri.rf.rt.ts, rr.rq.ri.rf.err, rr.rq.ri.rf.callMessages, retry, maxRetries, &rr.rq.ri.toolCallTruncationRepairUsed, rr.rq.ri.rf.rt.llmModel, rr.rq.ri.rf.rt.iteration); ok {
			rr.rq.ri.rf.callMessages = repaired
			continue
		}
		...
		if synthesizeImageRejection(pe, rr.rq.ri.rf.err) {
			break
		}
		...
		if downgradeResult := TryMediaDowngrade(rr.rq.ri.rf.rt.ts, rr.rq.ri.rf.callMessages, pe); downgradeResult.Applied {
			...
			helperCode := classifyByProviderError(pe, rr.rq.ri.rf.err.Error())
			logger.WarnCF("agent",
			...
			if rr.rq.ri.rf.err == nil {
				...
				if downgradeResult.Trigger == TriggerOutcomeFallback {
					rr.rq.ri.rf.rt.ts.setOutcomeRelabel(CodeMediaUnsupported)
				}
				break
			}
		...
		if rr.rq.ri.rf.rt.ts.hardAbortRequested() && errors.Is(rr.rq.ri.rf.err, context.Canceled) {
			rr.rq.ri.turnStatus = TurnEndStatusAborted
			rr.ret0, rr.ret1 = rr.rq.ri.rf.rt.al.abortTurn(rr.rq.ri.rf.rt.ts, "llm_call", hardInterruptAbortReason)
		...
		if errors.As(rr.rq.ri.rf.err, &exhaustedErr) {
			...
			if allTransient && retry < maxRetries {
				if rr.rq.ri.rf.rt.ts.hardAbortRequested() {
					break
				}
				...
				if sc, ok := rr.rq.ri.rf.rt.ts.lastStreamer.(interface{ StreamedContentLen() int }); ok && sc.StreamedContentLen() > 0 {
				...
				rr.rq.ri.rf.rt.al.emitEvent(
					EventKindLLMRetry,
					rr.rq.ri.rf.rt.ts.eventMeta("runTurn", "turn.llm.retry"),
					LLMRetryPayload{
						Attempt:    retry + 1,
				...
				if sleepErr := sleepWithContext(rr.rq.ri.rf.rt.turnCtx, backoff); sleepErr != nil {
					if rr.rq.ri.rf.rt.ts.hardAbortRequested() {
						rr.rq.ri.turnStatus = TurnEndStatusAborted
						rr.ret0, rr.ret1 = rr.rq.ri.rf.rt.al.abortTurn(rr.rq.ri.rf.rt.ts, "llm_retry_backoff", hardInterruptAbortReason)
						return agentLoopRunTurnResponseReturn
					}
					rr.rq.ri.rf.err = sleepErr
					break
				}
				continue
			}
		...
		activeProviderName := ""
		if len(rr.rq.ri.rf.rt.activeCandidates) > 0 {
			activeProviderName = rr.rq.ri.rf.rt.activeCandidates[0].Provider
		}
		failErr := providers.ClassifyError(rr.rq.ri.rf.err, activeProviderName, rr.rq.ri.rf.rt.llmModel)
		...
		if failErr != nil {
			...
			if isTransientStreamError(rr.rq.ri.rf.err) {
				isTimeoutError = true
			} else {
		...
		if isTimeoutError && retry < maxRetries {
			...
			rr.rq.ri.rf.rt.al.emitEvent(
				...
				rr.rq.ri.rf.rt.ts.eventMeta("runTurn", "turn.timeout"),
				TurnTimeoutPayload{
					TimeoutSeconds: rr.rq.ri.rf.rt.ts.agent.TimeoutSeconds,
			...
			if !compactionAttemptedOnTimeout && !rr.rq.ri.rf.rt.ts.opts.NoHistory && !rr.rq.ri.rf.rt.ts.agent.budgetChecksExempt() {
				// The sent surface, not the whole registry — same helper
				// windowTrim measures with (FR-028; see the pre-turn site).
				toolDefsTokens := rr.rq.ri.rf.rt.al.sentToolSurfaceTokens(rr.rq.ri.rf.rt.ts.agent, rr.rq.ri.rf.rt.ts.opts.TranscriptSessionID, rr.rq.ri.rf.rt.ts.sessionKey)
				retryMessages := rr.rq.ri.rf.callMessages
				if span := rr.rq.ri.rf.rt.al.activeRecallSpan(rr.rq.ri.rf.rt.ts.sessionKey); span != nil {
					retryMessages = append(append([]providers.Message(nil), rr.rq.ri.rf.callMessages...), span.Messages()...)
				}
				if isOverContextBudgetTokens(agentContextBudget(rr.rq.ri.rf.rt.ts.agent), retryMessages, toolDefsTokens) {
					...
					compression, ok := rr.rq.ri.rf.rt.al.windowTrim(rr.rq.ri.rf.rt.ts.agent, rr.rq.ri.rf.rt.ts.opts.TranscriptSessionID, rr.rq.ri.rf.rt.ts.sessionKey)
					if ok {
						rr.rq.ri.rf.rt.al.emitEvent(
							EventKindContextCompress,
							rr.rq.ri.rf.rt.ts.eventMeta("runTurn", "turn.context.compress"),
						...
						rr.rq.ri.rf.rt.al.emitEvent(
							...
							rr.rq.ri.rf.rt.ts.eventMeta("runTurn", "turn.compaction_retry"),
						...
						newHistory := rr.rq.ri.rf.rt.ts.agent.Sessions.GetHistory(rr.rq.ri.rf.rt.ts.sessionKey)
						rr.rq.ri.messages = rr.rq.ri.rf.rt.al.assembleMessages(rr.rq.ri.rf.rt.turnCtx, rr.rq.ri.rf.rt.ts, newHistory, "", nil, activeSkillNames(rr.rq.ri.rf.rt.ts.agent, rr.rq.ri.rf.rt.ts.opts))
						rr.continuationChain = nil
						if rr.rq.ri.rf.rt.ts.continuationUnresolved() {
							...
							// already written.
							rr.continuationChain = continuationChainMessages(rr.rq.ri.rf.rt.ts.continuationAccumulated())
							rr.rq.ri.messages = append(rr.rq.ri.messages, rr.continuationChain...)
						}
						rr.rq.ri.rf.callMessages = rr.rq.ri.messages
						if rr.rq.ri.gracefulTerminal {
							rr.rq.ri.rf.callMessages = append(append([]providers.Message(nil), rr.rq.ri.messages...), rr.rq.ri.rf.rt.ts.interruptHintMessage())
						}
						...
						logger.DebugCF("agent", "Window trim skipped during timeout recovery: nothing eligible to evict; proceeding with retry",
							map[string]any{"agent_id": rr.rq.ri.rf.rt.ts.agent.ID, "iteration": rr.rq.ri.rf.rt.iteration})
					} else {
			...
			if retry == 0 && !constants.IsInternalChannel(rr.rq.ri.rf.rt.ts.channel) {
				if notifyErr := rr.rq.ri.rf.rt.al.bus.PublishOutbound(rr.rq.ri.rf.rt.turnCtx, bus.OutboundMessage{
					...
					logger.WarnCF("agent", "Failed to send retry indicator",
						map[string]any{"channel": rr.rq.ri.rf.rt.ts.channel, "error": notifyErr.Error()})
				}
			}
			...
			if sleepErr := sleepWithContext(rr.rq.ri.rf.rt.turnCtx, backoff); sleepErr != nil {
				if rr.rq.ri.rf.rt.ts.hardAbortRequested() {
					rr.rq.ri.turnStatus = TurnEndStatusAborted
					rr.ret0, rr.ret1 = rr.rq.ri.rf.rt.al.abortTurn(rr.rq.ri.rf.rt.ts, "llm_timeout_backoff", hardInterruptAbortReason)
					return agentLoopRunTurnResponseReturn
				}
				rr.rq.ri.rf.err = sleepErr
				break
		...
		if isContextError && retry < maxRetries && !rr.rq.ri.rf.rt.ts.opts.NoHistory {
			...
			if contextCompressionFailed {
				logger.WarnCF("agent", "Context overflow persists after failed compression; aborting retry",
					map[string]any{"agent_id": rr.rq.ri.rf.rt.ts.agent.ID, "iteration": rr.rq.ri.rf.rt.iteration, "retry": retry})
				break
			}
			rr.rq.ri.rf.rt.al.emitEvent(
				EventKindLLMRetry,
			...
			if compression, ok := rr.rq.ri.rf.rt.al.windowTrimForce(rr.rq.ri.rf.rt.ts.agent, rr.rq.ri.rf.rt.ts.opts.TranscriptSessionID, rr.rq.ri.rf.rt.ts.sessionKey, true); ok {
				rr.rq.ri.rf.rt.al.emitEvent(
					EventKindContextCompress,
					rr.rq.ri.rf.rt.ts.eventMeta("runTurn", "turn.context.compress"),
					ContextCompressPayload{
