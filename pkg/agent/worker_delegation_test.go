...
func TestSeededGraph_JimToAvaTaskAllowed(t *testing.T) {
	...
	check := buildDelegationDenyCheckerForTaskReassignment(string(coreagent.IDJim), cfg.Agents.Defaults, config.DelegationModeTask)
...
func TestSeededGraph_BaseToWorkerAllowed(t *testing.T) {
	...
	for _, id := range []coreagent.CoreAgentID{
		...
		check := buildDelegationDenyCheckerForTaskReassignment(string(id), cfg.Agents.Defaults, config.DelegationModeTask)
...
func TestSeededGraph_DisallowedTargetDenied(t *testing.T) {
	...
	check := buildDelegationDenyCheckerForTaskReassignment(string(coreagent.IDMia), cfg.Agents.Defaults, config.DelegationModeTask)
	if denial := check(ctxWS(testWS, 0), string(coreagent.IDAva)); denial == nil {
...
func TestSeededGraph_JimAwaitModeAllowed(t *testing.T) {
	...
	jimCheck := buildDelegationDenyCheckerForDelegate(string(coreagent.IDJim), cfg.Agents.Defaults, config.DelegationModeAwait)
	...
	miaCheck := buildDelegationDenyCheckerForDelegate(string(coreagent.IDMia), cfg.Agents.Defaults, config.DelegationModeAwait)
