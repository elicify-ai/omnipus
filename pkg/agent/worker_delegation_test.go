...
func TestSeededGraph_JimToAvaTaskAllowed(t *testing.T) {
	...
	check := buildDelegationDenyChecker(string(coreagent.IDJim), nil, cfg.Agents.Defaults,
...
func TestSeededGraph_BaseToWorkerAllowed(t *testing.T) {
	...
	for _, id := range []coreagent.CoreAgentID{
		...
		check := buildDelegationDenyChecker(string(id), nil, cfg.Agents.Defaults,
			config.DelegationModeTask, nil)
...
func TestSeededGraph_DisallowedTargetDenied(t *testing.T) {
	...
	check := buildDelegationDenyChecker(string(coreagent.IDMia), nil, cfg.Agents.Defaults,
		config.DelegationModeTask, nil)
	if denial := check(ctxWS(testWS, 0), string(coreagent.IDAva)); denial == nil {
...
func TestSeededGraph_JimAwaitModeAllowed(t *testing.T) {
	...
	jimCheck := buildDelegationDenyChecker(string(coreagent.IDJim), nil, cfg.Agents.Defaults,
	...
	miaCheck := buildDelegationDenyChecker(string(coreagent.IDMia), nil, cfg.Agents.Defaults,
		config.DelegationModeAwait, nil)
