...
// call receives. Kept from release (#683): its loop.go post-dispatch arm
// still calls it. Note the round-3 reachability analysis above — with the
// pre-dispatch condition-2 refusal in place this arm is unreachable in
// practice; it is retained for the release loop.go call site, not as a live
// trip path.
