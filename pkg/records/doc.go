...
// PROPERTY TYPES (text, enum, relation, date, number, money, person) are
...
//
// FR-013 and FR-020b: money arithmetic is exact decimal, and money must never
// become a binary floating-point number ANYWHERE in the path. That rule is why
// this package parses frontmatter through yaml.Node and keeps every numeric
// value in its LEXICAL form (see frontmatter.go) rather than unmarshalling into
// `any`. Unmarshalling `349.98` into an `any` produces a float64 before any
// code of ours gets a say, and 349.98 is not representable in binary floating
