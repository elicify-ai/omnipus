...
`)
	...
	assertLeaf(t, branches[0], "state", generated.VaultFilterNodeOpEqual, "shipped")
	...
	assertLeaf(t, (*branches[1].All)[0], "state", generated.VaultFilterNodeOpEqual, "draft")
	assertLeaf(t, (*branches[1].All)[1], "batch", generated.VaultFilterNodeOpGreaterThanEqual, "7")

	// branch 2 — tree negation, which is NOT `<>` (spec §8 R-2).
	if branches[2].Not == nil {
		t.Fatalf("branch 2 is not a `not`: %+v", branches[2])
	}
	if branches[2].Not.Op == nil || *branches[2].Not.Op != generated.VaultFilterNodeOpISNULL {
