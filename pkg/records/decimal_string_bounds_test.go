...
// TestMoneyExponentRefusal_QuotesAPlainRendering pins the reason the
// abbreviation is not exponent notation.
//
// parseMoneyAmount refuses exponent notation and its message quotes
// d.String() as the form the operator should type instead — "write it out in
// full, e.g. %s". If String ever answered in exponent notation, that message
// would advise the very notation it is rejecting. The scales reachable here are
// always in-domain (they came through ParseDecimal), so the advice must be a
// plain, re-parseable literal every time.
func TestMoneyExponentRefusal_QuotesAPlainRendering(t *testing.T) {
	for _, text := range []string{"1e3", "1.5e2", "1e100", "1e-100", "-2.5e-3"} {
		_, err := parseMoneyAmount(text)
		if err == nil {
			t.Fatalf("parseMoneyAmount(%q) must refuse exponent notation", text)
		}
		msg := err.Error()
		if strings.ContainsAny(msg, "<>") {
			t.Fatalf("parseMoneyAmount(%q) advice contains an abbreviation marker: %s", text, msg)
		}

		// The advised form is the plain rendering of the same number, and it is
		// something money itself would accept on the next attempt.
		d, perr := ParseDecimal(text)
		if perr != nil {
			t.Fatalf("ParseDecimal(%q): %v", text, perr)
		}
		advice := d.String()
		if !strings.Contains(msg, advice) {
			t.Fatalf("parseMoneyAmount(%q) does not quote the plain form %q; message was: %s", text, advice, msg)
		}
		if strings.ContainsAny(advice, "eE") {
			t.Fatalf("parseMoneyAmount(%q) advises %q, which is exponent notation — the notation it just rejected", text, advice)
		}

		// The advice must at minimum be a literal that reads back as the same
		// number. It is NOT asserted that money itself accepts it: "1e-100"'s
		// plain form has 100 decimal places and money caps at maxMoneyScale, so
		// that advice is unusable for a separate, pre-existing reason of its
		// own. That is a different finding from this one and is deliberately
		// not smuggled into this test.
		back, berr := ParseDecimal(advice)
		if berr != nil {
			t.Fatalf("parseMoneyAmount(%q) advises writing %q, which ParseDecimal rejects: %v", text, advice, berr)
		}
		if !back.Equal(d) {
			t.Fatalf("parseMoneyAmount(%q) advises writing %q, which is a different number", text, advice)
		}
	}
}
