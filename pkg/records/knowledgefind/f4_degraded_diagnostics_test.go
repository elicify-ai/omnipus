...
func TestF4a_NearestTermsErrorIsReportedNotSwallowed(t *testing.T) {
	...
	for _, p := range resp.Problems {
		if p.Code == generated.RecordProblemCodeIndexUnavailable {
			found = true
		}
	}
	if !found {
...
func TestF4c_TextHashReadErrorIsDistinctFromNoHash(t *testing.T) {
	...
	for i := range resp.Problems {
		if resp.Problems[i].Code == generated.RecordProblemCodeIndexUnavailable {
			got = &resp.Problems[i]
		}
	}
	if got == nil {
	...
	for i := range resp.Problems {
		if resp.Problems[i].Code == generated.RecordProblemCodeStaleRecord {
