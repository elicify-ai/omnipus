...
func TestFind_WordSearchFanoutTruncationIsReportedNotSilent(t *testing.T) {
	...
	found := false
	for _, p := range resp.Problems {
		if p.Code == generated.RecordProblemCodeTextSearchTruncated {
			found = true
...
func TestFind_TaskKindReportsStalenessAgainstTheTextIndex(t *testing.T) {
	...
	found := false
	for _, p := range resp.Problems {
		if p.Code == generated.RecordProblemCodeStaleRecord && contains(p.Records, "garden/plant-0001.md") {
			found = true
		}
	}
	if !found {
...
func TestFind_PagingIsCursorBasedAndAStaleCursorIsAnError(t *testing.T) {
	...
	t.Run("a cursor from another epoch is refused", func(t *testing.T) {
		...
		if resp.Problems[0].Code != generated.RecordProblemCodeStaleCursor {
...
func TestFind_StaleRowIsReturnedAndFlagged(t *testing.T) {
	...
	found := false
	for _, p := range resp.Problems {
		if p.Code == generated.RecordProblemCodeStaleRecord {
			found = true
