...
func (e *evaluation) assemble(ctx context.Context, d Deps, echo string) generated.VaultFindResponse {
	...
	for _, s := range page {
		...
		case hashErr != nil:
			...
			t := true
			row.Stale = &t
			p := problem(generated.RecordProblemCodeIndexUnavailable,
		...
		case s.cand.Kind == propindex.KindAttachment:
			...
			if textHolds {
				...
				t := true
				row.Stale = &t
				p := problem(generated.RecordProblemCodeStaleRecord,
		...
		default:
			...
			if fresh == propindex.FreshnessAgree {
				...
				t := true
				row.Stale = &t
				p := problem(generated.RecordProblemCodeStaleRecord, s.cand.Path+": "+fresh.Reason(),
					"re-run to confirm; run knowledge_describe check_integrity if it persists",
					identityOf(s.cand))
				p.Paths = &[]string{s.cand.Path}
				e.problems = append(e.problems, p)
...
func finishVerdict(resp *generated.VaultFindResponse, q *query) {
	...
	if q.clamped {
		...
		resp.Problems = append(resp.Problems, problem(generated.RecordProblemCodePageSizeClamped,
