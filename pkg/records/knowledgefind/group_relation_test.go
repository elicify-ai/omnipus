...
func TestGroupByRelation_UnresolvedIsExcludedAndReported(t *testing.T) {
	...
	for _, p := range resp.Problems {
		if p.Code == generated.RecordProblemCodeDanglingRelation {
...
func TestGroupByInverse_IsRefusedNotAConfidentWrongAnswer(t *testing.T) {
	...
	if refusal.Problem.Code != generated.RecordProblemCodeUnknownProperty {
		t.Errorf("refusal code = %q, want %q (the ordinary unknown-property refusal, "+
			"not a bespoke inverse-specific one)", refusal.Problem.Code, generated.RecordProblemCodeUnknownProperty)
	}
