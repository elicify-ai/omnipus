		...
		t.Run(prop, func(t *testing.T) {
			...
			if p.Code != generated.RecordProblemCodeUnsupportedOperator {
				t.Errorf("problem.Code = %q, want %q (R-1's operatorDefinedForType has no ordering "+
					"operator for a relation/person, resolved or not)", p.Code, generated.RecordProblemCodeUnsupportedOperator)
			}
...
func TestFind_SortOnManyPropertyIsRefused(t *testing.T) {
	...
	if p.Code != generated.RecordProblemCodeOrderingOnManyProperty {
		t.Errorf("problem.Code = %q, want %q (R-13: ordering is not defined across arity)",
			p.Code, generated.RecordProblemCodeOrderingOnManyProperty)
	}
}
