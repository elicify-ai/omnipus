...
func TestRefusal_UnknownPropertyNamesTheValidOnes(t *testing.T) {
	...
	p := resp.Problems[0]
	if p.Code != generated.RecordProblemCodeUnknownProperty {
...
func TestRefusal_UnsupportedSQLNamesTheTenOperators(t *testing.T) {
	...
	for _, construct := range []string{"JOIN", "BETWEEN", "COALESCE", "EXISTS", "REGEXP", "!="} {
		t.Run(construct, func(t *testing.T) {
			...
			p := resp.Problems[0]
			if p.Code != generated.RecordProblemCodeUnsupportedOperator {
...
func TestRefusal_EmptyLikePatternAndEmptyInList(t *testing.T) {
	...
	t.Run("empty LIKE pattern", func(t *testing.T) {
		for _, pattern := range []string{"", "%"} {
			...
			p := resp.Problems[0]
			if p.Code != generated.RecordProblemCodeEmptyLikePattern {
	...
	t.Run("empty IN list", func(t *testing.T) {
		...
		if resp.Problems[0].Code != generated.RecordProblemCodeEmptyInList {
...
func TestRefusal_ThirdHop(t *testing.T) {
	...
	p := resp.Problems[0]
	if p.Code != generated.RecordProblemCodeHopLimitExceeded {
	...
	if !strings.Contains(deref(p.Fix), "second knowledge_find") {
