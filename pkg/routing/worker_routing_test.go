...
// available chat-target agent — a worker must never receive inbound messages.
//
// BDD: Given a worker marked Default=true and a base agent,
//
...
func TestResolveRoute_NeverPicksWorkerAsDefault(t *testing.T) {
	agents := []config.AgentConfig{
		...
		{ID: "worker", Type: config.AgentTypeWorker, Default: true},
	...
	if route.AgentID != "mia" {
		t.Errorf("AgentID = %q, want %q (worker skipped, first chat-target agent)", route.AgentID, "mia")
	}
}

// TestResolveRoute_WorkerSortsFirstButSkipped verifies the first-available-agent
...
func TestResolveRoute_WorkerSortsFirstButSkipped(t *testing.T) {
	agents := []config.AgentConfig{
		...
		{ID: "aworker", Type: config.AgentTypeWorker},
		{ID: "jim"},
	}
	cfg := testConfig(agents, nil)
	r := NewRouteResolver(cfg)
	...
	if route.AgentID == "aworker" {
		t.Fatalf("first-available fallback picked a worker (%q) — workers are not chat targets", route.AgentID)
	}
