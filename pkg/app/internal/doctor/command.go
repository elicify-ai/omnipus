...
// executablePathOverride is the companion test seam for the NEGATIVE case
// packageChromeRootOverride cannot express: "the resolver finds no root at
// all". The override above always reports ok=true, so a test that wants the
// bare-binary shape has to control where the resolver starts looking instead.
//
// Without it the bare-binary test was only ever asserting a property of the
// machine it ran on. `go test` puts the test binary in
// $TMPDIR/go-build*/b001/, and the darwin candidate list probes three levels
// up — which on macOS is $TMPDIR's parent, the very directory
// Chrome-for-Testing downloads its cache into. On any developer machine that
// has ever run the browser tool, the test saw a real chromium/ root and
// failed with WARN-BROWSER-008. Pointing the resolver at a path the test
// owns makes the assertion mean what it says.
//
// Production code MUST leave this empty, for the same reason as above.
var executablePathOverride string

// doctorExecutablePath returns the running binary's path, honouring the
// executablePathOverride test seam.
func doctorExecutablePath() (string, error) {
	if executablePathOverride != "" {
		return executablePathOverride, nil
	}
