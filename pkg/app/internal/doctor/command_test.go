...
//
// The bare-binary layout is synthesized rather than inherited from wherever
// `go test` happened to drop the test binary. Relying on the latter made this
// test a property of the machine: on darwin the candidate list probes three
// levels above the executable, which for a test binary is $TMPDIR's parent —
// exactly where Chrome-for-Testing caches its download. Any developer machine
// that had run the browser tool saw a real chromium/ root here and failed with
// WARN-BROWSER-008. The executable path is now the test's own, nested deep
// enough that every candidate the resolver probes stays inside t.TempDir().
func TestCheckBrowserPackageChrome_NoPackageChrome(t *testing.T) {
	// .../root/a/b/c/bin/omnipus — so both darwin candidates (../chromium and
	// ../../../chromium) and all three linux candidates land under root/.
	exeDir := filepath.Join(t.TempDir(), "a", "b", "c", "bin")
	require.NoError(t, os.MkdirAll(exeDir, 0o755))

	prev := executablePathOverride
	executablePathOverride = filepath.Join(exeDir, "omnipus")
	t.Cleanup(func() { executablePathOverride = prev })

	// Pre-condition: the seam really is what the resolver reads, and it finds
	// nothing. Without this a typo in the seam name would make the assertion
	// below pass for the wrong reason.
	root, ok := packageChromeRootForDoctor()
	require.False(t, ok, "synthesized bare-binary layout must resolve no chromium/ root, got %q", root)
