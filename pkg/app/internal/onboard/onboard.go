...
//
// The wizard configures an instance's LLM provider without requiring a running
// gateway or a browser — necessary for headless container deployments where the
// docker entrypoint previously gated boot on this command. See issue #159.
//
// WP1.5b (ADR-0010) composes it by config.EditionAuthMode(): in LOCAL mode
// (the open-source edition) it ALSO creates the admin account — upstream's
// behaviour, restored verbatim, because that edition's own password login is
// permanent (ADR-0010's founder decision). In PLATFORM mode (hosted/desktop)
// it creates NO account and NO password: under
// [ADR-0008](docs/adr/0008-account-is-the-login.md) the omnipus.ai account IS
// the login there — the only thing that may write a `gateway.users` row is the
// platform sign-in path, and the row it writes carries an EMPTY password hash.
// A CLI-seeded row with a different username would permanently refuse the first
// platform sign-in (`ensurePlatformUser`, pkg/gateway/rest_platform_auth.go),
// so no code path here may produce one on that edition.
//
...
//  4. writes a provider entry and the default model into config.json (and, in
//     local mode, the admin account alongside it);
//  5. mints the cli principal and token file (clitoken.EnsureCLIToken);
//  6. marks onboarding complete in system/state.json.
//
// After this runs, `omnipus start` boots with a working provider. In local
// mode the operator logs in with the credentials just entered; in platform
// mode they sign in to their omnipus.ai account in the app.
package onboard
...
import (
	...
	"github.com/elicify-ai/omnipus/pkg/app/internal/netinfo"
...
type Input struct {
	...
	APIKey     string
	Model      string
	// Username/Password are LOCAL-MODE ONLY (WP1.5b, ADR-0010): the admin
	// account this wizard creates on that edition. Always zero in platform
...
// NewOnboardCommand returns the `omnipus onboard` Cobra command, composed by
// config.EditionAuthMode() (WP1.5b, ADR-0010): the admin-account flags,
// wording and behaviour are upstream's in local mode, today's (no account)
// in platform mode.
func NewOnboardCommand() *cobra.Command {
	local := config.EditionAuthMode() == config.AuthModeLocal
	var (
		homeFlag           string
		providerFlag       string
		apiKeyFlag         string
	...
	cmd := &cobra.Command{
		Use:   "onboard",
		Short: onboardShort(local),
		Long:  onboardLong(local),
	...
	// LOCAL MODE ONLY (WP1.5b): the admin-account flags. Not registered in
	// platform mode at all — that edition's CLI creates no account, so these
	// flags would have nothing to do and would only confuse `--help`.
	if local {
...
// onboardShort is the command's one-line --help summary, per mode.
func onboardShort(local bool) string {
	if local {
		return "First-run setup (provider, API key, admin user) — interactive or headless"
	}
	return "First-run setup (provider, API key, default model) — interactive or headless"
}

// onboardLong is the command's --help body, per mode. Restored verbatim from
// upstream (merge base 184d7247) for local; today's text, unchanged, for
// platform.
func onboardLong(local bool) string {
	if local {
		return `Configure a fresh Omnipus install.
...
Examples:
  ...
  printf 'sk-or-v1-...\np@ssw0rd!\n' | omnipus onboard --non-interactive \
	...
	return `Configure a fresh Omnipus install's LLM provider.

This command creates no account and no password. Signing in happens in the
app, with your omnipus.ai account.

Two modes:

  Interactive (default)
    Presents a numbered provider menu, prompts for the API key and the
    default model, then mints the CLI token.

  Headless
    Pass --non-interactive together with --provider and --api-key (or
    --api-key-stdin). --model is optional and defaults to a sensible
    per-provider model.

In both modes the API key is encrypted into credentials.json (creating
master.key on first run); the provider entry and default model are written
to config.json; the cli principal and its token file are created; and
system/state.json is marked complete.

After running this command, ` + "`omnipus start`" + ` boots with a working
provider and you sign in to your omnipus.ai account in the app. The web
onboarding wizard remains available for users who prefer a browser.
...
Examples:
  ...
  # headless (key on command line — visible to other users on the box)
  omnipus onboard --non-interactive \
    --provider openrouter \
    --api-key sk-or-v1-... \
    --model 'z-ai/glm-5v-turbo'

  # headless (key from stdin — safer)
  printf 'sk-or-v1-...\n' | omnipus onboard --non-interactive \
    --provider openrouter \
    --api-key-stdin`
}
...
//
// username/password/adminPasswordStdin are LOCAL-MODE ONLY (WP1.5b): zero in
// platform mode, where NewOnboardCommand never registers the flags that
// would populate them.
...
// values are read from stdin when --api-key-stdin or (local mode only)
// --admin-password-stdin is set; the api key (if requested) is read first,
// then the admin password, each as a single line.
//
// In platform mode there is deliberately no credential collected here: the
// CLI creates no account there (ADR-0008 ruling 2), and f.username/f.password
// are always zero because NewOnboardCommand never registers the flags that
// would set them.
...
func RunHeadless(home string, wio wizardIO, in Input) error {
	...
	printSignInMessage(wio.stdout, in)
	printAccessURLBlock(wio.stdout, home)
	return nil
}

// printSignInMessage is the mode-composed closing line (WP1.5b, ADR-0010):
// upstream's "log in with the password you just entered" in local mode,
// today's "sign in to your omnipus.ai account" in platform mode.
func printSignInMessage(out io.Writer, in Input) {
	if config.EditionAuthMode() == config.AuthModeLocal {
		fmt.Fprintf(out, "Log in as %q with the password you just entered.\n", in.Username)
		return
	}
	fmt.Fprintln(out, "Sign in with your omnipus.ai account in the app — this command creates no local login.")
}
...
// Returns an error if input is malformed or empty where required.
//
// It asks for no username and no password: the account is the login
// (ADR-0008 ruling 2), and it is created on omnipus.ai, not here.
//
...
func prompt(wio wizardIO) (Input, error) {
	...
	// Admin user — LOCAL MODE ONLY (WP1.5b, ADR-0010), restored verbatim from
	// upstream (merge base 184d7247). In platform mode the account is the
	// omnipus.ai sign-in, created in the app, not here.
	if config.EditionAuthMode() == config.AuthModeLocal {
		username, usernameErr := readNonEmpty(wio.stdout, reader, "Admin username: ", "")
		if usernameErr != nil {
			return in, usernameErr
		}
		in.Username = username

		fmt.Fprint(wio.stdout, "Admin password (min 8 chars, input hidden): ")
		password, pwErr := wio.readPassword()
		if pwErr != nil {
			return in, fmt.Errorf("read password: %w", pwErr)
		}
		fmt.Fprintln(wio.stdout, "")
		if len(password) < 8 {
...
func applyInput(in Input, wio wizardIO) error {
	...
	// 4. LOCAL MODE ONLY (WP1.5b, ADR-0010): pre-compute the admin password
	// hash and a bearer token OUTSIDE any locks, restored from upstream.
	// Both stay zero in platform mode, where mutateConfigFile's admin
	// section never runs (see its own mode gate).
	local := config.EditionAuthMode() == config.AuthModeLocal
	var passwordHash, token, tokenHash string
	if local {
		...
		var tokenErr error
		token, tokenErr = clitoken.GenerateBearerToken()
		if tokenErr != nil {
			return fmt.Errorf("generate token: %w", tokenErr)
		}
		tHash, tHashErr := bcrypt.GenerateFromPassword([]byte(token), bcrypt.DefaultCost)
		if tHashErr != nil {
			return fmt.Errorf("hash token: %w", tHashErr)
		}
		tokenHash = string(tHash)
	}
	...
	// In platform mode NO account is written: gateway.users belongs to the
	// platform sign-in path alone (ADR-0008 ruling 2). In local mode the
	// admin row is written alongside the provider — see mutateConfigFile.
	configPath := filepath.Join(in.Home, "config.json")
	if err := mutateConfigFile(configPath, in, credRef, passwordHash, tokenHash); err != nil {
...
// mutateConfigFile writes the provider row and the default model in both
// modes, and — LOCAL MODE ONLY (WP1.5b, ADR-0010) — the admin row alongside
// them (passwordHash/tokenHash are the empty string in platform mode, and
// the admin section is skipped entirely there; see the package doc for why
// no code path may write gateway.users on that edition).
func mutateConfigFile(path string, in Input, credRef, passwordHash, tokenHash string) error {
	...
	// --- Admin user (LOCAL MODE ONLY, WP1.5b, ADR-0010) ---
	//
	// passwordHash is empty in platform mode (applyInput never computes it
	// there), and that emptiness is exactly the gate: this section must never
	// run on that edition, because gateway.users belongs to the platform
	// sign-in path alone — see the package doc for the one-way-door reasoning
	// (a CLI-seeded row with a different username permanently refuses the
	// first platform sign-in).
	if config.EditionAuthMode() == config.AuthModeLocal {
