...
// (Moved here from cmd/omnipus/main.go with the assembly it gates — WP1.5a.)
//go:build stdjson

// Package app assembles the Omnipus CLI and gateway into the cobra root
// command that `cmd/omnipus/main.go` runs. This is the public entry-point
// seam (WP1.5a, docs/plans/engine-divergence-workplan.md, ADR-0010): moving
// the assembly out from under cmd/omnipus/internal — which nothing outside
// cmd/omnipus may import (Go's internal-package rule) — lets a second main
// package (editions/cmd/omnipus) build the same binary plus edition-specific
// parts, without a fork of the CLI wiring.
//
// NewRootCommand and Main are behaviour-identical to the upstream assembly
// they replace: same commands, flags, help text and exit codes. This file's
// own onboard-related help text (onboardRosterLine/onboardHelpLine) is
// composed by config.EditionAuthMode() (WP1.5b, ADR-0010): upstream's
// wording in local mode, today's omnipus.ai wording in platform mode. The
// onboard/doctor packages carry the rest of that composition themselves —
// see pkg/app/internal/onboard's package doc and engine/DIVERGENCE.txt.
...
import (
	...
	"github.com/elicify-ai/omnipus/pkg/app/internal/gateway"
	"github.com/elicify-ai/omnipus/pkg/app/internal/onboard"
	recordscmd "github.com/elicify-ai/omnipus/pkg/app/internal/records"
	"github.com/elicify-ai/omnipus/pkg/app/internal/run"
	stopcmd "github.com/elicify-ai/omnipus/pkg/app/internal/stop"
	"github.com/elicify-ai/omnipus/pkg/app/internal/version"
...
// onboardRosterLine is the one-line description of `omnipus onboard` shown
// in the no-args roster/usage block, composed by config.EditionAuthMode()
// (WP1.5b, ADR-0010): upstream's wording in local mode (the CLI creates the
// admin account there), today's in platform mode (sign-in happens in the
// app).
func onboardRosterLine() string {
	if config.EditionAuthMode() == config.AuthModeLocal {
		return "first-time setup — configure a provider and admin account"
	}
	return "first-time setup — configure a provider (sign in happens in the app)"
}

// onboardHelpLine is the `onboard` entry in the root command's Commands:
// table (NewRootCommand's Long text), composed the same way as
// onboardRosterLine.
func onboardHelpLine() string {
	if config.EditionAuthMode() == config.AuthModeLocal {
		return "First-time setup — configure a provider and admin account."
	}
	return "First-time setup — configure a provider. Creates no account; you sign in with your omnipus.ai account in the app."
}
...
func printRosterAndUsage(w *os.File, roster []rosterLine) {
	...
	fmt.Fprintln(w, "  omnipus onboard      "+onboardRosterLine())
...
`, internal.Logo, config.GetVersion(), onboardHelpLine()),
