...
// lockEvent is one recorded acquire/release through the FR-101 seam.
...
func installLockRecorder(t *testing.T) (events *[]lockEvent, restore func()) {
	...
	sessionLockAcquireFn = func(shard uint32, m *sync.Mutex) {
		m.Lock()
		mu.Lock()
		recorded = append(recorded, lockEvent{acquire: true, shard: shard})
		mu.Unlock()
	}
	sessionLockReleaseFn = func(shard uint32, m *sync.Mutex) {
		mu.Lock()
		recorded = append(recorded, lockEvent{acquire: false, shard: shard})
		mu.Unlock()
		m.Unlock()
	}
...
func TestUnifiedStore_ConcurrentWritesToDifferentSessionsDoNotSerialize(t *testing.T) {
	...
	t.Logf("AC-20(a) slope: N=%d best-of-%d took %v, 2N=%d best-of-%d took %v (ratio %.2f)",
		n, trials, tN, 2*n, trials, t2N, float64(t2N)/float64(tN))
	...
	maxAllowed := time.Duration(float64(tN) * 2.5)
	assert.LessOrEqualf(t, t2N, maxAllowed,
		"2N concurrent session writers took %v (best of %d), more than 2.5x the N-writer time %v (ratio %.2f) — "+
			"this indicates writes to DIFFERENT sessions are still serializing",
		t2N, trials, tN, float64(t2N)/float64(tN))
}
...
func TestAppendTranscript_NotDelayedBySessionBCreate(t *testing.T) {
	...
	t.Logf("AC-20(c): A-only baseline (best of %d)=%v, A-with-B-contention (best of %d)=%v (ratio %.2f)",
		trials, baseline, trials, withContention, float64(withContention)/float64(baseline))

	// Same 2.5x slack rationale as AC-20(a) above (disk-fsync-floor
	// dominance verified empirically on this devpod).
	maxAllowed := time.Duration(float64(baseline) * 2.5)
	assert.LessOrEqualf(t, withContention, maxAllowed,
		"session A's append stream took %v with concurrent session-B creates, vs %v baseline (ratio %.2f) — "+
			"session B's creates are delaying session A's streamed appends",
		withContention, baseline, float64(withContention)/float64(baseline))
}
