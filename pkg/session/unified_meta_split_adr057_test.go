...
// readUnifiedSourcesForTest returns the concatenated non-test sources of the
// unified session store: every unified*.go file in pkg/session/, in name
// order, each preceded by a "// ---- file: <name> ----" banner. Guard tests
// that used to read "unified.go" alone must read this instead: unified.go was
// split into unified_*.go files by job on 2026-09-15, so a doc comment
// scanned by anchor text may live in any of them.
func readUnifiedSourcesForTest(t *testing.T) string {
	t.Helper()
	matches, err := filepath.Glob("unified*.go")
	require.NoError(t, err, "readUnifiedSourcesForTest: glob unified*.go")
	...
	for _, name := range matches {
		...
		require.NoErrorf(t, err, "readUnifiedSourcesForTest: read %s", name)
	...
	// anchor checks below would pass vacuously on a broken checkout.
	require.Greaterf(t, n, 0, "readUnifiedSourcesForTest: no non-test unified*.go sources found")
	return b.String()
}
...
func TestMetaDocComments_NoSingleFunnelClaim(t *testing.T) {
	text := readUnifiedSourcesForTest(t)
